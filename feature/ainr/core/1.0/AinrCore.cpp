...
void AinrCore::prepareJobs()
{
    ...
    m_homoMethod = [this](int index) -> enum AinrErr {
        ...
        rProc1In.i4RzWidth = m_rrzoWidth;
        rProc1In.i4RzHeight = m_rrzoHeight;
    ...
    m_matchingMethod = [this](int index) {
        ...
        {
            std::lock_guard<std::mutex> __lk(m_matchingMx);
            m_matchingCount++;
            m_matchingCond.notify_one();
            ainrLogD("notify matching task[%d] done, matchingCount(%d)",
                    index, m_matchingCount);
        }
        return;
    };
...
enum AinrErr AinrCore::doMatching()
{
...

    {
        ...
        std::unique_lock<std::mutex> __lk(m_matchingMx);
        ainrLogD("start wait matching done....");
        m_matchingCond.wait(__lk, [this](){ return m_matchingCount == (m_captureNum -1);});
        ainrLogD("Wait matching done....");
    }
...
enum AinrErr AinrCore::setUpTuning(NRCORE_PROC1_IN_STRUCT *unPackBayerCfg)
{
    ...
    if(CC_UNLIKELY(m_NvramPtr.get() == nullptr)) {
        ...
        // On device tuning
        if(data && ((m_onlineTuning == 2) || (m_autoTest))) {
...
void AinrCore::dumpAinrTuning(FEATURE_NVRAM_AINR_T *data)
{
    ...
    auto dump2Binary = [this](MINT32 uniqueKey, MINT32 requestNum, MINT32 frameNum, const char* buf, size_t size, string fn) -> bool {
        char filepath[256] = {0};
        int inUsed = ::snprintf(filepath, sizeof(filepath)-1, "%s/%09d-%04d-%04d-""%s",
                AINR_DUMP_PATH, uniqueKey, requestNum, frameNum, fn.c_str());
        if (inUsed < 0) {
            ainrLogW("snprintf fail");
            return false;
        }
...
enum AinrErr AinrCore::doNrCore()
{
    ...
    if (m_bIsLowMem)
    {
        ...
        if(CC_UNLIKELY(m_NvramPtr.get() == nullptr)) {
            ainrLogW("m_NvramPtr is null, we use default algo index");
        } else {
            // Dump
            size_t bufferSize = 0;
            char *pMutableChunk
            ...
            if (CC_LIKELY(pMutableChunk)) {
                FEATURE_NVRAM_AINR_T *data
                        = reinterpret_cast<FEATURE_NVRAM_AINR_T*>(pMutableChunk);
                algoIdx = data->rsv6; // rsv6 indicate algo model index
            } else {
                ainrLogW("getChunk fail, we use default algo index");
            }
        }
    }
    ainrLogD("NR model index(%d)", algoIdx);
    ...
    MUINT32 flipMode = 0;
    {
        ...
        char *pMutableChunk
         ...
         if (CC_LIKELY(pMutableChunk)) {
            FEATURE_NVRAM_AINR_T *data
                    = reinterpret_cast<FEATURE_NVRAM_AINR_T*>(pMutableChunk);
            flipMode = data->rsv7; // rsv7 indicate flip mode
        } else {
            ainrLogW("getChunk fail, we use default flip mode");
        }
    }

    ainrLogD("NR model index(%d) flip mode(%u)", algoIdx, flipMode);
...

    {
        ...
        if(mbNeedTileMode) {
            ...
            ainrLogI("Execute NR core in frame mode+");
...
void AinrCore::acquireJobQueue()
{
    ...
    if (mMinorJobQueue.get() == nullptr) {
        mMinorJobQueue
                = std::make_shared<NSCam::JobQueue<void()>>("AinrCoreJob");
...
void AinrCore::nextCapture()
{
    // Release RRZO and IMGO
    {
        ...
        if (!IMetadata::getEntry<MINT32>(m_vDataPackage[0].halMeta, MTK_PIPELINE_UNIQUE_KEY, processUniqueKey)) {
...
{
    ainrLogE("AINR1.0 not support %s", __func__);

    return eStage_MSNR_BFBLD;
}
