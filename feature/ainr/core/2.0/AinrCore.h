...
private:
    ...
    enum AinrErr releaseProcImgBuffers();
    ...
    MBOOL readBackRRZO(int index, const char* ispProfileName);
    MBOOL readBackIMGO(int index, const char* ispProfileName);
    MBOOL readBackBuf(std::vector<std::string> keys, IImageBuffer *pbuf,
...
private:
    ...
    //
    int                                m_onlineTuning;
    // We support two dump in position, refer to AinrOnlineDumpInPos
    int                             m_dumpInPos;
    // 0: force BPC off; 1: force BPC on;
    int                             m_BPCForceSel;
    ...
    std::shared_ptr<IWorkBufPool>      m_RgPool;
    std::shared_ptr<IWorkBufPool>      m_GbPool;

    // Processed IMGO
    std::vector<sp<IImageBuffer>>      m_vImgo;
    mutable std::mutex                 m_AllocImgoMx;
    std::condition_variable            m_AllocImgoCond;
    volatile bool                      mb_AllocImgoDone;
    bool                               mb_needProcImgo;

    /* value = 1 + FEATURE_NVRAM_AIISP_ISO_APU_Part1_RSV08_T
     * 0: Not MDLA out
     * 1: MDLA out 16 bits + DRC enable
     * 2: MDLA out 12 bits
     * 3: MDLA out 16 bits + DGN/4
     * others: violation
     */
    int32_t                           m_MDLAMode;
    ...
    std::shared_ptr<IWorkBufPool>      m_upkPool;
    ...
    volatile bool                      mb_upBaseDone;

    // Core mutex and condition
    mutable std::mutex                 m_coreInitMx;
    std::condition_variable            m_coreInitCond;
    volatile bool                      mb_coreInitDone;
