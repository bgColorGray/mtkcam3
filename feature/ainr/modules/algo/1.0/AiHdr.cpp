...
AiHdr::AiHdr()
    ...
    m_coreDrv = decltype(m_coreDrv)(MTKHDRCore::createInstance(DRV_HDRCORE_OBJ_SW),
            [](MTKHDRCore* core) {
                CAM_ULOGM_TAGLIFE("HDR core destroy");
...
void AiHdr::init(const AiParam &params,
    ...
    {
        CAM_ULOGM_TAGLIFE("HDR core init");
        std::lock_guard<std::mutex> __lk(mNRCoreMx);
...
enum AinrErr AiHdr::doNrCore(const AiParam &params, const AiCoreParam &coreParam) {
    ...
    if (coreParam.expInfos.size() !=
            static_cast<std::size_t>(mCaptureNum)) {
        ainrLogF("expInfos(%zu) size not correct", coreParam.expInfos.size());
        return AinrErr_UnexpectedError;
    }

    // sortedPairs container size should be capture number
    if (coreParam.sortedPairs.size() !=
            static_cast<std::size_t>(mCaptureNum)) {
        ainrLogF("sortedPairs(%zu) size not correct",
