...
void AiAlgo::init(const AiParam &params,
        std::shared_ptr<IAinrNvram> nvram) {
    mNvram = nvram;
...

    {
        ...
        std::lock_guard<std::mutex> __lk(mNRCoreMx);

        mNRCoreDrv = decltype(mNRCoreDrv)(MTKNRCore::createInstance(DRV_NRCORE_OBJ_SW),
                [](MTKNRCore* nrCore) {
        ...
        if (CC_UNLIKELY(mNRCoreDrv == nullptr))
        {
            ainrLogF("mNRCoreDrv is null");
            return;
        }
        ...
        debugInfo.uniqueKey = params.uniqueKey;
        debugInfo.requestNum = params.requestNum;
        debugInfo.frameNum = params.frameIndex;
        // TODO: core number should be queried
        rInitIn.u4CoreNum   = 8;
        rInitIn.u4Width     = params.width;
        rInitIn.u4Height    = params.height;
        rInitIn.u4AlgoIdx   = indexModelInfo.val;
        rInitIn.u4FrameNum  = params.captureNum;
        rInitIn.u4TileMode  = 1;  // Force tile mode, algo should define enum

        /* pass NVRAM information to NRCore to let it can choose model
         * with suitable tile size
         */
        rInitIn.tuningBuffer.fusion = apuTuning;
        rInitIn.tuningBuffer.postSw = postTuning;

        mNRCoreDrv->NRCoreInit(&rInitIn, nullptr);

        // Update basic information
        mCaptureNum = params.captureNum;

        // prepare IWorkBufPool
        {
            // query size/cnt
            NRCORE_SET_PROC_INFO_IN_STRUCT  proc_info;
            mNRCoreDrv->NRCoreFeatureCtrl(NRCORE_FTCTRL_GET_PROC_INFO ,
                    nullptr, &proc_info);

            if (proc_info.u4Num > 0) {
                CAM_ULOGM_TAGLIFE("MDLA in/out buffer allocate");
                std::shared_ptr<IIonDevice> ion_device =
                    IIonDeviceProvider::get()->makeIonDevice("AinrAPU", 0);
                mMDLAInOutPool =
                    IWorkBufPool::createInstance(ion_device, MTRUE);
                IWorkBufPool::BufferInfo buf_info(
                    proc_info.rBuf[0].u4Size, 1, eImgFmt_BLOB);
                mMDLAInOutPool->allocBuffers(buf_info, false, proc_info.u4Num);
            }
        }
...
enum AinrErr AiAlgo::doNrCore(const AiParam &params, const AiCoreParam &coreParam) {
    ...
    std::lock_guard<std::mutex> __lk(mNRCoreMx);
    if (CC_UNLIKELY(mInitDone == false)) {
        ainrLogF("mInitDone is false");
        return AinrErr_UnexpectedError;
    }
    ...
    rProc2In.i4OutRawBitDepth =
        (params.mdlaMode == MTK_FEATURE_AINR_MDLA_MODE_NNOUT_16BIT)
        ? NRCORE_OUT_RAW_16_BIT : NRCORE_OUT_RAW_12_BIT;

    rProc2Out.u4OutSize = unPackSize;

    NRCore_VerInfo rCoreVer;
    mNRCoreDrv->NRCoreFeatureCtrl(NRCORE_FTCTRL_GET_VERSION , nullptr, &rCoreVer);
    mNRCoreDrv->NRCoreFeatureCtrl(NRCORE_FTCTRL_GET_PROC_INFO , nullptr, &rGetProcInfoOut);
    ...
    std::vector< sp<IImageBuffer> >        vWorkingIonBuf;
    {
        ...
        for (int i = 0; i < rSetProcInfoIn.u4Num; i++)
        {
            ...
            sp<IImageBuffer> working = mMDLAInOutPool->acquireBufferFromPool();
    ...
    //
    mNRCoreDrv->NRCoreFeatureCtrl(NRCORE_FTCTRL_SET_PROC_INFO, &rSetProcInfoIn, nullptr);

    // N frame mv_unpackRaws
    for (int i = 0; i < mCaptureNum; i++)
    {
    ...
    {
        CAM_ULOGM_TAGLIFE("Core processing");
        ainrLogI("Execute NR core in tile mode+");
        mNRCoreDrv->NRCoreMain(NRCORE_PROC2, &rProc2In, &rProc2Out);
        ainrLogI("Execute NR core in tile mode-");
    }

    {
        ...
        for(auto && buf : vWorkingIonBuf) {
            mMDLAInOutPool->releaseBufferToPool(buf);
            buf = nullptr;
        }
        vWorkingIonBuf.clear();
        mMDLAInOutPool = nullptr;
    }
...
enum AinrErr AiAlgo::doDRC(const IAiAlgo::drcParam &params) {
    ...
    // get face infomation from RootNode
    IMetadata::IEntry entryFaceRects = halMeta->entryFor(MTK_FEATURE_FACE_RECTANGLES);
    if(entryFaceRects.count() > 0) {
        MtkCameraFaceMetadata *pCloneDetectedFaces = &fdData.facedata;
        pCloneDetectedFaces->number_of_faces = entryFaceRects.count();
        for (size_t i = 0;i < pCloneDetectedFaces->number_of_faces; i++)
        {
            MRect faceRect = entryFaceRects.itemAt(i, Type2Type<MRect>());
            pCloneDetectedFaces->faces[i].rect[0] = faceRect.p.x;
            pCloneDetectedFaces->faces[i].rect[1] = faceRect.p.y;
            pCloneDetectedFaces->faces[i].rect[2] = faceRect.s.w;
            pCloneDetectedFaces->faces[i].rect[3] = faceRect.s.h;
        }
        ainrLogD("Number of faces(%d)", pCloneDetectedFaces->number_of_faces);
    } else {
        ainrLogD("There is no face information");
    }
    ...
    // Query LCE gain and AE target
    if (!IMetadata::getEntry<MINT32>(dynamicMeta,
            MTK_3A_FEATURE_AE_METER_FD_TARGET, aeInfo.i4LCETarget)) {
        ainrLogF("cannot get MTK_3A_FEATURE_AE_METER_FD_TARGET");
        return AinrErr_UnexpectedError;
    }

    if (!IMetadata::getEntry<MINT32>(dynamicMeta,
            MTK_3A_FEATURE_AE_METER_FD_LINK_TARGET, aeInfo.i4AETarget)) {
        ainrLogF("cannot get MTK_3A_FEATURE_AE_METER_FD_LINK_TARGET");
        return AinrErr_UnexpectedError;
    }
    ainrLogD("LCE gain(%d), AE target(%d)", aeInfo.i4LCETarget, aeInfo.i4AETarget);
    ...
    if (hal3a.get()) {
        AE_HDR_Setting aeHdrInfo;
        hal3a->send3ACtrl(E3ACtrl_GetAIHDRExpSetting,  (MINTPTR)&aeHdrInfo, 0);

        auto buildAeInfo = [](const AE_EV_Setting &input,
                DRC_IMG_AE_INFO* aeInfo) {
            aeInfo->s8Exposuretime = input.u4Exposuretime_us;
            aeInfo->u4SensorGain = input.u4SensorGain;
            aeInfo->u4DGNGain = input.u4DGNGain;

        };

        buildAeInfo(aeHdrInfo.sNE_Preview, &(aeInfo.sNE_Preview));
        buildAeInfo(aeHdrInfo.sNE_Capture, &(aeInfo.sNE_Capture));
        buildAeInfo(aeHdrInfo.sLE, &(aeInfo.sLE));
        buildAeInfo(aeHdrInfo.sSE, &(aeInfo.sSE));

        aeInfo.u4EVidx = aeHdrInfo.u4EVidx;
        aeInfo.i4LENEEvdiff = aeHdrInfo.i4LENEEvdiff;
        aeInfo.i4NESEEvdiff = aeHdrInfo.i4NESEEvdiff;
        aeInfo.i4FaceEVdiff = aeHdrInfo.i4FaceEVdiff_real;
        aeInfo.i4TouchEVdiff = aeHdrInfo.i4TouchEVdiff;
        aeInfo.i4EVbarEVdiff = aeHdrInfo.i4EVbarEVdiff;
    } else {
        ainrLogW("IHal3A is nullptr");
    }
    ...
    {
        CAM_ULOGM_TAGLIFE("DRC_PROC_STATISTICS");
    ...
    {
        CAM_ULOGM_TAGLIFE("DRC_PROC_APPLY");
        ainrLogD("DRC+");
...
enum AinrErr AiAlgo::applyShadingWb(const int openId, NSCam::IImageBuffer* inBuf, NSCam::IImageBuffer* outBuf,
    ...
    IMetadata::setEntry<MINT32>(&inMetaSet.halMeta, MTK_ISP_AINR_MDLA_MODE,
            MTK_FEATURE_AINR_MDLA_MODE_DRCOUT_16BIT);
    ...
    if (rTuningParam.pLsc2Buf == nullptr)
        ainrLogF("no lsc buf...");
...
enum AinrErr AiAlgo::doProcessedRaw(
            int32_t openId,
            NSCam::IImageBuffer* pInRawBuf,
            NSCam::IImageBuffer* pOutRawBuf,
            NSCam::IMetadata* pInAppMeta,
            NSCam::IMetadata* pInHalMeta) {
    // Normal stream
    using uPtrNormalStream
            = std::unique_ptr<INormalStream, std::function<void(INormalStream*)>>;
    MUINT8 ispProfile = NSIspTuning::EIspProfile_AIISP_MainBPC;
    ...
    src.mPortID = PORT_IMGI;
    src.mPortID.group = 0;
    src.mBuffer = pInRawBuf;
    frameParams.mvIn.push_back(src);

    Output dst;
    dst.mPortID = PORT_TIMGO;
    dst.mPortID.group = 0;
    dst.mBuffer = pOutRawBuf;
    frameParams.mvOut.push_back(dst);

    // timgo special select
    MUINT32 timgoType
            = static_cast<MUINT32>(NSCam::NSIoPipe::EDIPTimgoDump_AFTER_DGN);
    ExtraParam extra;
    extra.CmdIdx = EPIPE_TIMGO_DUMP_SEL_CMD;  // In p2 driver timgo is a kind of debug port
    extra.moduleStruct = reinterpret_cast<void*>(&timgoType);
    frameParams.mvExtraParam.push_back(extra);
    ...
    if (mDumpWorking & AinrDumpPartial) {
        int requestNum = 0;
        if (!IMetadata::getEntry<MINT32>(pInHalMeta
                , MTK_PIPELINE_REQUEST_NUMBER, requestNum)) {
            ainrLogW("Get MTK_PIPELINE_REQUEST_NUMBER fail");
        }
        ...
        int64_t timestamp = 0; // set to 0 if not found.
#if MTK_CAM_DISPAY_FRAME_CONTROL_ON
        if (!IMetadata::getEntry<MINT64>(pInHalMeta
                , MTK_P1NODE_FRAME_START_TIMESTAMP, timestamp)) {
            ainrLogW("cannot get MTK_P1NODE_FRAME_START_TIMESTAMP");
        }
    ...
    NS3Av3::MetaSet_T inMetaSet(*(pInAppMeta), *(pInHalMeta));
    ...
    // BPCI if needed
    if (rTuningParam.pBpc2Buf) {
        Input bpcBuffer;
        bpcBuffer.mPortID = PORT_BPCI;
        bpcBuffer.mPortID.group = 0;
        bpcBuffer.mBuffer = reinterpret_cast<IImageBuffer*>(rTuningParam.pBpc2Buf);
        frameParams.mvIn.push_back(bpcBuffer);
    }
    ...
    enqueParams.mpfnCallback = [](QParams& rParams)->MVOID
    {
        ...
        std::lock_guard<std::mutex> __l(pAsync->getLocker());
        pAsync->notifyOne();
        ainrLogD("Raw2Raw pass2 BPC done");
    };

    // Start to enque request to pass2
    {
        CAM_ULOGM_TAGLIFE("RAW2Raw BPC processing");
        auto __l = p2Async.uniqueLock();
        MBOOL bEnqueResult = stream->enque(enqueParams);
        if (CC_UNLIKELY(!bEnqueResult)) {
            ainrLogF("Raw2Raw stream enque fail");
            ...
            ainrLogD("Wait for Raw2Raw BPC processing....");
            p2Async.wait(std::move(__l));
        }
    }
