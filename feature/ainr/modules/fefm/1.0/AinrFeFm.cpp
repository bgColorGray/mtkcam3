        ...
        std::vector<int> *matrix, MetaSet_T &metaset)
{
    ...
    // therefore we need to setIsp
    {
        ...
        int32_t p2InFmt = NSIspTuning::ERaw2Yuv;
        if (inputBuff->getImgFormat() == eImgFmt_MTK_YUV_P010) {
            p2InFmt = NSIspTuning::EYuv2Yuv;
        }
        ainrLogD("MTK_ISP_P2_IN_IMG_FMT:%d", p2InFmt);
        IMetadata::setEntry<MINT32>(&metaset.halMeta, MTK_ISP_P2_IN_IMG_FMT,
                p2InFmt);
...
enum AinrErr AinrFeFm::doFe(const IAinrFeFm::DataPackage *package)
{
    ...
    //
    QParams qParams = {};
    prepareFE(qParams, package->inBuf, package->outBuf, nullptr,
            nullptr /* color matrix*/, metaset);

    // Set up normalPipe callback
    qParams.mpCookie = static_cast<void*>(&m_p2Async);
    qParams.mpfnCallback = [](QParams& rParams)->MVOID
    {
        ...
        pAsync->notifyOne();
        ainrLogI("FE pass2 done");
    };

    // Start to enque request to pass2
    {
        auto __l = m_p2Async.uniqueLock();
...
enum AinrErr AinrFeFm::waitFeDone() {
    auto __l = m_p2Async.uniqueLock();
    ainrLogI("Wait doFeo");
    m_p2Async.wait(std::move(__l));
    ainrLogI("doFeo done");
