...
AinrNvram::AinrNvram()
    : m_ispProfile(EIspProfile_AIISP_NR_Capture_Before_Blend_12b_NNOut)
{
...
//-----------------------------------------------------------------------------
NSIspTuning::EIspProfile_T AinrNvram::queryIspProfile(IAinrNvram::ConfigParams& params)
{
    if (params.featureType == AIHDR)
        return EIspProfile_AIISP_HDR_Capture_Before_Blend_16b_DRCOut;

    /* For AINR, parse rsv08 @
     * EIspProfile_AIISP_NR_Capture_Before_Blend_12b_NNOut
     */
    std::shared_ptr<char> chunk = loadInterpolationChunk(params.iSensorId,
                NSIspTuning::EModule_AIISP_ISO_APU_Part1,
                NVRAM_AIISP_ISO_APU_Part1_SIZE,
                params.iso,
                EIspProfile_AIISP_NR_Capture_Before_Blend_12b_NNOut);
    if (CC_UNLIKELY(chunk == nullptr)) {
        ainrLogF("get NVRAM CHUNK fail");
            return EIspProfile_AIISP_NR_Capture_Before_Blend_12b_NNOut;
    }
    using ApuNvram = FEATURE_NVRAM_AIISP_ISO_APU_Part1_T;
    ApuNvram *data = reinterpret_cast<ApuNvram*>(chunk.get());

    /* only support NN16b and NN12b for AINR so far *
     * please refer to m_MDLAMode @ 2.0/ainrcore.h
     */
    auto reg = data->rsv08.val + 1;
    if (reg == MTK_FEATURE_AINR_MDLA_MODE_NNOUT_16BIT)
        return EIspProfile_AIISP_NR_Capture_Before_Blend_16b_NNOut;
    else
        return EIspProfile_AIISP_NR_Capture_Before_Blend_12b_NNOut;
}
...
enum AinrErr AinrNvram::init(IAinrNvram::ConfigParams& params)
{
    ...
    m_ispProfile = queryIspProfile(params);
    ...
    if (IMetadata::getEntry<IMetadata::Memory>(params.halMeta,
            MTK_ISP_ATMS_MAPPING_INFO, mappingInfo)) {
        // structure check
        constexpr auto mappingInfoSize = sizeof(m_mappingInfo);
        if (CC_UNLIKELY(mappingInfoSize != mappingInfo.size())) {
            ainrLogF("mapping size not sync(%zu, %zu)",
                    mappingInfoSize, mappingInfo.size());
            return AinrErr_UnexpectedError;
        }
        ::memcpy(&m_mappingInfo, mappingInfo.array(), sizeof(m_mappingInfo));

        int magicNum = 0;
        if (!IMetadata::getEntry<MINT32>(params.halMeta,
                MTK_P1NODE_PROCESSOR_MAGICNUM, magicNum)) {
            ainrLogW("Get MTK_P1NODE_PROCESSOR_MAGICNUM fail");
        }
        ainrLogD("MappingInfo magicNum(%d)", magicNum);
    } else {
        ainrLogW("Get MTK_ISP_ATMS_MAPPING_INFO fail, we get the latest");
        uint32_t sensorDev = SENSOR_DEV_NONE;
        IHalSensorList* const pHalSensorList = MAKE_HalSensorList();
        if (CC_UNLIKELY(pHalSensorList == nullptr)) {
            ainrLogF("%s: get IHalSensorList instance failed", __FUNCTION__);
        ...
        pMgr->getMappingInfo(static_cast<ESensorDev_T>(sensorDev), m_mappingInfo, magicNo);
    }
...
std::shared_ptr<char> AinrNvram::loadInterpolationChunk(int32_t openId,
    ...
    // Setup mappingInfo
    memcpy(&interpoInfo.mapping_info, &m_mappingInfo, sizeof(interpoInfo.mapping_info));

    // profile information
    CAM_IDX_QRY_COMB &mappingInfo = interpoInfo.mapping_info;
    mappingInfo.eFlash = EFlash_No;
    mappingInfo.eIspProfile = ispProfile;
...
std::shared_ptr<char> AinrNvram::loadInterpolationChunk(int32_t openId,
        NSIspTuning::EModule_T moduleId, size_t size, int32_t iso) {
    return loadInterpolationChunk(openId, moduleId, size, iso, m_ispProfile);
}
...
int AinrNvram::queryMDLAMode(void) const
{
    size_t chunkSize = 0;
    const char *pChunk = getChunk(&chunkSize);
    if (pChunk == nullptr) {
        ainrLogE("%s: read NVRAM failed, use AINR 12bit...", __FUNCTION__);
        return MTK_FEATURE_AINR_MDLA_MODE_NNOUT_12BIT;
    }

    const FEATURE_NVRAM_AIISP_ISO_APU_Part1_T *data =
        reinterpret_cast<const FEATURE_NVRAM_AIISP_ISO_APU_Part1_T*>(pChunk);

    return (data->rsv08.val + 1);
}
