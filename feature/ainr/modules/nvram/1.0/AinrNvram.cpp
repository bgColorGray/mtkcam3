    ...
    { // SpinLock locked
        ...
        //if (s_nvram_chunks[key].get() == nullptr) {
            ...
            if (IMetadata::getEntry<IMetadata::Memory>(params.halMeta,
                ...
                constexpr auto mappingInfoSize = sizeof(interpoInfo.mapping_info);
                if (CC_UNLIKELY(mappingInfoSize != mappingInfo.size())) {
                    ainrLogF("mapping size not sync(%zu, %zu)",
                            mappingInfoSize, mappingInfo.size());
                    goto lbErr;
                }
                ::memcpy(&interpoInfo.mapping_info, mappingInfo.array(), sizeof(interpoInfo.mapping_info));
                ...
                ainrLogW("Cannot get MTK_ISP_ATMS_MAPPING_INFO, therefore we get it from idxMgr");
