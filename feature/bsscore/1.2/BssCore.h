...
// BSS exec data, stored by doBssExec() and recover by doBss
struct BSSExecData {
    bool                        isValid;
    MINT32                      sensorId;
    MINT32                      uniqueKey;
    BSS_INPUT_DATA_G            bss_input;
    ...
    std::map<MINT32, MINT32>    exifData;
    int                         frameNumToSkip;
    BSSExecData () : isValid{false}, sensorId{-1}, uniqueKey{}, bss_input{}, bss_param{}, bssOutData{}, bssOrder{}, bssFdData{}, fdData{}, exifData{}, frameNumToSkip{} { };
    ~BSSExecData () { bssOrder.clear(); bssFdData.clear(); fdData.clear(); exifData.clear(); };

    void clone(BSSExecData& cloneInfo)
    {
        cloneInfo.isValid       = this->isValid;
        cloneInfo.uniqueKey     = this->uniqueKey;
        cloneInfo.bss_input     = this->bss_input;
        cloneInfo.bss_param     = this->bss_param;
        cloneInfo.bssOutData    = this->bssOutData;
        cloneInfo.bssOrder      = this->bssOrder;
        cloneInfo.bssFdData     = this->bssFdData;
        cloneInfo.fdData        = this->fdData;
        cloneInfo.exifData      = this->exifData;
        cloneInfo.frameNumToSkip= this->frameNumToSkip;
    }
};
//
static NSCam::FleetingQueue<BSSExecData, 30, 2> mBSSExecDataQueue;
...
public:
    ...
    MBOOL   doZipWithBss(IImageBuffer* input, IImageBuffer* output, MUINT32 ImgOutSize, IMetadata* pAppMeta, IMetadata* pAppMetaOut, IMetadata* pHalMeta);

private:
    MBOOL doBssExec(Vector<RequestPtr>& rvReadyRequests, BSSExecData& execData);
    MBOOL doBssRestore(Vector<RequestPtr>& rvReadyRequests, BSSExecData& execData);
    ...
    MBOOL appendBSSInput(Vector<RequestPtr>& rvRequests, BSS_INPUT_DATA_G& bss_input, vector<MTKBSSFDInfo>& bss_fddata, vector<FD_DATATYPE>& storeFdData);
    ...
    MINT32 queryBssTuningIndex(IMetadata* pHalMeta);
    MVOID updateAIShutterInfo(RequestPtr& mainRequests, BSS_PARAM_STRUCT& p);
    MVOID setScenarioInfo(IMetadata* pHalMeta, NSCam::TuningUtils::scenariorecorder::ResultParam& decParm);
