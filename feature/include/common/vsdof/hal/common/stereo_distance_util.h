...
public:
        ...
        , BUFFER_SRC(source)
    {
        ...
        //Get AF table
        // Notice: this operation may fail due to init order,
        //         if that happens, we'll get AF table later in runtime
        __getAFTable();
    ...
    {
        if((mpAFTable = __getAFTable()) == nullptr) {
            ALOGI("[StereoDistnace] AF table is not ready");
            return 0.0f;
        }
...
private:
    NS3Av3::DAF_TBL_STRUCT * __getAFTable()
    {
        if(mpAFTable)
        {
            return mpAFTable;
        ...
        if(NULL == pHal3A) {
            ...
            if(NULL == mpAFTable) {
                ALOGE("Cannot get AF table now");
            }
            else
            {
                ALOGD("Get AF table success");
            }
...
private:
    ...
    protected:
        ...
        virtual void _logResult(const int TARGET_DEPTH, const float FB)
        {
            ...
            {
                mLogger->FastLogD("%s", "============= Depth Histogram in AF ROI =============");
