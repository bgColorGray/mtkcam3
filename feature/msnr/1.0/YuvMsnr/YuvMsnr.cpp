...
#define MSS_YUV_IN_START 1
#define MSF_BUF_COUNT 7
#define MSF_YUV_IN_START 1
...
//-----------------------------------------------------------------------------
// YuvMsnrCore implementation
//-----------------------------------------------------------------------------
YuvMsnrCore::YuvMsnrCore()
    : mRegTableSize(0)
    , mNddDumpL1(-1)
    , mNddDumpL2(-1)
    , mIsInited(false)
...
//-----------------------------------------------------------------------------
enum MsnrErr YuvMsnrCore::init(IMsnr::ConfigParams const& params)
{
    CAM_ULOGM_APILIFE();
    ...
    mMssAllocThread = std::async(std::launch::async, &YuvMsnrCore::allocateMssBuf, this);
    // Allocate MSF working buffer
    mMsfAllocThread = std::async(std::launch::async, &YuvMsnrCore::allocateMsfBuf, this);
    // prepare Tuning working buffer
    mTunPrepareThread = std::async(std::launch::async, &YuvMsnrCore::prepareTunBuf, this);
...
enum MsnrErr YuvMsnrCore::doMsnr()
{
    CAM_ULOGM_APILIFE();
    enum MsnrErr err = MsnrErr_Ok;
    ...
    }
    {
        ...
        mssconfig.mss_scenario = MSS_BASE_S1_MODE; // yuv in
        ...
        for(int stage = MSF_BUF_COUNT-1; stage >= 0; stage--) {
            ...
            /* execute pass 2 operation */
            {
                ...
                // output: main yuv
                {
                    ...
                    Output p;
                    p.mBuffer          = (stage == 0)?outputBuffer:mvMsfBuf.at(stage).get();
        ...
        if (mNddDumpL1 == 1 || mNddDumpL2 == 1) {
            for (int i = 0; i < mvMsfBuf.size(); i++) {
                if (i == 0) {
                    ...
                    const char* pUserString = str.c_str();
                    NddDump(outputBuffer, msfDumpHint.at(i), YUV_PORT_NULL, pUserString);
...
enum MsnrErr YuvMsnrCore::makeDebugInfo(IMetadata* metadata)
{
    CAM_ULOGM_APILIFE();
#if 0
    bool haveExif = false;
...
enum MsnrErr YuvMsnrCore::checkParams(IMsnr::ConfigParams const& params)
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::allocateMssBuf()
{
    CAM_ULOGM_FUNCLIFE();
    ...
    for(int i = 0; i < MSS_BUF_COUNT; i++) {
        ...
        // s0 is already provided
        if (i < MSS_YUV_IN_START)
            continue;
...
enum MsnrErr YuvMsnrCore::deallocateMssBuf()
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::allocateMsfBuf()
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::deallocateMsfBuf()
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::deallocateDebugBuf()
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::prepareTunBuf()
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::releaseTunBuf()
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::generateMsfNDDHint(const IMetadata* pInHalMeta, int stage, FILE_DUMP_NAMING_HINT& hint)
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::generateMssNDDHint(const IMetadata* pInHalMeta, FILE_DUMP_NAMING_HINT& hint)
{
    CAM_ULOGM_FUNCLIFE();
...
enum MsnrErr YuvMsnrCore::generateP2NDDHint(const IMetadata* pInHalMeta, FILE_DUMP_NAMING_HINT& hint)
{
    CAM_ULOGM_FUNCLIFE();
    ...
    hint.IspProfile = getIspProfile(-1);
...
inline EIspProfile_T YuvMsnrCore::getIspProfile(int stage)
{
    CAM_ULOGM_FUNCLIFE();
    ...
    switch(stage) {
        case -1:
            return fnQueryISPProfile(eStage_MSNR_DCE, EIspProfile_Capture_DCE);
        break;
        case 0:
            return fnQueryISPProfile(eStage_MSNR_MSF_Y0, EIspProfile_Capture_MSF_YUV_0);
        break;
        case 1:
            return fnQueryISPProfile(eStage_MSNR_MSF_1, EIspProfile_Capture_MSF_1);
        break;
        case 2:
            return fnQueryISPProfile(eStage_MSNR_MSF_2, EIspProfile_Capture_MSF_2);
        break;
        case 3:
            return fnQueryISPProfile(eStage_MSNR_MSF_3, EIspProfile_Capture_MSF_3);
        break;
        case 4:
            return fnQueryISPProfile(eStage_MSNR_MSF_4, EIspProfile_Capture_MSF_4);
        break;
        case 5:
            return fnQueryISPProfile(eStage_MSNR_MSF_5, EIspProfile_Capture_MSF_5);
        break;
        case 6:
            return fnQueryISPProfile(eStage_MSNR_MSF_6, EIspProfile_Capture_MSF_6);
        break;
        default:
...
inline int32_t YuvMsnrCore::getIspStage(int32_t iterationId)
{
    switch(iterationId) {
        case -1:
...
void YuvMsnrCore::generateNDDHintFromMetadata(FILE_DUMP_NAMING_HINT& hint, const IMetadata* pMetadata)
{
    CAM_ULOGM_FUNCLIFE();
