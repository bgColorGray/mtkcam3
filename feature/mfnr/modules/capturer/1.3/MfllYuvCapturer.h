...
public:
    ...
    enum MfllErr    queueSourceMssImage(sp<IMfllImageBuffer>& img) override;

    enum MfllErr    queueSourceDcesoBuf(sp<IMfllImageBuffer>& buf) override;
...
public:
    ...
    /* mutex/cond for Mss S1 */
    mutable std::mutex              m_mutexMssS1;
    vector< sp<IMfllImageBuffer> >  m_vMssS1;

    /* mutex/cond for Mss S1 */
    mutable std::mutex              m_mutexDceso;
    vector< sp<IMfllImageBuffer> >  m_vDceso;
