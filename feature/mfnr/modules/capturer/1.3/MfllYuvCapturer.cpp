...
public:
    void onCaptureYuvQ(MfllEventStatus_t& status, MfllCore* c, int index)
    {
        ...
        constexpr const int MSS_S1       = SourceImageSyncPack::MSS_S1;
        constexpr const int DCESO        = SourceImageSyncPack::DCESO;
        ...
        if (status.ignore || status.err != MfllErr_Ok) {
            ...
            mfllLogE("%s index < 0: %d, skip this", __FUNCTION__, index);
            return;
        }
        ...
        SourceImageSyncPack& pack = m_pCapturer->m_srcSyncs[index];
        {
            ...
            while (true) {
                ...
                mfllLogD3("waiting, status(fullSrc,resizedY8,mss,dceso,GMV)=(%d,%d,%d,%d,%d)",
                        pack.bReadyFlags[FULLSIZE_SRC],
                        pack.bReadyFlags[RESIZED_Y8],
                        pack.bReadyFlags[MSS_S1],
                        pack.bReadyFlags[DCESO],
        ...
        if (m_pCapturer->m_vMssS1[index]->isInited()) {
            std::lock_guard<std::mutex> lk(m_pCapturer->m_mutexMssS1);
            c->m_imgMssYuvs[index].images[1].setImgBufLocked(m_pCapturer->m_vMssS1[index]);
            m_pCapturer->m_vMssS1[index] = nullptr;

            mfllLogD3("%s: Mss S1 addr=%p", __FUNCTION__, c->m_imgMssYuvs[index].images[1].imgbuf.get());
            mfllLogD3("%s: size=%dx%d, format=%d(mfll)",
                    __FUNCTION__,
                    c->m_imgMssYuvs[index].images[1].imgbuf->getWidth(), c->m_imgMssYuvs[index].images[1].imgbuf->getHeight(),
                    c->m_imgMssYuvs[index].images[1].imgbuf->getImageFormat()
                    );
        } else {
            mfllLogD3("%s: By-Pass Mss S1 enque, due to input buffer is not inited", __FUNCTION__);
        }
        if (m_pCapturer->m_vDceso[index]->isInited()) {
            std::lock_guard<std::mutex> lk(m_pCapturer->m_mutexDceso);
            c->m_imgDcesoBuf[index].setImgBufLocked(m_pCapturer->m_vDceso[index]);
            m_pCapturer->m_vDceso[index] = nullptr;

            mfllLogD3("%s: DCESO addr=%p", __FUNCTION__, c->m_imgDcesoBuf[index].imgbuf.get());
            mfllLogD3("%s: size=%dx%d, format=%d(mfll)",
                    __FUNCTION__,
                    c->m_imgDcesoBuf[index].imgbuf->getWidth(), c->m_imgDcesoBuf[index].imgbuf->getHeight(),
                    c->m_imgDcesoBuf[index].imgbuf->getImageFormat()
                    );

        } else {
            mfllLogD3("%s: By-Pass DCESO enque, due to input buffer is not inited", __FUNCTION__);
        }
        {
...
)
{
    ...
    if (CC_UNLIKELY(requestNo >= MFLL_MAX_FRAMES))
...
enum MfllErr MfllYuvCapturer::queueSourceMssImage(sp<IMfllImageBuffer>& img)
{
    size_t cnt = 0;
    /* push_back element */
    {
        std::lock_guard<std::mutex> __l(m_mutexMssS1);
        cnt = m_vMssS1.size();
        m_vMssS1.push_back(img);
    }

    /* mark ready and notify condition_variable */
    {
        std::lock_guard<std::mutex> __l(m_srcSyncs[cnt].mx);
        m_srcSyncs[cnt].markReadyLock(SourceImageSyncPack::MSS_S1);
        m_srcSyncs[cnt].cv.notify_one();
    }
...
enum MfllErr MfllYuvCapturer::queueSourceDcesoBuf(sp<IMfllImageBuffer>& buf)
{
    size_t cnt = 0;
    /* push_back element */
    {
        std::lock_guard<std::mutex> __l(m_mutexDceso);
        cnt = m_vDceso.size();
        m_vDceso.push_back(buf);
    }

    /* mark ready and notify condition_variable */
    {
        std::lock_guard<std::mutex> __l(m_srcSyncs[cnt].mx);
        m_srcSyncs[cnt].markReadyLock(SourceImageSyncPack::DCESO);
        m_srcSyncs[cnt].cv.notify_one();
    }
