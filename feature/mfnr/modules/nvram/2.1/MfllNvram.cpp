...
static bool doQueryIndex(IMfllNvram::ConfigParams& params)
{
    ...
    }
    {
        ...
        if (params.iSensorMode > 0)
            rMapping_Info.eSensorMode = static_cast<NSIspTuning::ESensorMode_T>(params.iSensorMode);
    ...
    mfllLogD3("mappingInfo sensorId(%d), iSensorMode(%d), flashOn(%d) to index BSS(%d), Mfnr(%d), MfnrTh(%d)"
        , params.iSensorId, params.iSensorMode, params.bFlashOn, params.iQueryIndex_Bss, params.iQueryIndex_Mfnr, params.iQueryIndex_MfnrTh);
...
std::shared_ptr<char> MfllNvram::chunkMfnr(size_t *bufferSize)
{
    if (getChunk(bufferSize) != NULL) {
        ...
        std::shared_ptr<char> __chunk(new char[size], std::default_delete<char[]>());
