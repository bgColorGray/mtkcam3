    ...
    for(size_t index = 0; index < rQParam.mvFrameParams.size(); ++index) {
        ...
        for ( size_t index2 = 0; index2 < frameParam.mvIn.size(); ++index2 ) {
            ...
            if ( data.mBuffer != NULL ) {
                ...
                mfllLogI("%s: mvIn.mBuffer->getImageBufferHeap = %p", __FUNCTION__, reinterpret_cast<void*>(data.mBuffer->getImageBufferHeap()) );
...
{
    if (base == NULL) {
        mfllLogE("%s: base cannot be NULL", __FUNCTION__);
        return MfllErr_NullPointer;
    }

    if (ref == NULL) {
        mfllLogE("%s: ref cannot be NULL", __FUNCTION__);
        return MfllErr_NullPointer;
    }

    if (out == NULL) {
        mfllLogE("%s: out cannot be NULL", __FUNCTION__);
        return MfllErr_NullPointer;
    }

    if (wt_out == NULL) {
        mfllLogE("%s: wt_out cannot be NULL", __FUNCTION__);
        return MfllErr_NullPointer;
    }
    enum MfllErr err = MfllErr_Ok;
    MBOOL bRet = MTRUE;
     ...
     */
    {
        ...
        /* input: source frame */
        {
            ...
            mfllLogI("input(base): va/pa(%p/%p) fmt(%#x), size=(%d,%d), planes(%zu), stride(%zu)",
                    // va/pa
        ...
        /* input: reference frame */
        {
            ...
            mfllLogI("input(ref): va/pa(%p/%p) fmt(%#x), size=(%d,%d), planes(%zu), stride(%zu)",
                    // va/pa
...
{
    if (src == 0 || dst == 0) {
        mfllLogE("%s: src and dst argument cannot be NULL", __FUNCTION__);
        return MfllErr_BadArgument;
    }
    enum MfllErr err = MfllErr_Ok;
    MBOOL bRet = MTRUE;
