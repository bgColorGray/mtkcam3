        ...
        const IMfllImageBuffer_Flag_t &flag /* = Flag_Undefined */)
{
    ...
    if (q.get() != nullptr) {
        ...
        if (p == nullptr) {
            mfllLogF("MfllImageBufferQueue accquireBuffer returns nullptr!!");
        } else {
    ...
    )
{
    ...
    m_bufferName = bufferName ? bufferName : "MfllImageBuffer";
    ...
    if (m_bHasOwnership && !commonDump) {
        ...
        for (size_t i = 0; i < m_imgBuffer->getPlaneCount(); i++) {
            ...
            for (size_t h = 0; h < planeHeight; h++) {
                ...
                if (fwrite((void*)addr, 1L, lineSize, fp) != lineSize) {
                    mfllLogW("fwrite element number is wrong");
                }
...
enum MfllErr MfllImageBuffer::loadFile(const char *name)
{
    ...
    }
    {
        ...
        if (fread(addr, 1L, bufferSize, fp) != bufferSize) {
            mfllLogW("fread element number is wrong");
        }
    }
...
MUINT32 MfllImageBuffer::mapImageFormat(const enum ImageFormat &fmt, bool dontCare /* = false */)
{
    ...
    case ImageFormat_YUV_P010:
        ...
        break;
    case ImageFormat_YUV_P012:
        f = eImgFmt_MTK_YUV_P012;
        break;
    case ImageFormat_YUV_P012_UP:
        f = eImgFmt_YUV_P012;
        break;
...
enum ImageFormat MfllImageBuffer::mapImageFormat(const MUINT32 &fmt, bool dontCare /* = false */)
{
    ...
    case eImgFmt_MTK_YUV_P012:
        f = ImageFormat_YUV_P012;
        break;
    case eImgFmt_YUV_P012:
        f = ImageFormat_YUV_P012_UP;
        break;
    case eImgFmt_NV16:
        f = ImageFormat_Nv16;
        break;
...
void MfllImageBuffer::dumpDebugLog(const char* name)
{
    ...
    mfllLogD("%s: getImgFormat() = %x, getImgSize().w = %d, getImgSize().h = %d, getPlaneCount() = %zu"
        , name
        , m_imgBuffer->getImgFormat()
    ...
    for (int i = 0 ; i < m_imgBuffer->getPlaneCount() ; i++) {
        mfllLogD("%s: getBufStridesInBytes(%d) = %zu, getBufSizeInBytes(%d) = %zu, getBufVA(%d) = 0x%p, getBufPA(%d) = 0x%p"
            , name
            , i, m_imgBuffer->getBufStridesInBytes(i)
            , i, m_imgBuffer->getBufSizeInBytes(i)
            , i, (void*)m_imgBuffer->getBufVA(i)
            , i, (void*)m_imgBuffer->getBufPA(i)
        );
    }
}
