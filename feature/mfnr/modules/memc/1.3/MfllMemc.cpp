...
enum MfllErr MfllMemc::motionEstimation()
{
    ...
    if (CC_UNLIKELY(pChunk == NULL)) {
        ...
        param.Proc1_noise_level = (MUINT32)pNvram->me_noise_lv;
        {
            ...
            if (bssData != nullptr) {
                IFDContainer::eFDContainer_Opt fdOPT = (m_pCore->getIspTuningHint() == MTK_CONTROL_CAPTURE_HINT_FOR_ISP_TUNING_DEFAULT_NONE)?
                    IFDContainer::eFDContainer_Opt_Read:IFDContainer::eFDContainer_ISP_Opt_Read;
                mfllLogD3("fdOPT:0x%d", fdOPT);
        ...
        // For On-line device tuning, re-load me params
        if (mfll::MfllProperty::readProperty(mfll::Property_LoadMeParam, 0) == 1) {
            mfllLogI("%s: ME param re-load", __FUNCTION__);

            #define REPLACE_ME_PARAM_FROM_PROPERTY(var)\
                do { \
                    string propString = string(mfll::PropertyString[mfll::Property_LoadMeParam]); \
                    propString += "." + string(#var); \
                    param.var = static_cast<decltype(param.var)>(property_get_int32(propString.c_str(), static_cast<int32_t>(param.var))); \
                    mfllLogI("%s: reload, %s = %u", __FUNCTION__, propString.c_str(), static_cast<MUINT32>(param.var)); \
                } while (0)

            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_bad_range);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_bad_TH);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_noise_level);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_large_mv_thd);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_large_mv_SAD_thd);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_large_mv_ratio);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_large_mv_txtr_en);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_large_mv_txtr_wei);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_large_mv_txtr_thd);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_en);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_mv_ratio);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_face_ratio_ThL);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_face_ratio_ThH);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_full_ratio_ThL);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_full_ratio_ThH);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_edge_ccl_Th0);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_edge_ccl_Th1);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_edge_cclnum_Th);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_edge_FDAreaThL);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_refine_edge_FDAreaThH);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_lcl_deconf_en);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_lcl_deconf_noise_lv);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_lcl_deconf_bg_bss_ratio);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_lcl_deconf_fd_bss_ratio);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_lcl_deconf_dltvar_en);
            REPLACE_ME_PARAM_FROM_PROPERTY(Noise_lvl);
            REPLACE_ME_PARAM_FROM_PROPERTY(Proc1_me_blink_eye_en);

            #undef REPLACE_ME_PARAM_FROM_PROPERTY
        }
...
#   endif
    ...
    auto dump2Binary = [this](MINT32 uniqueKey, MINT32 requestNum, MINT32 frameNum, const char* buf, size_t size, string fn) -> bool {
        ...
        char filepath[256] = {0};
        int n = snprintf(filepath, sizeof(filepath)-1, "%s/%09d-%04d-%04d-""%s", MFLLMEMC_DUMP_PATH, uniqueKey, requestNum, frameNum, fn.c_str());
        if (n < 0) {
            mfllLogW("snprintf failed writing MEMC dump name, filepath:%s", filepath);
        }
