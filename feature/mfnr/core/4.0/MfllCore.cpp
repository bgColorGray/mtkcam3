...
#if MFLL_LOG
#define PYRAMID_NAME(name, i, j)  (string(name) + to_char(i) + string(".") + to_char(j)).c_str()
#else
#define PYRAMID_NAME(name, i, j)
#endif
...
const char* IMfllCore::getEventTypeName(enum EventType e)
{
    ...
    if (e >= (unsigned int)EventType_Size)
    ...
    , m_downscaleDivisor(16)
{
    ...
    for (unsigned int i = 0; i < MFLL_MAX_FRAMES; i++) {
        ...
        lockSyncObject(&m_syncAllocMss[i]);
        lockSyncObject(&m_syncCapturedYuvQ[i]);
        lockSyncObject(&m_syncCapturedMss[i]);
...
enum MfllErr MfllCore::doMfll()
{
    ...
    if (CC_LIKELY( m_mrpMode != MrpMode_BadPerformance )) {
        /* thread to do Mss */
        tellsFutureMss();
        ...
        /* thread for mss->blending->mixing->NR->Postview->JPEG */
        tellsFuturePhase2_Mss();
    }
...
enum MfllErr MfllCore::doCancel()
{
    ...
    m_bypass.bypassAllocWeighting[2] = 1;
    ...
    MfllMotionVector        mv)
{
    ...
    if (m_bypass.bypassEncodeQYuv[requestNo]) {
        ...
        pCapturer->queueSourceImage(sourceImg);
        pCapturer->queueSourceMonoImage(quarterImg);
        pCapturer->queueSourceMssImage(mssImg);
        pCapturer->queueSourceDcesoBuf(dceso);
        pCapturer->queueGmv(mv);
    ...
    if (getBlendFrameNum() >= nums) {
        ...
        for (size_t i = (nums <= 0 ? 0 : nums-1); i < MFLL_MAX_FRAMES; i++) {
            ...
            if (i+1 < MFLL_MAX_FRAMES) {
                m_bypass.bypassEncodeMss[i+1] = 1;
                m_bypass.bypassAllocMssBuffer[i+1] = 1;
...
enum MfllErr MfllCore::setIspTuningHint(const int ISP_TuningHint)
{
    enum MfllErr err = MfllErr_Ok;
    ...
    m_ispTuningHint = ISP_TuningHint;

lbExit:
    pthread_mutex_unlock(&m_mutexShoot);
    if (err == MfllErr_Shooted)
        mfllLogW("MFLL has shooted, cannot set ISP stage");
    return err;
}

sp<IMfllImageBuffer> MfllCore::retrieveBuffer(const enum MfllBuffer &s, int index /* = 0 */)
{
    return retrievePyramidBuffer(s, index , 0);
}

sp<IMfllImageBuffer> MfllCore::retrievePyramidBuffer(const enum MfllBuffer &s, int index, int stage)
{
    enum MfllErr err = MfllErr_Ok;
    ...
    case MfllBuffer_Rrzo:
        ...
        break;
    case MfllBuffer_MssYuv:
        if (CC_UNLIKELY(index < 0 || index >= static_cast<int>(getCaptureFrameNum()))) {
            mfllLogE("%s: get mss buffer but index is wrong (index, max) = (%d, %d)", __FUNCTION__, index, getCaptureFrameNum());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        if (CC_UNLIKELY(stage < 0 || stage >= static_cast<int>(m_imgMssYuvs[index].getSize()))) {
            mfllLogE("%s: get mss buffer but stage is wrong (stage, max) = (%d, %zu)", __FUNCTION__, stage, m_imgMssYuvs[index].getSize());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = m_imgMssYuvs[index].images[stage].getImgBufLocked();
        break;
    ...
    case MfllBuffer_BaseYuv:
        if (CC_UNLIKELY(m_ptrImgYuvBase && (stage < 0 || stage >= static_cast<int>(m_ptrImgYuvBase->getSize())))) {
            mfllLogE("%s: get msf_base buffer but stage is wrong (stage, max) = (%d, %zu)", __FUNCTION__, stage, m_ptrImgYuvBase->getSize());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = (m_ptrImgYuvBase ? m_ptrImgYuvBase->images[stage].getImgBufLocked() : nullptr);
        break;
    case MfllBuffer_ReferenceYuv:
        if (CC_UNLIKELY(m_ptrImgYuvRef && (stage < 0 || stage >= static_cast<int>(m_ptrImgYuvRef->getSize())))) {
            mfllLogE("%s: get msf_ref buffer but stage is wrong (stage, max) = (%d, %zu)", __FUNCTION__, stage, m_ptrImgYuvRef->getSize());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = (m_ptrImgYuvRef ? m_ptrImgYuvRef->images[stage].getImgBufLocked() : nullptr);
        break;
    case MfllBuffer_BlendedYuv:
        if (CC_UNLIKELY(m_ptrImgYuvBlended && (stage < 0 || stage >= static_cast<int>(m_ptrImgYuvBlended->getSize())))) {
            mfllLogE("%s: get msf_out buffer but stage is wrong (stage, max) = (%d, %zu)", __FUNCTION__, stage, m_ptrImgYuvBlended->getSize());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = (m_ptrImgYuvBlended ? m_ptrImgYuvBlended->images[stage].getImgBufLocked() : nullptr);
        break;
    ...
    case MfllBuffer_WeightingIn:
        if (CC_UNLIKELY(m_ptrImgWeightingIn && (stage < 0 || stage >= static_cast<int>(m_ptrImgWeightingIn->getSize())))) {
            mfllLogE("%s: get msf_wt_in buffer but stage is wrong (stage, max) = (%d, %zu)", __FUNCTION__, stage, m_ptrImgWeightingIn->getSize());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = (m_ptrImgWeightingIn ? m_ptrImgWeightingIn->images[stage].getImgBufLocked() : nullptr);
        break;
    case MfllBuffer_WeightingOut:
        if (CC_UNLIKELY(m_ptrImgWeightingOut && (stage < 0 || stage >= static_cast<int>(m_ptrImgWeightingOut->getSize())))) {
            mfllLogE("%s: get msf_wt_out buffer but stage is wrong (stage, max) = (%d, %zu)", __FUNCTION__, stage, m_ptrImgWeightingOut->getSize());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = (m_ptrImgWeightingOut ? m_ptrImgWeightingOut->images[stage].getImgBufLocked() : nullptr);
        break;
    case MfllBuffer_WeightingDs:
        if (CC_UNLIKELY(m_ptrImgWeightingDs && (stage < 0 || stage >= static_cast<int>(m_ptrImgWeightingDs->getSize())))) {
            mfllLogE("%s: get msf_wt_ds buffer but stage is wrong (stage, max) = (%d, %zu)", __FUNCTION__, stage, m_ptrImgWeightingDs->getSize());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = (m_ptrImgWeightingDs ? m_ptrImgWeightingDs->images[stage].getImgBufLocked() : nullptr);
        break;
    default:
...
enum MfllErr MfllCore::releaseBuffer(const enum MfllBuffer &s, int index)
{
    enum MfllErr err = MfllErr_Ok;
    ...
    case MfllBuffer_WeightingOut:
        ...
        break;
    case MfllBuffer_WeightingDs:
        if (m_ptrImgWeightingDs) (*m_ptrImgWeightingDs) = 0;
        break;
    default:
        mfllLogE("%s: unsupport buffer %d", __FUNCTION__, (int)s);
        err = MfllErr_NotSupported;
...
void MfllCore::updateImageBufferQueueLimit()
{
    ...
    //int maxWeighting = 3*MFLL_PYRAMID_SIZE; // fix to 2
    ...
    //m_imgQueue.weighting    = IMfllImageBufferQueue::getInstance(Flag_WeightingTable); // don't use due to pyramid
...
ImageFormat MfllCore::getInputYuvFmtMss() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_YUV_P012;
        default: break;
    }
...
ImageFormat MfllCore::getInputYuvFmtMssSn() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_YUV_P012_UP;
        default: break;
    }
    mfllLogE("%s: m_internalYuvFmt is undefined", __FUNCTION__);
    *(volatile uint32_t*)(0x00000000) = 0xDEADFEED;
...
ImageFormat MfllCore::getInputYuvFmtMsfDL() const
{
    switch (m_internalYuvFmt.load()) {
...
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_Sta8;
        //case InputYuvFmt_Nv21:
        //case InputYuvFmt_Nv12:       return ImageFormat_Sta8;
        default: break;
    }
    mfllLogE("%s: m_internalYuvFmt is undefined", __FUNCTION__);
    *(volatile uint32_t*)(0x00000000) = 0xDEADFEED;
...
/**
 *  Encode YUV to Pyramid YUVs. This operation must be invoked after QYuv Encode
 *  while these conditions are matched:
 *
 *  1. BSS has been done
 */
enum MfllErr MfllCore::doEncodeMss(void *void_index)
{
    unsigned int index = (unsigned int)(long)void_index;
    enum MfllErr err = MfllErr_Ok;
    MfllEventStatus_t status;

    syncWaitDone(&m_syncCapturedYuvQ[index]);
    if (index)
    ...
    /* invokes event */
    m_event->onEvent(EventType_CaptureMss, status, this, (void*)(long)index);

    if (m_bypass.bypassEncodeMss[index] || status.ignore) {
        mfllLogD("%s: Bypass encode Mss(%d)", __FUNCTION__, index);
        status.ignore = 1;
    }
    else {
        err = do_EncodeMss(void_index);
        if (err != MfllErr_Ok) {
            status.ignore = 1;
        }
    }
...
lbExit:
    status.err = err;
    syncAnnounceDone(&m_syncCapturedMss[index]);
    m_event->doneEvent(EventType_CaptureMss, status, this, (void*)(long)index);
...
/**
 *  Multi-Scale Fusion using MTK hardware, furthurmore, operation fusion depends on that
 *  the previous fusion output. Therefore, the conditions of this operaion are:
 *
 *  1. Pyramid YUV working buffer for blending is ready.
 *  2. The i-th motion compensation has been done.
 *  3. The (i-1)-th fusion has been done.
 */
enum MfllErr MfllCore::doMsf(void *void_index)
{
    ...
    /* conditions */
    syncWaitDone(&m_syncCapturedMss[index]);
    syncWaitDone(&m_syncCapturedMss[index+1]);
    ...
    if (m_blendedCount > 0) {
        ...
        swap<PyramidImages*>(m_ptrImgYuvBase, m_ptrImgYuvBlended);
        swap<PyramidImages*>(m_ptrImgWeightingIn, m_ptrImgWeightingOut);
    } else {
        m_ptrImgYuvBase = &(m_imgMssYuvs[index]);
        m_ptrImgYuvBlended = &(m_imgMsfBlended[0]);
        m_ptrImgWeightingIn = &(m_imgWeightings[0]);
        m_ptrImgWeightingOut = &(m_imgWeightings[1]);
    }

    m_ptrImgYuvRef = &(m_imgMssYuvs[index+1]);
    m_ptrImgWeightingDs = &(m_imgWeightings[2]);

    //last blending
    if (getFrameCapturedNum() == index+2) {
        ...
        //
        m_ptrImgYuvBlended = &m_imgMsfBlended[2];
        if (getMixingBufferType() != MixYuvType_Working) {
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        m_ptrImgYuvBlended->images[0].imgbuf = m_ptrImgYuvMixed->imgbuf;
    }
...
lbExit:
    ...
    {
        mfllLogD3("YUV and QYUV buffer of index(%u) is released", index);
        /* release buffer */
        m_imgYuvs[index + 1].releaseBufLocked();
        m_imgQYuvs[index + 1].releaseBufLocked();
        m_imgMssYuvs[index].releaseBufLocked();
        m_imgMssYuvs[index + 1].releaseBufLocked();
    }
    ...
    if (status.ignore == 0 && err == MfllErr_Ok) {
        ...
        if (m_blendedCount == 1) {
            ...
            mfllLogD3("%s: use blended[1] yuv buffer as ping-pong buffer for next round", __FUNCTION__);
            m_ptrImgYuvBase = &(m_imgMsfBlended[1]);
        }
    }
...
{
    const unsigned int index = (unsigned int)(long)void_index;
    enum MfllErr err = MfllErr_NotSupported;

    mfllFunctionIn();
    mfllTraceCall();

    mfllLogE("Not support for doBlending(%d)", index);
...
enum MfllErr MfllCore::doMixing(JOB_VOID)
{
    ...
    mfllLogD("%s: updae jpeg exifinfo.", __FUNCTION__);
...
enum MfllErr MfllCore::doAllocMssBuffer(void *void_index)
{
    unsigned int index = (unsigned int)(long)void_index;
    enum MfllErr err = MfllErr_Ok;
    MfllEventStatus_t status;
    ...
    /* thread-safe operation lock */
    std::lock_guard<std::mutex> lk(m_opLockDoAllocMssBuffer[index]);

    mfllFunctionIn();
    mfllTraceCall();

    /* trigger events */
    m_event->onEvent(EventType_AllocateMssBuffer, status, this, (void*)(long)index);


    if (m_bypass.bypassAllocMssBuffer[index] || status.ignore) {
        mfllLogD3("%s: Bypass allocate Mss buffers", __FUNCTION__);
        status.ignore = 1;
    }
    else {
        err = do_AllocMssBuffer(void_index);
    }

lbExit:
    status.err = err;
    m_event->doneEvent(EventType_AllocateMssBuffer, status, this, (void*)(long)index);
    m_bypass.bypassAllocMssBuffer[index] = 1; // ignore the next round
    syncAnnounceDone(&m_syncAllocMss[index]);
    mfllFunctionOut();
    return err;
}
...
enum MfllErr MfllCore::doAllocWeighting(void *void_index)
{
    ...
    /* index should be smaller than 3 or it's the critical condition */
    if (CC_UNLIKELY(index >= 3)) {
...
void MfllCore::tellsFutureAllocateMemory()
{
    ...
    {
        MFLL_THREAD_NAME("thread_allocate");
        ...
        if (bIsMultiFrame) {
            ...
            /* weighting table 3 */
            doAllocWeighting((void*)(long)2);
        }

        /* allocate Mss buffers */
        for (int i = 0; i < blendNum; i++)
            doAllocMssBuffer((void*)(long)i);
        for (int i = blendNum; i < captureNum; i++) {
            m_bypass.bypassAllocMssBuffer[i] = 1;
            doAllocMssBuffer((void*)(long)i);
        }
...
void MfllCore::tellsFutureAllocateMemoryLite()
{
    std::lock_guard<std::mutex> __l(m_futureMemory.mx);
    ...
    m_futureMemory.fu = std::async(std::launch::async, [this]()->void
    {
        ...
        mfllLogD("future allocate memory lite +");
...
void MfllCore::tellsFuturePhase2()
{
    ...
    m_futurePhase2.fu = std::async(std::launch::async, [this]()->void
    {
        ...
        MfllOperationSync::getInstance()->removeJob(MfllOperationSync::JOB_MSF);

        doMixing(reinterpret_cast<intptr_t>(nullptr));

        mfllLogD("future phase2 -");
    });
}

void MfllCore::tellsFuturePhase2_Mss()
{
    std::lock_guard<std::mutex> __l(m_futurePhase2_Mss.mx);

    if (m_futurePhase2_Mss.fu.valid())
        return;

    m_futurePhase2_Mss.fu = std::async(std::launch::async, [this]()->void
    {
        MFLL_THREAD_NAME("thread_phase2_s");

        mfllLogD("future phase2 seq +");

        int times = static_cast<int>(getBlendFrameNum() - 1);

        //do MSS(0) first for MSF(0)'s Base input
        doEncodeMss((void*)(long)0);

        for (int i = 0; i < times; i++) {
            //do MSS(i+1) for MSF(i)'s Ref input
            doEncodeMss((void*)(long)(i+1));

            //do MSF(i) for next MSF(i+1)'s Base input
            doMsf((void*)(long)i);
        }

        MfllOperationSync::getInstance()->removeJob(MfllOperationSync::JOB_MSS);
        MfllOperationSync::getInstance()->removeJob(MfllOperationSync::JOB_MSF);
...
void MfllCore::tellsFutureMss()
{
    std::lock_guard<std::mutex> __l(m_futureMss.mx);

    if (m_futureMss.fu.valid())
        return;

    m_futureMss.fu = std::async(std::launch::async, [this]()->void
    {
        MFLL_THREAD_NAME("thread_mss");

        mfllLogD("future mss +");

        int times = static_cast<int>(getBlendFrameNum());

        for (int i = 0; i < times; i++)
            doEncodeMss((void*)(long)i);
