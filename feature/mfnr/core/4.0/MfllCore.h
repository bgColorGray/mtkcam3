...
#define MFLL_IMAGE_FORMAT_MSS               getInputYuvFmtMss()
#define MFLL_IMAGE_FORMAT_MSS_SN            getInputYuvFmtMssSn()
#define MFLL_IMAGE_FORMAT_MSF_DL            getInputYuvFmtMsfDL()
...
/* TO-DO: move to pyramidimages' header file*/

class PyramidImages
{
public:
    PyramidImages(void) { m_Size = 0; };
    ~PyramidImages(void) { };

    void setSize(size_t t) { m_Size = t; };
    size_t getSize(void) { return m_Size; };

    vector<IMfllImageBuffer *> getBuffers() {
        std::lock_guard<std::mutex> __l(this->locker);

        vector<IMfllImageBuffer *> ret;

        if (m_Size > MFLL_MAX_PYRAMID_SIZE)
            return ret;

        ret.resize(m_Size);
        for (size_t i = 0 ; i < ret.size(); i++)
            ret[i] = images[i].getImgBufLocked().get();
...
public:
    ...
    {
        std::lock_guard<std::mutex> __l(this->locker);
        for (size_t i = 0 ; i < MFLL_MAX_PYRAMID_SIZE ; i++)
            images[i].releaseBufLocked();
    }

    PyramidImages& operator=(PyramidImages* pBuffer)
    {
        for (size_t i = 0 ; i < MFLL_MAX_PYRAMID_SIZE ; i++)
            this->images[i].imgbuf = (pBuffer)?pBuffer->images[i].imgbuf:nullptr;

        return *this;
    }

public:
    ImageBufferPack images[MFLL_MAX_PYRAMID_SIZE];
...
protected:
    ...
    std::mutex          m_opLockDoAllocMssBuffer[MFLL_MAX_FRAMES];
    ...
    std::mutex          m_opLockDoAllocWeighting[3];
...
public:
    ...
    PyramidImages       m_imgMssYuvs[MFLL_MAX_FRAMES]; // saves mss yuv buffers
    ImageBufferPack     m_imgDcesoBuf[MFLL_MAX_FRAMES]; // saves DCESO buffer from P2A
    ...
    PyramidImages       m_imgMsfBlended[3]; // ping-pong buffers, takes [0], [1], output takes [2]
    PyramidImages       m_imgWeightings[3]; // ping-pong buffers takes [0], [1], ds takes [2]
     ...
     *  1st:    m_imgMssYuvs[0]    + m_imgMssYuvs[1] = m_imgMsfBlended[0]    (ping)
     *  2nd:    m_imgMsfBlended[0] + m_imgMssYuvs[2] = m_imgMsfBlended[1]    (pong)
     *  3rd:    m_imgMsfBlended[1] + m_imgMssYuvs[3] = m_imgMsfBlended[0]    (ping)
     *  ...
     *  last:   m_imgMsfBlended[x] + m_imgMssYuvs[n] = m_imgMsfBlended[2]    (output)
     *
     *  Hence here we use these pointers to indicate the REAL meanful buffers.
     */
    PyramidImages*      m_ptrImgYuvBase; // indicates to full size base YUV
    PyramidImages*      m_ptrImgYuvRef; // indicates to any size reference YUV
    PyramidImages*      m_ptrImgYuvBlended; // indicates to blended YUV (same size with YUV base)
    ...
    PyramidImages*      m_ptrImgWeightingIn; // indicates to input of weighting table during blending
    PyramidImages*      m_ptrImgWeightingOut; // indicates to output of weighting table during blending
    PyramidImages*      m_ptrImgWeightingDs; // indicates to ds of weighting table during blending
...
public:
    ...
    MfllSyncObj_t m_syncAllocMss[MFLL_MAX_FRAMES]; // annonced done while Mss buffers are allocated
    ...
    MfllSyncObj_t m_syncCapturedMss[MFLL_MAX_FRAMES]; // annonced done while a single mss is captured
...
public:
    ...
    virtual enum MfllErr queueYuvCapture(int32_t requestNo, sp<IMfllImageBuffer>& sourceImg, sp<IMfllImageBuffer>& quarterImg, sp<IMfllImageBuffer>& mssImg, sp<IMfllImageBuffer>& dceso, MfllMotionVector mv);
    ...
    virtual sp<IMfllImageBuffer> retrievePyramidBuffer(const enum MfllBuffer &s, int index = 0, int stage = 0);
    ...
    virtual enum MfllErr setIspTuningHint(const int ISP_TuningHint);
    virtual int  getIspTuningHint(void) { return m_ispTuningHint; }
...
public:
    ...
    virtual ImageFormat getInputYuvFmtMss() const;
    virtual ImageFormat getInputYuvFmtMssSn() const;
    virtual ImageFormat getInputYuvFmtMsfDL() const;
...
public:
    ...
    virtual enum MfllErr doEncodeMss(void *void_index);
    virtual enum MfllErr doMsf(void *void_index);
...
/* Buffer allocations */
    ...
    virtual enum MfllErr doAllocMssBuffer(void *void_index);
...
protected:
    ...
    virtual enum MfllErr do_EncodeMss(void* void_index);
    virtual enum MfllErr do_Msf(void* void_index);
    ...
    virtual enum MfllErr do_AllocMssBuffer(void* void_index);
