...
enum MfllErr MfllCore::do_AllocRawBuffer(void *void_index)
{
...

    {
        ...
        if (!pImg->isInited()) {
            ...
            mfllLogD3("%s: raw buffer %u is inited, ignore here", __FUNCTION__, index);
        }
    }
...

    {
        ...
        if (bNeedResizedImg) {
            if (!pImg->isInited()) {
                ...
                mfllLogD3("%s: QYUV buffer %u re-init(%dx%d)", __FUNCTION__, index, m_width, m_height);
        ...
        /* full size YUV, and it's been inited */
            ...
            mfllLogD3("%s: full size YUV buffer %u re-init(%dx%d)", __FUNCTION__, index, m_width, m_height);
            pFullSizeImg->setAligned(16, 16);
            pFullSizeImg->setResolution(m_width, m_height);
            pFullSizeImg->convertImageFormat(MFLL_IMAGE_FORMAT_YUV_FULL);
        }
...
enum MfllErr MfllCore::do_AllocWeighting(void *void_index)
{
...

    {
        ...
        if (!pImg->isInited()) {
            ...
            mfllLogD3("%s: YUV weighting table(%u) buffer re-init(%dx%d)", __FUNCTION__, index, m_width, m_height);
            pImg->setResolution(m_width, m_height);
            // OMC_MFB need 16 align
            pImg->setAligned(16, 16);
            pImg->convertImageFormat(MFLL_IMAGE_FORMAT_WEIGHTING);
