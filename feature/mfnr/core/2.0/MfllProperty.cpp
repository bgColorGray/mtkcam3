    ...
    if (0 == strcmp(str, "-1")) {
        ...
        while (pch != NULL)
        {
            ...
            if (setting_count%2 != 0) {
                mfllLogD("%s: GMV[%zu].x=%d", __FUNCTION__, frame_count, value);
                globalMv[frame_count].x = value;
            }
            else {
                mfllLogD("%s: GMV[%zu].y=%d", __FUNCTION__, frame_count, value);
                globalMv[frame_count].y = value;

                frame_count++;
                if (frame_count >= MFLL_MAX_FRAMES) {
                    mfllLogW("%s: out of globalMv's MFLL_MAX_FRAMES:%zu", __FUNCTION__, frame_count);
                    break;
                }
            }
