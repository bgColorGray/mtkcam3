...

    {
        ...
        if (pImg == NULL) {
            ...
            if (CC_UNLIKELY(pImg == NULL)) {
                mfllLogE("%s: create IMfllImageBuffer(%u) failed", __FUNCTION__, index);
                err = MfllErr_CreateInstanceFailed;
                goto lbExit;
            }
        ...
        if (!pImg->isInited()) {
            ...
            if (CC_UNLIKELY(err != MfllErr_Ok)) {
                mfllLogE("%s: init raw buffer(%u) failed", __FUNCTION__, index);
                err = MfllErr_UnexpectedError;
                goto lbExit;
            }
        }
        else {
            mfllLogD("%s: raw buffer %u is inited, ignore here", __FUNCTION__, index);
        }
    }
...
enum MfllErr MfllCore::do_AllocQyuvBuffer(void *void_index)
{
...

    {
        ...
        if (pImg == NULL) {
            ...
            if (CC_UNLIKELY(pImg == NULL)) {
                mfllLogE("%s: create QYUV buffer instance (%u) failed", __FUNCTION__, index);
                err = MfllErr_CreateInstanceFailed;
                goto lbExit;
            }
        ...
        if (m_isFullSizeMc && pFullSizeImg == NULL) {
            ...
            if (CC_UNLIKELY(pFullSizeImg == NULL)) {
                mfllLogE("%s: create full size YUV buffer instance (%u) failed", __FUNCTION__, index);
                err = MfllErr_CreateInstanceFailed;
                goto lbExit;
            }
        ...
        if (!pImg->isInited()) {
            ...
            err = pImg->initBuffer();
            if (CC_UNLIKELY(err != MfllErr_Ok)) {
                mfllLogE("%s: init QYUV buffer(%u) failed", __FUNCTION__, index);
                goto lbExit;
            }
        }
        else {
            mfllLogD("%s: QYuv %u has been inited, ignored here", __FUNCTION__, index);
        ...
        if (m_isFullSizeMc && pFullSizeImg->isInited() == false) {
            ...
            if (CC_UNLIKELY(err != MfllErr_Ok)) {
                mfllLogE("%s: init full size YUV buffer (%u) failed", __FUNCTION__, index);
                goto lbExit;
            }
        }
        /* full size YUV, and it's been inited */
        else if (m_isFullSizeMc){
            mfllLogD("%s: full yuv %u has been inited, ignored here", __FUNCTION__, index);
            pFullSizeImg->setAligned(16, 16);
...
enum MfllErr MfllCore::do_AllocWeighting(void *void_index)
{
...

    {
        ...
        if (!pImg->isInited()) {
            ...
            mfllLogD("%s: weighting table %u is inited, ignored here", __FUNCTION__, index);
        }
    }
...
enum MfllErr MfllCore::do_AllocMemcWorking(void *void_index)
{
    ...
    {
        unsigned int bufferIndex = index % getMemcInstanceNum();
        ...
        if (CC_UNLIKELY(pImg == NULL)) {
            ...
            if (pImg == NULL) {
                mfllLogE("%s: create MEMC working buffer(%u) instance failed", __FUNCTION__, bufferIndex);
                err = MfllErr_CreateInstanceFailed;
                goto lbExit;
            }
        ...
        if (!pImg->isInited()) {
            ...
            if (CC_UNLIKELY(memc.get() == NULL)) {
                mfllLogE("%s: memc instance(index %u) is NULL", __FUNCTION__, bufferIndex);
                err = MfllErr_NullPointer;
                goto lbExit;
            }
            size_t bufferSize = 0;
            ...
            err = pImg->initBuffer();
            if (CC_UNLIKELY(err != MfllErr_Ok)) {
                mfllLogE("%s: init MEMC working buffer(%u) failed", __FUNCTION__, bufferIndex);
                err = MfllErr_UnexpectedError;
                goto lbExit;
            }
        }
            ...
            mfllLogD("%s: memc buffer %u is inited, ignored here", __FUNCTION__, bufferIndex);
        }
    }
