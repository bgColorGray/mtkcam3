...
enum MfllErr MfllCore2::releaseBuffer(const enum MfllBuffer& s,
                                      int index /* = 0 */) {
  MfllErr err = MfllErr_Ok;

  if (index < 0) {
    mfllLogE("index out of range, index<0 ");
...
sp<IMfllImageBuffer> MfllCore2::retrieveBuffer(const enum MfllBuffer& s,
                                               int index /* = 0 */) {
  MfllErr err = MfllErr_Ok;
...
enum MfllErr MfllCore2::do_MotionEstimation(void* void_index) {
...

  {
    ...
    if (CC_UNLIKELY(memc.get() == NULL)) {
      mfllLogE("%s: MfllMemc is necessary to be created first (index=%d)",
               __FUNCTION__, index);
    ...
    if (CC_UNLIKELY(err != MfllErr_Ok)) {
      mfllLogE("%s: allocate MEMC working buffer(%d) failed", __FUNCTION__,
               memcIndex);
      err = MfllErr_UnexpectedError;
      goto lbExit;
    }
    ...
    if (CC_UNLIKELY(err != MfllErr_Ok)) {
      ...
      mfllLogE("%s: IMfllMemc::motionEstimation failed, returns %d",
...
enum MfllErr MfllCore2::do_Blending(void* void_index) {
...

  {
    ...
    /**
     * while index == 0, which means the first time to blend, the input
     * weighting table should be sent
     */
    err = m_spMfb->blend(
        m_ptrImgYuvBase->imgbuf.get(), m_ptrImgYuvRef->imgbuf.get(),
        confmap.get(), m_ptrImgYuvBlended->imgbuf.get(),
...
android::sp<IMfllImageBuffer> MfllCore2::createConfidenceMap(size_t width,
                                                             size_t height) {
