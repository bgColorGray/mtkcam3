...
{
    char str[PROPERTY_STRING_MAX] ="-1";
    ...
    if (0 == strcmp(str, "-1")) {
        ...
        while (pch != NULL)
        {
            ...
            if (setting_count%2 != 0) {
                mfllLogD("%s: GMV[%u].x=%d", __FUNCTION__, frame_count, value);
                globalMv[frame_count].x = value;
            }
            else {
                mfllLogD("%s: GMV[%u].y=%d", __FUNCTION__, frame_count, value);
                globalMv[frame_count].y = value;

                frame_count++;
                if (frame_count >= MFLL_MAX_FRAMES) {
                    mfllLogW("%s: out of globalMv's MFLL_MAX_FRAMES:%u", __FUNCTION__, frame_count);
                    break;
                }
            }
...
void MfllProperty::setProperty(const Property_t &t, const int &v)
{
    ...
    int n = snprintf(value, PROPERTY_VALUE_MAX, "%d", v);
    if (n < 0) {
        mfllLogW("snprintf failed writing property value, value:%s", value);
    }
