...
{
    char str[128];
    int ret = snprintf(str, sizeof(str), "%s cam %d", name, sensorID);
    if( ret < 0) str[0] = '\0';
...
{
    char str[128];
    int ret = snprintf(str, sizeof(str), "%s cam %d MWFrame:#%d MWReq:#%d, frame %d ", name, sensorID, mwFrameID, mwRequestID, frameID);
    if( ret < 0) str[0] = '\0';
...
{
    char str[128];
    int ret = snprintf(str, sizeof(str), "%s cam %d MWFrame:#%d MWReq:#%d, frame %d-%d ", name, sensorID, mwFrameID, mwRequestID, frameID, requestID);
    if( ret < 0) str[0] = '\0';
