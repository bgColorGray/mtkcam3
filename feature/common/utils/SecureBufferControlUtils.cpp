...
MVOID SecureBufferControl::init(const NSCam::SecType &secureType)
{
    std::lock_guard<std::mutex> _m(mMapLock);
    mSecureType = secureType;
    mSecureEnum = toSecEnum(mSecureType);
}

MVOID SecureBufferControl::uninit()
{
    std::lock_guard<std::mutex> _m(mMapLock);
    mMap.clear();
    mSecureType = NSCam::SecType::mem_normal;
    mSecureEnum = toSecEnum(mSecureType);
}

MBOOL SecureBufferControl::needSecure() const
{
    return mSecureType != NSCam::SecType::mem_normal;
}
...
SecureBufferControl::HANDLE_T SecureBufferControl::registerAndGetSecHandle(IImageBuffer *buf, MBOOL isVA)
{
    ...
    if (search != mMap.end()) {
        ...
        if (isVA) {
            ...
            sechandle = (HANDLE_T)(intptr_t)secbuf->getFD(0);
        }
