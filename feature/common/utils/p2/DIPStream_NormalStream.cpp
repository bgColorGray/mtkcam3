...
MBOOL DIPStream_NormalStream::init(char const* szCallerName, NSCam::NSIoPipe::EStreamPipeID StreamPipeID, const DipUsageHint &dipUsageHint)
{
    MBOOL ret = MFALSE;
    //init NormalStream
    DipUserParam dipUserParam = toDipUserParam(dipUsageHint);
    if( ((mNormalStream = NSIoPipe::NSPostProc::INormalStream::createInstance(mSensorIndex, dipUserParam)) != NULL) &&
        (mNormalStream->init(szCallerName, StreamPipeID, dipUsageHint.mSecTag)) != MFALSE )
    {
        ret = MTRUE;
    }
    else
    {
        MY_LOGE("NormalStream(%s : %p) sensor(%u) secTag(%u) init failed", szCallerName, mNormalStream, mSensorIndex, dipUsageHint.mSecTag);
        ret = MFALSE;
        uninit(szCallerName);
    }
    return ret;
...
bool DIPStream_NormalStream::normalStreamEarlyCB(const FrameParams &f, NSIoPipe::EFrameCallBackMsg /*msg*/)
{
    // Cookie convert
    struct CookieExt *pCookieExt = (CookieExt *)f.mpCookie;
    if(pCookieExt != NULL && pCookieExt->mpfnDIPEarlayCallback && pCookieExt->mMagicCheck == MAGIC_PASS_NUM)
    {
        pCookieExt->mpfnDIPEarlayCallback(pCookieExt->mpOriCookie);
    }
    else
    {
        MY_LOGW("FrameParam Early CB cookie return error, pCookieExt = %p", pCookieExt);
        if(pCookieExt != NULL)
            MY_LOGW("MagicCheck = 0x%X, earlyCB(%p)", pCookieExt->mMagicCheck, pCookieExt->mpfnDIPEarlayCallback);
    }
    return true;
}
...
QParams DIPStream_NormalStream::convertToQParams(const DIPParams &dipParams)
{
    ...
    for( const auto &dipFrameParam : dipParams.mvDIPFrameParams )
    {
        ...
        if( dipFrameParam.mpCookie && dipFrameParam.mpfnCallback )
        {
            pCookieExt->mpfnDIPEarlayCallback = dipFrameParam.mpfnCallback;
            frameParam.mpCookie = (MVOID *)pCookieExt;
            frameParam.mpfnCallback = normalStreamEarlyCB;
        }
        ...
        frameParam.Resolution_Width = dipFrameParam.Resolution_Width;
        frameParam.Fps = dipFrameParam.Fps;
...
DIPParams DIPStream_NormalStream::convertToDIPParams(const QParams &qParams)
{
    ...
    for( const auto &frameParam : qParams.mvFrameParams )
    {
        ...
        // No convert mpCookie & mpfnCallback
