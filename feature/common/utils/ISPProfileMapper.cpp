...
)
{
    if (rMappingKey.type == eMappingKey_Proprietary) {
        ...
        MY_LOGD(" No remove feature=0x%" PRIx64 " becasue it is not proprietary", featureID);
    }
    return MTRUE;
}
...
printIspProfileTable()
{
    auto sensorCombtoString = [](ESensorCombination& sensorComb) -> const char*
    {
        switch (sensorComb)
        {
            case eSensorComb_Single:
                return "eSensorComb_Single";
                break;
            case eSensorComb_BayerBayer:
                return "eSensorComb_BayerBayer";
                break;
            case eSensorComb_BayerMono:
                return "eSensorComb_BayerMono";
                break;
            default:
                return "unknown eSensorComb";
                break;
        }
    };

    auto printTable = [&](MBOOL isISPHidl)
    {
        String8 str_titlehead(" ");
        std::map<MappingKey, StageProfileMap> *pMappingItem = NULL;
        if(isISPHidl)
        {
            str_titlehead += String8::format("ISPHidl,");
            pMappingItem = &mMappingItems_ISPHIDL;
        }
        else
        {
            str_titlehead += String8::format("Proprietary,");
            pMappingItem = &mMappingItems;
        }

        std::map<MappingKey, StageProfileMap>::iterator iterator;
        for(iterator = pMappingItem->begin(); iterator != pMappingItem->end() ; ++iterator)
        {
            tuple<MUINT64, ESensorCombination, EProfileMappinScenario> key = iterator->first;
            MUINT64 featureId = std::get<MUINT64>(key);
            ESensorCombination sensorComb = std::get<ESensorCombination>(key);
            // EProfileMappinScenario scenario = std::get<EProfileMappinScenario>(key);
            String8 str_title;
            str_title = str_titlehead + String8::format("\tfeature: 0x%" PRIx64 ",\tSensorCombination: %s", featureId, sensorCombtoString(sensorComb));
            MY_LOGD("----------------------------------------");
            MY_LOGD("%s", str_title.string());
            std::map<EProfileMappingStages, EIspProfile_T>::iterator stageIter;
            for(stageIter = iterator->second.begin(); stageIter != iterator->second.end(); ++stageIter)
            {
                EProfileMappingStages stage = stageIter->first;
                EIspProfile_T profile = stageIter->second;
                MY_LOGD("Stage=%d\tIspProfile=%s(%d)\n", stage, TuningUtils::getIspProfileName(profile), profile);
            }
        }
    };

    MY_LOGD("printIspProfileTable start");
    // Proprietary isp profile table
    printTable(MFALSE);
    // ISPHidl isp profile table
    printTable(MTRUE);
    MY_LOGD("printIspProfileTable end");
}
