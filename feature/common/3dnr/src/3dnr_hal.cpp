...
Hal3dnr *Hal3dnrObj<sensorIdx>::getInstance(char const *userName)
{
    ...
    static Hal3dnrObj<sensorIdx> spInstance(userName);
    return &spInstance;
}
...
// mktodo: make checkIso static function
    ...
    MINT adjustIsoThr = m3dnrIsoThrProp ?  m3dnrIsoThrProp : nr3dHalParam.isoThreshold;

    MY_LOGD_IF(mLogLevel >= 1, "s(%u): fm(%d), r(%4d), f(%4d), uK(%d): iso(%d), nr3dHal_isoThr(%d), final_isoThr(%d)"
        , mSensorIdx, nr3dHalParam.featMask, nr3dHalParam.requestNo, nr3dHalParam.frameNo, nr3dHalParam.uniqueKey
        , nr3dHalParam.iso, nr3dHalParam.isoThreshold, adjustIsoThr);

    if ( nr3dHalParam.iso < adjustIsoThr )
    {
...
MBOOL Hal3dnr::handleFrameZoomAlign(const NR3DHALParam &nr3dHalParam,
    const NR3DMVInfo &gmvInfo, // ori-gmv(pixelx256) for algo
    NR3DHwParam &outNr3dHwParam, NR3DHALResult &nr3dHalResult)
{
    FUNC_START;
    MINT32 nr3dPathID = determine3DNRPath(nr3dHalParam);
    ...
    outNr3dHwParam.vipi_ofst_x_in_pixel = VIPI_OFST_X &~1;    //in pixel
    outNr3dHwParam.vipi_ofst_y_in_pixel = VIPI_OFST_Y &~1;    //in pixel
...
MBOOL Hal3dnr::handleFrameAlign(const NR3DHALParam &nr3dHalParam,
    const NR3DMVInfo &gmvInfo /* nMV + pixel-base */, NR3DHwParam &outNr3dHwParam)
{
    FUNC_START;
    if (nr3dHalParam.vipiInfo.isValid != MTRUE)
    {
    ...
    outNr3dHwParam.dramBitsPerPixel = dramBitsPerPixel;
    outNr3dHwParam.onOff_onOfStX = u4Nr3dOffset_X &~1;
    outNr3dHwParam.onOff_onOfStY = u4Nr3dOffset_Y &~1;
    outNr3dHwParam.onSiz_onWd = imgSizeOvlp.w &~1;
    outNr3dHwParam.onSiz_onHt = imgSizeOvlp.h &~1;

    outNr3dHwParam.vipi_offst = vipi_offst &~1;
    outNr3dHwParam.vipi_readW = imgSizeOvlp.w &~1;    //in pixel
    outNr3dHwParam.vipi_readH = imgSizeOvlp.h &~1;    //in pixel

    outNr3dHwParam.vipi_ofst_x_in_pixel = u4VipiOffset_X &~1;    //in pixel
    outNr3dHwParam.vipi_ofst_y_in_pixel = u4VipiOffset_Y &~1;    //in pixel
...
Hal3dnr::Hal3dnr(const char *userName, const MUINT32 sensorIdx)
    : mSensorIdx(sensorIdx), mUserName(userName ? userName : "")
{
    mDebugEnable = ::property_get_int32(NR3D_PROP_DEBUG_ENABLE, 0);
    mLogLevel = ::property_get_int32(NR3D_PROP_LOG_LEVEL, 0);
    m3dnrIsoThrProp = ::property_get_int32(NR3D_PROP_ISO_THRESHOLD, m3dnrIsoThrProp);
    mSupportZoom3DNR = ::property_get_int32(NR3D_PROP_ZOOM, 1); // zoom_3dnr default ON

    MY_LOGD_IF(mLogLevel >= 1, "User(%s) constructs 3dnr<%u> instance, "
        " Property(debugEnable(%d), loglevel(%d), isoThrProp(%d), supportZoomProp(%d) )",
        mUserName.c_str(), mSensorIdx,
        mDebugEnable, mLogLevel, m3dnrIsoThrProp, mSupportZoom3DNR);
}
    ...
    const NR3DHALParam& nr3dHalParam, NR3DHALResult &nr3dHalResult)
{
    ...
    MBOOL ret = MTRUE;
    nr3dHalResult.nr3dHwParam.ctrl_onEn = 0; // reset to off for later change

    // NOTES: check 3dnrlib-calculated GMV should be done outside Hal3dnr
    if (nr3dHalResult.isGMVInfoUpdated != MTRUE)
    {
        MY_LOGW("!!warn: s(%u): fm(%d), r(%4d), f(%4d), uK(%d): "
            "GMV not updated yet, please call update3DNRMvInfo before do3dnrFlow"
            , mSensorIdx, nr3dHalParam.featMask, nr3dHalParam.requestNo, nr3dHalParam.frameNo, nr3dHalParam.uniqueKey);
    }
...
MBOOL Hal3dnr::update3DNRMvInfo(
    ...
    // output
    NR3D::NR3DMVInfo outGmvInfoResults[], MBOOL &outIsGMVInfoUpdated, IImageBuffer *omcmv
    )
{
    (void) featMask;
    ...
    // --- fill tuning params ---
    tnrSetProcInfo.nvram_param.msyuv_ms5_rsv1 = nr3dHalParam.mNvramInfo.mMS5_RSV1;
    tnrSetProcInfo.nvram_param.msyuv_ms5_rsv2 = nr3dHalParam.mNvramInfo.mMS5_RSV2;
    tnrSetProcInfo.nvram_param.msyuv_ms5_rsv3 = nr3dHalParam.mNvramInfo.mMS5_RSV3;
    tnrSetProcInfo.nvram_param.msyuv_ms5_rsv4 = nr3dHalParam.mNvramInfo.mMS5_RSV4;
    ...
    // --- fill gyroInfo ---
    tnrSetProcInfo.gyro_info.gyro_in_mv = nr3dHalParam.gyroData.gyroInfo.gyro_in_mv;
    tnrSetProcInfo.gyro_info.gyroMvWidth = nr3dHalParam.gyroData.gyroInfo.gyroMvWidth;
    tnrSetProcInfo.gyro_info.gyroMvHeight = nr3dHalParam.gyroData.gyroInfo.gyroMvHeight;
    // --- fill mvInfo ---
    // LMV
    ...
    // dsdn omc mvmap stride
    if (omcmv)
    {
        tnrSetProcInfo.MVMapStrideInByte = omcmv->getBufStridesInBytes(0);
    }
    ...
    else
    {
        ...
        tnrResult.tnr_mv_map = omcmv ? (MINT32*)omcmv->getBufVA(0) : NULL;
    ...
    NR3DHALResult &nr3dHalResult)
{
    ...
    const NR3DHALParam::VipiInfo &vipiInfo = nr3dHalParam.vipiInfo;
    const MRect &dstRect = nr3dHalParam.dstRect;
    if ( dstRect.s == vipiInfo.imgSize )
    {
        // !!NOTES: calGMV: (1) transforms gmvX/Y to pixel-base (2) output as nMV
        NR3DMVInfo calGmvInfo = nr3dHalParam.gmvInfo;
        calGMV(nr3dHalParam, calGmvInfo, nr3dHalResult);
        if ( ! handleFrameAlign(nr3dHalParam, calGmvInfo, outNr3dHwParam) )
        {
    ...
    {
        // === Algo code: start: align vipi/imgi ===
        if ( ! handleFrameZoomAlign(nr3dHalParam, nr3dHalParam.gmvInfo/* ori-gmv(x256) for algo */,
            outNr3dHwParam, nr3dHalResult) )
        {
    ...
    MBOOL isOutOfBound = MFALSE;
    if ( ((outNr3dHwParam.onOff_onOfStX + outNr3dHwParam.onSiz_onWd) > dstRect.s.w) ||
         ((outNr3dHwParam.onOff_onOfStY + outNr3dHwParam.onSiz_onHt) > dstRect.s.h) ||
         (( outNr3dHwParam.vipi_ofst_x_in_pixel + outNr3dHwParam.vipi_readW) > vipiInfo.imgSize.w) ||
         (( outNr3dHwParam.vipi_ofst_y_in_pixel + outNr3dHwParam.vipi_readH) > vipiInfo.imgSize.h) )
    {
        isOutOfBound = MTRUE;
    }
    if ( isOutOfBound || mLogLevel >= 1 )
    {
        char tmpStr[1024];
        const NR3DMVInfo &gmvInfo = nr3dHalParam.gmvInfo;
        const NSCam::NR3D::GyroData &gyroData = nr3dHalParam.gyroData;
        const char *pStrPrefix = isOutOfBound ? "!!err: vipi/img3o misalign: " : "";

        int n = snprintf(tmpStr, sizeof(tmpStr), "s(%u): fm(%d), r(%4d), f(%4d), uK(%d): "
            "3dnr: %sST=%d, path(%d), gmvX/Y-by-pixel(%5d,%5d), int_x/y=(%5d,%5d), confX/Y(%d, %d),"
            "Gyro( valid(%d), value(%f,%f,%f)), isZoom(%d), "
            "vipiOvlp( (%d,%d), %dx%d, full(%dx%d), stride(%d), offstInBytes(%d), fmt(0x%x) ), OnRegion( (%d,%d), %dx%d, full(%dx%d) )"
            , mSensorIdx, nr3dHalParam.featMask, nr3dHalParam.requestNo, nr3dHalParam.frameNo, nr3dHalParam.uniqueKey
            , pStrPrefix, nr3dHalResult.state, (MINT32)determine3DNRPath(nr3dHalParam)
                , gmvInfo.gmvX/LMV_GMV_VALUE_TO_PIXEL_UNIT, gmvInfo.gmvY/LMV_GMV_VALUE_TO_PIXEL_UNIT
                , gmvInfo.x_int, gmvInfo.y_int, gmvInfo.confX, gmvInfo.confY
            , (gyroData.isValid ? 1 : 0), gyroData.x, gyroData.y, gyroData.z, (dstRect.s != vipiInfo.imgSize)
            , outNr3dHwParam.vipi_ofst_x_in_pixel, outNr3dHwParam.vipi_ofst_y_in_pixel
                , outNr3dHwParam.vipi_readW, outNr3dHwParam.vipi_readH, vipiInfo.imgSize.w
                , vipiInfo.imgSize.h, vipiInfo.bufStridesInBytes_p0, outNr3dHwParam.vipi_offst, vipiInfo.imgFmt
            , outNr3dHwParam.onOff_onOfStX, outNr3dHwParam.onOff_onOfStY
                , outNr3dHwParam.onSiz_onWd, outNr3dHwParam.onSiz_onHt, dstRect.s.w, dstRect.s.h );
         if ( n < 0 ) tmpStr[0] = '\0';
         if ( isOutOfBound )  MY_LOGE("%s", tmpStr);
         else                 MY_LOGD("%s", tmpStr);
    }
...
void Hal3dnr::print_TNR_SET_PROC_INFO(const NR3DHALParam &nr3dHalParam, const TNR_SET_PROC_INFO_STRUCT& info)
{
   MY_LOGD_IF(mLogLevel >= 1,
        " === s(%u): fm(%d), r(%4d), f(%4d), uK(%d) === { "
        "lmv_info: (valid=%d, lmv_gmv_conf(%d,%d), lmv_gmv(%.2f,%.2f), lmv_max_gmv(%d,%d) ), "
        "gyro_info: (gyro_in_mv=%p, gyroMvWidth=%d, gyroMvHeight=%d), "
        ...
        " }"
        , mSensorIdx, nr3dHalParam.featMask, nr3dHalParam.requestNo, nr3dHalParam.frameNo, nr3dHalParam.uniqueKey
        , info.lmv_info.lmv_valid, info.lmv_info.lmv_gmv_conf[0], info.lmv_info.lmv_gmv_conf[1], info.lmv_info.lmv_gmv[0], info.lmv_info.lmv_gmv[1], info.lmv_info.lmv_max_gmv[0], info.lmv_info.lmv_max_gmv[1]
        , info.gyro_info.gyro_in_mv, info.gyro_info.gyroMvWidth,info.gyro_info.gyroMvHeight
        ...
        , nr3dHalParam.mNvramInfo.mMS5_RSV1
        , nr3dHalParam.mNvramInfo.mMS5_RSV2
        , nr3dHalParam.mNvramInfo.mMS5_RSV3
        , nr3dHalParam.mNvramInfo.mMS5_RSV4
   );
   return;
}
