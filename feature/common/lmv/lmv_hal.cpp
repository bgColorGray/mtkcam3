...
#define MY_LOGE_IF(c, fmt, arg...)    LMV_DO(if(c) LMV_LOG(E, fmt, ##arg))
...
#define LMV_HAL_BUF_DUMP        "vendor.debug.lmv.buf.dump"
...
LMVHal *LMVHalImp::GetInstance(const MUINT32 &aSensorIdx)
{
    ...
    return LMVHalObj::GetInstance(aSensorIdx);
}
    ...
    , mSensorIdx(aSensorIdx)
{
    ...
    mBufferDump = ::property_get_int32(LMV_HAL_BUF_DUMP, 0);
...
MINT32 LMVHalImp::Init(const MUINT32 eisFactor)
{
    ...
    if( UNLIKELY(mDebugDump >= 1) || (mBufferDump >=1))
    {
...
MINT32 LMVHalImp::CreateMultiMemBuf(MUINT32 memSize, MUINT32 num, android::sp<IImageBuffer>& spMainImageBuf, android::sp<IImageBuffer> spImageBuf[MAX_LMV_MEMORY_SIZE])
{
    ...
    for( MUINT32 index = 0; index < num; index++)
    {
        ...
        portBufInfo.offset = ((index)*(memSize));
        sp<IImageBufferHeap> imgBufHeap = IDummyImageBufferHeap::create(mName.c_str(), imgParam_t, portBufInfo);
        if( imgBufHeap == NULL )
        {
...
MVOID LMVHalImp::DumpMemBuf()
{
    String8 str_main;
    int ret = 0;
    str_main += String8::format(
        "LMVOMainBuffer=(%p)(va:%p,pa:%p)",
        m_pLMVOMainBuffer.get(),
        (void*)m_pLMVOMainBuffer->getBufVA(0),
        (void*)m_pLMVOMainBuffer->getBufPA(0));
    if (mBufferDump >=1) {
        MY_LOGD0("%s", str_main.string());
    }
    if ((mBufferDump >=2) &&
        (mDumpFile != nullptr)) {
        ret = fprintf(mDumpFile, "%s\n", str_main.string());
        MY_LOGE_IF((ret < 0), "Make Dump File fprintf FAIL1");
        //20210915--ALPS06140495-coverity:10933558:CERT-C Error Handling
        MINT32 rt1 = fflush(mDumpFile);
        if(rt1 !=0)
            MY_LOGD0("1:fflush return != 0 !");
        //20210915--ALPS06140495
    }

    String8 str_slice("LMVOSliceBuffer=");
    if (mBufferDump >=1) {
        MY_LOGD0("%s", str_slice.string());
    }
    if ((mBufferDump >=2) &&
        (mDumpFile != nullptr)) {
        ret = fprintf(mDumpFile, "%s\n", str_slice.string());
        MY_LOGE_IF((ret < 0), "Make Dump File fprintf FAIL2");
        //20210915--ALPS06140495-coverity:10933558:CERT-C Error Handling
        MINT32 rt1 = fflush(mDumpFile);
        if(rt1 !=0)
            MY_LOGD0("2:fflush return != 0 !");
    }

    for (int index = 0; index < LMVO_BUFFER_NUM; index++)
    {
        String8 str_slice_element;
        str_slice_element += String8::format("[%d](%p)(va:%p,pa:%p)",
            index, m_pLMVOSliceBuffer[index].get(),
            (void*)m_pLMVOSliceBuffer[index]->getBufVA(0),
            (void*)m_pLMVOSliceBuffer[index]->getBufPA(0));
        if (mBufferDump >=1) {
            MY_LOGD0("%s", str_slice_element.string());
        }
        if ((mBufferDump >=2) &&
            (mDumpFile != nullptr)) {
            ret = fprintf(mDumpFile, "%s\n", str_slice_element.string());
            MY_LOGE_IF((ret < 0), "Make Dump File fprintf FAIL3");
            //20210915--ALPS06140495-coverity:10933558:CERT-C Error Handling
            MINT32 rt1 = fflush(mDumpFile);
            if(rt1 !=0)
                MY_LOGD0("3:fflush return != 0 !");
            //20210915--ALPS06140495
        }
    }
}
...
MINT32 LMVHalImp::ConfigLMV(const LMV_HAL_CONFIG_DATA &aLmvConfig)
{
    ...
    if( mIsLmvConfig == 0 )
    {
        ...
        if( UNLIKELY(mDebugDump >= 1) )
        {
            ...
            {
                str_gmv_pan.appendFormat("(%u)", eisAlgoInitData.eis_tuning_data.advtuning_data.gmv_pan_array[i]);
                str_gmv_sm.appendFormat("(%u)", eisAlgoInitData.eis_tuning_data.advtuning_data.gmv_sm_array[i]);
                str_cmv_pan.appendFormat("(%u)", eisAlgoInitData.eis_tuning_data.advtuning_data.cmv_pan_array[i]);
                str_cmv_sm.appendFormat("(%u)", eisAlgoInitData.eis_tuning_data.advtuning_data.cmv_sm_array[i]);
            }
...
MINT32 LMVHalImp::PrepareLMV(const LMV_HAL_ENQUEUE_DATA &aLmvEnqueData)
{
    if( aLmvEnqueData.magicNum > 0 && m_pLMVDrv ) {
        MBOOL isNeedBypass = MFALSE;
        MINT32 hdrHalMode = MTK_HDR_FEATURE_HDR_HAL_MODE_OFF;
        {
            IMetadata::IEntry const& entry = aLmvEnqueData.inHal->entryFor(MTK_HDR_FEATURE_HDR_HAL_MODE);
            if( !entry.isEmpty() ) {
                hdrHalMode = entry.itemAt(0, Type2Type< MINT32 >());
            }
        }
        if( hdrHalMode & MTK_HDR_FEATURE_HDR_HAL_MODE_MSTREAM_CAPTURE_PREVIEW )
        {
            MINT32 expLevel = MTK_3A_FEATURE_AE_EXPOSURE_LEVEL_NONE;
            IMetadata::IEntry const& entry = aLmvEnqueData.inHal->entryFor(MTK_3A_FEATURE_AE_EXPOSURE_LEVEL);
            if( !entry.isEmpty() ) {
                expLevel = entry.itemAt(0, Type2Type< MINT32 >());
            }
            isNeedBypass = ( expLevel == MTK_3A_FEATURE_AE_EXPOSURE_LEVEL_SHORT || expLevel == MTK_3A_FEATURE_AE_EXPOSURE_LEVEL_LONG )
                           ? MTRUE : MFALSE;
        }
        LMV_ENQUEUE_INFO lmvEnqueInfo {
            .magicNum = aLmvEnqueData.magicNum,
            .bypass   = isNeedBypass,
        };
        m_pLMVDrv->PushLMVStatus(lmvEnqueInfo);
    }
    else {
        MY_LOGW("LMVDrv(%p): invalid magicNum(%d)", m_pLMVDrv, aLmvEnqueData.magicNum);
        return LMV_RETURN_API_FAIL;
    }
    return LMV_RETURN_NO_ERROR;
}
...
MINT32 LMVHalImp::GetBufLMV(android::sp<IImageBuffer>& spBuf)
{
    ...
    if( !mLMVOBufferList.empty() )
    {
        ...
        if (mBufferDump >=1) {
            MY_LOGD0("GetBufLMV[sz=%zu]:(%p)(va:%p,pa:%p)",
                     mLMVOBufferList.size(),
                     spBuf.get(), (void*)spBuf->getBufVA(0), (void*)spBuf->getBufPA(0));
        }
    }
    else
    {
...
MINT32 LMVHalImp::NotifyLMV(QBufInfo& pBufInfo)
{
    ...
    for( size_t i = 0; i < pBufInfo.mvOut.size(); i++ )
    {
        if( pBufInfo.mvOut[i].mPortID.index == PORT_EISO.index )
        {
            ...
            if (mBufferDump >=1) {
                MY_LOGD0("NotifyLMV[sz=%zu]:(%p)(va:%p,pa:%p)",
                         mLMVOBufferList.size(), retBuf.get(),
                         (void*)retBuf->getBufVA(0), (void*)retBuf->getBufPA(0));
            }
    ...
    DumpMemBuf();
    MY_LOGA("Error: Cannot Find LMVO in QBufInfo"
            "\nCRDISPATCH_KEY:LMV no find LMVO in QBufInfo");
    return LMV_RETURN_NO_ERROR;
}
...
MINT32 LMVHalImp::NotifyLMV(android::sp<NSCam::IImageBuffer>& spBuf)
{
    ...
    if( spBuf != 0 )
    {
        ...
        if (mBufferDump >=1) {
            MY_LOGD0("NotifyLMV[sz=%zu] - Drop:(%p)(va:%p,pa:%p)",
                     mLMVOBufferList.size(), spBuf.get(),
                     (void*)spBuf->getBufVA(0), (void*)spBuf->getBufPA(0));
        }
    }
...
MVOID LMVHalImp::DumpStatistic(const EIS_STATISTIC_STRUCT &aLmvStat)
{
    ...
    if( mDebugDump >= 3 )
    {
        ...
        if( mDumpFile != NULL )
        {
            int ret = fprintf(mDumpFile, "GMV(%6d,%6d),CMV(%4d,%4d):Raw(%6d,%6d),Center(%4d,%4d)\n",
                    mGMV_X, mGMV_Y, mCmvX_Int, mCmvY_Int, rawLMV.x, rawLMV.y, mMVtoCenterX, mMVtoCenterY);
            MY_LOGE_IF((ret < 0), "Make Dump File fprintf FAIL");
        }
    }
}
