...
#define LMV_DRV_BYPASS_CONTROL          "vendor.debug.LMVDrv.bypassctrl"

#define LMV_DRV_BYPASS_CONTROL_KEY_VAL  1
...
MUINT32 gHalBypassControl = 0;
const MUINT32 LMV_MIN_SIZE = 100;

static const MINT32 MIN_VALID_MAGIC_NUM = 1;
...
void LMVP1Cb::p1TuningNotify(MVOID *pInput,MVOID *pOutput)
{
    ...
    const MBOOL isBypass = _this->GetLMVStatusAndUpdate(aLmvInInfo->magNum)/*always update queue*/ &&
        (aLmvInInfo->magNum >= MIN_VALID_MAGIC_NUM) && gHalBypassControl;

    bool isUsePrviousSetting = false;
    if( aLmvInInfo->bYUVFmt == MFALSE )    // RAW sensor specific register
    {
        ...
        if( _this->mImgWidth != tempW || _this->mImgHeight != tempH )
        {
            ...
            _this->mIsFirst = 1;
            isUsePrviousSetting = false;
        }
        else
        {
            _this->mIsFirst = 0;
            isUsePrviousSetting = true;
        }
    ...
    if (_this->mSensor_Width != aLmvInInfo->sTGOut.w ||
        _this->mSensor_Height != aLmvInInfo->sTGOut.h  ||
        _this->mRRZ_in_Width != aLmvInInfo->RRZ_IN_CROP.in_size_w ||
        _this->mRRZ_in_Height != aLmvInInfo->RRZ_IN_CROP.in_size_h ||
        ...
        _this->mRrz_scale_Width != (( _this->mIs2Pixel > 0) ?
         (_this->mImgWidth << (_this->mIs2Pixel)) : (_this->mImgWidth )) ||
        _this->mRrz_scale_Height != _this->mImgHeight) {
      MY_LOGD0("M(%d)enLMV(%d):TG(%u,%u),RRZ In(%u,%u),RRZ crop x/y(%u,%u),"
               "RRZ crop w/h(%u,%u),RMX(%u,%u),HBIN(%u,%u),pixelMode(%d),YUV(%d)",
            aLmvInInfo->magNum, !isBypass,
    ...
    {
        a_pLmvCfgData->enLMV = (isBypass == MTRUE) ? MFALSE : MTRUE;
    }
    else
    {
    ...
    if( isUsePrviousSetting )
    {
        ...
        if( UNLIKELY(gLogLevel > 0) )
        {
            ...
            MY_LOGD0("win_numV:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0xF0000000) >> 28);
            MY_LOGD0("win_numH:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0xE000000) >> 25);
            MY_LOGD0("rp_numV:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0x1E00000) >> 21);
            MY_LOGD0("knee_1:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0x1E0000) >> 17);
            MY_LOGD0("knee_2:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0x1E000) >> 13);
            MY_LOGD0("rp_numH:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0x1F00) >> 8);
            MY_LOGD0("subG_en:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0x40) >> 6);
            MY_LOGD0("lmv_op_vert:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0x38) >> 3);
            MY_LOGD0("lmv_op_hori:(0x%08x)",_this->mLmvRegSetting.reg_lmv_prep_me_ctrl1 & 0x7);
            ...
            MY_LOGD0("dc_dl:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl2 & 0x3F00) >> 8);
            MY_LOGD0("vert_shr:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl2 & 0xE0) >> 5);
            MY_LOGD0("hori_shr:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_prep_me_ctrl2 & 0x1C) >> 2);
            MY_LOGD0("proc_gain:(0x%08x)",_this->mLmvRegSetting.reg_lmv_prep_me_ctrl2 & 0x3);
            ...
            MY_LOGD0("rp_offsetH:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_mb_offset&0xFFF0000)>>16);
            MY_LOGD0("rp_offsetV:(0x%08x",_this->mLmvRegSetting.reg_lmv_mb_offset&0xFFF);

            MY_LOGD0("reg_lmv_mb_interval(0x%08x)",_this->mLmvRegSetting.reg_lmv_mb_interval);
            MY_LOGD0("win_sizeH:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_mb_interval&0xFFF0000)>>16);
            MY_LOGD0("win_sizeV:(0x%08x)",_this->mLmvRegSetting.reg_lmv_mb_interval&0xFFF);

            MY_LOGD0("reg_lmv_image_ctrl(0x%08x)",_this->mLmvRegSetting.reg_lmv_image_ctrl);
            MY_LOGD0("Width:(0x%08x)",(_this->mLmvRegSetting.reg_lmv_image_ctrl&0x1FFF0000)>>16);
            MY_LOGD0("Height:(0x%08x)",_this->mLmvRegSetting.reg_lmv_image_ctrl&0x1FFF);
        }

        if (a_pLmvCfgData->enLMV == MTRUE) {
            _this->mHasEnableLMV = MTRUE;
        }
    ...
    MUINT32 first_frame_bit = (_this->mHasEnableLMV) ? 0 : 1 /*first frame*/;
    ...
    _this->mLmvRegSetting.reg_lmv_prep_me_ctrl2 = (first_frame_bit << 15) | (1 << 14) | (dc_dl << 8) | (vert_shr << 5) | (hori_shr << 2) | proc_gain;
    ...
    , mIsSupportSGG2Cb(MTRUE)
{
    ...
    gHalBypassControl = ::property_get_int32(LMV_DRV_BYPASS_CONTROL, LMV_DRV_BYPASS_CONTROL_KEY_VAL);

    MY_S_LOGI("LMV(%d/%p@%p),SGG2(%d@%p),isHalBypassControl(%d)",
              mIsSupportLMVCb, mLMVCb, obj, mIsSupportSGG2Cb, mSGG2Cb, gHalBypassControl);
