...
MINT32 LMVDrvImp::PushLMVStatus(const LMV_ENQUEUE_INFO &enqueInfo)
{
    MY_TRACE_FUNC_LIFE();
    const static size_t MAX_DEPTH_LMV_QUEUE = 60;
    std::lock_guard<std::mutex> lock(mQueueMutex);
    if( mPendingQueue.size() > MAX_DEPTH_LMV_QUEUE ) {
        MY_LOGW("Too many pending frames(%zu)", mPendingQueue.size());
    }

    // monotonically increasing
    if( enqueInfo.magicNum > 0 ) {
        if( !mPendingQueue.empty() ) {
            if( enqueInfo.magicNum <= mPendingQueue.back().magicNum ) {
                MY_LOGD("Suspicous non-monotonic incresement, may new pipeline started: magicNum=back(%d),new(%d)",
                        mPendingQueue.back().magicNum, enqueInfo.magicNum);
            }
        }
        mPendingQueue.push(enqueInfo);
    }
    return LMV_RETURN_NO_ERROR;
}

MBOOL LMVDrvImp::GetLMVStatusAndUpdate(const MINT32 magicNum)
{
    MY_TRACE_FUNC_LIFE();
    std::lock_guard<std::mutex> lock(mQueueMutex);
    // clear fade-out data
    while( !mPendingQueue.empty() ) {
        if( magicNum == mPendingQueue.front().magicNum ) {
            break;
        }
        else {
            if( mPendingQueue.front().magicNum < magicNum ) {
                MY_LOGD("Suspicous clear, driver may drop frame: magicNum(%d)=bypass(%d)", mPendingQueue.front().magicNum, mPendingQueue.front().bypass);
            }
            else {
                MY_LOGD("Suspicous clear, driver may drop frame and now in new pipeline: magicNum(%d)=bypass(%d)", mPendingQueue.front().magicNum, mPendingQueue.front().bypass);
            }
            mPendingQueue.pop();
        }
    }
    if( !mPendingQueue.empty() ) {
        LMV_ENQUEUE_INFO enqueInfo = mPendingQueue.front();
        if( magicNum == enqueInfo.magicNum ) {
            MY_LOGD3("Find magicNum(%d):bypass(%d)", magicNum, enqueInfo.bypass);
            mPendingQueue.pop();
            return enqueInfo.bypass;
        }
    }

    MY_LOGW("Cannot find magicNum(%d) in queue(%zu)", magicNum, mPendingQueue.size());
    return MFALSE;
}
