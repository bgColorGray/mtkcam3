...
private:
    ...
    LMVData getDefaultData(QBufInfo const &pBufInfo, const MUINT32 rrzId);
...
public:
    ...
    static LMVHal *GetInstance(MUINT32 sensorIdx) {
        static std::mutex mutex;
        static std::map<MUINT32/*sensorIdx*/, LMVHalImp> lmvHalObj;
        std::lock_guard<std::mutex> lock(mutex);
        if (lmvHalObj.count(sensorIdx) <= 0) {
            lmvHalObj.emplace(std::piecewise_construct,
                              std::forward_as_tuple(sensorIdx),
                              std::forward_as_tuple(sensorIdx));
        }
        return &lmvHalObj.at(sensorIdx);
    }
};
