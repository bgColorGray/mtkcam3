...
MINT32 YUY2toRGB888(MINT32 srcFD, MINT32 width, MINT32 height, MINT32 target_width, MINT32 target_height, MINT32 targetFd, MINT32 dstStride = 0, DP_COLOR_ENUM format = eRGB888)
{
    ...
    // source config
    status = DpStream.setSrcBuffer(srcFD, src_size, 1);
    if (status != DP_STATUS_RETURN_SUCCESS)
    {
        MY_LOGE("[%s] %s -> setSrcBuffer fail!\n", LOG_TAG, __FUNCTION__);
    ...
    // target config
    status = DpStream.setDstBuffer(targetFd, dst_size, 1);
    if (status != DP_STATUS_RETURN_SUCCESS)
    {
...
)
{
    ...
    struct tuning_para *pParams = &tuning_50;
    ...
    FDFTInitInfo.FDRefresh = pParams->FDRefresh;
    ...
    FDFTInitInfo.SilentModeFDSkipNum = pParams->SilentModeFDSkipNum;
...
)
{
    ...
    pMDPDstImgBuf = mpSqzRpnFd->map_input_buffer_fd();
    ...
    YUY2toRGB888(pPara->FD_Y, pPara->InWidth, pPara->InHeight, target_w, target_h, pMDPDstImgBuf, dstStride, eRGB888);
...
)
{
    ...
    //YUY2toRGB888(pPara->FD_Y, pPara->InWidth, pPara->InHeight, target_w, target_h, pMDPDstImgBuf, 0, eBGR888);
...
void halFDVT::dumpFDImages(MUINT8* pFDImgBuffer __unused, MUINT8* pFTImgBuffer __unused)
{
}
// Dump FD information
