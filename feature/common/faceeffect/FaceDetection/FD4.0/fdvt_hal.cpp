...
)
{
    ...
    {
        FDFTInitInfo.LandmarkEnableCnt = 0;
        MY_LOGD("Secure FD mode");
    }

    FDFTInitInfo.FLDAttribConfig = 1;
    #if MTK_ALGO_PSD_MODE && USE_HW_FD
    #if (MTKCAM_HWFD_MAIN_VERSION == 40)
    FdDrv_InitParam_Struct FdDrvInitInfo;
    FdDrvInitInfo.isSecure = misSecureFD;
    FdDrvInitInfo.secMemType = MEM_TYPE_PROT;
    FDVT_OpenDriverWithUserCount(0, FdDrvInitInfo);
...
)
{
    ...
    MY_LOGD("[P dbg] buffer PA before hw: %p", FDOps.ImageBufferPhyPlane1);
