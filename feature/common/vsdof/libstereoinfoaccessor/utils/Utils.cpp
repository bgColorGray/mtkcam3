...
StereoString Utils::buffer2Str(const StereoBuffer_t &buffer) {
    if (buffer.data == nullptr) {
        StereoLogE("<buffer2Str 1> buffer is invalid");
        StereoString str("");
        return str;
    }
    else {
...
StereoString Utils::buffer2Str(const StereoBuffer_t &buffer, S_UINT32 size) {
    if (buffer.data == nullptr) {
        StereoLogE("<buffer2Str 2> buffer is invalid");
        StereoString str("");
        return str;
    }
    else {
...
StereoString Utils::intToHexString(int val) {
    char hex[16] = { 0 };
    if ( sprintf(hex, "0x%X", val) < 0 ) {
        StereoLogE("<intToHexString> sprintf fail");
    }
...
void Utils::writeBufferToFile(const StereoString &destFile, const StereoBuffer_t &buffer) {
    ...
    if (fp) {
        int status = 0;
        status = fwrite(buffer.data, buffer.size * sizeof( S_UINT8 ), 1, fp );
        if (status != buffer.size * sizeof( S_UINT8 ) ) {
            StereoLogE("<writeBufferToFile> fwrite fail, status %d, target %lu"
                , status, buffer.size * sizeof( S_UINT8 ));
        }
        status = fclose(fp);
        if (status != 0) {
            StereoLogE("<writeBufferToFile> fclose fail");
        }
    } else {
        StereoLogE(
...
void Utils::readFileToBuffer(const StereoString &filePath, StereoBuffer_t &outBuffer) {
    ...
    if (isFileExist(filePath.c_str()) != 0) {
        StereoLogE("<readFileToBuffer> file not exist, %s", filePath.c_str());
        return;
    }
    ...
    if (fp) {
        ...
        if(stat(filePath.c_str(), &statbuff) < 0){
            ...
            if (status != 0) {
                StereoLogE("<readFileToBuffer> fclose fail");
            }
            return;
        }else{
        ...
        if((size * sizeof( S_UINT8 ) == 0))
            StereoLogE("<readFileToBuffer> nothing to read, error: %s ", strerror(errno));
        if(realRead != 1)
            StereoLogE("<readFileToBuffer> read fail, error: %s ", strerror(errno));

        int status = fclose(fp);
        if (status != 0) {
            StereoLogE("<readFileToBuffer> fclose fail 2");
        }
    } else {
        StereoLogE(
...
static void PngWriteCallback(png_structp  png_ptr, png_bytep data, png_size_t length) {
    ...
    if(p != nullptr)
        ...
        StereoLogE("<PngWriteCallback> null io ptr");
}
...
void Utils::encodeDepthToPng(const StereoBuffer_t &inputBuffer,
        int width, int height, StereoBuffer_t &outBuffer) {
    png_structp png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
    if (!png_ptr) {
        StereoLogE("png_create_write_struct fail");
    ...
    std::vector<ui8*> rows(height);
    for (int y = 0; y < height; ++y) {
        rows[y] = (ui8*)inputBuffer.data + y * width * 1;
    }
    png_set_rows(png_ptr, info_ptr, &rows[0]);
    png_set_write_fn(png_ptr, &tmpOut, PngWriteCallback, NULL);
    png_write_png(png_ptr, info_ptr, PNG_TRANSFORM_IDENTITY, NULL);
