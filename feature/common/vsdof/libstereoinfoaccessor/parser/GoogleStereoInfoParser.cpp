...
GoogleStereoInfoParser:: GoogleStereoInfoParser(const StereoBuffer_t &standardBuffer,
        const StereoBuffer_t &extendedBuffer, GoogleStereoInfo *info, StereoString &packInfoMd5) {
    ...
    /* make md5 digest hash code makes xmp and Exmp column
    have unique and correspond relationship*/
    info->hasExtendedXMP = PackUtils::getMD5(info->jpgBuffer);
    packInfoMd5 = info->hasExtendedXMP;
...
void GoogleStereoInfoParser::initSimpleValue() {
    ...
    simpleValue = getSimpleValueInstance();
    simpleValue->name = ATTRIBUTE_XMPNOTE_HASEXTENDEDXMP;
    simpleValue->nameSpaceItem.nameSpace = NS_XMPNOTE;
    simpleValue->nameSpaceItem.nameSpacePrefix = PRIFIX_XMPNOTE;
    pListOfSimpleValue->push_back(simpleValue);
...
void GoogleStereoInfoParser::writeSimpleValue() {
    ...
    for (auto iter = pListOfSimpleValue->begin();
        ...
        } else if (ATTRIBUTE_XMPNOTE_HASEXTENDEDXMP == pSimpleValue->name
            && PRIFIX_XMPNOTE == pSimpleValue->nameSpaceItem.nameSpacePrefix) {
            pSimpleValue->value = pGoogleStereoInfo->hasExtendedXMP;
        }
    }
}
...
void GoogleStereoInfoParser::writeBufferItem() {
    ...
    for (auto iter = pListOfBufferItem->begin();
        ...
        if (ATTRIBUTE_CLEAR_IMAGE == pBufferItem->name
            ...
            && PRIFIX_GDEPTH == pBufferItem->nameSpaceItem.nameSpacePrefix
            && pGoogleStereoInfo->depthBuffer.isValid()) {

            StereoLogD("<pGoogleStereoInfo->depthBuffer> width: %d, height: %d, size: %d",
                pGoogleStereoInfo->depth_width, pGoogleStereoInfo->depth_height,
                pGoogleStereoInfo->depthBuffer.size);

            if (pGoogleStereoInfo->depthMime == "image/png") {
                Utils::encodeDepthToPng(pGoogleStereoInfo->depthBuffer,
                       pGoogleStereoInfo->depth_width,
                       pGoogleStereoInfo->depth_height,
                       pBufferItem->value);
            } else if  (pGoogleStereoInfo->depthMime == "image/jpeg") {
                ; // encode Depth map to jpeg
            } else {
                StereoLogE("<writeBufferItem> Invalid DepthMIME type");
            }

            if (Utils::ENABLE_BUFFER_DUMP) {
                dumpSingleBuffer(pBufferItem->value,
                    "depthMap_EncodedToPNG", "png");
            }
        }
    }
}

void GoogleStereoInfoParser::dumpSingleBuffer(const StereoBuffer_t &buffer,
    StereoString name, StereoString filetype)
{
    ...
    if (buffer.isValid()) {
        ...
        Utils::writeBufferToFile(
            dumpPath.append(dumpFolder).append("GoogleStereoInfo_")
            .append(name).append(".").append(filetype),
            buffer);
    }
}
