...
HW_WMFTuning::retrieveTuningParams(TuningQuery_T &query)
{
    ...
    ::memcpy(ctrl, &__ctrls[(size_t)queryRound], sizeof(NSCam::NSIoPipe::WMFECtrl));
...
HW_WMFTuning::_initDefaultValues()
{
    ...
    __ctrls[E_HOLE_FILLING_ROUND_1].WmfeHorzScOrd   = (StereoSettingProvider::getSensorRelativePosition() == 1)
    ...
    __ctrls[E_HOLE_FILLING_ROUND_2].WmfeHorzScOrd   = (StereoSettingProvider::getSensorRelativePosition() == 1)
                                                      ? WMFE_HORZ_RIGHT2LEFT
                                                      : WMFE_HORZ_LEFT2RIGHT;
            ...
            if(wmfValue[VALUE_KEY_PARAMETERS][QUERY_KEY_WMF_ROUND] == QUERY_KEY_WMF_ROUND_NAME[round])
            {
                ...
                switch(round) {
                    ...
                    case E_HOLE_FILLING_ROUND_1:
                        ctrl->WmfeHorzScOrd = (StereoSettingProvider::getSensorRelativePosition() == 1)
                                              ? WMFE_HORZ_LEFT2RIGHT
                                              : WMFE_HORZ_RIGHT2LEFT;
                        ctrl->WmfeVertScOrd = WMFE_VERT_TOP2BOTTOM;
                    break;
