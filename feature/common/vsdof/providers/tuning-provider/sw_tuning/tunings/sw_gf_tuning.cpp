...
SW_GFTuning::retrieveTuningParams(TuningQuery_T &query)
{
    ...
    auto iter = __gfMap.find(scenario);
    if(iter == __gfMap.end() &&
       scenario == eSTEREO_SCENARIO_RECORD)
    {
        iter = __gfMap.find(eSTEREO_SCENARIO_PREVIEW);
        MY_LOGD_IF(iter != __gfMap.end(), "Record is not configured in tuning, use preview instead");
    }

    if(iter != __gfMap.end()) {
        SW_GF_TUNING_T &tuning = iter->second;
        if(coreNumber) {
...
SW_GFTuning::log(FastLogger &logger, bool inJSON)
{
    ...
    logger.FastLogD("%s", "======== GF Parameters ========");
...
SW_GFTuning::_loadValuesFromDocument(const json& gfValues)
{
    ...
    for(auto &gfValue : gfValues) {
        ...
        for(json::const_iterator it = tuningValues.begin(); it != tuningValues.end(); ++it) {
            ...
            auto key = it.key();
            if(key.length() > TUNING_PREFIX_SIZE &&
               !strncmp(key.c_str(), TUNING_PREFIX, TUNING_PREFIX_SIZE))
            {
                auto value = it.value().get<int>();
                if(key == "gf.max_blur_level" ||
                   key == "gf.max_blur_level_fg" ||
                   key == "gf.max_blur_gain" ||
                   key == "gf.max_blur_gain_gf")
                {
                    MSize pvSize = StereoSizeProvider::getInstance()->getPreviewSize();
                    value = (int)std::roundf(value*pvSize.h/1080.0f);
                }

                tuning.params.push_back({key, value});
            }
        }
