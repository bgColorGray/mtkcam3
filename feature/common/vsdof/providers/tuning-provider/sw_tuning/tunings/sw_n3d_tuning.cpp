...
SW_N3DTuning::SW_N3DTuning(json &tuningJson)
{
    ...
    __n3dMap[eSTEREO_SCENARIO_RECORD]  = SW_N3D_TUNING_T();
...
SW_N3DTuning::retrieveTuningParams(TuningQuery_T &query)
{
    ...
    auto iter = __n3dMap.find(scenario);
    if(iter == __n3dMap.end() &&
       scenario == eSTEREO_SCENARIO_RECORD)
    {
        iter = __n3dMap.find(eSTEREO_SCENARIO_PREVIEW);
        MY_LOGD_IF(iter != __n3dMap.end(), "Record is not configured in tuning, use preview instead");
    }

    if(iter != __n3dMap.end()) {
        if(maskMargin) {
            *maskMargin = iter->second.maskMargin;
        }

        if(params) {
            *params = iter->second.params;
            for(auto &param : *params) {
                // Since the config is based on "main cam is on the rhs"
                // if the main cam is on hte lhs, need to reverse the sign(direction)
                if(param.first == "n3d.c_offset")
                {
                    if(param.second != 0)
                    {
                        if(StereoSettingProvider::getSensorRelativePosition() == 0) {
...
SW_N3DTuning::log(FastLogger &logger, bool inJSON)
{
    ...
    logger.FastLogD("%s", "======== N3D Parameters ========");
    for (N3D_MAP_T::iterator it = __n3dMap.begin(); it != __n3dMap.end(); ++it) {
        ...
        for(auto &param : n3dTuning.params) {
            if(param.first == "n3d.c_offset" &&
               param.second != 0)
            {
                logger.FastLogD("\"%s\": %d(Main on right)/-%d(Main on left))", param.first.c_str(), param.second, param.second);
            }
            else
            {
