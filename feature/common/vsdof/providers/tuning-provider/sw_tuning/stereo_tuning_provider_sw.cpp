...
bool
StereoTuningProvider::getBlendingTuningInfo(MUINT32 &coreNumber, std::vector<MUINT8> &alphaTable, TUNING_PAIR_LIST_T &tuningParams, ENUM_STEREO_SCENARIO eScenario)
{
    TuningQuery_T query;
    query.intParams[QUERY_KEY_SCENARIO] = eScenario;

    query.results[QUERY_KEY_CORE_NUMBER]   = &coreNumber;
    query.results[QUERY_KEY_ALPHATABLE]    = &alphaTable;
    query.results[QUERY_KEY_TUNING_PARAMS] = &tuningParams;
    return StereoTuningProviderKernel::getInstance()->getTuningParams(E_TUNING_SW_BLENDING, query);
}
