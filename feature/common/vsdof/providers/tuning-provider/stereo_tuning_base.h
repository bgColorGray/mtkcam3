...
protected:
    ...
    {
        auto key = getTuningKey();
        if(key &&
           LogicalCamJSONUtil::HasMember(tuningJson, key))
        {
            _loadValuesFromDocument(tuningJson.at(key));
        } else {
            _initDefaultValues();
        }
    }
