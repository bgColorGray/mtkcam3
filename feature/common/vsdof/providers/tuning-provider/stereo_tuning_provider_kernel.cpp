...
#define TUNING_HEADER_SDCARD_PATH       "/sdcard/camera_custom_stereo_tuning.h"
#define TUNING_HEADER_VENDOR_PATH       "/data/vendor/camera_dump/camera_custom_stereo_tuning.h"
...
StereoTuningProviderKernel::~StereoTuningProviderKernel()
{
    ...
    for(auto const &element : __tuningHolderMap)
    {
        if(element.second) {
            ...
            MY_LOGE("Cannot delete tuning of %s", STEREO_TUNING_NAME[(MUINT32)element.first]);
        }
    }
...
StereoTuningProviderKernel::init()
{
    ...
    else
    {
        ...
        // 1. Logical device name, case sensitive
        IHalLogicalDeviceList* pHalDeviceList = MAKE_HalLogicalDeviceList();
        if(pHalDeviceList)
        {
            auto s = pHalDeviceList->queryDriverName(StereoSettingProvider::getLogicalDeviceID());
            if(s)
            {
                std::string name(s);
                if(LogicalCamJSONUtil::HasMember(loadedJson, name))
                {
                    isSettingFound = true;
                }
                else
                {
                    // Remove sensor name prefix and try again
                ...
                if(isSettingFound)
                {
                    MY_LOGD("Init tuning parameters of %s", name.c_str());
                    tuningJson = std::move(loadedJson[name]);
                }
            }
        }
        ...
        // 3. Use whole file as tuning param
        if(!isSettingFound)
        {
    ...
    //Create map
    {
        ...
        __tuningHolderMap[E_TUNING_SW_BLENDING] = new SW_BLENDINGTuning(tuningJson);
        __tuningHolderMap[E_TUNING_SW_AIDEPTH] = new SW_AIDEPTHTuning(tuningJson);
        __tuningHolderMap[E_TUNING_SW_VIDEO_AIDEPTH] = new SW_VIDEO_AIDEPTHTuning(tuningJson);
    }
...
StereoTuningProviderKernel::getTuningParams(ENUM_STEREO_TUNING tuningID, TuningQuery_T &param)
{
    ...
    if(NULL == holder) {
        MY_LOGE("Cannot get holder of %s", STEREO_TUNING_NAME[(MUINT32)tuningID]);
        return false;
    }
...
StereoTuningProviderKernel::__saveTuningToFile(const KeepOrderJSON &loadedJson)
{
    ...
    {
        struct stat st;
        bool isSdcardFileExist = (stat(STEREO_TUNING_FILE_SDCARD_PATH, &st) == 0);
        bool isVendorFileExist = (stat(STEREO_TUNING_FILE_VENDOR_PATH, &st) == 0);
        if(!isSdcardFileExist &&
           !isVendorFileExist)
        {
            FILE *fp = fopen(STEREO_TUNING_FILE_SDCARD_PATH, "w");
            if(fp) {
            ...
            {
                fp = fopen(STEREO_TUNING_FILE_VENDOR_PATH, "w");
                if(fp) {
                    MY_LOGE_IF(fwrite(data, 1, strlen(data), fp) == 0, "Write failed");
    ...
    if(__IS_EXPORT_ENABLED) {
        FILE *fp = fopen(TUNING_HEADER_SDCARD_PATH, "w");
        if(NULL == fp)
        {
            fp = fopen(TUNING_HEADER_VENDOR_PATH, "w");
        }
