...
public:
    ...
    virtual StereoArea getWDMAArea( P2SizeQueryParam param ) const {
        return Pass2A_Crop_SizeProvider::instance()->getWDMAArea(param);
    }
    ...
    virtual StereoArea getIMG2OArea( P2SizeQueryParam param ) const  {
        ...
        StereoSizeProvider::getInstance()->getPass1Size( { eSTEREO_SENSOR_MAIN1,
                                                           (eSTEREO_SCENARIO_CAPTURE == param.eScenario) ? eImgFmt_BAYER10 : eImgFmt_FG_BAYER10,
                                                           (eSTEREO_SCENARIO_CAPTURE == param.eScenario) ? EPortIndex_IMGO : EPortIndex_RRZO,
                                                           param.eScenario,
                                                           param.previewCropRatio
                                                         },
                                                         //below are outputs
                                                         tgCropRect,
                                                         szMain1Output,
