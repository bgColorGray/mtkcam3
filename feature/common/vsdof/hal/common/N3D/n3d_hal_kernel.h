...
struct N3D_HAL_KERNEL_INIT_PARAM
{
    ...
    N3D_HAL_KERNEL_INIT_PARAM(const N3D_HAL_INIT_PARAM &initParam)
    {
    ...
    N3D_HAL_KERNEL_INIT_PARAM(const N3D_HAL_INIT_PARAM_WPE &initParam)
    {
        eScenario          = initParam.eScenario;
        fefmRound          = initParam.fefmRound;
...
public:
    ...
    virtual void init(N3D_HAL_KERNEL_INIT_PARAM n3dInitParam);
...
private:
    ...
    void __getRectInS();        //run in __rectInSThread
    void __getRectInSThread();  //run in __rectInSThread

    void __updateSceneInfo(N3D_HAL_OUTPUT &n3dOutput);

    void __updateRuntimeRRZOInfo(float zoomRatio, ENUM_STEREO_SENSOR sensor, STEREO_KERNEL_RRZ_INFO_STRUCT &rrzoInfo);
...
private:
    ...
    // multithread main2 RECT_IN
    thread                  __rectInSThread;
    std::mutex              __rectInSMutex;
    std::condition_variable __rectInSCondVar;
    std::condition_variable __rectInSCondVarCaller;
    bool                    __wakeupGetRectInS = false;
    NSCam::IImageBuffer     *__rectInSInput  = NULL;
    NSCam::IImageBuffer     *__rectInSOutput = NULL;
