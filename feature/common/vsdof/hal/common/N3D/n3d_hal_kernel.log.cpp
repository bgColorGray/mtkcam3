...
N3D_HAL_KERNEL::__logInitInfo(const STEREO_KERNEL_SET_ENV_INFO_STRUCT &initInfo)
{
    ...
    FAST_LOGD("[iio_main.wpe_img_rotate_type] %d", initInfo.iio_main.wpe_img_rotate_type);
    ...
    FAST_LOGD("[iio_auxi.wpe_img_rotate_type] %d", initInfo.iio_auxi.wpe_img_rotate_type);
    ...
    for(i = 0; i < MAX_GEO_LEVEL; i++) {
        ...
        FAST_LOGD("[geo_img][%zu][size] %d", i, initInfo.geo_img[i].size);

        if(snprintf(logPrefix, PREFIX_SIZE, "[geo_img][%zu][main]", i) > 0) {
            __logImgInfo(logPrefix, initInfo.geo_img[i].img_main);
        }

        if(snprintf(logPrefix, PREFIX_SIZE, "[geo_img][%zu][auxi]", i) > 0) {
    ...
    FAST_LOGD("[module_angle]     %d", initInfo.module_angle);
...
N3D_HAL_KERNEL::__logImgInfo(const char *prefix, const STEREO_KERNEL_IMG_INFO_STRUCT &imgInfo)
{
    ...
    FAST_LOGD("%s[rot_width]  %d", prefix, imgInfo.rot_width);
    FAST_LOGD("%s[rot_height] %d", prefix, imgInfo.rot_height);
}
...
N3D_HAL_KERNEL::__logSetProcInfo(const char *prefix, const STEREO_KERNEL_SET_PROC_INFO_STRUCT &setprocInfo)
{
    ...
    FAST_LOGD("%s[rrz_proc_main][rrz_offset_x]     %d", prefix, setprocInfo.rrz_proc_main.rrz_offset_x);
    FAST_LOGD("%s[rrz_proc_main][rrz_offset_y]     %d", prefix, setprocInfo.rrz_proc_main.rrz_offset_y);
    FAST_LOGD("%s[rrz_proc_main][rrz_usage_width]  %d", prefix, setprocInfo.rrz_proc_main.rrz_usage_width);
    FAST_LOGD("%s[rrz_proc_main][rrz_usage_height] %d", prefix, setprocInfo.rrz_proc_main.rrz_usage_height);
    FAST_LOGD("%s[rrz_proc_main][rrz_out_width]    %d", prefix, setprocInfo.rrz_proc_main.rrz_out_width);
    FAST_LOGD("%s[rrz_proc_main][rrz_out_height]   %d", prefix, setprocInfo.rrz_proc_main.rrz_out_height);

    FAST_LOGD("%s[rrz_proc_auxi][rrz_offset_x]     %d", prefix, setprocInfo.rrz_proc_auxi.rrz_offset_x);
    FAST_LOGD("%s[rrz_proc_auxi][rrz_offset_y]     %d", prefix, setprocInfo.rrz_proc_auxi.rrz_offset_y);
    FAST_LOGD("%s[rrz_proc_auxi][rrz_usage_width]  %d", prefix, setprocInfo.rrz_proc_auxi.rrz_usage_width);
    FAST_LOGD("%s[rrz_proc_auxi][rrz_usage_height] %d", prefix, setprocInfo.rrz_proc_auxi.rrz_usage_height);
    FAST_LOGD("%s[rrz_proc_auxi][rrz_out_width]    %d", prefix, setprocInfo.rrz_proc_auxi.rrz_out_width);
    FAST_LOGD("%s[rrz_proc_auxi][rrz_out_height]   %d", prefix, setprocInfo.rrz_proc_auxi.rrz_out_height);
...
N3D_HAL_KERNEL::__logResult(const char *prefix, const STEREO_KERNEL_RESULT_STRUCT &n3dResult)
{
    ...
    //         FAST_LOGD("%s[verify[%zu]]   %d", prefix, i, verityOutputs[i]);
    //     }
    // }
