...
protected:
    virtual void SetUp() {
        ...
        FILE *fp = NULL;
        char filePath[256];
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/LensINFO") >= 0)
        {
    ...
    virtual void loadBuffers()
    {
        ...
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN1_%dx%d.yuv", _n3dInitInfo.iio_main.src_w, _n3dInitInfo.iio_main.src_h) >= 0)
        {
            readImage(filePath, _inputImageMain1.get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN2_%dx%d.yuv", _n3dInitInfo.iio_auxi.src_w, _n3dInitInfo.iio_auxi.src_h) >= 0)
        {
            readImage(filePath, _inputImageBuffers[_ringIdx].get());
        }

        bool hasFEFM = false;
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE1BO_%dx%d.yuv", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = readImage(filePath, _feoMain1[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE1CO_%dx%d.yuv", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _feoMain1[1].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE2BO_%dx%d.yuv", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _feoMain2[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE2CO_%dx%d.yuv", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _feoMain2[1].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMBO_LR_%dx%d.yuv", _fmSize.w, _fmSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _fmoLR[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMBO_RL_%dx%d.yuv", _fmSize.w, _fmSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _fmoLR[1].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMCO_LR_%dx%d.yuv", _fmSize.w, _fmSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _fmoRL[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMCO_RL_%dx%d.yuv", _fmSize.w, _fmSize.h) >= 0)
        {
