...
TEST_F(N3D_Preview, TEST)
{
    ...
    if(sprintf(dumpPath, UT_CASE_OUT_FOLDER"/BID_N3D_OUT_MV_Y_%dx%d.yuv", dumpSize.w, dumpSize.h) >= 0)
    {
        _outputImageMain1.get()->saveToFile(dumpPath);
    }

    dumpSize = _outputMaskMain1.get()->getImgSize();
    if(sprintf(dumpPath, UT_CASE_OUT_FOLDER"/BID_N3D_OUT_MASK_M_%dx%d.yuv", dumpSize.w, dumpSize.h) >= 0)
    {
        _outputMaskMain1.get()->saveToFile(dumpPath);
    }

    dumpSize = _previewOutput.rectifyImgMain2->getImgSize();
    if(sprintf(dumpPath, UT_CASE_OUT_FOLDER"/BID_N3D_OUT_SV_Y_%dx%d.yuv", dumpSize.w, dumpSize.h) >= 0)
    {
        _previewOutput.rectifyImgMain2->saveToFile(dumpPath);
    }

    dumpSize = _previewOutput.maskMain2->getImgSize();
    if(sprintf(dumpPath, UT_CASE_OUT_FOLDER"/BID_N3D_OUT_MASK_S_%dx%d.yuv", dumpSize.w, dumpSize.h) >= 0)
    {
