...
protected:
    virtual void SetUp() {
        ...
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/N3D_NVRAM_IN") >= 0)
        {
    ...
    virtual void loadBuffers()
    {
        ...
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN1__%dx%d_8_s0.yv12", _n3dInitInfo.iio_main.inp_w, _n3dInitInfo.iio_main.inp_h) >= 0)
        {
            readImage(filePath, _inputImageMain1.get());
        }

        // if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN2__%dx%d_8_s0.yv12", _n3dInitInfo.iio_auxi.inp_w, _n3dInitInfo.iio_auxi.inp_h) >= 0)
        // {
        //     readImage(filePath, _inputImageBuffers[_ringIdx].get());
        // }

        bool hasFEFM = false;
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE1BO__%dx%d_8_s0.unknown", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = readImage(filePath, _feoMain1[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE1CO__%dx%d_8_s0.unknown", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _feoMain1[1].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE2BO__%dx%d_8_s0.unknown", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _feoMain2[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FE2CO__%dx%d_8_s0.unknown", _feSize.w, _feSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _feoMain2[1].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMBO_LR__%dx%d_8_s0.unknown", _fmSize.w, _fmSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _fmoLR[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMBO_RL__%dx%d_8_s0.unknown", _fmSize.w, _fmSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _fmoLR[1].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMCO_LR__%dx%d_8_s0.unknown", _fmSize.w, _fmSize.h) >= 0)
        {
            hasFEFM = hasFEFM && readImage(filePath, _fmoRL[0].get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_FMCO_RL__%dx%d_8_s0.unknown", _fmSize.w, _fmSize.h) >= 0)
        {
