...
protected:
    ...
    virtual void loadParam()
    {
        ...
        if(!parseSuccess) {
            CAM_ULOGE(NSCam::Utils::ULog::MOD_VSDOF_HAL, "Fail to parse VIDEO_AIDEPTH_INIT_INFO.json");
            return;
        }

        _input.mainCamPos              = document["TuningInfo"]["flipFlag"].get<int>();
        _tuningInfo.imgWidth           = document["TuningInfo"]["imgWidth"].get<int>();
        _tuningInfo.imgHeight          = document["TuningInfo"]["imgHeight"].get<int>();
        ...
        if(!parseSuccess) {
            CAM_ULOGE(NSCam::Utils::ULog::MOD_VSDOF_HAL, "Fail to parse VIDEO_AIDEPTH_PROC_INFO.json");
            return;
        }

        _input.iso          = document["ParamInfo"]["ISOValue"].get<int>();
        _input.exposureTime = document["ParamInfo"]["ExposureTime"].get<int>();
        _imgSize.w          = document["ImgInfo"]["ImgLRWidth"].get<int>();
        _imgSize.w          = document["ImgInfo"]["ImgLRHeight"].get<int>();
        _depthSize.w        = document["ImgInfo"]["Width"].get<int>();
        _depthSize.h        = document["ImgInfo"]["Height"].get<int>();
        ...
        StereoDpUtil::allocImageBuffer(LOG_TAG, eImgFmt_Y8, _depthSize, !IS_ALLOC_GB, _disparityMap);
        StereoDpUtil::allocImageBuffer(LOG_TAG, eImgFmt_Y8, _depthSize, !IS_ALLOC_GB, _confidenceMap);
    ...
    {
        char filePath[256];
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_N3D_OUT_MV_Y_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight) >= 0)
        {
            readImage(filePath, _imgMain1.get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_N3D_OUT_SV_Y_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight) >= 0)
        {
            readImage(filePath, _imgMain2.get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_OCC_OUT_NOC_M_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight) >= 0)
        {
            readImage(filePath, _disparityMap.get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_OCC_OUT_CFM_M_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight) >= 0)
        {
