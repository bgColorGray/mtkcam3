...
BOKEH_HAL_IMP::__dumpImageData(BOKEH_HAL_PARAMS &param)
{
    ...
    if(sprintf(fileName, "CleanImage_%dx%d_NV21.yuv", inputSize.w, inputSize.h) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        param.cleanImage->saveToFile(dumpPath);
    }

    if(sprintf(fileName, "CleanImage_%dx%d_YV12_workingImage.yuv", __imgInfo.TargetWidth, __imgInfo.TargetHeight) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        __workingImage->saveToFile(dumpPath);
    }

    if(sprintf(fileName, "CleanImage_%dx%d_YV12_TargetImg.yuv", __imgInfo.TargetWidth, __imgInfo.TargetHeight) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        FILE *fp = fopen(dumpPath, "wb");
        if(fp) {
            MY_LOGE_IF(fwrite(__imgInfo.TargetImgAddr, 1, __imgInfo.TargetWidth*__imgInfo.TargetHeight*3/2, fp) == 0, "Write failed");
    ...
    if(sprintf(fileName, "DepthBuffer_%dx%dx4.yuv", depthSize.w, depthSize.h) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        fp = fopen(dumpPath, "wb");
        if(fp) {
            MY_LOGE_IF(fwrite(__imgInfo.DepthBufferAddr, 1, __imgInfo.DepthBufferSize, fp) == 0, "Write failed");
            MY_LOGE_IF(fflush(fp), "Flush failed");
            MY_LOGE_IF(fclose(fp), "Close failed");
        } else {
            MY_LOGE("Cannot open %s", dumpPath);
    ...
    for(int i = 0; i < 4; i++, src += PER_BUFFER_SIZE) {
        if(sprintf(fileName, FILENAME_TEMPLATE[i], depthSize.w, depthSize.h) >= 0)
        {
            genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
            fp = fopen(dumpPath, "wb");
            if(fp) {
                MY_LOGE_IF(fwrite(src, 1, PER_BUFFER_SIZE, fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            } else {
                MY_LOGE("Cannot open %s", dumpPath);
...
BOKEH_HAL_IMP::__dumpResult(BOKEH_HAL_OUTPUT &output)
{
    ...
    if(fp) {
        ...
        MY_LOGE_IF(fwrite(__resultInfo.RefocusedYUVImageAddr, 1, fileSize, fp) == 0, "Write failed");
        MY_LOGE_IF(fflush(fp), "Flush failed");
        MY_LOGE_IF(fclose(fp), "Close failed");
    } else {
        MY_LOGE("Cannot open %s", dumpPath);
