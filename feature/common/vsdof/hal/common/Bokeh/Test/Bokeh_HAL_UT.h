...
protected:
    ...
    virtual void TearDown() {
        ...
        if(sprintf(imagePath, UT_CASE_PATH"/%s/%s/BokehImage_%dx%d.yuv", getUTCaseName(), UT_CASE_OUT_FOLDER, imageSize.w, imageSize.h) >= 0)
        {
        ...
        if(sprintf(imagePath, UT_CASE_PATH"/%s/%s/PackedDepthBuffer_%dx%d.yuv", getUTCaseName(), UT_CASE_OUT_FOLDER, DEPTH_SIZE.w, DEPTH_SIZE.h*4) >= 0)
        {
    ...
    virtual bool isUTCaseExist()
    {
        ...
        char utPath[256];
        if(sprintf(utPath, "%s/%s/%s", UT_CASE_PATH, getUTCaseName(), UT_CASE_IN_FOLDER) >= 0)
        {
            if(stat(utPath, &st) == -1) {
        ...
        if(sprintf(utPath, "%s/%s/%s", UT_CASE_PATH, getUTCaseName(), UT_CASE_GOLDEN_FOLDER) >= 0)
        {
            if(stat(utPath, &st) == -1) {
                MY_LOGD("%s does not exist", utPath);
                return false;
        ...
        if(sprintf(utPath, "%s/%s/%s", UT_CASE_PATH, getUTCaseName(), UT_CASE_OUT_FOLDER) >= 0)
        {
            if(stat(utPath, &st) == -1) {
                MY_LOGD("%s does not exist", utPath);
                return false;
    ...
    void loadImages()
    {
        ...
        if(sprintf(imagePath, UT_CASE_PATH"/%s/%s/CleanImage_%dx%d.yuv", getUTCaseName(), UT_CASE_IN_FOLDER, imageSize.w, imageSize.h) >= 0)
        {
            MY_LOGD("Read %s...\n", imagePath);
        ...
        for(int i = 0; i < 3; i++) {
            ...
            if(sprintf(imagePath, UT_CASE_PATH"/%s/%s/%s_%dx%d.y", getUTCaseName(), UT_CASE_IN_FOLDER, BUFFER_NAME[i], DEPTH_SIZE.w, DEPTH_SIZE.h) >= 0)
            {
                MY_LOGD("Read %s...\n", imagePath);
        ...
        if(sprintf(imagePath, UT_CASE_PATH"/%s/%s/BokehImage_%dx%d.yuv", getUTCaseName(), UT_CASE_GOLDEN_FOLDER, imageSize.w, imageSize.h) >= 0)
        {
            MY_LOGD("Read %s...\n", imagePath);
