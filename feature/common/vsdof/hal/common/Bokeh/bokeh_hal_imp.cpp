    ...
    if(__needABF) {
        ...
        __convertABFResultThread = std::thread(
            [&]() mutable
            {
                ...
                if(!StereoDpUtil::transformImage(__workingImage.get(), param.cleanImage, rotation)) {
...
BOKEH_HAL_IMP::__parseExtraData(char *extraData)
{
    ...
    if(LogicalCamJSONUtil::HasMember(extraDataJson, EXTRA_DATA_CAPTURE_ORIENTATION)) {
        ...
        __capOrientation = rotation;
        if(StereoHAL::isVertical(__capOrientation)) {
        ...
        __depthRotation = rotation;

        if(StereoHAL::isVertical(__depthRotation)) {
