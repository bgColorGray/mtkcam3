    ...
    , __fastLogger(LOG_TAG, LOG_PERPERTY)
{
    ...
    for(size_t i = 0; i < OCC_COUNT; i++) {
        ...
        CAM_ULOG_ASSERT(NSCam::Utils::ULog::MOD_VSDOF_HAL, __pOCC[i] != nullptr, "Cannot create instance of OCC(%s)", OCC_NAME[i]);
...
OCC_HAL_IMP::__splitDisparityMapAndDump(const short *dispL, const short *dispR)
{
    ...
    FILE* fp = NULL;
    if(sprintf(fileName, "DPE_DISP_L_X_%dx%d.y", szDMP.w, szDMP.h) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        fp = fopen(dumpPath, "wb");
        if(fp) {
            MY_LOGE_IF(fwrite(l_x_map, 1, LEN, fp) == 0, "Write failed");
    ...
    if(sprintf(fileName, "DPE_DISP_L_Y_%dx%d.y", szDMP.w, szDMP.h) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        fp = fopen(dumpPath, "wb");
        if(fp) {
            MY_LOGE_IF(fwrite(l_y_map, 1, LEN, fp) == 0, "Write failed");
            MY_LOGE_IF(fflush(fp), "Flush failed");
            MY_LOGE_IF(fclose(fp), "Close failed");
        } else {
            MY_LOGE("Cannot write to %s, error: %s", fileName, strerror(errno));
    ...
    if(sprintf(fileName, "DPE_DISP_R_X_%dx%d.y", szDMP.w, szDMP.h) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        fp = fopen(dumpPath, "wb");
        if(fp) {
            MY_LOGE_IF(fwrite(r_x_map, 1, LEN, fp) == 0, "Write failed");
            MY_LOGE_IF(fflush(fp), "Flush failed");
            MY_LOGE_IF(fclose(fp), "Close failed");
        } else {
            MY_LOGE("Cannot write to %s, error: %s", fileName, strerror(errno));
    ...
    if(sprintf(fileName, "DPE_DISP_R_Y_%dx%d.y", szDMP.w, szDMP.h) >= 0)
    {
        genFileName_VSDOF_BUFFER(dumpPath, DUMP_PATH_SIZE, __dumpHint, fileName);
        fp = fopen(dumpPath, "wb");
        if(fp) {
            MY_LOGE_IF(fwrite(r_y_map, 1, LEN, fp) == 0, "Write failed");
            MY_LOGE_IF(fflush(fp), "Flush failed");
            MY_LOGE_IF(fclose(fp), "Close failed");
        } else {
            MY_LOGE("Cannot write to %s, error: %s", fileName, strerror(errno));
