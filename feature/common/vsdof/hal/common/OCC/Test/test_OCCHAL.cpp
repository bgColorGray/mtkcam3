...
TEST(HAL_TEST, OCCHAL_TEST)
{
    ...
    //Read LDC
    if(sprintf(fileName, "%s/XVEC_N3D.raw", OCC_UT_CASE_IN_PATH) >= 0)
    {
        MY_LOGD("Read %s...\n", fileName);
    ...
    do {
        ...
        if(sprintf(fileName, "%s/DV_HW_L_%05d.raw", OCC_UT_CASE_IN_PATH, index) >= 0)
        {
            if(stat(fileName, &st) == -1) {
        ...
        if(sprintf(fileName, "%s/DV_HW_R_%05d.raw", OCC_UT_CASE_IN_PATH, index) >= 0)
        {
            MY_LOGD("Read %s...", fileName);
            if(stat(fileName, &st) == -1) {
                MY_LOGE("error: %s", ::strerror(errno));
        ...
        if(sprintf(fileName, "%s/m_pDown_L_%05d.yv12", OCC_UT_CASE_IN_PATH, index) >= 0)
        {
            MY_LOGD("Read %s...", fileName);
            if(stat(fileName, &st) == -1) {
                MY_LOGE("error: %s", ::strerror(errno));
        ...
        if(sprintf(fileName, "%s/m_pDown_R_%05d.yv12", OCC_UT_CASE_IN_PATH, index) >= 0)
        {
            MY_LOGD("Read %s...", fileName);
            if(stat(fileName, &st) == -1) {
                MY_LOGE("error: %s", ::strerror(errno));
        ...
        //Read goldens
        //Read DS_xxxxx.yv12
        if(sprintf(fileName, "%s/DS_%05d.yv12", OCC_UT_CASE_OUT_PATH, index) >= 0)
        {
            MY_LOGD("Read %s...", fileName);
            if(stat(fileName, &st) == -1) {
                MY_LOGE("error: %s", ::strerror(errno));
        ...
        if(sprintf(fileName, "%s/WMI_DS8_pre_%05d.raw", OCC_UT_CASE_OUT_PATH, index) >= 0)
        {
            MY_LOGD("Read %s...", fileName);
            if(stat(fileName, &st) == -1) {
                MY_LOGE("error: %s", ::strerror(errno));
        ...
        if(sprintf(fileName, "%s/imageMain1_%05d.yv12", OCC_UT_CASE_HAL_IN_PATH, index) >= 0)
        {
            ...
            if(NULL == fp) {
                ...
                expectSize = sizeof(MUINT8)*IMAGE_SIZE.w*IMAGE_SIZE.h;
                MY_LOGE_IF(fwrite((void *)occHalParam.imageMain1->getBufVA(0), 1, expectSize,    fp) == 0, "Write failed");
                MY_LOGE_IF(fwrite((void *)occHalParam.imageMain1->getBufVA(1), 1, expectSize>>2, fp) == 0, "Write failed");
                MY_LOGE_IF(fwrite((void *)occHalParam.imageMain1->getBufVA(2), 1, expectSize>>2, fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            }
        }

        if(sprintf(fileName, "%s/imageMain2_%05d.yv12", OCC_UT_CASE_HAL_IN_PATH, index) >= 0)
        {
            MY_LOGD("Write %s...", fileName);
            ...
            if(NULL == fp) {
                ...
                expectSize = sizeof(MUINT8)*IMAGE_SIZE.w*IMAGE_SIZE.h;
                MY_LOGE_IF(fwrite((void *)occHalParam.imageMain2->getBufVA(0), 1, expectSize,    fp) == 0, "Write failed");
                MY_LOGE_IF(fwrite((void *)occHalParam.imageMain2->getBufVA(1), 1, expectSize>>2, fp) == 0, "Write failed");
                MY_LOGE_IF(fwrite((void *)occHalParam.imageMain2->getBufVA(2), 1, expectSize>>2, fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            }
        }

        if(sprintf(fileName, "%s/disparityLeftToRight_%05d.raw", OCC_UT_CASE_HAL_IN_PATH, index) >= 0)
        {
            MY_LOGD("Write %s...", fileName);
            fp = fopen(fileName, "wb");
            if(NULL == fp) {
                ...
                expectSize = sizeof(MUINT16)*IMAGE_SIZE.w*IMAGE_SIZE.h;
                MY_LOGE_IF(fwrite((void *)occHalParam.disparityLeftToRight, 1, expectSize,    fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            }
        }

        if(sprintf(fileName, "%s/disparityRightToLEft_%05d.raw", OCC_UT_CASE_HAL_IN_PATH, index) >= 0)
        {
            MY_LOGD("Write %s...", fileName);
            fp = fopen(fileName, "wb");
            if(NULL == fp) {
                ...
                expectSize = sizeof(MUINT16)*IMAGE_SIZE.w*IMAGE_SIZE.h;
                MY_LOGE_IF(fwrite((void *)occHalParam.disparityRightToLEft, 1, expectSize,    fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            }
        }

        if(sprintf(fileName, "%s/ldcMain1_%05d.raw", OCC_UT_CASE_HAL_IN_PATH, index) >= 0)
        {
            MY_LOGD("Write %s...", fileName);
            fp = fopen(fileName, "wb");
            if(NULL == fp) {
                ...
                expectSize = sizeof(MUINT8)*IMAGE_SIZE.w*IMAGE_SIZE.h;
                MY_LOGE_IF(fwrite((void *)occHalParam.ldcMain1, 1, expectSize,    fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            }
        }
        ...
        if(sprintf(fileName, "%s/DS32_M_OCC_HAL_%05d.raw", OCC_UT_CASE_HAL_OUT_PATH, index) >= 0)
        {
            MY_LOGD("Write %s...", fileName);
            fp = fopen(fileName, "wb");
            if(NULL == fp) {
                ...
                expectSize = sizeof(MUINT8)*CROPPED_SIZE.w*CROPPED_SIZE.h;
                MY_LOGE_IF(fwrite((void *)occHalOutput.downScaledImg->getBufVA(0), 1, expectSize,    fp) == 0, "Write failed");
                MY_LOGE_IF(fwrite((void *)occHalOutput.downScaledImg->getBufVA(1), 1, expectSize>>2, fp) == 0, "Write failed");
                MY_LOGE_IF(fwrite((void *)occHalOutput.downScaledImg->getBufVA(2), 1, expectSize>>2, fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            }
        }

        if(sprintf(fileName, "%s/WMI_DS8_pre_OCC_HAL_%05d.raw", OCC_UT_CASE_HAL_OUT_PATH, index) >= 0)
        {
            MY_LOGD("Write %s...", fileName);
            fp = fopen(fileName, "wb");
            if(NULL == fp) {
                ...
                MY_LOGE_IF(fwrite(occHalOutput.depthMap, 1, sizeof(MUINT8)*CROPPED_SIZE.w*CROPPED_SIZE.h, fp) == 0, "Write failed");
                MY_LOGE_IF(fflush(fp), "Flush failed");
                MY_LOGE_IF(fclose(fp), "Close failed");
            }
        }
