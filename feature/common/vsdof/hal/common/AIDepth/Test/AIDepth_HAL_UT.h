...
protected:
    ...
    {
        char filePath[256];
        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN1_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight) >= 0)
        {
            readImage(filePath, _imgMain1.get());
        }

        if(sprintf(filePath, UT_CASE_IN_FOLDER"/BID_P2A_OUT_RECT_IN2_%dx%d.yuv", _tuningInfo.imgWidth, _tuningInfo.imgHeight) >= 0)
        {
