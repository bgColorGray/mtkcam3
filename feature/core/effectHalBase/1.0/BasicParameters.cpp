...
flattenString8(void*& buffer, size_t& size, const String8& string8)
{
    ...
    const char16_t* str = string16.string();
    if (str == nullptr) {
        ALOGE("string16 mString is null");
    }else{
        memcpy(static_cast<char16_t*>(buffer), str, len);
    }
...
getFlattenedSize() const
{
    ...
    size_t variableSize{};
    if (string16.size() > (string16.size() + 1)) {
        ALOGE("String16 size overflow");
        return variableSize;
    }
    variableSize =
...
set(const char *key, int value)
{
    FUNCTION_LOG_START;
    char str[16]={0};
    int err = snprintf(str, sizeof(str)-1, "%d", value);
    if ( err >= 0 )
        set(key, str);
    FUNCTION_LOG_END;
}
...
setInt64(const char *key, int64_t value)
{
    FUNCTION_LOG_START;
    char str[32]={0};
    int err = snprintf(str, sizeof(str)-1, "%" PRId64 "", value);
...
setPtr(const char *key, void *value)
{
    ...
    int err = snprintf(str, sizeof(str)-1, "%#" PRIxPTR "", val);
    if ( err >= 0 )
        set(key, str);
    FUNCTION_LOG_END;
}
...
setFloat(const char *key, float value)
{
    ...
    int err = snprintf(str, sizeof(str)-1, "%g", value);
    if ( err >= 0 )
        set(key, str);
    FUNCTION_LOG_END;
}
