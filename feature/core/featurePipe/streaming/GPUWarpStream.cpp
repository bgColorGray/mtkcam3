...
MBOOL GPUWarpStream::init(const MUINT32 &sensorIdx, const MUINT32 &timerIdx, const MSize &maxImageSize, const MSize &maxWarpSize)
{
    TRACE_FUNC_ENTER();
    (void)(sensorIdx);
    ...
    {
        if( (mProcessThread = new ProcessThread(timerIdx)) != NULL )
        {
...
GPUWarpStream::ProcessThread::ProcessThread(const MUINT32 &timerIdx)
    : mStop(MFALSE)
    , mTimerIdx(timerIdx)
{
    TRACE_FUNC_ENTER();
...
MVOID GPUWarpStream::ProcessThread::processParam(WarpParam param)
{
    ...
    if( param.mResult && param.mMDPOut.size() )
    {
        ...
        else
        {
            ...
            if( param.mByPass )
            {
                param.mResult = mMDP.process(param.mScenarioFPS, param.mIn->getImageBufferPtr(), param.mMDPOut, param.mRequest->needPrintIO());
            }
            else
            {
                String8 str(" ");
                ...
                param.mResult = mMDP.process(param.mScenarioFPS, param.mWarpOut->getImageBufferPtr(), param.mMDPOut, param.mRequest->needPrintIO());
