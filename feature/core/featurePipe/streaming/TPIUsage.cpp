...
MUINT32 TPIUsage::getTPILength(const TPIOEntry &entry, const bool skipQueueCount) const
{
    MUINT32 len = 0;
    auto it = mGroupMap.find(entry);
    if( it != mGroupMap.end() && it->second.mEnable )
    {
        len += skipQueueCount?0:it->second.mTotalQueueCount;
        len += it->second.mNodes.size();
        len += 1; // vmdp
    }
    return len;
}

MUINT32 TPIUsage::getNumInBuffer(const TPIOEntry &entry) const
{
    MUINT32 num = 0;
    auto it = mGroupMap.find(entry);
    if( it != mGroupMap.end() && it->second.mEnable )
...
MUINT32 TPIUsage::getNumOutBuffer(const TPIOEntry &entry) const
{
    MUINT32 num = 0;
    auto it = mGroupMap.find(entry);
    if( it != mGroupMap.end() && it->second.mEnable )
