...
{
    TRACE_FUNC_ENTER();
    TRACE_FUNC("Frame %d: %s arrived, CBMsg=%d", data.mRequest->mRequestNo, ID2Name(id), data.mData.mCBMsg);
    MBOOL ret = MFALSE;
    if( id == ID_P2G_TO_HELPER ||
        id == ID_DEPTH_TO_HELPER ||
        ...
        id == ID_XNODE_TO_HELPER ||
        id == ID_TRACKING_TO_HELPER ||
        0
        )
    {
...
MBOOL HelperNode::onInit()
{
    ...
    {
        mWaitSet.insert(HMSG_RSSO_DONE);
        mDirectCBSet.insert(HMSG_RSSO_DONE);
    }

    if (mPipeUsage.supportDepthP2() || mPipeUsage.supportP2GWithDepth()) {
        mWaitSet.insert(HMSG_YUVO_DONE);
    }

    if( mPipeUsage.supportWarpNode(SFN_HINT::TWIN) || mPipeUsage.supportVMDPNode(SFN_HINT::TWIN) )
    {
        mWaitSet.insert(HMSG_DISPLAY_DONE);
    }

    if( mPipeUsage.supportTrackingFocus() )
    {
        mWaitSet.insert(HMSG_TRACKING_DONE);
    }

    if( mPipeUsage.supportP2AP2() ||
        mPipeUsage.supportSMP2() ||
        mPipeUsage.supportDepthP2() )
    {
            ...
            !request->mHelperNodeData.isReady(HMSG_ASYNC_DONE) )
        {
            ...
            if( needAsync )
            {
                ...
                if( popImg != NULL )
                {
                    ...
                    if( display && copy )
                    {
                        mdp = mMDP.process(request->getAppFPS(), display, copy);
