...
TPI_AsyncNode::TPI_AsyncNode(const char* name)
    : CamNodeULogHandler(Utils::ULog::MOD_STREAMING_TPI_ASYNC)
...
NextIO::Policy TPI_AsyncNode::getIOPolicy(const NextIO::ReqInfo &reqInfo) const
{
    ...
    {
        StreamMask run;
        run.enable(STREAMTYPE_ASYNC);
        policy.setRun(run, reqInfo.mMasterID);

        NextIO::NextAttr attr;
        attr.setAttrByPool(mBufferPool);
        // return MSize(0,0) to indicate directly using display size
        //attr.mResize = mInputInfo.mSize;
        policy.add(reqInfo.mMasterID, NextIO::NORMAL, attr, mBufferPool);
    }
    return policy;
}
...
MBOOL TPI_AsyncNode::onInit()
{
    ...
    if( mEnable )
    {
        ...
        mNumWorkingBuffer = mTPIUsage.getNumInBuffer(TPIOEntry::ASYNC) + mPipeUsage.getP2GLength() + mPipeUsage.getDispPostProcLength();
