...
MVOID TimgoHal::init(const MSize &streamingSize)
{
    ...
    {
        mBufferSize = MSize(256, 256);
        mBufferFmt = eImgFmt_BAYER10;
        MY_LOGD("Not Support This Dip Timgo Dump Type(%u). Ignore It.", mDipType);
    }

    MY_LOGD("Timgo enable(%d) type(%s:%d) size(%dx%d) fmt(0x%x)",
            mIsSupport, mTypeStr, mDipType,
            mBufferSize.w, mBufferSize.h, mBufferFmt);
}
...
EImageFormat TimgoHal::toBufferFmt(NSIoPipe::EDIPTimgoDumpEnum dipType)
{
    switch(dipType)
    {
        ...
        default:                                return eImgFmt_BAYER10;
    }
}
