               ...
               Feature::SecureBufferControl&   secBufCtrl)
{
    ...
    if (secureEnum != NSIoPipe::EDIPSecureEnum::EDIPSecure_NONE)
    {
        ...
        for( auto &in : frameParam.mvIn )
        {
            ...
            else if((secDMAType == NSIoPipe::EDIPSecDMAType::EDIPSecDMAType_TUNE_SHARED ||
                     secDMAType == NSIoPipe::EDIPSecDMAType::EDIPSecDMAType_TUNE_SHARED_FD) &&
                     in.mBuffer != NULL)
            {
                auto isVA = (secDMAType == NSIoPipe::EDIPSecDMAType::EDIPSecDMAType_TUNE_SHARED);
                MVOID *secHandle = secBufCtrl.registerAndGetSecHandle(in.mBuffer, isVA);
                if ( secHandle != NULL )
                {
