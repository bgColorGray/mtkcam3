...
void TPIMgr_PluginWrapper::initSelection(T_Selection &sel, const std::shared_ptr<NSCam::IMetadata> &meta)
{
  if( sel != NULL )
  {
    ...
    sel->mCfgRun = false;
    sel->mCfgEnablePerFrameNegotiate = true;
    sel->mP2Run = false;
...
void TPIMgr_PluginWrapper::print(const T_Selection &sel)
{
    ...
    else
    {
        ...
        MY_LOGD("sel->mCfgCropIntput=%d", sel->mCfgCropInput);
        MY_LOGD("sel->mCfgSharedInput=%d", sel->mCfgSharedInput);
        ...
        MY_LOGD("sel->mCfgEnablePerFrameNegotiate=%d", sel->mCfgEnablePerFrameNegotiate);
...
bool TPIMgr_PluginWrapper::createSession(unsigned mainSensorID, const std::vector<MUINT32> &allSensorIDs, unsigned numSensor, TP_MASK_T masks, MFLOAT marginRatio, const MSize &maxRawSize, const MSize &maxOutSize, const IMetadata &meta, const NSPipelinePlugin::TPICustomConfig &tpiCustomConfig, bool hasEISNode)
{
  bool ret = false;
  ...
  if( mState == TPIMgr_PluginWrapper::STATE_IDLE )
  {
    ...
    createPluginSession(mainSensorID, allSensorIDs, masks, meta, marginRatio, tpiCustomConfig, hasEISNode);
      ...
      sel != NULL && metaPtr != NULL)
  {
    ...
    for( auto &info : mPluginMap )
    {
          ...
          info.second.mPlugin != NULL )
      {
        ...
        {
          if( info.second.mSelection->mCfgEnablePerFrameNegotiate )
          {
...
void TPIMgr_PluginWrapper::createPluginSession(unsigned mainSensorID, const std::vector<MUINT32> &allSensorIDs, TP_MASK_T masks, const IMetadata &meta, MFLOAT marginRatio, const  NSPipelinePlugin::TPICustomConfig &tpiCustomConfig, bool hasEISNode)
{
    TRACE_FUNC_ENTER();
    ...
    if( mPluginPool != NULL )
    {
        ...
        for( auto &plugin : mPluginPool->getProviders(masks) )
        {
            ...
            {
                print(sel);
                checkPluginConfig(sel, marginRatio, hasEISNode);
            }
        }
    }
    TRACE_FUNC_EXIT();
...
void TPIMgr_PluginWrapper::checkPluginConfig(const T_Selection &sel, MFLOAT marginRatio, bool hasEISNode)
{
    TRACE_FUNC_ENTER();
    if( sel != NULL )
    {
        ...
        if(hasEISNode)
        {
            CAM_ULOGM_ASSERT(sel->mCfgJoinEntry != NSPipelinePlugin::eJoinEntry_S_DIV_2,
                    "TPI DIV_2 can't enable when MTK EIS ON!\nCRDISPATCH_KEY:TPI.EIS.conflict");
        }
    }
    TRACE_FUNC_EXIT();
}
...
void TPIMgr_PluginWrapper::updateImageInfo(TPI_Image &img, const JoinImageInfo &info)
{
    ...
    case eJoinImageID_MAIN2:
        ...
        break;
    default:
        MY_LOGW_IF(img.mUseSrcImageBuffer, "img(%d) use Src img replace, but unknown srcImgId(%d), ignore src inplace!!", img.mBufferID, info.mOSrcImageID);
