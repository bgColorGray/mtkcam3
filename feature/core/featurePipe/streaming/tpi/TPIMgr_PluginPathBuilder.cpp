...
private:
    ...
    using T_Property = TPIMgr_PluginWrapper::T_Property;
...
private:
    ...
    static T_Property getProperty(const PluginMap &pluginMap, T_NodeID id);
    ...
    static TPI_NodeInfo toNodeInfo(unsigned id, PathType pathType, const T_Selection &sel, const T_Property &prop);
    static bool toNodeInfo(TPI_NodeInfo &node, PathType pathType, const T_Selection &sel, const T_Property &prop);
...
PluginPathBuilder::T_Property PluginPathBuilder::getProperty(const PluginMap &pluginMap, T_NodeID id)
{
    T_Property prop;
    PluginMap::const_iterator it = pluginMap.find(id);
    if( it != pluginMap.end() )
    {
        prop = it->second.mProperty;
    }
    return prop;
}
...
TPI_NodeInfo PluginPathBuilder::toNodeInfo(unsigned id, PathType pathType, const T_Selection &sel, const T_Property &prop)
{
    TPI_NodeInfo node(id);
    toNodeInfo(node, pathType, sel, prop);
    return node;
}

bool PluginPathBuilder::toNodeInfo(TPI_NodeInfo &node, PathType pathType, const T_Selection &sel, const T_Property &prop)
{
    bool ret = false;
    if( sel != NULL )
    {
        ...
        node.mName = std::move(std::string(prop.mName ? prop.mName : "unknown"));
...
void PluginPathBuilder::buildPath(TPI_Session &session, const PluginMap &pluginMap, PathType pathType, const NodeIDList &list)
{
    ...
    for( T_NodeID nodeID : list )
    {
        ...
        T_Property prop = getProperty(pluginMap, nodeID);
        if( sel != NULL )
        {
            TPI_NodeInfo node = toNodeInfo(nodeID, pathType, sel, prop);
