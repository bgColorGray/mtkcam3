...
const char* Fmt2Name(MINT fmt)
{
    ...
    case eImgFmt_YUV_P010:          return "p010";
...
MBOOL isGrallocFormat(MINT fmt)
{
    switch(fmt)
    {
...
MBOOL isTargetOutput(IO_TYPE target, const SFPOutput &output)
{
    switch(target)
    {
    case IO_TYPE_DISPLAY:   return isDisplayOutput(output);
    case IO_TYPE_RECORD:    return isRecordOutput(output);
    case IO_TYPE_EXTRA:     return isExtraOutput(output);
    case IO_TYPE_FD:        return isFDOutput(output);
    case IO_TYPE_PHYSICAL:  return isPhysicalOutput(output);
    case IO_TYPE_APP_DEPTH: return isAppDepthOutput(output);
    default:                MY_LOGW("Invalid output target type = %d", target); return MFALSE;
    }
}
...
MBOOL isAppDepthOutput(const SFPOutput &output)
{
    return (output.mTargetType == SFPOutput::OUT_TARGET_APP_DEPTH);
}
...
MBOOL toDpColorFormat(const NSCam::EImageFormat fmt, DpColorFormat &dpFmt)
{
    ...
    case eImgFmt_YUV_P010: dpFmt = DP_COLOR_NV12_10L; break;
    default:
...
MUINT32 allocate(const char *prefix, const sp<IBufferPool> &pool, MUINT32 count)
{
    ...
    if( pool != NULL && count )
    {
        MY_LOGI("%s %s %d/%d buf (0x%x/%s)(%dx%d) in %d ms", prefix, STR_ALLOCATE, pool ? count : 0, count, fmt, toName(fmt), size.w, size.h, timer.getElapsed());
    }
    return count;
}
...
MBOOL isValid(const MRectF &crop)
{
...
MSizeF toMSizeF(const MSize &size)
{
    return MSizeF(size.w, size.h);
}

MRect toMRect(const MSize &size)
{
    return MRect(MPoint(0,0), size);
}

MRectF toMRectF(const MSize &size)
{
    return toMRectF(toMSizeF(size));
}

MRectF toMRectF(const MSizeF &size)
{
    return MRectF(MPointF(0,0), size);
}
...
MVOID print(const char *prefix, const P2IO &io)
{
    MY_LOGD("%s(%p) pq(%d) size=(%dx%d) crop=(%fx%f@(%f,%f)",
            prefix, io.mBuffer, (io.mPQCtrl != NULL),
            io.mCropDstSize.w, io.mCropDstSize.h,
            io.mCropRect.s.w, io.mCropRect.s.h,
            io.mCropRect.p.x, io.mCropRect.p.y);
}
