...
enum eDepthPipeCBTarget {
    eTARGET_DEPTH_OUT = 0,
    eTARGET_BUFFER_RELEASE,
    eTARGET_TOTAL,
};
...
public:
    RequestPtr mRequest;
    BasicImgData mYuvOut;
    DepthImg   mDepthOut;
    std::bitset<eTARGET_TOTAL> mTargetBitSet;
};
...
private:
    ...
    MUINT32 mDebugImgPoolAllocateNeed    = 0;
    MUINT32 mDreTuningPoolAllocateNeed   = 0;
    ...
    android::sp<IBufferPool> mDebugImgPool       = NULL;
    android::sp<TuningBufferPool> mDreTuningPool = NULL;
#if (SUPPORT_ISP_VER >= 60)
    ...
    MUINT32 setOutputData(sp<IDepthMapEffectRequest> pDepMapReq, DepthEnqueData& out,
                                    const SFPIOMap &generalIO);

    IImageBuffer* createImageBufferFromFile(const IImageBufferAllocator::ImgParam imgParam,
                                    const char* path,
                                    const char* name,
                                    MINT usage);
...
private:
    ...
    android::Mutex mLock;
    int miLogEnable    = 0;
    int miShowDepthMap = 0;
    int miFeautreMode  = 0;
    std::vector<MUINT8> mSensorID;

    // DRE HAL
    MBOOL mbApplyDREFeature = MFALSE;
    DreHal mDreHal;
    MUINT32 mDreTuningByteSize = 0;
    std::shared_ptr<P2G::SensorContextMap> mSensorContextMap = nullptr;
