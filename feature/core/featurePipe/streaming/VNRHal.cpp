...
MBOOL VNRHal::isSupport(EImageFormat format)
{
    ...
    case eImgFmt_NV21:  return MTRUE;
...
MBOOL VNRHal::init(const MSize &maxSize, MBOOL use10Bit)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MFALSE;
    if( mDSDN != NULL )
...
MBOOL VNRHal::initDSDN()
{
    ...
    initInfo.useHighBit = mUse10Bit ? 1 : 0;
...
MBOOL VNRHal::addBufferInfo(struct DSDNProcInfo &info, DSDN_BUFFER_TYPE_ENUM type, const IImageBuffer *img) const
{
    ...
    if( img )
    {
        ...
        else
        {
            ...
            MINT32 baseFD = img->getPlaneFD(0);
            MUINT8 *baseVA = (MUINT8*)img->getBufVA(0) - img->getPlaneOffsetInBytes(0);
            ...
            for( unsigned i = 0; i < planeCount; ++i )
            {
                ...
                if( heapCount != 1 )
                {
                    planeInfo->fd = img->getPlaneFD(i);
                    planeInfo->offset = img->getPlaneOffsetInBytes(i);
                }
