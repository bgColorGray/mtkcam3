...
MVOID EISQControl::update(const EISQActionInfo& status) {
  ...
  MBOOL isQR =
      mUseQ && (status.mAppMode == IStreamingFeaturePipe::APP_VIDEO_RECORD);
  MBOOL isQS =
      mUseQ && (status.mAppMode == IStreamingFeaturePipe::APP_VIDEO_STOP);
  ...
  switch (action) {
    ...
    case EISQ_ACTION_READY:
      action = !isQR ? EISQ_ACTION_NO
                     : (status.mIsAppEIS && status.mRecordCount > mPSize &&
                        status.mIsReady)
                           ? EISQ_ACTION_INIT
                           : EISQ_ACTION_READY;
      break;
    ...
    case EISQ_ACTION_PUSH:
      action = isQR ? ((counter < mQSize) ? EISQ_ACTION_PUSH : EISQ_ACTION_RUN)
                    : (isQS ? EISQ_ACTION_STOP : EISQ_ACTION_NO);
      counter = isQR ? ((counter < mQSize) ? counter + 1 : counter)
                     : (isQS ? counter - 1 : 0);
      mPushCount = (isQR || isQS) ? std::max(counter, mPushCount) : 0;
      break;
    case EISQ_ACTION_RUN:
      action = isQR ? action : isQS ? EISQ_ACTION_STOP : EISQ_ACTION_NO;
      counter = isQR ? counter : isQS ? mPushCount - 1 : 0;
      break;
    case EISQ_ACTION_STOP:
      action = EISQ_ACTION_NO;
      counter = 0;
  ...
  TRACE_FUNC("AppMode=%d recNo=%d QAction=%d QCounter=%d", status.mAppMode,
             status.mRecordCount, mCurrent.mAction, mCurrent.mCounter);
  TRACE_FUNC_EXIT();
}
