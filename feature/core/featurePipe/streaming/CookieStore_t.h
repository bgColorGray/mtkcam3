...
public:
    ...
    static void staticProcessCB(const T_Msg &msg, T_Param param, T_Token token, bool last = true);

private:
    static T_Token store(T_Store *store, T_User *user, const T_Cookie &cookie, const T_Data &data);
    static bool fetch(T_Token token, T_Store* &store, T_User* &user, T_Cookie &cookie, T_Data &data, bool last);
