...
void CookieStore<T_User>::enque(T_User *user, T_Stream *stream, T_Param param, const T_Data &data)
{
    ...
    if( !user->onCookieStoreEnque(stream, param) )
    {
        ...
        staticProcessCB(T_User::MSG_COOKIE_FAIL, param, token);
    }
    TRACE_FUNC_EXIT();
}
...
template <typename T_User>
void CookieStore<T_User>::staticProcessCB(const T_Msg &msg, T_Param param, T_Token token, bool last)
{
    TRACE_FUNC_ENTER();
    T_User *user = NULL;
    T_Store *store = NULL;
    ...
    if( !T_Store::fetch(token, store, user, cookie, data, last) )
    {
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_STREAMING, "Invalid data from token=%p, store=%p, user=%p last=%d", token, store, user, last);
    }
    else if( !user )
    {
...
template <typename T_User>
bool CookieStore<T_User>::fetch(T_Token token, T_Store* &store, T_User* &user, T_Cookie &cookie, T_Data &data, bool last)
{
    TRACE_FUNC_ENTER();
    bool ret = false;
