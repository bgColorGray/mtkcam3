...
PluginBufferHandle::acquireAll(MINT usage)
{
    (void) usage;
    if(mpImageBuffers.empty()) {
        auto pNodeRequest = mpNodeRequest.promote();
        if (pNodeRequest == NULL)
            return mpImageBuffers;

        mpImageBuffers = pNodeRequest->acquireBufferAll(mBufferId);
        if (mpImageBuffers.empty()) {
            MY_LOGW("acquire an empty buffer, bufId:%d", mBufferId);
        }
#ifdef __DEBUG
        for (auto& pImageBuffer : mpImageBuffers) {
            MY_LOGI("Fmt(0x%x) ImgSize(%dx%d) BufSize(%d) Stride(%d) VA/PA(%#" PRIxPTR "/%#" PRIxPTR ")",
...
{
    struct AccessPrivConstructor : public PluginSmartBufferHandle
    {
        AccessPrivConstructor(const char* name, const android::sp<IIBuffer>& imgBuffer)
            : PluginSmartBufferHandle(name, imgBuffer) {}
    };
    return std::make_shared<AccessPrivConstructor>(name, imgBuffer);
}
