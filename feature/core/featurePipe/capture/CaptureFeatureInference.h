...
private:
    ...
    struct DstData
    {
        ...
        template <typename Type>
        void setSupportFormats(std::vector<Type> vecfmt) {
            mvSupportFormats = vecfmt;
        }
        inline void setAllFmtSupport(MBOOL bSupport) {
            mbAllFmtSupported = bSupport;
        }
        ...
        inline void setAlignInputFmt(bool bAlign) {
            mbAlignInputFmt = bAlign;
        }
        inline bool getAlignInputFmt() {
            return mbAlignInputFmt;
        }
...
private:
    ...
    Format_T getFormat(TypeID_T typeId);
    ...
    inline MINT32 getMultiframeType() {
        return mMultiframeType;
    }
    ...
    inline MBOOL isRawInput() {
        return mIsRawInput;
    }

    inline MBOOL isMasterIndex() {
        return mIsMasterIndex;
    }

    // after connection finish, check the bypass possibility
    MBOOL refineBypassNodes(sp<CaptureFeatureRequest> pRequest);
...
public:
    ...
    shared_ptr<IMetadata>   mpIMetadataHal2;
    shared_ptr<IMetadata>   mpIMetadataDynamic2;
