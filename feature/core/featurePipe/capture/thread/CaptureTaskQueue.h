...
public:
    ...
    virtual MVOID addTask(std::function<void()>& func, MBOOL isNeedSync=MFALSE);
...
private:
    ...
    MBOOL                               mStop;
    MBOOL                               mIsSyncTaskThreadExist = MFALSE;
    MUINT8                              mThreadMax;
    MUINT8                              mThreadCount;
    std::deque<std::function<void()>>   mTasks;
    // for p2a::onRequestFinish to use the same thread
    // to guarantee in-order dispatch of multiframe request
    std::deque<std::function<void()>>   mSyncTasks;
    std::mutex                          mTaskLock;
    std::mutex                          mSyncTaskLock;
    std::mutex                          mThreadLock;
    std::condition_variable             mTaskCond;
    std::condition_variable             mSyncTaskCond;
