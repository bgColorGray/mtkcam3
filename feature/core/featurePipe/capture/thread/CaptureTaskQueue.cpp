...
CaptureTaskQueue::~CaptureTaskQueue()
{
 ...
 MVOID CaptureTaskQueue::addTask(std::function<void()>& func, MBOOL isNeedSync)
{
    if(!isNeedSync) {
        ...
        std::unique_lock<std::mutex> lock(mSyncTaskLock);
        mSyncTasks.push_back(std::move(func));

        if(!mIsSyncTaskThreadExist) {

            mIsSyncTaskThreadExist = MTRUE;
            std::thread t([this]() {
                while (!mStop) {
                    std::function<void()> task;
                    {
                        std::unique_lock<std::mutex> lock(mSyncTaskLock);
                        if(mSyncTasks.empty()) {
                            mSyncTaskCond.wait(lock);
                            continue;
                        }
                        task = std::move(mSyncTasks.front());
