...
{
    return ImageTransformPtr(IImageTransform::createInstance(PIPE_CLASS_TAG, -1, IImageTransform::OPMode_DisableWPE), [](IImageTransform *p) -> MVOID
    {
        p->destroyInstance();
    });
}
...
createPluginRequestPtr(NodeID_T nodeId, MINT32 sensorIndex, const vector<MINT>& inputFmts, sp<CaptureBufferPool> bufPool, PluginPtr& pluginPtr, PipeRequestPtr& pipeRequestPtr) -> PluginRequestPtr
{
    ...
    if ( mainFmt != subFmt )
    {
        ...
        mainBufHadlePtr = PluginSmartBufferHandle::createInstance("TID_MAN_FULL_YUV_CONVERTED", subFmt, sensorIndex, pMainImgBuf, bufPool);
        if ( getIsDumpImage<NID_DEPTH>() )
        {
            pipeRequestPtr->dump("DEPTH_IN_MAIN_WORKING_BUF", nodeId, mainBufHadlePtr->getSmartImageBuffer()->getImageBufferPtr());
        }
...
process(ProviderPtr& providerPtr, RequestPtr& pipeRequestPtr)
{
    ...
    //
    auto pluginRequestPtr = DepthNodeUtility::createPluginRequestPtr(getNodeID(), sensorId, inputFmts, mpBufferPool, mPluginPtr, pipeRequestPtr);
    {
    ...
    if ( getIsDumpImage<NID_DEPTH>() )
    {
        pipeRequestPtr->dump("DEPTH_IN_TID_MAN_FULL_YUV", getNodeID(), TID_MAN_FULL_YUV, INPUT);
        pipeRequestPtr->dump("DEPTH_IN_TID_SUB_FULL_YUV", getNodeID(), TID_SUB_FULL_YUV, INPUT);
    }
...
evaluate(NodeID_T nodeId, CaptureFeatureInferenceData& rInfer)
{
    ...
    for (auto& item : mProviderInfoTable)
    {
        ...
        if ( isOK && rSel.mIBufferLCESHO.getRequired() )
        {
            const TypeID_T typeId = TID_MAN_LCESHO;
            const MBOOL hasType = rInfer.hasType(typeId);
            const MBOOL isValid = MTRUE;
        ...
        if ( isOK && rSel.mIBufferLCESHO2.getRequired() )
        {
            const TypeID_T typeId = TID_SUB_LCESHO;
            const MBOOL hasType = rInfer.hasType(typeId);
            const MBOOL isValid = MTRUE;
...
onCompleted(PluginRequestPtr pluginRequestPtr, MERROR status)
{
    ...
    if (status != OK) {
        pipeRequestPtr->addParameter(PID_FAILURE, 1);
    }
    //
    if ( getIsDumpImage<NID_DEPTH>() )
    {
        pipeRequestPtr->dump("DEPTH_OUT_TID_MAN_DEPTH", getNodeID(), TID_MAN_DEPTH, OUTPUT);
    }
    //
    dispatch(pipeRequestPtr);
    //
