...
MBOOL FDNode::onRequestProcess(RequestPtr& pRequest)
{
    ...
    CAM_TRACE_FMT_BEGIN("fd:process|r%df%ds%d", requestNo, frameNo, sensorId);
    MY_LOGD("+, R/F/S Num: %d/%d/%d", requestNo, frameNo, sensorId);
    //
    ...
    // [2] FD init
    int ret = mpFDHalObj->halFDInit(fdSize.w, fdSize.h, mpFDWorkingBuffer, mpPureYBuffer ? FD_WORKING_BUF_SIZE : FD_40_WORKING_BUF_SIZE, 1, HAL_FD_MODE_MANUAL);
    ...
    if (!NSIoPipe::NSPostProc::INormalStream::queryDIPInfo(dipInfo)) {
        MY_LOGE("queryDIPInfo Error! Please check the error msg above!");
    }
    bool isISP3_0 = (dipInfo[NSIoPipe::EDIPINFO_DIPVERSION] == NSIoPipe::EDIPHWVersion_30);

    MRect cropRegion(MPoint(0, 0), MSize(0, 0));
    if (isISP3_0) {
        cropRegion = mpCropCalculator->getActiveArray(sensorId);
        MY_LOGD("fd crop region from active region(%d,%d)(%dx%d)",
                cropRegion.p.x, cropRegion.p.y, cropRegion.s.w, cropRegion.s.h);
    } else {
        const MBOOL isIgnoreCrop = pRequest->hasParameter(PID_IGNORE_CROP);
        const MBOOL isReproc     = pRequest->hasParameter(PID_REPROCESSING);
        const MRect atRegion     = mpCropCalculator->getActiveArray(sensorId);

        sp<CropCalculator::Factor> pFactor = mpCropCalculator->getFactor(pAppMeta, pHalMeta, sensorId);
        if (isIgnoreCrop) {     // disable cropping
            cropRegion = atRegion;
        } else if (isReproc) {  // reproc with cropping
            CropCalculator::evaluate(atRegion.s, fdSize, pFactor->mActiveCrop, cropRegion);
        } else {
            MRect sensorCropRegion(MPoint(0, 0), MSize(0, 0));
            mpCropCalculator->evaluate(pFactor, fdSize, sensorCropRegion);
            // fd crop region transform back to active domain
            pFactor->mSensor2Active.transform(sensorCropRegion, cropRegion);
        }

        MY_LOGD("fd crop region(%d,%d)(%dx%d) active region(%d,%d)(%dx%d)",
                cropRegion.p.x, cropRegion.p.y, cropRegion.s.w, cropRegion.s.h,
                atRegion.p.x, atRegion.p.y, atRegion.s.w, atRegion.s.h);
    }
    ...
    for (size_t i = 0; i < faceCount; i++) {
        ...
        if (!mpFOVCalculator->getIsEnable() || !mpFOVCalculator->transform(sensorId, tgLeftTop, tgRightBottom, pipeLeftTop, pipeRightBottom)) {
