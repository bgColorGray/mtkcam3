...
process(ProviderPtr& providerPtr, RequestPtr& pipeRequestPtr)
{
    ...
    {
        pipeRequestPtr->dump("BOKEH_IN_TID_MAN_FULL_YUV", getNodeID(), TID_MAN_FULL_YUV, INPUT);
        pipeRequestPtr->dump("BOKEH_IN_TID_MAN_DEPTH", getNodeID(), TID_MAN_DEPTH, INPUT);
    }
...
onCompleted(PluginRequestPtr pluginRequestPtr, MERROR status)
{
    ...
    if ( getIsDumpImage<NID_BOKEH>() )
    {
        pipeRequestPtr->dump("BOKEH_OUT_TID_MAN_FULL_YUV", getNodeID(), TID_MAN_FULL_YUV, OUTPUT);
    }
    //
    dispatch(pipeRequestPtr);
    //
