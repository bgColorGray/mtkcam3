...
public:
    virtual MERROR offer(RawPlugin::Selection& sel)
    {
        sel.mIBufferFull
            ...
            .addSupportFormat(eImgFmt_BAYER10_MIPI)
...
MBOOL RAWNode::onRequestProcess(RequestPtr& pRequest)
{
    ...
    CAM_TRACE_FMT_BEGIN("raw:process|r%df%ds%d", requestNo, frameNo, sensorId);
    ...
    if (pProvider->process(pPluginReq, mpCallback) != OK) {
        onRequestFinish(pRequest);
        AEE_ASSERT("R/F/S Num: %d/%d/%d, RAW plugin process failed! fid=%s\nCRDISPATCH_KEY:CaptureFeaturePipe",
                    pRequest->getRequestNo(), pRequest->getFrameNo(), pRequest->getMainSensorIndex(), FeatID2Name(featId));
        return MFALSE;
    }
...
MVOID RAWNode::onRequestFinish(RequestPtr& pRequest)
{
    MINT32 requestNo = pRequest->getRequestNo();
    MINT32 frameNo = pRequest->getFrameNo();
    MINT32 sensorId = pRequest->getMainSensorIndex();
    CAM_TRACE_FMT_BEGIN("raw:finish|r%df%ds%d", requestNo, frameNo, sensorId);
    MY_LOGI("(%p)+, R/F/S Num: %d/%d/%d", this, requestNo, frameNo, sensorId);
        ...
        if (isValid && sel.mIBufferFull.getRequired()) {
            if (sel.mIBufferFull.isValid()) {
                ...
                if (rProperty.mInPlace) {
                    ...
                    dst_0.mInPlace = true;
                    dst_0.setAlignInputFmt(sel.mAlignInputFmt);
                }
            } else
                isValid = MFALSE;
        }
