...
public:
    ...
    virtual MVOID                       queryTypeId(BufferID_T, TypeID_T&, Direction&);

    virtual MBOOL                       hasMetadata(MetadataID_T);

    //
    virtual IImageBuffer*               acquireBuffer(BufferID_T);
    virtual std::vector<IImageBuffer*>  acquireBufferAll(BufferID_T bufId);
    virtual MBOOL                       isMultiBuffer(BufferID_T bufId);
...
public:
    ...
    virtual MVOID                   addTuningBuffer(BufferID_T, SmartTuningBuffer);
    ...
    virtual SmartTuningBuffer       getTuningBuffer(BufferID_T);
    ...
    virtual MBOOL                   isFeatureSupportSub(FeatureID_T);
    virtual MVOID                   setSensorIndex(MINT32, MINT32, MINT32);
...
public:
    ...
    virtual MBOOL                   isMasterIndex(MINT32 sensorIndex);
    ...
    // in case that onRequestFinish is performed earlier than onRequestProcess
    // add check to callback next capture
    std::mutex                      mLockNextCapture;
    MBOOL                           mIsNextCaptureCallBacked = MFALSE;
// boost operators
public:
