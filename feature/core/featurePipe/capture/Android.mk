...
ifeq ($(MTKCAM_OPT_ISP_VER),65)
    LOCAL_CFLAGS += -DCAPTURE_PIPE_IPVER=65
    LOCAL_SRC_FILES += nodes/isp6s/P2ANode.cpp
    LOCAL_SRC_FILES += nodes/isp6s/EnqueISPWorker.cpp

else
    # default version
    LOCAL_CFLAGS += -DCAPTURE_PIPE_IPVER=0
    LOCAL_SRC_FILES += nodes/default/P2ANode.cpp
endif
...
ifeq ($(MTKCAM_OPT_CAPTURE_ISP_VERSION),6)
MTK_CAPTURE_ISP_VERSION         := 6
endif

ifeq ($(MTKCAM_OPT_RAW_FORMAT_UTILITY),yes)
    P2_RAW_FORMAT_UTILITY       := 1
else
    P2_RAW_FORMAT_UTILITY       := 0
endif

LOCAL_CFLAGS += -DMTK_CAPTURE_ISP_VERSION=$(MTK_CAPTURE_ISP_VERSION)
LOCAL_CFLAGS += -DP2_RAW_FORMAT_UTILITY=$(P2_RAW_FORMAT_UTILITY)
################################################################################
# MFLL
################################################################################
...
else ifeq ($(MTKCAM_OPT_CAP_MFLL_APPLY_RAW_BSS),yes)
MFLL_APPLY_RAW_BSS           := 1
endif
...
################################################################################
SUPPORT_SWNR := 1
ifeq ($(MTKCAM_OPT_SWNR),no)
SUPPORT_SWNR := 0
endif
LOCAL_CFLAGS += -DSUPPORT_SWNR=$(SUPPORT_SWNR)
################################################################################
# AINR
################################################################################
...
# MSNR
LOCAL_SHARED_LIBRARIES += libmsnr
