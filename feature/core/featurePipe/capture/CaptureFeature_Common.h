...
#define AEE_ASSERT(fmt, arg...)                                            \
    do {                                                                   \
        ...
        CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_CAPTURE,                   \
          "ASSERT(%s) fail", str.string());                                \
        aee_system_exception("mtkcam/CaptureFeaturePipe", NULL,            \
          DB_OPT_DEFAULT, str.string());                                   \
        int res = raise(SIGKILL);                                          \
        if (res != 0) {                                                    \
          CAM_ULOGE(NSCam::Utils::ULog::MOD_FPIPE_CAPTURE,                 \
          "raise failed, res = %d", res);                                  \
        }                                                                  \
    } while(0)
#else
#define AEE_ASSERT(fmt, arg...) \
...
#define ISP_PROFILE_WITH_IDENTITY_EFFECT EIspProfile_Thumbnail
/******************************************************************************
 *  Utility
 ******************************************************************************/
...
template<typename T>
static inline MBOOL isZoomMode(T& t)
{
    const MUINT32 dualMode = t.mDualMode;
    const MBOOL isZoom = (dualMode & v1::Stereo::E_DUALCAM_FEATURE_ZOOM);
    return isZoom;
}
...
{
    const MUINT32 dualMode = t.mDualMode;
    const MBOOL isZoom = (dualMode & v1::Stereo::E_STEREO_FEATURE_MULTI_CAM);
    return isZoom;
}
/******************************************************************************
 *  Metadata Access
...
public:
    ...
    static MBOOL isSupport(HWCapability cap);
    static MVOID setSupport(HWCapability cap, MBOOL issupport);

private:
    static MBOOL mData[HWCap_SIZE];
    static MBOOL mReady[HWCap_SIZE];
};
typedef PlatformCapability PlatCap;

MVOID dumpQParams(int logLevel, NSIoPipe::QParams& rInputQParam);

static inline MINT64 generatePQUserID(MINT32 RequestNo, MUINT8 sensorID, MBOOL isPhysicalStream)
{
    /**
     *    The arrangement would be like
     *
     *    Bit order:  63  ...  39  ...  8   7  ...  2          1           0
     *    Content  : | 0 | ..| Request No | Sensor ID | isPhysicalStream | 1 |
     *
     */
    return ((RequestNo << 8) | (sensorID << 2) | ((int)isPhysicalStream << 1) | 1);
}

enum MultiframeType {
    MULTIFRAME_TYPE_NONE     = -1,
    MULTIFRAME_TYPE_MULTIRAW =  0,
    MULTIFRAME_TYPE_MULTIYUV =  1,
};
