...
#define DEFAULT_CAPTURE_REQUEST_BOOST MFALSE
...
const CaptureFeatureNodeConfigTable gCaptureFeatureNodeConfigTable =
{
    ...
    {NID_FUSION_VSDOF, {SCHED_NORMAL, DEFAULT_CAMTHREAD_PRIORITY + DUALCAM_NODE_THREAD_PRIORITY_OFFSET}},
    {NID_FUSION_ZOOM, {SCHED_NORMAL, DEFAULT_CAMTHREAD_PRIORITY + DUALCAM_NODE_THREAD_PRIORITY_OFFSET}}
};
    ...
    , CamNodeULogHandler(Utils::ULog::MOD_FPIPE_CAPTURE)
{
    ...
    mIsZoom               = ((sensorList.size() > 1) && (usageHint.mDualMode & v1::Stereo::E_DUALCAM_FEATURE_ZOOM));
    ...
    mpYUVNode             = CreateNode<YUVNode>(NID_YUV, APPEND_POSTFIX("YUV"), (mIsVSDOF || mIsZoom));
    ...
    if (mIsVSDOF) {
        ...
        mpFusionVsdofNode = CreateNode<FusionNode>(NID_FUSION_VSDOF, APPEND_POSTFIX("Fusion_V"));
        mpFusionZoomNode  = NULL;
    } else if (mIsZoom) {
        mpYUV2Node        = CreateNode<YUVNode>(NID_YUV2, APPEND_POSTFIX("YUV2"), mIsZoom);
        mpFusionZoomNode  = CreateNode<FusionNode>(NID_FUSION_ZOOM, APPEND_POSTFIX("Fusion_Z"));
        mpDepthNode       = NULL;
        mpBokehNode       = NULL;
        mpFusionVsdofNode = NULL;
    } else {
        mpYUV2Node        = NULL;
        mpDepthNode       = NULL;
        mpBokehNode       = NULL;
    ...
    if (mIsVSDOF) {
        ...
        mpNodes.push_back(mpFusionVsdofNode);
        mpNodes.push_back(mpDepthNode);
        mpNodes.push_back(mpBokehNode);
    } else if (mIsZoom) {
        mpNodes.push_back(mpYUV2Node);
        mpNodes.push_back(mpFusionZoomNode);
    }
    ...
    if (mIsVSDOF) {
        ...
        mInference.addNode(NID_FUSION_VSDOF, mpFusionVsdofNode);
        ...
        mInference.addNode(NID_YUV2, mpYUV2Node);
        mInference.addNode(NID_FUSION_ZOOM, mpFusionZoomNode);
    }
...
#define DELETE_NODE(node)       \
    ...
    if (mIsVSDOF) {
        ...
        DELETE_NODE(mpFusionVsdofNode);
    } else if (mIsZoom) {
        DELETE_NODE(mpYUV2Node);
        DELETE_NODE(mpFusionZoomNode);
    }
#undef DELETE_NODE
...
MERROR CaptureFeaturePipe::enque(sp<ICaptureFeatureRequest> request)
{
    ...
    else
    {
        ...
        function<void()> fnInference = [=]() {
            ...
            if (!rRequest.hasParameter(PID_DROPPED_FRAME)) {
                for (size_t i = 0; i < rRequest.mBufferItems.size(); i++) {
                    ...
                    MY_LOGD_IF(mLogLevel, "Check to create working buffer, bufID=%d, cond: mCreated=%d Size=%d, buffer size=%dx%d format=%d",
                                bufID, !rItem.mCreated, !!rItem.mSize, rItem.mSize.w, rItem.mSize.h, rItem.mFormat);
        ...
        {
            MY_LOGD("R/F:%d/%d Missing meta handle, inHal missing:%x outHal missing:%x", pRequest->getRequestNo(),
                pRequest->getFrameNo(), inHalMetaHandle == NULL, outHalMetaHandle == NULL);
        }
        mpRootNode->markNodeEnter(pRequest);
...
MBOOL CaptureFeaturePipe::onInit()
{
    ...
    mpFeatureBufferPool =
        ICaptureFeatureBufferPool::getInstance("capturePipe");
...
MBOOL CaptureFeaturePipe::prepareNodeSetting()
{
    ...
    mpMultiFrameNode->setBufferPool(mpBufferPool);
    if (mIsVSDOF) {
...
MBOOL CaptureFeaturePipe::prepareNodeConnection()
{
    ...
    if (mIsVSDOF) {
        ...
        this->connectData(PID_P2A_TO_FUSION_V, *mpP2Node, *mpFusionVsdofNode);
        this->connectData(PID_P2A_TO_YUV2, *mpP2Node, *mpYUV2Node);
        this->connectData(PID_FD_TO_DEPTH, *mpFDNode, *mpDepthNode);
        this->connectData(PID_FD_TO_FUSION_V, *mpFDNode, *mpFusionVsdofNode);
        ...
        this->connectData(PID_FUSION_V_TO_YUV, *mpFusionVsdofNode, *mpYUVNode);
        this->connectData(PID_FUSION_V_TO_YUV2, *mpFusionVsdofNode, *mpYUV2Node);
        this->connectData(PID_FUSION_V_TO_MDP, *mpFusionVsdofNode, *mpMDPNode);
        ...
        this->connectData(PID_P2A_TO_FUSION_Z, *mpP2Node, *mpFusionZoomNode);
        this->connectData(PID_P2A_TO_YUV2, *mpP2Node, *mpYUV2Node);
        this->connectData(PID_FD_TO_FUSION_Z, *mpFDNode, *mpFusionZoomNode);
        this->connectData(PID_FD_TO_YUV2, *mpFDNode, *mpYUV2Node);
        this->connectData(PID_MULTIFRAME_TO_YUV2, *mpMultiFrameNode, *mpYUV2Node);
        this->connectData(PID_MULTIFRAME_TO_FUSION_Z, *mpMultiFrameNode, *mpFusionZoomNode);
        this->connectData(PID_FUSION_Z_TO_YUV2, *mpFusionZoomNode, *mpYUV2Node);
        this->connectData(PID_FUSION_Z_TO_MDP, *mpFusionZoomNode, *mpMDPNode);
        this->connectData(PID_YUV_TO_FUSION_Z, *mpYUVNode, *mpFusionZoomNode);
        this->connectData(PID_YUV_TO_YUV2, *mpYUVNode, *mpYUV2Node);
        this->connectData(PID_YUV2_TO_MDP, *mpYUV2Node, *mpMDPNode);
    }
...
MVOID CaptureFeaturePipe::releaseNodeSetting()
{
    ...
    mpMultiFrameNode->setBufferPool(NULL);
    if (mIsVSDOF) {
...
std::vector<IImageBuffer*> PipeBufferHandle::nativeAll()
{
    // wait first
    mFuture.wait();
    ...
    std::vector<IImageBuffer*> vpSmartBuffer;
    vpSmartBuffer.push_back(mpSmartBuffer->getImageBufferPtr());
    return vpSmartBuffer;
}

MBOOL PipeBufferHandle::isMultiBuffer()
{
    return MFALSE;
}
