...
namespace util{
    ...
    /**
     * @brief support 1In2Out for pure mdp
     */
    struct sDpIspOut
    {
        DpPqParam* pDpPqParam    = nullptr;
        int32_t portIndex        = 0; // WROT = 0, WDMA = 1
        MINT32 rotAngle          = 0;
        IImageBuffer* pDstBuffer = nullptr;
        MRect cropRect;
    };
    struct sDpIspConfigs
    {
        DpIspStream* pDpIspStream = nullptr;
        IImageBuffer* pSrcBuffer = nullptr;
        std::vector<sDpIspOut> vDpIspOut = {};
    };

    MBOOL executeDpIspStream(
            sDpIspConfigs config
            );
}
}
