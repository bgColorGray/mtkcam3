...
getDpColorFormat(MINT32 format)
{
    switch(format)
    {
        ...
        case eImgFmt_MTK_YUV_P010:
            LOG_DBG("eImgFmt_MTK_YUV_P010, return DP_COLOR_NV12_10P for special MDP path");
            return DP_COLOR_NV12_10P;
        case eImgFmt_RGB48:
    ...
    MINT usage)
{
    ...
    if ( pImgBuf )
        ...
        LOG_ERR("failed to allocate ImageBuffer");
    return pImgBuf;
}
...
excuteDpIspStream(sDpIspConfig config)
{
    if(config.pDpIspStream == nullptr)
        return MFALSE;
    //***************************src****************************//
    MUINT32 src_addr_list[3] = {0, 0, 0};
    unsigned int src_size_list[3] = {0, 0, 0};
    MINT32 src_fd_list[3] = {0, 0, 0};
    MUINT32 src_offset_list[3] = {0, 0, 0};
    size_t planeCount = config.pSrcBuffer->getPlaneCount();
    for(size_t i=0;i<planeCount;++i)
    {
        src_addr_list[i] = config.pSrcBuffer->getBufPA(i);
        src_size_list[i] = config.pSrcBuffer->getBufSizeInBytes(i);
        src_fd_list[i] = config.pSrcBuffer->getPlaneFD(i);
        src_offset_list[i] = config.pSrcBuffer->getPlaneOffsetInBytes(i);
    }
    MINT32 uvPitch = (planeCount > 1) ? config.pSrcBuffer->getBufStridesInBytes(1) : 0;
    DP_STATUS_ENUM status;
#if defined(SUPPORT_ION_FD)
    status = config.pDpIspStream->queueSrcBuffer(src_fd_list, src_offset_list, src_size_list, planeCount);
#else
    status = config.pDpIspStream->queueSrcBuffer(src_addr_list, src_size_list, planeCount);
#endif
    ...
    //***************************dst********************************//
    MUINT32 dst_addr_list_pa[3] = {0, 0, 0};
    MINTPTR dst_addr_list_va[3] = {0, 0, 0};
    unsigned int dst_size_list[3] = {0, 0, 0};
    MINT32 dst_fd_list[3] = {0, 0, 0};
    MUINT32 dst_offset_list[3] = {0, 0, 0};
    planeCount = config.pDstBuffer->getPlaneCount();
    for(size_t i=0;i<planeCount;++i)
    {
        dst_addr_list_pa[i] = config.pDstBuffer->getBufPA(i);
        dst_addr_list_va[i] = config.pDstBuffer->getBufVA(i);
        dst_size_list[i] = config.pDstBuffer->getBufSizeInBytes(i);
        dst_fd_list[i] = config.pDstBuffer->getPlaneFD(i);
        dst_offset_list[i]   = config.pDstBuffer->getPlaneOffsetInBytes(i);
    }
    uvPitch = (planeCount > 1) ? config.pDstBuffer->getBufStridesInBytes(1) : 0;
    // currently only support
    int portIndex = 0;
#if defined(SUPPORT_ION_FD)
    status= config.pDpIspStream->queueDstBuffer(portIndex, dst_fd_list, dst_offset_list, dst_size_list, planeCount);
#else
    status= config.pDpIspStream->queueDstBuffer(portIndex, dst_addr_list_pa, dst_size_list, planeCount);
#endif
    ...
    status = config.pDpIspStream->dequeueDstBuffer(portIndex, (void**)dst_addr_list_va, true);
...
MBOOL
executeDpIspStream(sDpIspConfigs config)
{
    if(config.pDpIspStream == nullptr)
        return MFALSE;
    //***************************src****************************//
    ...
    LOG_DBG("src buffer=%dx%d", config.pSrcBuffer->getImgSize().w, config.pSrcBuffer->getImgSize().h);
    //***************************dst********************************//
    if(config.vDpIspOut.size() > 2)
    {
        LOG_ERR("MDP cannot support more than two outputs per round, output size:%lu", config.vDpIspOut.size());
        return MFALSE;
    }
    MUINT32 dst_addr_list_pa[2][3] = {{0, 0, 0}, {0, 0, 0}};
    unsigned int dst_size_list[2][3] = {{0, 0, 0}, {0, 0, 0}};
    MINT32 dst_fd_list[2][3] = {{0, 0, 0}, {0, 0, 0}};
    MUINT32 dst_offset_list[2][3] = {{0, 0, 0}, {0, 0, 0}};
    for(int id = 0; id<config.vDpIspOut.size(); id++)
    {
        sDpIspOut out = config.vDpIspOut[id];
        planeCount = out.pDstBuffer->getPlaneCount();
        for(size_t i=0;i<planeCount;++i)
        {
            dst_addr_list_pa[id][i] = out.pDstBuffer->getBufPA(i);
            dst_size_list[id][i] = out.pDstBuffer->getBufSizeInBytes(i);
            dst_fd_list[id][i] = out.pDstBuffer->getPlaneFD(i);
            dst_offset_list[id][i] = out.pDstBuffer->getPlaneOffsetInBytes(i);
        }
        uvPitch = (planeCount > 1) ? out.pDstBuffer->getBufStridesInBytes(1) : 0;
        int32_t portIndex = out.portIndex;
        LOG_DBG("config.vDpIspOut[%d], port:%d", id, portIndex);
#if defined(SUPPORT_ION_FD)
        status= config.pDpIspStream->queueDstBuffer(portIndex, dst_fd_list[id], dst_offset_list[id], dst_size_list[id], planeCount);
#else
        status= config.pDpIspStream->queueDstBuffer(portIndex, dst_addr_list_pa[id], dst_size_list[id], planeCount);
#endif
        CHECK_DP_SUCCESS(queueDstBuffer, status);
        status = config.pDpIspStream->setSrcCrop(portIndex,
                                                out.cropRect.p.x,
                                                0,
                                                out.cropRect.p.y,
                                                0,
                                                ...
                                                0);
        CHECK_DP_SUCCESS(setSrcConfig, status);
        status= config.pDpIspStream->setDstConfig(portIndex,
                                        out.pDstBuffer->getImgSize().w,
                                        out.pDstBuffer->getImgSize().h,
                                        out.pDstBuffer->getBufStridesInBytes(0),
                                        uvPitch,
                                        getDpColorFormat(out.pDstBuffer->getImgFormat()),
                                        DP_PROFILE_FULL_BT601,
                                        eInterlace_None,
                                        0,
                                        false);
        CHECK_DP_SUCCESS(setDstConfig, status);
        status = config.pDpIspStream->setRotation(portIndex, out.rotAngle);
        CHECK_DP_SUCCESS(setRotation, status);
        if(out.pDpPqParam != nullptr)
        {
            status = config.pDpIspStream->setPQParameter(portIndex, *out.pDpPqParam);
            CHECK_DP_SUCCESS(setPQParameter, status);
        }
        LOG_DBG("dst buffer=%dx%d", out.pDstBuffer->getImgSize().w, out.pDstBuffer->getImgSize().h);
    }
    //*******************************************************************//
    status = config.pDpIspStream->startStream();
    CHECK_DP_SUCCESS(startStream, status);
    ...
    for(int id = 0; id<config.vDpIspOut.size(); id++)
    {
        int portIndex = config.vDpIspOut[id].portIndex;
        void* va[3] = {0};
        status = config.pDpIspStream->dequeueDstBuffer(portIndex, va, true);
        CHECK_DP_SUCCESS(dequeueDstBuffer, status);
    }
    status = config.pDpIspStream->dequeueFrameEnd();
    CHECK_DP_SUCCESS(dequeueFrameEnd, status);
