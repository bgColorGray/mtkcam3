...
QParamTemplateGenerator::checkValid()
{
    ...
    {
        MY_LOGE("FrameID:%d one or both of In/Out buffer size is zero, in:%zu out:%zu",
...
{
    FrameParams& frameParam = mTargetQParam.mvFrameParams.editItemAt(frameID);
    if(frameParam.mvCropRsInfo.size() == 0)
    {
