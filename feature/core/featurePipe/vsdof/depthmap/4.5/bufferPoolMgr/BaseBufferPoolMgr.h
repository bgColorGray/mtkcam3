...
#define CREATE_IMGBUF_POOL_USER_STRIDE(rBufPool, name, size, format, usage, strideHint) \
    VSDOF_MDEPTH_LOGD("Create FatImageBufferPool:: "#name " size=%dx%d format=%x usage=%x", \
                size.w, size.h, format, usage); \
    rBufPool = FatImageBufferPool::create(name, size.w, size.h, format, usage, strideHint);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    /**
     * @brief request static buffer from NodeBufferPoolMgr_VSDOF
     * @param [in] id buffer id used to request buffer.
     * @return
     * - reuqest static buffer
     */
    ...
    {
        if ( iBlockSize == 0){
            VSDOF_MDEPTH_LOGE("invalid block size for FEFM");
            return MFALSE;
        }
