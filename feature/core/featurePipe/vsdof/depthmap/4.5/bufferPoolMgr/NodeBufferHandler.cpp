...
)
{
    ...
    else
    {
        ...
        // request static buffer
        if(pOutImgBuf == NULL)
        {
            VSDOF_LOGD2("request static buffer, srcNodeID=%d bufferID=%s", srcNodeID, BID2Name(bufferID));
            pOutImgBuf = mpBufferPoolMgr->requestStaticBuffer(bufferID);
        }
        if(pOutImgBuf == NULL)
        {
...
)
{
    ...
    MBOOL bUseInternalMeta = MFALSE;
    // convert user spec to decision
    if( dataSource == METADATA_SOURCE::USE_FLOW_DEFINED ) // user do not need specific meta
    {
        /**
         * deteremin to use internal/external meta depend on flowtpye & user defined
         * For QueuedDepth flow, critical flow is clear yuv flow
         * For Standatd flow, critcal flow is depth flow
         * PS : P2ABayerNode is used for FEFM only, always use internal data
         */
        if( mFlowType == eDEPTH_FLOW_TYPE_QUEUED_DEPTH )
        {
            if (   srcNodeID == eDPETHMAP_PIPE_NODEID_P2ABAYER
                || srcNodeID == eDPETHMAP_PIPE_NODEID_N3D
                || srcNodeID == eDPETHMAP_PIPE_NODEID_DVS
                || srcNodeID == eDPETHMAP_PIPE_NODEID_DVP
                || srcNodeID == eDPETHMAP_PIPE_NODEID_GF
                || srcNodeID == eDPETHMAP_PIPE_NODEID_WPE
                || srcNodeID == eDPETHMAP_PIPE_NODEID_DLDEPTH )
            {
                bUseInternalMeta = MTRUE;
            }
        }
        else
        {
            if (   srcNodeID == eDPETHMAP_PIPE_NODEID_P2A
                || srcNodeID == eDPETHMAP_PIPE_NODEID_P2ABAYER )
            {
                bUseInternalMeta = MTRUE;
            }
        }
    }
    else if( dataSource == METADATA_SOURCE::USE_INTERNAL ) // user need internal meta
    {
        bUseInternalMeta = MTRUE;
    }
    else                        // user need external meta
    {
        bUseInternalMeta = MFALSE;
    }
    // request metadata by decision
    VSDOF_LOGD2("srcNodeID:%d bufferID:%s scenario:%d METADATA_SOURCE:%d InternalMetaAvailable:%d",
                srcNodeID, BID2Name(bufferID), mReqAttr.bufferScenario, dataSource, mpDepthMapEffectReq->isInternalMetaAvailable());
    if( bUseInternalMeta )
    {
        pOutMeta = mapMetaBIDIntoInternal(bufferID);
        if(pOutMeta == nullptr)
        {
            MY_LOGE("Failed to request internal metadata");
        }
        return pOutMeta;
    }
    else
...
IMetadata*
NodeBufferHandler::
mapMetaBIDIntoInternal(DepthMapBufferID BID)
{
    switch(BID)
    {
        case BID_META_IN_APP:
            return mpDepthMapEffectReq->mInternalData.mMetaSet.pMeta_InApp.get();
        case BID_META_IN_HAL_MAIN1:
            return mpDepthMapEffectReq->mInternalData.mMetaSet.pMeta_Main1_InHal.get();
        case BID_META_IN_HAL_MAIN2:
            return mpDepthMapEffectReq->mInternalData.mMetaSet.pMeta_Main2_InHal.get();
        case BID_META_OUT_APP:
            return mpDepthMapEffectReq->mInternalData.mMetaSet.pMeta_OutApp.get();
        case BID_META_OUT_HAL:
            return mpDepthMapEffectReq->mInternalData.mMetaSet.pMeta_OutHal.get();
        case BID_META_IN_P1_RETURN:
            return mpDepthMapEffectReq->mInternalData.mMetaSet.pMeta_P1_InRet.get();
        default:
            break;
    }
    return nullptr;
