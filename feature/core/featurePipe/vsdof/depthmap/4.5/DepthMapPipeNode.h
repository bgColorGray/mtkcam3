...
public:
    ...
    /**
     * @brief map the data ID to corresponding one by NDD level
     */
    DepthMapDataID mapNDDL1DataID(DataID id);
    /**
     * @brief map the debug enum to correspoding buffer ID
     */
    DepthMapBufferID mapDebugInkBufferID(DepthMapRequestPtr pRequest, DepthDebugImg id);
    /**
    ...
    MBOOL _copyBufferIntoRequestWithCropAndRotate(
                IImageBuffer* pSrcBuffer,
                sp<DepthMapEffectRequest> pRequest,
                DepthMapBufferID bufferID,
                ENUM_ROTATION targetRotation);

    static MBOOL copyBuffer(
                    IImageBuffer* pSrcBuffer,
                    IImageBuffer* pDstBuffer);

    static MBOOL copyBufferWithCrop(
                    IImageBuffer* pSrcBuffer,
                    IImageBuffer* pDstBuffer);

    /**
     * @brief handle Depth flow done
     */
    MBOOL onHandle3rdPartyFlowDone(DepthMapRequestPtr pRequest);

    MBOOL mapOutputOnFlowType(
                    DepthMapRequestPtr pRequest,
...
protected:
    ...
    // Depth Delay
    MUINT32 miDepthDelay;
    // NDD level
    MUINT32 miNDDLevel;
    // Bit true
    MUINT32 miBitTrueFrame;
private:
};
