...
)
{
    ...
    DepthPipeLoggingSetup::miDebugLog = ::property_get_int32("vendor.STEREO.log.DepthMapNode", 0);
#endif

    MY_LOGD("LoggingSetup: mbProfileLog =%d miDebugLog=%d ",
            DepthPipeLoggingSetup::mbProfileLog, DepthPipeLoggingSetup::miDebugLog);
...
getInstance()
{
    ...
    MY_LOGD("DepthPipeHolder::getInstance  %p", pPipe.get());
    return pPipe;
}
...
, mpPipeOption(pPipeOption)
{
    ...
    //
    MY_LOGD("mSensorType=%d mFeatureMode=%d  FlowType=%d scenario=%d",
        mpPipeOption->mSensorType, mpPipeOption->mFeatureMode, mpPipeOption->mFlowType,
        mpPipeOption->mStereoScenario);
}
...
MBOOL DepthMapPipe::onInit()
{
    ...
    TEST_AND_CREATE_NODE(eDPETHMAP_PIPE_NODEID_DVS, DVSNode);
    TEST_AND_CREATE_NODE(eDPETHMAP_PIPE_NODEID_DVP, DVPNode);
    ...
    TEST_AND_CREATE_NODE(eDPETHMAP_PIPE_NODEID_SLANT, SlantNode);
...
flush()
{
    ...
    MY_LOGD("DepthMapPipe::flush +:  req size=%lu", mvRequestDataMap.size());
    // set flush status
...
enque(sp<IDepthMapEffectRequest>& request)
{
    ...
    // initialize N3D phase
    if (hasWPEHw())
        pDpeEffReq->mN3DPhase = eN3D_PVPHASE_MAIN1_PADDING;
    else
        pDpeEffReq->mN3DPhase = eN3D_PVPHASE_GPU_WARPING;
    // overall timer
    ...
    // autolock for request map
    {
        ...
        {
            mvRequestDataMap.add(reqID,
                    {pDpeEffReq, pDpeEffReq->isSkipDepth(mpPipeOption)?
                        eREQ_STATUS_WAIT_FG_FLOW : eREQ_STATUS_FG_FLOW});
        }
        else
        {
        ...
        VSDOF_LOGD("reqID=%d mvRequestDataMap size=%lu ids=%s",
    ...
    // Register users to input request buffer map, and if buffers are
    // no longer needed, must do unRegisterInputBufferUser
    MBOOL ret = pDpeEffReq->registerInputBufferUser(mpPipeOption);
    // handle flow type task
    ret &= handleFlowTypeOnEnque(pDpeEffReq);
    // call parent class:FeaturePipe enque function
    ret &= CamPipe::enque(ROOT_ENQUE, pDpeEffReq);
...
handleFlowTypeOnEnque(DepthMapRequestPtr pRequest)
{
    ...
    pRequest->copyInputDataIfNeed(mpPipeOption);
    VSDOF_LOGD("-");
    return MTRUE;
}
...
MBOOL DepthMapPipe::onData(DataID id, DepthMapRequestPtr &request)
{
    ...
    switch(id)
    {
        ...
        case GF_OUT_DMBG:
        case DVS_OUT_DEPTHMAP:
        case DVP_OUT_DEPTHMAP:
        case DLDEPTH_OUT_DEPTHMAP:
        case GF_OUT_DEPTHMAP:
        case SLANT_OUT_DEPTHMAP:
        ...
        // Release HAL buffer flow
        case DEPTHPIPE_BUF_RELEASED: {
            bRet = onHandleBufReleased(request);
            break;
        }
        ...
        case REQUEST_DEPTH_NOT_READY:
        {
            ...
            request->mTimer.showPerFrameSummary(request->getRequestNo(), request->getRequestAttr(), mpPipeOption);
...
MBOOL DepthMapPipe::onHandleBufReleased(DepthMapRequestPtr request) {
    MUINT32 reqID = request->getRequestNo();
    VSDOF_LOGD("+ [lock] reqID=%d", reqID);
    {
        // autolock for request map
        ...
        ssize_t reqPos =  mvRequestDataMap.indexOfKey(reqID);
        if (reqPos < 0) {
            MY_LOGE("non-existed reqID=%u, need to check!", reqID);
            // return MTRUE;
        }
    }
    // Release buffer callback
    request->launchFinishCallback(eRESULT_BUF_RELEASED);
