...
: miFrameID(frameID)
{
    TuningUtils::FILE_DUMP_NAMING_HINT dumpHint;
    extract_by_SensorOpenId(&dumpHint, iSensorIdx);
    mPass2EnqueFrame.mStreamTag = streamTag;
    mPass2EnqueFrame.mSensorIdx = iSensorIdx;
    mPass2EnqueFrame.FrameNo    = frameID;
    mPass2EnqueFrame.SensorDev  = dumpHint.SensorDev;
}
...
QParamTemplateGenerator::checkValid()
{
    ...
    {
        MY_LOGE("FrameID:%d In/Out buffer size is not consistent, in:%lu out:%lu",
...
, mbSuccess(MTRUE)
{
    IMetadata* halMeta = pRequest->getBufferHandler()->requestMetadata(nodeId, BID_META_IN_HAL_MAIN1);
    MINT32 uniqueKey = 0;
    MINT32 requestNo = 0;
    if (!tryGetMetadata<MINT32>(halMeta, MTK_PIPELINE_UNIQUE_KEY, uniqueKey))
    {
        MY_LOGW("Failed to get timestamp value, use default value 0");
    }
    for(size_t idx=0;idx<mTargetQParam.mvFrameParams.size();++idx)
    {
        mTargetQParam.mvFrameParams.editItemAt(idx).RequestNo = pRequest->getRequestNo();
        mTargetQParam.mvFrameParams.editItemAt(idx).Timestamp = uniqueKey;
        mTargetQParam.mvFrameParams.editItemAt(idx).UniqueKey = uniqueKey;
    }
}
QParamTemplateFiller&
QParamTemplateFiller::
...
)
{
    ...
    frameParam.IspProfile = (MINT32)profile;
    return *this;
}
...
addWPEQParam(
    ...
    MBOOL const bUseHalfWPEConfigSize
)
{
    WPEQParams* pWpeQParam = new WPEQParams();
    rpWpeQParamHolder->mvData.push_back((void*)pWpeQParam);
    // wpe mode
    pWpeQParam->wpe_mode = WPEMode;
    // no z plane
    pWpeQParam->vgen_hmg_mode = 0;
    // plane 0, x
    pWpeQParam->warp_veci_info.width = pWarpMatrix_X->getImgSize().w;
    pWpeQParam->warp_veci_info.height = pWarpMatrix_X->getImgSize().h;
    pWpeQParam->warp_veci_info.stride = pWarpMatrix_X->getBufStridesInBytes(0);
    pWpeQParam->warp_veci_info.virtAddr = pWarpMatrix_X->getBufVA(0);
    pWpeQParam->warp_veci_info.phyAddr = pWarpMatrix_X->getBufPA(0);
    pWpeQParam->warp_veci_info.bus_size = NSIoPipe::NSWpe::WPE_BUS_SIZE_32_BITS;
    pWpeQParam->warp_veci_info.addr_offset = 0;
    pWpeQParam->warp_veci_info.veci_v_flip_en = 0;
    // plane 1, y
    pWpeQParam->warp_vec2i_info.width = pWarpMatrix_Y->getImgSize().w;
    pWpeQParam->warp_vec2i_info.height = pWarpMatrix_Y->getImgSize().h;
    pWpeQParam->warp_vec2i_info.stride = pWarpMatrix_Y->getBufStridesInBytes(0);
    pWpeQParam->warp_vec2i_info.virtAddr = pWarpMatrix_Y->getBufVA(0);
    pWpeQParam->warp_vec2i_info.phyAddr = pWarpMatrix_Y->getBufPA(0);
    pWpeQParam->warp_vec2i_info.bus_size = NSIoPipe::NSWpe::WPE_BUS_SIZE_32_BITS;
    pWpeQParam->warp_vec2i_info.addr_offset = 0;
    pWpeQParam->warp_vec2i_info.veci_v_flip_en = 0;
    //
    pWpeQParam->wpecropinfo.x_start_point = 0;
    pWpeQParam->wpecropinfo.y_start_point = 0;
    if ( bUseHalfWPEConfigSize ) {
        pWpeQParam->wpecropinfo.x_end_point = (pInputImg->getImgSize().w/2)-1;
        pWpeQParam->wpecropinfo.y_end_point = (pInputImg->getImgSize().h/2)-1;
    } else {
        pWpeQParam->wpecropinfo.x_end_point = pInputImg->getImgSize().w-1;
        pWpeQParam->wpecropinfo.y_end_point = pInputImg->getImgSize().h-1;
    }
    // vgen crop
    MCrpRsInfo vgenCropInfo;
    vgenCropInfo.mCropRect = MCropRect(MPoint(0,0), pWarpMatrix_X->getImgSize());
    pWpeQParam->mwVgenCropInfo.push_back(vgenCropInfo);
    // tuning parameters
    pWpeQParam->tbl_sel_v = 24;
    pWpeQParam->tbl_sel_h = 24;
    // add to ExtraParam
    this->addExtraParam(frameID, EPIPE_WPE_INFO_CMD, (void*)pWpeQParam);
    //
    auto logWPEParam = [](char* prefix, NSIoPipe::NSWpe::WarpMatrixInfo& veciInfo)
    {
        VSDOF_LOGD2("%s: width=%d height=%d stride=%d virtAddr=%lX phyAddr=%lX bus_size=%d offset=%d flip=%d",
    ...
    logWPEParam("warp_veci_info", pWpeQParam->warp_veci_info);
    logWPEParam("warp_vec2i_info", pWpeQParam->warp_vec2i_info);
    return *this;
}
...
MBOOL is3DNROn(IMetadata *appInMeta)
{
    ...
    if (DepthPipeLoggingSetup::miDebugLog)
    {
    ...
    for(size_t index = 0; index < rInputQParam.mvFrameParams.size(); ++index)
    {
        ...
        for(size_t index2=0;index2<frameParam.mvIn.size();++index2)
        {
            ...
            if(data.mBuffer !=NULL)
            {
                ...
                MY_LOGD("mvIn.mBuffer->getPlaneCount = %lu", data.mBuffer->getPlaneCount());
                for(int j=0;j<data.mBuffer->getPlaneCount();j++)
                {
                    MY_LOGD("mvIn.mBuffer->getBufVA(%d) = %lX", j, data.mBuffer->getBufVA(j));
                    MY_LOGD("mvIn.mBuffer->getBufStridesInBytes(%d) = %lu", j, data.mBuffer->getBufStridesInBytes(j));
                }
            }
            else
            {
        ...
        for(size_t index2=0;index2<frameParam.mvOut.size(); index2++)
        {
            ...
            if(data.mBuffer != NULL)
            {
                ...
                MY_LOGD("mvOut.mBuffer->getPlaneCount = %lu", data.mBuffer->getPlaneCount());
                for(size_t j=0;j<data.mBuffer->getPlaneCount();j++)
                {
                    MY_LOGD("mvOut.mBuffer->getBufVA(%zu) = %lX", j, data.mBuffer->getBufVA(j));
                    MY_LOGD("mvOut.mBuffer->getBufStridesInBytes(%zu) = %lu", j, data.mBuffer->getBufStridesInBytes(j));
                }
            }
            else
            {
...
MINT32 remapTransform(int rot)
{
    switch(rot)
    {
        ...
        // slant case, take as sensor is placed in horizon
        case 45:
        case 315:
            return 0;
        default:
...
)
{
    ...
    IMetadata* inP1RetHal = pBufferHandler->requestMetadata(srcNodeID, BID_META_IN_P1_RETURN);
    MINT32 iso;
    ...
    IMetadata* inHal = pBufferHandler->requestMetadata(srcNodeID, BID_META_IN_HAL_MAIN1);
    MINT32 timestamp = 0;
...
)
{
    ...
    VSDOF_LOGD("scenario=%d enable=%d timestamp=%u iso=%d frameNo=%d requestNo=%d lensId=%d",
...
: miThreshold(threshold)
{
    mTimer.start();
    //
    if ( strlen(fmt) < LOG_LEN )
        strncpy(mLog, fmt, strlen(fmt));
}
...
~TimeThreshold()
{
    ...
    {
        char log[LOG_LEN]={};
        int err = sprintf(log, "[TimeThreshold] %s, execution time(%dms) is larger than %dms",
