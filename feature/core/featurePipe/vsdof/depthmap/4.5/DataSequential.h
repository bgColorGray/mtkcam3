...
public:
    ...
    SequenceOPState sequentialize(const T& data)
    {
        ...
        VSDOF_MDEPTH_LOGD("%s sequentialize %s=%d mOnGoingDataQueue size=%lu",
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_MDEPTH_LOGD("%s Mark onging %s=%d", mpUser, mpKeyString, mfSeqGetter(data));
    ...
    lbExit:
        VSDOF_MDEPTH_LOGD("%s finish markFinishAndPop=%d, debugSeq=%s",
                    mpUser, iFinishSeq, debugSeq());
        return MTRUE;
    }
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_MDEPTH_LOGD("%s  markFinishNoInOder=%d, debugSeq=%s",
                    mpUser, iFinishSeq, debugSeq());
        if(!mbAllowNotInOrder)
        {
        ...
        VSDOF_MDEPTH_LOGD("%s  finish markFinishNoInOder=%d, debugSeq=%s",
                    mpUser, iFinishSeq, debugSeq());
        return MTRUE;
    }
...
public:
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_MDEPTH_LOGD("%s Mark enque %s=%d", mpUser, mpKeyString, mfSeqGetter(data));
    ...
    {
        RWLock::AutoWLock _l(mLock);
        VSDOF_MDEPTH_LOGD("%s Mark deque %s=%d latest:%d",
