...
public:
    ...
    AAATuningResult(MVOID* pTuningBuf, MVOID* pLcsBuf = nullptr, MVOID* pLce4CALTM = nullptr, NSIspTuning::EIspProfile_T profile = NSIspTuning::EIspProfile_Preview)
    : tuningResult()
    {
        tuningResult.pRegBuf    = pTuningBuf;
        tuningResult.pLcsBuf    = pLcsBuf;
        tuningResult.pLce4CALTM = pLce4CALTM;
        tuningBuffer            = pTuningBuf;
        ispProfile              = profile;
    }
public:
    NSIspTuning::EIspProfile_T ispProfile = NSIspTuning::EIspProfile_Preview;
...
public:
    ...
    virtual MBOOL buildQParam(
                    ...
                    QParams& rOutParam,
                    EnqueCookieContainer*& rpWpeQParamHolder) = 0;
    ...
    /**
     * @brief build the enque QParams for wpe run
     * @param [in] pReq DepthMap EffectRequest
     * @param [in] WPEQparamHolder lifeCycle holder for WPEQParam
     * @param [out] rOutParam output QParams
     * @return
     * - MTRUE indicates success.
     * - MFALSE indicates failure.
     */
    virtual MBOOL buildQParam_Warp(
                    sp<DepthMapEffectRequest> pReq,
                    QParams& rOutParam,
                    EnqueCookieContainer*& WPEQparamHolder) = 0;
