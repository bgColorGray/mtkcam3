    ...
    if(pRequest->isRequestBuffer(bufferID))
    {
        ...
        else
        {
            if ( srcSize.w == dstSize.h || srcSize.h == dstSize.w) {
                ...
                if ( rot == 90) targetRotation = 90;
                else if ( rot == 180) targetRotation = 180;
                else if ( rot == 270) targetRotation = 270;
                MY_LOGD("Dst size(%dx%d) > src size(%dx%d)", dstSize.w, dstSize.h, srcSize.w, srcSize.h);
                MY_LOGD("Do copy with rotation & crop  %u", rot);
