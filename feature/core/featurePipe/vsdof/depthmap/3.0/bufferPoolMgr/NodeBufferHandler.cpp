...
)
{
    ...
    if(pOutImgBuf == NULL)
    {
        MY_LOGF("Failed to request buffer, src node:%d, buffer id:%s, scenario:%d",
...
)
{
    ...
    IImageBuffer* pOutImgBuf = NULL;
    {
        ...
        if(pOutImgBuf == NULL)
        {
            MY_LOGF("Failed to request working buffer, buffer id:%s, scenario:%d", BID2Name(bufferID), mReqAttr.bufferScenario);
            return nullptr;
        }
    }
...
)
{
    ...
    if(smTuningBuf != NULL && smTuningBuf.get() != NULL)
    {
        addEnquedWorkingBuffer(srcNodeID, smTuningBuf);
        return smTuningBuf->mpVA;
    }

    MY_LOGF("Failed to request tuning buffer!! bufferID:%s scenario:%d", BID2Name(bufferID), mReqAttr.bufferScenario);
    return NULL;
}
MBOOL
NodeBufferHandler::
