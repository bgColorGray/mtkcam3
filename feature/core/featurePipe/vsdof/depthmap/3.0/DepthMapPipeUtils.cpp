...
QParamTemplateGenerator::checkValid()
{
    ...
    {
        MY_LOGE("FrameID:%d In/Out buffer size is not consistent, in:%ld out:%ld",
...
)
{
    ...
    MY_LOGE("wrong LMV region count %d", entry.count());
    return MFALSE;
}
    ...
    for(size_t index = 0; index < rInputQParam.mvFrameParams.size(); ++index)
    {
        ...
        for(size_t index2=0;index2<frameParam.mvIn.size();++index2)
        {
            ...
            if(data.mBuffer !=NULL)
            {
                ...
                for(int j=0;j<data.mBuffer->getPlaneCount();j++)
                {
                    ...
                    MY_LOGD("mvIn.mBuffer->getBufStridesInBytes(%d) = %ld", j, data.mBuffer->getBufStridesInBytes(j));
                }
            }
            else
            {
        ...
        for(size_t index2=0;index2<frameParam.mvOut.size(); index2++)
        {
            ...
            if(data.mBuffer != NULL)
            {
                ...
                {
                    MY_LOGD("mvOut.mBuffer->getBufVA(%ld) = %lX", j, data.mBuffer->getBufVA(j));
                    MY_LOGD("mvOut.mBuffer->getBufStridesInBytes(%ld) = %ld", j, data.mBuffer->getBufStridesInBytes(j));
                }
            }
            else
            {
        ...
        for(size_t i=0;i<frameParam.mvModuleData.size(); i++)
        {
            ...
            MY_LOGD("SrzInfo->crop_w=%ld", SrzInfo->crop_w);
            MY_LOGD("SrzInfo->crop_h=%ld", SrzInfo->crop_h);
...
{
    ENUM_PASS2_ROUND pass2Round = PASS2A;
...
)
{
    ...
    VSDOF_LOGD("iFrameNo=%d scenario=%d enable=%d timestamp=%d iso=%d frameNo=%d requestNo=%d lensId=%d iMDPPortMask=%d",
