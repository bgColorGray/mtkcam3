...
debugDPEConfig(NSIoPipe::DVEConfig& config)
{
    ...
    #define LOG_DPEBuf(bufInfo) \
        ...
        MY_LOGD("DVEConfig." #bufInfo ".u4BufVA=%p", config.bufInfo.u4BufVA); \
        MY_LOGD("DVEConfig." #bufInfo ".u4BufPA=%p", config.bufInfo.u4BufPA); \
...
)
{
    ...
    if(pLastDMP_L == nullptr)
    {
       MY_LOGE("pLastDMP_L is nullptr!");
       return MFALSE;
    }
    IImageBuffer* pLastDMP_R =  pBufferHandler->requestWorkingBuffer(getNodeId(), BID_DPE_INTERNAL_LAST_DMP);
    if(pLastDMP_R == nullptr)
    {
       MY_LOGE("pLastDMP_R is nullptr!");
       return MFALSE;
    }
