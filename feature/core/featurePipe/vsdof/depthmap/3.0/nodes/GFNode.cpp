...
)
{
    ...
    MY_LOGD("images.size()=%ld", inData.images.size());
    for(ssize_t idx=0;idx<inData.images.size();++idx)
    {
        MY_LOGD("images[%ld]=%p", idx, inData.images[idx]) ;
        if(inData.images[idx])
        {
            MY_LOGD("images[%ld], image size=%dx%d", idx,
    ...
    bool bIsCapture)
{
    ...
    if(pRequest->isQueuedDepthRequest(mpPipeOption)
        && mpPipeOption->mFeatureMode == eDEPTHNODE_MODE_MTK_SW_OPTIMIZED_DEPTH)
