...
onData(DataID data, DepthMapRequestPtr &request)
{
  ...
  {
  case BAYER_ENQUE:
    VSDOF_PRFLOG("+ : reqID=%d size=%ld", request->getRequestNo(), mRequestQue.size());
...
)
{
    ...
    MVOID* pTuningBuf = (void*)pBufferHandler->requestWorkingTuningBuf(getNodeId(), BID_P2A_TUNING);
    // in/out meta set
...
)
{
    ...
    if(pRequest->isRequestBuffer(bufferID))
    {
        ...
        {
            MY_LOGE("buffer size(id:%d %s) is not consistent!request buffer size=%dx%d(%ld bytes) src_buffer size=%dx%d(%ld bytes)",
