...
BufferPool<T>::BufferPool(const char *name)
  ...
  , mWaitTimeNS(milliseconds_to_nanoseconds(POOL_WAIT_TIME_MS))
  , mAutoFree(-1)
  , mAutoAllocate(-1)
  , mAllocatingCount(0)
{
...
sb<T> BufferPool<T>::request()
{
  ...
  do
  {
    ...
    {
      #if POOL_WAIT_TIME_MS
      status_t status = mCondition.waitRelative(mMutex, mWaitTimeNS);
      if( status == ::android::TIMED_OUT )
      {
        MY_LOGW(  "pool[%s] request timeout(%d ms): retry",
                  mName, POOL_WAIT_TIME_MS);
      }
      #else // POOL_WAIT_TIME_MS
      mCondition.wait(mMutex);
      #endif // POOL_WAIT_TIME_MS
    }
  }while(true);

#if TIME_BUFFERPOOL_REQUEST
    timer.stop(MTRUE);
    MY_LOGD("BufferPool %s request time: %u (ms)", getName(), timer.getElapsed());
#endif
...
template <typename T>
P2F::ImageInfo BufferPool<T>::getImageInfo() const
{
  TRACE_FUNC_ENTER();
  TRACE_FUNC_EXIT();
  return P2F::ImageInfo();
}
...
MVOID BufferPool<T>::releaseAll()
{
  ...
  {
    MY_LOGE("%s: Some buffers are not released before pool released",
