...
public:
  static android::sp<FatImageBufferPool> create(const char* name, MUINT32 width, MUINT32 height, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint = std::vector<MUINT32>());
  static android::sp<FatImageBufferPool> create(const char* name, const MSize &size, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint = std::vector<MUINT32>());
...
protected:
  ...
  MBOOL init(MUINT32 width, MUINT32 height, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint);
  MVOID uninit();
  virtual android::sp<FatImageBufferHandle> doAllocate();
  virtual MBOOL doRelease(FatImageBufferHandle *handle);

private:
  MBOOL initConfig(MUINT32 width, MUINT32 height, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint);
