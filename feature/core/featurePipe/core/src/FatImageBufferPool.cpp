...
sp<IImageBufferHeap> FatHeap::createDummyHeap()
{
  ...
  if( this->isValid() )
  {
    ...
    portBufInfo.offset = mChunkUsed*mChunkSize;
...
android::sp<FatImageBufferPool> FatImageBufferPool::create(const char *name, MUINT32 width, MUINT32 height, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint)
{
  TRACE_FUNC_ENTER();
  ...
  else if( !pool->init(width, height, format, usage, strideHint) )
  {
...
android::sp<FatImageBufferPool> FatImageBufferPool::create(const char *name, const MSize &size, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint)
{
  TRACE_FUNC_ENTER();
  sp<FatImageBufferPool> pool;
  pool = FatImageBufferPool::create(name, size.w, size.h, format, usage, strideHint);
  TRACE_FUNC_EXIT();
  return pool;
}
...
P2F::ImageInfo FatImageBufferPool::getImageInfo() const
{
  TRACE_FUNC_ENTER();
  TRACE_FUNC_EXIT();
  return mImageInfo;
...
MBOOL FatImageBufferPool::init(MUINT32 width, MUINT32 height, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint)
{
    TRACE_FUNC_ENTER();
    ...
    else if( initConfig(width, height, format, usage, strideHint) )
    {
      mReady = MTRUE;
      ret = MTRUE;
    }
...
MBOOL FatImageBufferPool::initConfig(MUINT32 width, MUINT32 height, EImageFormat format, MUINT32 usage, const std::vector<MUINT32> &strideHint)
{
  TRACE_FUNC_ENTER();
  MBOOL ret = MFALSE;
  if( !usage )
  ...
  else
  {
    ...
    MBOOL useStrideHint = !strideHint.empty();
    if ( useStrideHint && (strideHint.size() != mPlane) )
    {
      MY_LOGW("invalid strideHint, strideHint.size(%zu) != mPlane(%u)", strideHint.size(), mPlane);
      useStrideHint = MFALSE;
    }
    ...
    {
      mImageInfo.mFormat = mFormat;
      mImageInfo.mSize = MSize(mWidth, mHeight);
      mImageInfo.mStrideInBytes.resize(mPlane);
      ...
      for( unsigned i = 0; i < mPlane; ++i )
      {
        ...
        if( useStrideHint )
        {
          if( strideHint[i] >= mStride[i] )
            mStride[i] = strideHint[i];
          else
            MY_LOGW("plane[%u]: strideHint(%u) < stride(%zu), not used", i, strideHint[i], mStride[i]);
        }
        mBufferSize += mStride[i] * queryPlaneHeightInPixels(mFormat, i, mHeight);
        mImageInfo.mStrideInBytes[i] = mStride[i];
      }
