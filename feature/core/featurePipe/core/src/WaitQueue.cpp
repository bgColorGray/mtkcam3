...
MUINT32 WaitHub::waitSignal(MBOOL useTimeout)
{
  TRACE_N_FUNC_ENTER(mName);
  ...
  {
    if( useTimeout || mTimeoutMS == 0 )
    {
        mCondition.wait(lock);
    }
    else if( mCondition.wait_for(lock, std::chrono::milliseconds(mTimeoutMS)) == std::cv_status::timeout )
    {
        MY_LOGD("%s wait timeout %u ms", mName, mTimeoutMS);
        break;
    }
...
MVOID WaitHub::setTimeoutMS(MUINT32 timeoutMS)
{
    TRACE_N_FUNC_ENTER(mName);
    Autolock lock(mMutex);
