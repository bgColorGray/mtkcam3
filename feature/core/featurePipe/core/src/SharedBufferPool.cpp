...
P2F::ImageInfo SharedBufferPool::getImageInfo() const
{
  TRACE_FUNC_ENTER();
  TRACE_FUNC_EXIT();
  ...
  {
    {
      ...
      while( mQuotaMap[ownerID] <= 0 )
      {
          ...
          {
            MY_LOGW("pool[%s] request(%d) timeout(%d ms): retry",
                    mName, ownerID, POOL_WAIT_TIME_MS);
          }
  ...
  if( buffer != NULL && buffer->getOwnerID(this, oldID) )
  {
    ...
    if( newID != oldID )
    {
      ...
      while( mQuotaMap[newID] <= 0 )
      {
          ...
          if( status == ::android::TIMED_OUT )
          {
            MY_LOGD("pool[%s] change owner(%d) timeout(%d ms): retry",
                    mName, newID, POOL_WAIT_TIME_MS);
          }
        #else // POOL_WAIT_TIME_MS
          mCondition.wait(mMutex);
        #endif // POOL_WAIT_TIME_MS
