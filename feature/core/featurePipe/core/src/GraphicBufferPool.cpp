...
android::sp<GraphicBufferPool> GraphicBufferPool::create(const char *name, MUINT32 width, MUINT32 height, android_pixel_format_t format, MUINT32 usage, NativeBufferWrapper::ColorSpace color, const std::vector<MUINT32> &strideHint)
{
  TRACE_FUNC_ENTER();
  (void)strideHint;
...
android::sp<GraphicBufferPool> GraphicBufferPool::create(const char *name, const MSize &size, EImageFormat format, MUINT32 usage, NativeBufferWrapper::ColorSpace color, const std::vector<MUINT32> &strideHint)
{
  TRACE_FUNC_ENTER();
  (void)strideHint;
  sp<GraphicBufferPool> pool;
...
P2F::ImageInfo GraphicBufferPool::getImageInfo() const
{
  TRACE_FUNC_ENTER();
  TRACE_FUNC_EXIT();
  return mImageInfo;
...
MBOOL GraphicBufferPool::initConfig(MUINT32 width, MUINT32 height, EImageFormat format, MUINT32 usage, NativeBufferWrapper::ColorSpace color)
{
  ...
  else
  {
    ...
    {
      mImageInfo.mFormat = mImageFormat;
