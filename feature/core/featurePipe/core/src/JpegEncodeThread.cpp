...
JpegEncodeThread::JpegEncodeThread(const MSizeF& finalCrop, const char* filename)
{
    ...
    else
    {
        ...
        int res = snprintf(fileName, sizeof(fileName), "/data/vendor/camera_dump/jpegencodethread/%" PRId64 "", timeNow);
        MY_LOGE_IF((res < 0), "JpegEncode snprintf FAIL!");
        if (mkdir(fileName, S_IRWXU | S_IRWXG))
        {
            MY_LOGW("mkdir for jpegencodethread file folder failed");
        }
        res = snprintf(fileName, sizeof(fileName), "/data/vendor/camera_dump/jpegencodethread/%" PRId64 "/%" PRId64 ".jpg", timeNow, timeNow);
        MY_LOGE_IF((res < 0), "JpegEncode snprintf FAIL!");
        mFilePath.assign(fileName);
    }
...
JpegEncodeThread::~JpegEncodeThread()
{
    ...
    if (mpJpegFp != NULL)
    {
        MY_LOGE_IF( fclose(mpJpegFp) != 0, "Close file failed !!!");
        mpJpegFp = NULL;
    }
...
bool JpegEncodeThread::encode(const sp<IIBuffer>& srcBuf)
{
    ...
    if (format == eImgFmt_YV12)
    {
        ret = mJpgHal.setEncSize(encodeSize.w, encodeSize.h, JpgEncHal::kENC_YV12_Format) &&
              mJpgHal.setSrcAddr(
    ...
    {
        ret = mJpgHal.setEncSize(encodeSize.w, encodeSize.h, JpgEncHal::kENC_YUY2_Format) &&
    ...
    {
        ret = mJpgHal.setEncSize(encodeSize.w, encodeSize.h, JpgEncHal::kENC_NV21_Format) &&
              mJpgHal.setSrcAddr(
                (void*)srcBuffer->getBufVA(0),
                (void*)srcBuffer->getBufVA(1)) &&
              mJpgHal.setSrcBufSize(
    ...
    if (!ret) {
        MY_LOGE("config JpgEncHal failed");
        mJpgHal.unlock();
        return false;
    }
    ...
    if (!mCropInfoSet)
    {
        success = success && (fwrite(&mFinalCrop.w, sizeof(mFinalCrop.w), 1, mpJpegFp) > 0)
                          && (fwrite(&mFinalCrop.h, sizeof(mFinalCrop.h), 1, mpJpegFp) > 0)
                          && (fwrite(&imgSize.w, sizeof(imgSize.w), 1, mpJpegFp) > 0)
                          && (fwrite(&imgSize.h, sizeof(imgSize.h), 1, mpJpegFp) > 0)
                          && (fwrite(&encodeSize.w, sizeof(encodeSize.w), 1, mpJpegFp) > 0)
                          && (fwrite(&encodeSize.h, sizeof(encodeSize.h), 1, mpJpegFp) > 0);
        mCropInfoSet = true;
    }

    MUINT32 writeSize = mExifSize + jpegSize;
    success = success && (fwrite(&writeSize, sizeof(MUINT32), 1, mpJpegFp) > 0)
                      && (fwrite(mExifBuf, mExifSize, 1, mpJpegFp) > 0)
                      && (fwrite((void*)mpJpegBuf->getBufVA(0), jpegSize, 1, mpJpegFp) > 0);
    MY_LOGE_IF( !success, "Write file has error !");
