...
#define BuildStream(_type_, _IStreamInfo_)                                     \
    ...
    BuildStream(eStreamType_IMG_APP, OutInfo.pISPImage_Thumb_Yuv_In);
...
)
{
    ...
    NodeT::ConfigParams cfgParam;
    {
        ...
        add_image_stream( inStreamSet, cfgParam.pInResized,     OutInfo.pISPImage_Resized_In);
...
)
{
    ...
    NodeT::ConfigParams cfgParam;
    {
        ...
        cfgParam.pInYuv_Thumbnail           = OutInfo.pISPImage_Thumb_Yuv_In != nullptr ? OutInfo.pISPImage_Thumb_Yuv_In : OutInfo.pHalImage_Thumbnail_YUV;
...
#undef ALIGN
    sp<IImageStreamInfo>
        pStreamInfo = ImageStreamInfoBuilder()
                        .setStreamName(streamName == nullptr ? "ISPModel-Unknown" : streamName)
...
{
    MY_LOGI("@@@PipelineModelISP::%s", __FUNCTION__);
    ...
    {
        UserOnFrameUpdated params;
        params.requestNo = arg.frame->getRequestNo();
        params.userId = arg.nodeId;
        params.bFrameEnd = false;
        params.bisEarlyCB = true;
        params.pMetadata = arg.resultMetadata;
        pCallback->onFrameUpdated(params);
    }
...
) -> int
{
    ...
    SET_IMAGESTREAM(eSTREAMID_IMAGE_ISP_DEVICE_INPUT_THUMBNAIL, mConfigureInfo.pISPImage_Thumb_Yuv_In);
    ...
    if (mConfigureInfo.pISPImage_JpegOut != nullptr)
    {
        ...
        // Thumbnail YUV
        if (mConfigureInfo.pISPImage_Thumb_Yuv_In == nullptr)
        {
...
) -> int
{
    ...
    if (request->pFDData != nullptr && request->pFDData->size() != 0)
    {
        ...
        MY_LOGD("store to fd isp container, key(%lld), size = %lu", request->FDDataTimestamp, request->pFDData->size());
        if (fddata != NULL)
        {
            MY_LOGD("size of MtkCameraFaceMetadata = %lu", sizeof(MtkCameraFaceMetadata));
    ...
    for( const auto& buffer : request->vIImageBuffers )
    {
        switch (buffer.first)
        {
            ...
            case eSTREAMID_IMAGE_ISP_DEVICE_INPUT_RESIZED:
                ...
                break;
            case eSTREAMID_IMAGE_ISP_DEVICE_INPUT_THUMBNAIL:
                FrameBuilder->setAppImageStreamBuffer(buffer.first, buffer.second);
                MY_LOGD("request has input thumbnail");
                break;
                //
            default:
        ...
        else
        {
            switch (buffer.second->getStreamInfo()->getImgFormat())
            {
                ...
                case eImgFmt_HEIF:
                    ...
                    if (mConfigureInfo.pISPImage_Thumb_Yuv_In != nullptr)
                    {
                        JpegImageMap.addIn(eSTREAMID_IMAGE_ISP_DEVICE_INPUT_THUMBNAIL);
                    }
                    else if (!mConfigureInfo.WithoutThumbnail)
                    {
        ...
        switch (buffer.first)
        {
            ...
            case eSTREAMID_META_PIPE_DYNAMIC_01:
                ...
                if (pHalMetaControl != nullptr)
                {
                    ...
                    // modify request number
                    IMetadata::IEntry reqEntry(MTK_PIPELINE_REQUEST_NUMBER);
                    reqEntry.push_back(request->requestNo, Type2Type<MINT32>());
                    pHalMetaControl->update(MTK_PIPELINE_REQUEST_NUMBER, reqEntry);

                    // Record NDD information
                    auto pRecorder = IScenarioRecorder::getInstance();
                    if (CC_UNLIKELY(pRecorder->isScenarioRecorderOn())) {
                      MINT32 uniqueKey = -1;
                      if (!IMetadata::getEntry<MINT32>(pHalMetaControl,
                      ...
                      MINT32 requestNo = -1;
                      if (!IMetadata::getEntry<MINT32>(pHalMetaControl,
                              MTK_PIPELINE_REQUEST_NUMBER, requestNo)) {
                          MY_LOGW("Get MTK_PIPELINE_REQUEST_NUMBER fail");
                      }

                      MINT32 frameNo = -1;
                      if (!IMetadata::getEntry<MINT32>(pHalMetaControl,
                              MTK_PIPELINE_FRAME_NUMBER, frameNo)) {
                          MY_LOGW("Get MTK_PIPELINE_FRAME_NUMBER fail");
                      }

                      MY_LOGD("NDD value(%d, %d, %d)", uniqueKey, requestNo, frameNo);
                      pRecorder->recordNddInfo(uniqueKey, requestNo, frameNo, true);
                    }
                }
                break;
        }
    ...
    if (needJpeg)
    {
        ...
        if (mConfigureInfo.WithoutThumbnail == false && mConfigureInfo.pISPImage_Thumb_Yuv_In == nullptr)
        {
