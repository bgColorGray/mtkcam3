...
registerBatch(const android::Vector<Request> requests) -> int
{
    ...
    batch->mFirstFrame = requests[0].requestNo;
    ...
    MY_LOGD("Batch %d is registered. requestNo: %d - %d",
...
dumpStateLocked(android::Printer& printer) const -> void
{
    for(CallbackParcel cb : mMergedParcels){
        android::String8 str;
        str += android::String8::format("CallbackParcel: requestNo(%u) valid(%d) shutter(%" PRIu64 ")", cb.requestNo, cb.valid, cb.timestampShutter);
        str += "vInputImage: ";
...
appendParcel(CallbackParcel srcParcel, CallbackParcel &dstParcel) -> void
{
    if (srcParcel.shutter != nullptr && srcParcel.shutter->timestamp != 0){
        if (dstParcel.shutter != nullptr){
            MY_LOGW("Redundantly update shutter timestamp for requestNo %u", srcParcel.requestNo);
        }
...
#define ELEMENT_FOUND(l, v) (std::find(l.begin(), l.end(), v) != l.end())
    ...
    for (CallbackParcel cbParcel : cbParcels) {
        ...
        auto batch = getBatchLocked(cbParcel.requestNo);
        ...
        if(cbParcel.needRemove)batch->mFrameRemoved.push_back(cbParcel.requestNo);

        isPending = false;
        hasPendingMetaResult = false;
        for (CallbackParcel &cbPending: mMergedParcels) {
            if (cbParcel.requestNo != cbPending.requestNo)
                continue;
        ...
        for (CallbackParcel::MetaItem mi : cbParcel.vOutputMetaItem) {
            if (mi.bufferNo == mCommonInfo->mAtMostMetaStreamCount) {
                    batch->mHasLastPartial.push_back(cbParcel.requestNo);
            }
        }
    ...
    while (itParcel != mMergedParcels.end()) {
        ...
        MY_LOGD_IF(getLogLevel() >= 2, "updated parcel requestNo:%d - output meta:%zu input image:%zu output image:%zu",
                cbParcel.requestNo,
        ...
        // Shutter timestamp
        {
            if (cbParcel.shutter != NULL && !ELEMENT_FOUND(batch->mShutterReturned, cbParcel.requestNo))
            {
                cbTemp.timestampShutter = cbParcel.timestampShutter;
                cbTemp.shutter = cbParcel.shutter;
                batch->mShutterReturned.push_back(cbParcel.requestNo);
                isUpdated = true;
            }
        }
        ...
        if (cbParcel.vOutputMetaItem.size() > 0) {
            ...
            if ((batch->mHasLastPartial.size() == batch->mBatchSize) ||
                ...
                (ELEMENT_FOUND(batch->mFrameRemoved, cbParcel.requestNo)))
            {
        ...
        if (isUpdated || mPendingNonBatchParcels.size() > 0) {
            ...
            cbTemp.requestNo = cbParcel.requestNo;
            cbTemp.needIgnore = cbParcel.needIgnore;
            while (mPendingNonBatchParcels.size() > 0 &&
                   mPendingNonBatchParcels[0].requestNo < cbParcel.requestNo)
            {
                MY_LOGD("Callback non-batch result requestNo:%d",  mPendingNonBatchParcels[0].requestNo);
