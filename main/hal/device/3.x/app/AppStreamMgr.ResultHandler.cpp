...
enqueResult(UpdateResultParams const& params) -> int
{
    ...
    profile.print_overtime(1, "requestNo:%u userId:%#" PRIxPTR " ResultQueue#:%zu", requestNo, userId, mResultQueue.size());
    //
    if  ( 0 <= index ) {
        sp<ResultItem> pItem = mResultQueue.editValueAt(index);

        MY_LOGW(
            "requestNo:%u existed in mResultQueue[%zd] - hasLastPartial:%d->%d",
            requestNo, index, pItem->lastPartial, hasLastPartial
        );
        ...
        // compared in priority : HARD > SOFT > NON
        pItem->realTimePartial = (pItem->realTimePartial > params.realTimePartial) ?
                                  pItem->realTimePartial : params.realTimePartial;
    }
    else {
    ...
    if ( params.physicalResult.size() != 0){
        sp<ResultItem> pItem = mResultQueue.valueFor(requestNo);
    ...
    mResultQueueCond.broadcast();
    //
    profile.print_overtime(1, "- requestNo:%u userId:%#" PRIxPTR, requestNo, userId);
    return OK;
}
