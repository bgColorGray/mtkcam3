...
class AppImageStreamBufferProviderImpl
    : public AppImageStreamBufferProvider
{
...
typedef std::map<uint32_t, std::map<StreamId_T, android::sp<AppImageStreamBuffer>>>
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
public:     ////    Operations.
                    AppImageStreamBufferProviderImpl(
...
private:
    ...
    virtual auto    dumpL(const std::map<StreamId_T, android::sp<AppImageStreamBuffer>>& buffers) const -> void;
...
{
    auto pInstance = std::make_shared<AppImageStreamBufferProviderImpl>(pHalBufHandler);
...
AppImageStreamBufferProviderImpl(
    ...
    , mLogLevel()
{
    mHandleAcquireFence = ::property_get_int32("vendor.debug.camera.appimagesbpvdr.acquirefence", 0);
    ...
    if ( mLogLevel==0 ) {
        mLogLevel = ::property_get_int32("vendor.debug.camera.log.deviceutils.appimagesbpvdr", 0);
    }

    MY_LOGD("HalBufHandler(%p) handle_acquire_fence(%d)", pHalBufHandler.get(), mHandleAcquireFence);
}
...
{
    // MY_LOGD("requestNumber(%u) store ImageStreamBuffer.size(%zu)", requestNumber, buffers.size());
    MY_LOGD("requestNumber(%u) store inputSBuffer.size(%zu) outputSBuffer.size(%zu)", requestNumber, vInputImageBuffers.size(), vOutputImageBuffers.size());
    {
        ...
        for ( size_t i=0; i<vInputImageBuffers.size(); ++i ) {
            mMap[requestNumber][vInputImageBuffers.keyAt(i)] = vInputImageBuffers.valueAt(i);
        }
        for ( size_t i=0; i<vOutputImageBuffers.size(); ++i ) {
            mMap[requestNumber][vOutputImageBuffers.keyAt(i)] = vOutputImageBuffers.valueAt(i);
        }
...
) -> int
{
    auto waitAcquireFence = [&](auto& pStreamBuffer) -> int {
        ...
        if( pStreamBuffer->getAcquireFence() != -1) { // waiting acquire_fence before createImageStreamBuffer
            ...
            const StreamId_T streamId = (pStreamInfo!=nullptr)? pStreamInfo->getStreamId() : -1;
            MY_LOGD_IF(/*mLogLevel>=1 || */t >= 10,
                    "StreamBuffer:%p streamId:(%" PRId64 ") bufferId:%" PRIu64 "imageBufferHeap(%p)"
                    "pre wait acquireFence %d, time %d ms",
                    pStreamBuffer.get(), streamId, pStreamBuffer->getBufferId(),
                    pStreamBuffer->getImageBufferHeap().get(), acquire_fence, (int)t);
    ...
    if ( mHalBufHandler ) {
        ...
        if ( (err = mHalBufHandler->requestStreamBuffer(cb, in)) == 0 ) {
            ...
            if ( CC_UNLIKELY( mMap.find(in.requestNo)==mMap.end() ||
                              mMap[in.requestNo].find(streamId)==mMap[in.requestNo].end() ) ) {
                MY_LOGD("not stored");
            } else {
...
auto
ThisNamespace::
dumpL(const std::map<StreamId_T, android::sp<AppImageStreamBuffer>>& buffers) const -> void
{
    for ( auto& it : buffers ) {
