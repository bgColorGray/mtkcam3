...
{
    auto pInstance = new HalBufHandler(creationInfo, pAppStreamMgr/*, pAppRequestUtil*/ );
    if  ( ! pInstance ) {
        delete pInstance;
        return nullptr;
    }
...
: mInstanceName{std::to_string(creationInfo.mInstanceId) + "-HalBufHandler"}
, mCommonInfo(std::make_shared<CommonInfo>())
, mAppStreamMgr(pAppStreamMgr)
// , mAppRequestUtil(pAppRequestUtil)
{
    mBufMinNum = ::property_get_int32("vendor.debug.camera.halbuf.num", HAL_BUF_MIN_NUM);
    ...
    const std::string threadName{std::to_string(mCommonInfo->mInstanceId)+":HalBufHdl"};
...
// auto
// ThisNamespace::
// getHalBufManagerStreamProvider() -> std::shared_ptr<IImageStreamBufferProvider>
// {
...
//         }
//         auto requestStreamBuffer( android::sp<IImageStreamBuffer>& rpImageStreamBuffer, IImageStreamBufferProvider::RequestStreamBuffer const& in) -> int
//         {
...
//     auto provider = std::make_shared<ImageStreamBufferProvider>(mAppStreamMgr, mAppRequestUtil);
//     return provider;
// }
 ...
 *
 ******************************************************************************/
// "not finished yet, different i/f"
auto
ThisNamespace::
requestStreamBuffer(IImageStreamBufferProvider::requestStreamBuffer_cb cb __attribute__((unused)),
                    IImageStreamBufferProvider::RequestStreamBuffer const& in __attribute__((unused)) ) -> int
{
//    Mutex::Autolock _l(mInterfaceLock);
    ...
    //         .streamBuffer       = rpStreamBuffer,
    //     };
    //     android::sp<AppImageStreamBuffer> appImageStreamBuffer = makeAppImageStreamBuffer(params);
    ...
    //     // mAppRequestUtil->convertStreamBuffer(rpStreamBuffer, appImageStreamBuffer);
    ...
    //             MY_LOGD_IF(mCommonInfo->mLogLevel>=2,
    //                 "end wait acquire fence. err %d", (int)err); //TBD: calculate time
    ...
    // }
    // // TODO : need interface
    // mAppStreamMgr->updateStreamBuffer(in.requestNo, in.pStreamInfo->getStreamId(), rpImageStreamBuffer);
...
auto
ThisNamespace::
_allocHalBuf(StreamId_T streamId, int num, HalBufPoolSP poolSP) -> int
{
    ...
    while(true) {
        ...
        MY_LOGW("Allocate failed. Reach max numers. streamId %d, usedCnt %d, maxCnt %d", (int)streamId, usedCnt, maxCnt);
        return -1;
    }
    ...
    if (auto cb = mCommonInfo->mDeviceCallback.lock()) {
        status = cb->requestStreamBuffers(mHalBufferReqs, &bufRets);
    } else {
    ...
    // auto err = mCommonInfo->mDeviceCallback->requestStreamBuffers(mHalBufferReqs,
    //     [&status, &bufRets] (
    ...
    // Only BufferRequestStatus::STATUS_OK and BufferRequestStatus::FAILED_PARTIAL reaches here
    ...
    auto bufSize = (int)hBufs.size();
    for(int i=0;i< bufSize;i++) {
        MY_LOGD_IF(getLogLevel() >= 1,
                "push_back bufferId: %d, buffer %p, acquireFence %p",
                (int)hBufs[i].bufferId, hBufs[i].buffer, hBufs[i].acquireFenceOp.hdl);
...
auto
ThisNamespace::
_getHalBufMaxCnt(StreamId_T streamId) -> int
{
...
auto
ThisNamespace::
setHalBufMaxCnt(const HalStreamConfiguration& halConfiguration) -> void
{
    Mutex::Autolock _l1(mCntLock);
...
auto
ThisNamespace::
_incHalBufAllocCnt(StreamId_T streamId, int cnt) -> int
{
    ...
    if(it == mCntAllocMap.end()) {
        ...
        MY_LOGD_IF(getLogLevel() >= 1,
                            "streamId %d, buf %d+%d",
                            (int)streamId, (int)0, (int)cnt);
        return cnt;
    }
    else {
        MY_LOGD_IF(getLogLevel() >= 1,
                            "streamId %d, buf %d+%d",
                            (int)streamId, (int)it->second, (int)cnt);
        it->second+=cnt;
        return it->second;
    }
}
...
auto
ThisNamespace::
_decHalBufAllocCnt(StreamId_T streamId, int cnt) -> int
{
    auto it = mCntAllocMap.find(streamId);
    if(it == mCntAllocMap.end()) {
    ...
    if(it->second<=0) {
        MY_LOGE("stream %d in mCntAllocMap, or count %d <=0 , decCnt %d!", (int)streamId, it->second, cnt);
        return 0;
    }
...
inline auto
ThisNamespace::
_getHalBufAllocCnt(StreamId_T streamId) -> int
{
    auto it = mCntAllocMap.find(streamId);
    if(it == mCntAllocMap.end())
        return 0;
...
auto
ThisNamespace::
_incHalBufInflightCnt(StreamId_T streamId, int cnt) -> int
{
...
auto
ThisNamespace::
_decHalBufInflightCnt(StreamId_T streamId, int cnt) -> int
{
    auto it = mCntInflightMap.find(streamId);
    if(it == mCntInflightMap.end()) {
    ...
    if(it->second<=0) {
        MY_LOGE("stream %d in mCntInflightMap, or count %d <=0 , decCnt %d!", (int)streamId, it->second, cnt);
        return 0;
    }
...
inline auto
ThisNamespace::
_getHalBufInflightCnt(StreamId_T streamId) -> int
{
    auto it = mCntInflightMap.find(streamId);
    if(it == mCntInflightMap.end())
        return 0;
...
auto
ThisNamespace::
updateHalBufCnt(const vector<CaptureRequest>& requests) -> void
{
            ...
            auto bufferId = buffer.bufferId;
                ...
                if(bufferId == 0) { // bufferId means it use hal buf feature
                    ...
                    if(mBufPoolMap.find(streamId) == mBufPoolMap.end()) {
                        MY_LOGD_IF(getLogLevel() >= 1, "register stream %d to mBufPoolMap", (int)streamId);
                        //create empty pool
...
auto
ThisNamespace::
updateHalBufCnt(const vector<CaptureResult>& vBufferResult) -> void
{
...
auto
ThisNamespace::
_dequeueHalBufPool(StreamId_T streamId, HalBufSP &rpStreamBufferSP) -> bool
{
    ...
    if(poolSP->size() == 0) {
        MY_LOGD_IF(getLogLevel() >= 1,
            "direct allocate due to pool already empty. streamId %d", (int)streamId);
    ...
    MY_LOGD_IF(getLogLevel() >= 1,
        "pop_front bufferId: %d, buffer %p", (int)rpStreamBufferSP->bufferId, rpStreamBufferSP->buffer);
    ...
    if(poolSP->size() < mBufMinNum ) {
        MY_LOGD_IF(getLogLevel() >= 1,
            "Trigger pre-allocate pool size() %d, mBufMinNum %d", (int)poolSP->size(), (int)mBufMinNum);
        mBufCond.broadcast();
    }
    return true;
}
...
ThisNamespace::
allocHalBuf(
        HalBufSP &rpStreamBufferSP,
        IImageStreamBufferProvider::RequestStreamBuffer const& in) -> int
{
    MY_LOGD_IF(getLogLevel()>=1,
            ...
            (int)in.timeout,
            in.pStreamInfo->getStreamName(),
            (int)in.pStreamInfo->getStreamId(),
            in.pStreamInfo->getStreamType()
            );

    nsecs_t current = ::systemTime();
    if(false == _dequeueHalBufPool(in.pStreamInfo->getStreamId(), rpStreamBufferSP)) {
        MY_LOGE("_dequeueHalBufPool fail");
        return UNKNOWN_ERROR;
    }

    if(rpStreamBufferSP->buffer == nullptr) {
        auto it = mHalBufImportMap.find(std::make_pair(rpStreamBufferSP->streamId, rpStreamBufferSP->bufferId));
        if(it != mHalBufImportMap.end()) {
            rpStreamBufferSP = it->second;
            MY_LOGD("replace with mHalBufImportMap streamId %d, bufferId %d, buf %p",
                    (int)rpStreamBufferSP->streamId,
                    (int)rpStreamBufferSP->bufferId,
    ...
    MY_LOGD_IF(getLogLevel()>=1  || t >= 10,
            "requestNo %d, timeout %d, streamName %s, streamId %d, streamType %d time %d ms",
            in.requestNo,
            (int)in.timeout,
            in.pStreamInfo->getStreamName(),
            (int)in.pStreamInfo->getStreamId(),
...
auto
ThisNamespace::
_releaseAllHalBuf(StreamId_T streamId, HalBufPoolSP poolSP) -> void
{
    // Pre-allocate bufferId & handle will be reuse between different configuration.
    // we can't release those buffer during flush/waitUntilDrain stage
    // i.e. bufferId will assume we keep last handle in previous session
    //      this session allocation will only get bufferId with null handle
    //
    // Another race condition will be pre-allocate thread may run after waitUntilDrain
    // and cause some buffer not return.
    //
    //
    ...
    MY_LOGD("releaseStreamBuffers streamId %d, %d buffers +", (int)streamId, (int)numBuffers);
    if(numBuffers >0) {
        // reduce allocate count
        {
            Mutex::Autolock _l1(mCntLock);
            _decHalBufAllocCnt(streamId, (int)numBuffers);
        }
        ...
        for(auto bufSP: *poolSP) {
            ...
            if(bufSP->releaseFenceOp.hdl) {
                MY_LOGW("streamId %d, bufferId %d, releaseFence %p exists!",
                    (int)bufSP->streamId, (int)bufSP->bufferId, bufSP->releaseFenceOp.hdl);
            }
            i++;

            //
            if( bufSP->buffer) {
                MY_LOGD("mHalBufImportMap insert streamId %d, bufferId %d, buf %p", (int) bufSP->streamId, (int) bufSP->bufferId, bufSP->buffer);
                mHalBufImportMap.insert_or_assign( std::make_pair(bufSP->streamId, bufSP->bufferId), bufSP);
            }
        }

        if (auto cb = mCommonInfo->mDeviceCallback.lock()) {
            cb->returnStreamBuffers( hBufs );
        }

        // if (!ret.isOk()) {
        //     MY_LOGE("Transaction error in mDeviceCallback->returnStreamBuffers: %s", ret.description().c_str());
        // }
        poolSP->clear();
    }
