...
#define MY_LOGV_LV1(...)       do { if ( (mLogLv > 0) ) { MY_LOGV(__VA_ARGS__); } }while(0)
#define MY_LOGD_LV1(...)       do { if ( (mLogLv > 0) ) { MY_LOGD(__VA_ARGS__); } }while(0)
#define MY_LOGI_LV1(...)       do { if ( (mLogLv > 0) ) { MY_LOGI(__VA_ARGS__); } }while(0)
#define MY_LOGW_LV1(...)       do { if ( (mLogLv > 0) ) { MY_LOGW(__VA_ARGS__); } }while(0)
#define MY_LOGE_LV1(...)       do { if ( (mLogLv > 0) ) { MY_LOGE(__VA_ARGS__); } }while(0)

using namespace NSCam::v3;

auto
ZoomRatioConverter::
Create(const bool supportZoomRatio,
       Dimension d,
       DimensionMap map
) -> std::shared_ptr<ZoomRatioConverter>
{
    int32_t logLv = ::property_get_int32("vendor.debug.camera.log.ZoomRatioConverter", 0);
    MY_LOGD("Create ZoomRatioConverter support(%d), logLv(%d)", supportZoomRatio, logLv);
    return std::make_shared<ZoomRatioConverter>(supportZoomRatio, d, map, logLv);
}

auto
ZoomRatioConverter::
UpdateCaptureRequestMeta(const uint32_t reqNum,
                         std::unordered_map<StreamId_T, android::sp<IMetaStreamBuffer>> &requestMetaMap
) -> void
{
    ...
    for(auto&& [streamId, pMetaBuffer] : requestMetaMap)
    {
        ...
        {
            MY_LOGE("Get dimension failed, R%d stream(%s)", reqNum, pMetaBuffer->getName());
            continue;
        }

        IMetadata* pMetadata = pMetaBuffer->tryWriteLock(LOG_TAG, true);
                ...
                {
                    {
                        ...
                        MY_LOGD_LV1("Set: R%u, stream(%s), zoom_ratio(%f), size(%zu)",
                                reqNum, pMetaBuffer->getName(), zoom_ratio, mReqRatioMap.size());
                    }
...
auto
ZoomRatioConverter::
UpdateCaptureResultMeta(
    UpdateResultParams &resultMeta
) -> void
{
    const uint32_t reqNum = resultMeta.requestNo;
    android::Vector<android::sp<IMetaStreamBuffer>> &vResultMeta = resultMeta.resultMeta;
    android::Vector<android::sp<PhysicalResult>> &vResultMeta_phy = resultMeta.physicalResult;
...

    {
        ...
        if(mReqRatioMap.find(reqNum) != mReqRatioMap.end()) {
            ...
            MY_LOGD_LV1("R%d no zoom ratio found in map", reqNum);
            return;
        }
    ...
    for(int i = 0; i < vResultMeta.size(); i++)
    {
        ...
        {
            MY_LOGD("Skip error metabuffer: R%d, stream(%s)",
                        reqNum, vResultMeta[i]->getName());
            continue;
        }
        ...
        IMetadata* pMetadata = vResultMeta[i]->tryWriteLock(LOG_TAG, true);
        if(pMetadata != nullptr)
        {
            MY_LOGD_LV1("Get: R%d, stream(%s), zoom_ratio(%f)",
                        reqNum, vResultMeta[i]->getName(), zoom_ratio);
            ApplyZoomRatio(pMetadata, zoom_ratio, d, false, false);
            vResultMeta[i]->unlock(LOG_TAG, pMetadata);
        }
        else {
            MY_LOGE("Get pMetadata failed: R%d, stream(%s)",
                        reqNum, vResultMeta[i]->getName());
        }
    }
    ...
    for(int i = 0; i < vResultMeta_phy.size(); i++)
    {
        ...
        IMetadata* pMetadata = physicRes->tryWriteLock(LOG_TAG, true);
        if(pMetadata != nullptr)
        {
            MY_LOGD_LV1("Get: R%d, stream(%s), zoom_ratio(%f)",
                        reqNum, physicRes->getName(), zoom_ratio);
...
auto
ZoomRatioConverter::
UpdateCropRegionWithoutConvert(
...
auto
ZoomRatioConverter::
ApplyZoomRatio(IMetadata* pMetadata,
...
) -> void
{
    ...
    UpdateCropRegion(pMetadata, zoom_ratio, active_array_dimension, is_request, is_phy);
    if(is_request) {
...
auto
ZoomRatioConverter::
UpdateCropRegion(IMetadata* pMetadata,
                 const float zoom_ratio,
                 const Dimension& active_array_dimension,
                 const bool is_request,
                 const bool is_phy
) -> void
{
    ...
    else // result
    {
        if(is_phy)
        {
            ...
            {
                MRect rect = entry.itemAt(0, Type2Type<MRect>());
                RevertZoomRatio(zoom_ratio, rect, active_array_dimension);
...
auto
ZoomRatioConverter::
Update3ARegion(IMetadata* pMetadata,
               const float zoom_ratio,
...
auto
ZoomRatioConverter::
Update3AFeatureRegion(IMetadata* pMetadata,
                      const float zoom_ratio,
                      const mtk_camera_metadata_tag_t tag_id,
                      const Dimension& active_array_dimension
) -> void
{
    ...
    if( !entry.isEmpty() )
    {
        ...
        if (entryCount < 7 || entryCount != entry.itemAt(1, Type2Type<MINT32>()) * 5 + 2)
        {
        ...
        entryReverted.push_back( entry.itemAt(0, Type2Type<MINT32>()), Type2Type<MINT32>() );
        entryReverted.push_back( entry.itemAt(1, Type2Type<MINT32>()), Type2Type<MINT32>() );
...
auto
ZoomRatioConverter::
UpdateFace(IMetadata* pMetadata,
           const float zoom_ratio,
           const Dimension& active_array_dimension
) -> void
{
    ...
    ////// Face Landmarks
    IMetadata::IEntry entryLandmark = pMetadata->entryFor(MTK_STATISTICS_FACE_LANDMARKS);
...
auto
ZoomRatioConverter::
ConvertZoomRatio(const float zoom_ratio,
                 MRect& rect,
                 const Dimension& active_array_dimension
) -> void
{
...
auto
ZoomRatioConverter::
RevertZoomRatio(const float zoom_ratio,
                MRect& rect,
                const Dimension& active_array_dimension
) -> void
{
...
auto
ZoomRatioConverter::
CorrectRegionBoundary(int32_t &left, int32_t &top, int32_t &width, int32_t &height,
...
auto
ZoomRatioConverter::
GetDimensionByMetaBuffer(const int32_t phyId,
                         Dimension& d
) -> bool
{
    // logical
    if (phyId == -1)
    {
        ...
        if( mActiveArrayDimension_Physical.find(phyId) != mActiveArrayDimension_Physical.end() ) {
            d.width = mActiveArrayDimension_Physical[phyId].width;
            d.height = mActiveArrayDimension_Physical[phyId].height;
            MY_LOGD_LV1("get phy(%d) dimension, w(%d), h(%d)", phyId, d.width, d.height);
            return true;
        }
    }
...
auto
ZoomRatioConverter::
Dump(const IMetadata* pMetadata) -> void
{
...
auto
ZoomRatioConverter::
DumpTag(const IMetadata* pMetadata,
