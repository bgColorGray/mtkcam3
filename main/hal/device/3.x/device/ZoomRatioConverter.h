...
public:
    ...
    ZoomRatioConverter  (bool supportZoomRatio, Dimension d, DimensionMap map, int32_t logLv) :
                        mSupportZoomRatio(supportZoomRatio), mActiveArrayDimension(d), mActiveArrayDimension_Physical(map), mLogLv(logLv)
                        { }

    static
    auto                Create(bool supportZoomRatio, Dimension d, DimensionMap map)
                            -> std::shared_ptr<ZoomRatioConverter>;

    auto                UpdateCaptureRequestMeta(
                            const uint32_t reqNum,
                            std::unordered_map<StreamId_T, android::sp<IMetaStreamBuffer>> &requestMetaMap) -> void;
...
private:
    ...
    auto                CorrectRegionBoundary(
                            int32_t &left, int32_t &top, int32_t &width, int32_t &height,
                            const int32_t bound_w, const int32_t bound_h) -> void;

    auto                GetDimensionByMetaBuffer(
                            const int32_t physicalId,
                            Dimension& d
                        ) -> bool;
