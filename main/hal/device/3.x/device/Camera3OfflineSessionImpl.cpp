...
#include "Camera3OfflineSessionImpl.h"
#include "MyUtils.h"
...
#define ThisNamespace   Camera3OfflineSessionImpl
...
Camera3OfflineSessionImpl(CreationInfo const& info)
    : ICamera3OfflineSession()
    , mStaticInfo(info)
    , mLogPrefix(std::to_string(info.mStaticDeviceInfo->mInstanceId)+"-offSess")
    , mLogLevel(0)
...
void
ThisNamespace::
setCallback(const std::shared_ptr<CameraDevice3SessionCallback>& callback)
{
