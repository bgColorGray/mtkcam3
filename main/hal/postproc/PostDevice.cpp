...
) -> Return<int32_t>
{
    ...
    auto srcListSize = sizeof(param.src_sizeList);
    auto dstListSize = sizeof(nrParam.src_sizeList);
    if (srcListSize > dstListSize) {
        MY_LOGF("param.src_sizeList(%d) is larger than nrParam.src_sizeList(%d)",
            srcListSize, dstListSize);
        return -1;
    }
    memcpy(&(nrParam.src_sizeList), &(param.src_sizeList), srcListSize);
