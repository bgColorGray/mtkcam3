...
) const -> int32_t
{
    ...
    auto& pVirtualDevice = mVirtEnumDeviceMap.valueFor(mVirInstanceMap.valueAt(index));
    if ( CC_UNLIKELY(!pVirtualDevice.get() || !pVirtualDevice->mVirtDevice.get()) ) {
        MY_LOGF("get deviceInfo %d fail: VirtualDevice(%p)", deviceId, pVirtualDevice.get(), pVirtualDevice->mVirtDevice.get());
        return -EINVAL;
    }
    ...
    return (pVirtualDevice->mVirtDevice)? HARDWARE_DEVICE_API_VERSION(major, minor) : 0;
}*/
