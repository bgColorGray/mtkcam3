...
initialize() -> bool
{
    ...
    mVirtEnumDeviceFactoryMap[3] = loadDeviceFactory("libmtkcam_device3_hal.so", "createVirtualCameraDevice");
    ...
    for (size_t i = 0; i < mVirtEnumDeviceMap.size(); i++) {
        ...
        if (!isHiddenCamera(pVirtEnumDevice->mInstanceId)) {
            ...
            ids.push_back(pVirtDevice->getVirtualInstanceId());
        }
    }
    CAM_ULOGM_ASSERT(
        NSCam::NSTemplateRequestManager::constructTemplateRequests(ids),
        "constructTemplateRequests fail");
    return (OK==status);
}
...
auto
CameraDeviceManagerBase::
getConcurrentStreamingDeviceNameList(
    std::vector<std::vector<std::string>>& rDeviceNames
) -> ::android::status_t
{
    RWLock::AutoRLock _l(mDataRWLock);
    //
    ...
    auto concurrentStramingCamIdList = pipInfo.getConcurrentStreamingCameraIds();

    rDeviceNames.reserve(concurrentStramingCamIdList.size());
    for (auto concurrentStreamingCamId : concurrentStramingCamIdList) {
        std::vector<std::string> camIdPair;
        for (auto camId : concurrentStreamingCamId) {
            camIdPair.push_back(std::to_string(camId));
        }
        rDeviceNames.push_back(camIdPair);
    }
    //
    return OK;
}
...
) -> ::android::status_t
{
    ...
    //
    rpDevice = ::android::sp<CameraDevice3Impl>(
                static_cast<CameraDevice3Impl*>(pDevice.get()));
    if  ( rpDevice == nullptr ) {
        MY_LOGE("[%s] cannot get ::android::sp<CameraDevice3Impl>", deviceName.c_str());
        logLocked();
        return BAD_VALUE;
    }
    return OK;
...
isConcurrentStreamCombinationSupported(
    const std::vector<int> vCamIds,
    const std::vector<MSize> vSize
) -> bool
{
    auto& pipInfo = PIPInfo::getInstance();
    pipInfo.initializeConcurrentStreamSetting();
    auto vConcurrentStreamCombinations = pipInfo.getConcurrentStreamCombination();

    if (vCamIds.size() > 2) return false;
    for (auto concurrentStreamCombination : vConcurrentStreamCombinations) {
        bool found = true;
        for (auto streamConfig : concurrentStreamCombination) {
            if (std::find(vCamIds.begin(), vCamIds.end(), streamConfig.camId) == vCamIds.end()) {
                found = false;
                break;
            }
        }
        if (found) {
            if (concurrentStreamCombination[0].camId == vCamIds[0]) {
                if ((concurrentStreamCombination[0].maxWidth >= vSize[0].w && concurrentStreamCombination[0].maxHeight >= vSize[0].h)
                    && concurrentStreamCombination[1].maxWidth >= vSize[1].w && concurrentStreamCombination[1].maxHeight >= vSize[1].h)
                {
                    return true;
                }
            } else {
                if ((concurrentStreamCombination[0].maxWidth >= vSize[1].w && concurrentStreamCombination[0].maxHeight >= vSize[1].h)
                    && concurrentStreamCombination[1].maxWidth >= vSize[0].w && concurrentStreamCombination[1].maxHeight >= vSize[0].h)
                {
                    return true;
                }
            }
