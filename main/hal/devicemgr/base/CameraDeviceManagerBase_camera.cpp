    ...
    auto add_virtual_device = [&](IVirtualDevice* pVirtualDevice){
            if ( pVirtualDevice != nullptr ) {
                if  ( auto pInfo = new VirtEnumDevice ) {
                    ...
                    pInfo->mVirtDevice = ::android::sp<IVirtualDevice> (pVirtualDevice);
                    pInfo->mInstanceId = pVirtualDevice->getInstanceId();
                    auto pName = pVirtualDevice->getInstanceName();
                    if(pName)
                    {
                        mVirtEnumDeviceMap.add(pName, pInfo);
                        // Legacy HAL API MUST use incremental Camera ID from 0 to N-1.
                        // However, we have remapping mechanism to generate jumping Camera ID as mVirtualInstanceId.
...
) const -> ::android::status_t
{
    ...
    //
    auto pEnumInfo = (deviceName) ? mVirtEnumDeviceMap.valueFor(deviceName) : nullptr;
    if  ( pEnumInfo == 0 ) {
