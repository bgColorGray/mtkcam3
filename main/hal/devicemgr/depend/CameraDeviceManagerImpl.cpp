...
onEnumerateDevicesLocked() -> ::android::status_t
{
    ...
    int queryScenario = SENSOR_SCENARIO_ID_NORMAL_CAPTURE;  //The value doesn't matter, full_w/h will be the same for all sensor scenarios
    for (size_t sensorId = 0; sensorId < sensorNum; sensorId++)
    {
        ...
        if (pHalSensorList)
        {
            ...
            if(pIHalSensor)
            {
                ...
                int err = pIHalSensor->sendCommand(sensorDevIndex,
                                                   SENSOR_CMD_GET_SENSOR_CROP_WIN_INFO,
                                                   (MUINTPTR)&queryScenario,
                                                   (MUINTPTR)&rSensorCropInfo, 0);
                MY_LOGE_IF(err != OK, "pIHalSensor->sendCommand failed, err:%d(%s)", err, ::strerror(-err));
                pIHalSensor->destroyInstance(LOG_TAG);

                MY_LOGD("[addSensorSizeToCompare] Sensor %zu, %dx%d", sensorId, rSensorCropInfo.full_w, rSensorCropInfo.full_h);
                DipUtils::addSensorSizeToCompare(rSensorCropInfo.full_w, rSensorCropInfo.full_h);
            }
            else
            {
...
) const -> ::android::status_t
{
    ...
    if  ( 0 != mOpenDeviceMap.size() )
    {
        ...
        if (!pipInfo.checkOpenAbility(i4OpenId)) {
            return -EUSERS;
        }
        // Check ISP Resource
