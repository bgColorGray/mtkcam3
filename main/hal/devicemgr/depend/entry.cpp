...
getLegacyCameraModule()
{
    ...
    static NSCam::legacy_dev3::LegacyCameraModule* module = createLegacyCameraModule(&singleton);
    if ( module == nullptr ) {
...
set_callbacks(camera_module_callbacks_t const* callbacks)
{
    ...
    return  getLegacyCameraModule()->setCallbacks(callbacks);
}
...
{
    return getLegacyCameraModule()->getPhysicalCameraInfo(physical_camera_id, static_metadata);
}
...
{
    return getLegacyCameraModule()->isStreamCombinationSupported(camera_id, streams);
}


static
void
notify_device_state_change(uint64_t deviceState)
{
