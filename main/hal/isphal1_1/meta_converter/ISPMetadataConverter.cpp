...
static std::unordered_map<uint32_t, uint32_t> gMtkTagMap = {
        {MTK_SCALER_CROP_APP_REGION, (uint32_t)(MTK_TYPE_ENUM::TYPE_MRect)},
        {MTK_SCALER_CROP_REGION, (uint32_t)(MTK_TYPE_ENUM::TYPE_MRect)},
        {MTK_SINGLEHW_SETTING_SOURCE_CROP, (uint32_t)(MTK_TYPE_ENUM::TYPE_MRect)},
        {MTK_SINGLEHW_SETTING_WARP_OUTPUT, (uint32_t)(MTK_TYPE_ENUM::TYPE_MRect)},
        {MTK_CONTROL_CAPTURE_P2_CROP_REGION_CUSTOMIZE, (uint32_t)(MTK_TYPE_ENUM::TYPE_MRect)},
        {MTK_CONTROL_CAPTURE_P2_RESIZER_SIZE_CUSTOMIZE, (uint32_t)(MTK_TYPE_ENUM::TYPE_MRect)},
};
...
auto ISPMetadataConverter::convertToIMetadata(const void* src, IMetadata& dst) -> MBOOL
{
    ...
    // assign MTK_SCALER_CROP_APP_REGION to MTK_SCALER_CROP_REGION
    IMetadata::IEntry entry = dst.entryFor(MTK_SCALER_CROP_APP_REGION);
    if( !entry.isEmpty() )
    {
        ...
        if ( rect.s.w != 0 && rect.s.h != 0 )
        {
            ...
            dst.update(MTK_SCALER_CROP_REGION, entryNew);
        }
    }
