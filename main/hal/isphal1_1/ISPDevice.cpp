...
using vendor::mediatek::hardware::camera::isphal::V1_0::FeatureSetting;
...
Return<void> ISPDevice::getISPSupportOutputFormat(getISPSupportOutputFormat_cb _hidl_cb)
{
    ...
    static hidl_vec<::vendor::mediatek::hardware::camera::isphal
                  ::V1_0::ISPFormat> out;
    if (out.size() == 0) {
        std::vector<::vendor::mediatek::hardware::camera::isphal
        ::V1_0::ISPFormat> ispFormat;
        ::vendor::mediatek::hardware::camera::isphal::V1_0::ISPFormat _ispFormat{
        PixelFormat::BLOB,
        static_cast<V1_0::DataspaceFlags>(Dataspace::V0_JFIF)
        };
...
Return<void> ISPDevice::getISPSupportInputFormat(getISPSupportInputFormat_cb _hidl_cb)
{
    ...
    if (out.size() == 0) {
        std::vector<::vendor::mediatek::hardware::camera::isphal
        ::V1_0::ISPFormat> ispFormat;
        // TODO(MTK): check if need to fill a specific dataspace
        ::vendor::mediatek::hardware::camera::isphal::V1_0::ISPFormat _ispFormat {
        PixelFormat::YV12,
        static_cast<V1_0::DataspaceFlags>(Dataspace::UNKNOWN)
        };
        ispFormat.push_back(std::move(_ispFormat));
        out = ispFormat;
    }
...
{
    FUNC_START;
    if (mMetaConverter == nullptr || mConfigParams == nullptr)
    {
    ...
    if (mConfigParams->HWModule != MTK_SINGLEHW_SETTING_MODULE_NONE)
    {
        MY_LOGW("SingleHW module mode is not support queryFeatureSetting");
        hidl_vec<FeatureSetting> out;
        _hidl_cb(Status::OK, 0, out);
        return Void();
    }
    if ( mPipelineModel == nullptr )
    {
        std::lock_guard<std::timed_mutex> _l(mLock);
        MY_LOGD("configure pipeline for get feature setting");
        mCamId = 0;
        mSensorIds.clear();
        mSensorIds.push_back(0);
        // create new pipeline and config
        MY_LOGD("create new isp pipeline and then configure");
        mPipelineModel = IISPPipelineModel::createInstance(mId, mCamId, mSensorIds, MTK_SINGLEHW_SETTING_MODULE_NONE); // camera id current is not used
...
{
    CAM_ULOG_APILIFE_GUARD(MOD_ISP_HAL_SERVER);
    FUNC_START;
    std::lock_guard<std::timed_mutex> _l(mLock);
    ...
    {
        MY_LOGE("[configureISPDevice]ISP stream controller failed");
        return Status::INTERNAL_ERROR;
    }

    mCamId = mConfigParams->CamId;
    mSensorIds.clear();
    mSensorIds.push_back(mCamId);
    if (mConfigParams->HWModule != MTK_SINGLEHW_SETTING_MODULE_NONE)
    {
        mPipelineModel = IISPPipelineModel::createInstance(mId, mCamId, mSensorIds, mConfigParams->HWModule);
        // pipeline configure
        MY_LOGD("Open/Configure singleHW Pipeline");
        mPipelineModel->open("ISPDevice(HW)", mPipelineCallback);
        mPipelineModel->configure(mConfigParams);
        MY_LOGD("Configure singleHW Pipeline done");
    }
    else if (mCamId == -1)
    {
        // use sensor id = -1 to set default value
        // Do not configure pipeline while unknown cam id
        mSensorIds.clear();
        mSensorIds.push_back(-1);
    }
    else
    {
        mPipelineModel = IISPPipelineModel::createInstance(mId, mCamId, mSensorIds, MTK_SINGLEHW_SETTING_MODULE_NONE);
        // pipeline configure
...
Return<Status> ISPDevice::configureISPDevice_1_1(
    const V1_1::ISPStreamConfiguration& config,
    const ::android::sp<V1_0::ICallback>& Cb) {
        return Status::OK;
}
...
Return<Status> ISPDevice::processImgRequest(const hidl_vec<V1_0::ISPRequest>& requests)
{
    ...
    if (mConfigParams->HWModule != MTK_SINGLEHW_SETTING_MODULE_NONE)
    {
        ret = mStreamCtrl->parseRequests_singleHW(requests, ppl_requests, reconfig_params);
    }
    else
    {
        ret = mStreamCtrl->parseRequests(requests, ppl_requests, reconfig_params);
    }
    if (ret)
    {
    ...
    {
        IMetadata data;
        mMetaConverter->convertToIMetadata(requests[i].settings.data(), data);
        mPipelineCallback->registerReq(requests[i].reqNumber, data);
    }
...
Return<Status> ISPDevice::processImgRequest_1_1(
    const hidl_vec<V1_1::ISPRequest>& requests __unused) {
    return Status::OK;
}

Return<void> ISPDevice::getISPSupportOutputFormat_1_1(
    getISPSupportOutputFormat_1_1_cb _hidl_cb) {
    return Void();
}

Return<void> ISPDevice::getISPSupportInputFormat_1_1(
    getISPSupportInputFormat_1_1_cb _hidl_cb) {
    return Void();
}
...
) -> MBOOL
{
    ...
    if (currentParam->HWModule != MTK_SINGLEHW_SETTING_MODULE_NONE)
    {
        MY_LOGD("Single HW module mode!!, no need reconfigure");
        return false;
    }
