...
#define THUMB_NAIL_WIDTH (320)
#define ISPHIDL_DUMP_PATH               "/data/vendor/camera_dump/"
#define ISPHIDL_DUMP_COMMAND            "vendor.debug.camera.dump.ispstreamctrl"
#define ISPHIDL_LOG_COMMAND             "vendor.debug.camera.log.ispstreamctrl"
...
{
    mLogLevel = property_get_int32(ISPHIDL_LOG_COMMAND, 0);
    mDump = property_get_int32(ISPHIDL_DUMP_COMMAND, 0);
    mUseYUVO = property_get_int32("vendor.debug.camera.yuvo.en",
                                  MTK_CAM_PREFER_RRZO_USE_YUVFMT/* pre-definition in mtkcam.mk */);
    mIonDevice = IIonDeviceProvider::get()->makeIonDevice("ISPDevice");
...
) -> int
{
    ...
    mHasThumbnail = false;
    params->HWModule = MTK_SINGLEHW_SETTING_MODULE_NONE;
    ...
    {
        IMetadata::IEntry const& e1 = params->sessionParams.entryFor(MTK_SINGLEHW_SETTING_MODULE);
        if ( ! e1.isEmpty() ) {
            params->HWModule = e1.itemAt(0, Type2Type<MINT32>());
            MY_LOGD("params->HWModule: 0x%X", params->HWModule);
        }
        if (params->HWModule != MTK_SINGLEHW_SETTING_MODULE_NONE)
        {
            mSingleHWMode = true;
        }
    }
    IMetadata::IEntry const& entrySensorId = params->sessionParams.entryFor(MTK_HAL_REQUEST_SENSOR_ID);
    // get sensor id
    if  ( !entrySensorId.isEmpty() )
    {
        params->CamId = entrySensorId.itemAt(0, Type2Type<MINT32>());
        MY_LOGD("Configure CamId : %d", params->CamId);
    }
    else
    {
        params->CamId = -1;
    ...
    //create image stream info
    {
        int32_t rawbpp = 0;
        {
            IMetadata::IEntry const& bppEntry = params->sessionParams.entryFor(MTK_CONTROL_CAPTURE_RAW_BPP);
            if ( ! bppEntry.isEmpty() ) {
                rawbpp = bppEntry.itemAt(0, Type2Type<MINT32>());
                MY_LOGD("bppOfRaw: %d", rawbpp);
            }
        }
        ...
        //input
        for (size_t i = 0; i < config.InputStreams.size(); i++){
            auto pImageStreamInfo = createImageStreamInfo(config.InputStreams[i], eSTREAMTYPE_IMAGE_IN, rawbpp);
        ...
        for (size_t i = 0; i < config.OutputStreams.size(); i++){
            ...
            if (pImageStreamInfo->getImgFormat() != eImgFmt_BLOB && pImageStreamInfo->getImgFormat() != eImgFmt_JPEG
                    && pImageStreamInfo->getImgFormat() != eImgFmt_HEIF)
            {
                needOutTuning = true;
            }
        }
        //HAL stream
        if (params->HWModule == MTK_SINGLEHW_SETTING_MODULE_NONE) // use isp pipeline
        {
            {
...
#define ADD_METASTREAMBUF(_id_, _metadata_, _request_)                                                                         \
    ...
    for ( size_t req = 0; req < hidl_reqs.size(); ++req ) {
        ...
        for (size_t i = 0; i < request.OutputBuffers.size(); i++){
            ...
            if ( 0 > convertImageStreamBuffer("r"+std::to_string(request.reqNumber), request.OutputBuffers[i].buffer,
                    request.OutputBuffers[i].id, pStreamBuffer, rawInfo) ){
                return -1;
            }
            if (pStreamBuffer->getStreamInfo() != nullptr) {
            ...
            if (request.OutputBuffers[i].tuningData != nullptr){
                ...
                if (pTunningStreamBuffer->getStreamInfo() != nullptr) {
        ...
        for (size_t it = 0; it < parsed_reqs.size(); it++){
            ...
            if ( 0 > convertTuningMetaBuffer("r"+std::to_string(request.reqNumber)+"idx"+std::to_string(it),
                                             request.InputBuffers[it], pImageBufferHeap, importedTuningBufferHandle) ){
                MY_LOGD("without input tuning buffer");
            ...
            ADD_METASTREAMBUF(eSTREAMID_META_PIPE_DYNAMIC_01, unpackParam.halResultMeta, pItem);
            {
                IMetadata::IEntry const& e0 = unpackParam.appControlMeta.entryFor(MTK_HAL_REQUEST_RAW_IMAGE_INFO);
                rawInfo.format = 0;
                if ( ! e0.isEmpty() ) {
                    rawInfo.format = e0.itemAt(0, Type2Type<MINT32>());
                    rawInfo.stride = e0.itemAt(1, Type2Type<MINT32>());
                    rawInfo.width  = e0.itemAt(2, Type2Type<MINT32>());
                    rawInfo.height = e0.itemAt(3, Type2Type<MINT32>());
                    MY_LOGD("app raw : format: 0x%X, stride : %d, wxh : %dx%d", rawInfo.format, rawInfo.stride, rawInfo.width, rawInfo.height);
                }
                IMetadata::IEntry const& e1 = unpackParam.halResultMeta.entryFor(MTK_HAL_REQUEST_RAW_IMAGE_INFO);
                if ( ! e1.isEmpty() ) {
                    rawInfo.format = rawInfo.format == 0 ? e1.itemAt(0, Type2Type<MINT32>()) : rawInfo.format;
                    rawInfo.stride = rawInfo.stride == 0 ? e1.itemAt(1, Type2Type<MINT32>()) : rawInfo.stride;
                    rawInfo.width  = rawInfo.width == 0  ? e1.itemAt(2, Type2Type<MINT32>()) : rawInfo.width;
                    rawInfo.height = rawInfo.height == 0 ? e1.itemAt(3, Type2Type<MINT32>()) : rawInfo.height;
            ...
            //LCSO input
            {
                ...
                android::sp<IImageStreamInfo> streamInfo = nullptr;
                if (unpackParam.pSTTData == nullptr)
                {
                    streamInfo = mStreamConfigMap.valueAt(index);
                }
                else
                {
                    IImageStreamInfo::BufPlanes_t bufPlanes;
                    bufPlanes.count = unpackParam.pSttoImageBuffer->getPlaneCount();
                    bufPlanes.planes[0].sizeInBytes      = unpackParam.pSttoImageBuffer->getBufSizeInBytes(0);
                    bufPlanes.planes[0].rowStrideInBytes = unpackParam.pSttoImageBuffer->getBufStridesInBytes(0);
                    streamInfo = ImageStreamInfoBuilder((mStreamConfigMap.valueAt(index)).get())
                            .setAllocImgFormat(unpackParam.pSttoImageBuffer->getImgFormat())
                            .setAllocBufPlanes(bufPlanes)
                            .setImgFormat(unpackParam.pSttoImageBuffer->getImgFormat())
                            .setBufPlanes(bufPlanes)
                            .setImgSize(unpackParam.pSttoImageBuffer->getImgSize())
                            .setBufStep({0})
                            .setStartOffset(0)
                            .setPrivateData(ImageStreamInfoBuilder::SetPrivateData{
                                    .privData = NSCam::plugin::streaminfo::create_privdata(unpackParam.pSTTData->data()),
                                    .privDataId = NSCam::plugin::streaminfo::get_dataid(unpackParam.pSTTData->data()),
                                })
                            .build();
                }
        ...
        for (size_t i = 0; i < request.InputBuffers.size(); i++){
            //
            android::sp<ISPImageStreamBuffer> pStreamBuffer;
            if ( 0 > convertImageStreamBuffer("r"+std::to_string(request.reqNumber)+"idx"+std::to_string(i),
                                              request.InputBuffers[i].buffer, request.InputBuffers[i].id, pStreamBuffer, rawInfo) ){
                return -1;
            }
            if (mHasThumbnail)
            {
                if (pStreamBuffer->getStreamInfo() != nullptr) {
                    parsed_reqs[0]->vIImageBuffers.insert({pStreamBuffer->getStreamInfo()->getStreamId(), pStreamBuffer});
                }
            }
            else
            {
...
#define ADD_METASTREAMBUF(_id_, _metadata_, _request_)                                                                         \
    ...
    for ( size_t req = 0; req < hidl_reqs.size(); ++req ) {
        ...
        //  pItem->vOImageBuffers <- ISPRequest.OutputBuffers
        for (size_t i = 0; i < request.OutputBuffers.size(); i++){
            android::sp<ISPImageStreamBuffer> pStreamBuffer;
            ...
            if (pStreamBuffer->getStreamInfo() != nullptr) {
                pItem->vOImageBuffers.insert({pStreamBuffer->getStreamInfo()->getStreamId(), pStreamBuffer});
            }
        }

        //  pItem->vIImageBuffers <- ISPRequest.InputBuffer
        for (size_t i = 0; i < request.InputBuffers.size(); i++){
            //
            android::sp<ISPImageStreamBuffer> pStreamBuffer;
            ...
            if (pStreamBuffer->getStreamInfo() != nullptr) {
                pItem->vIImageBuffers.insert({pStreamBuffer->getStreamInfo()->getStreamId(), pStreamBuffer});
            }
        }

        //  pItem->vIMetaBuffers <- ISPRequest.settings
        IMetadata ISPmeta = IMetadata();
        {
            //convert the configParams to IMetadata
            if ( ! mMetaConverter->convertToIMetadata(request.settings.data(), ISPmeta) )
            ...
            ADD_METASTREAMBUF(eSTREAMID_META_APP_CONTROL, ISPmeta, pItem);
        }
    }
    //
    //dump the inoutput UserRequestParams
...
) -> android::sp<IImageStreamInfo>
{
    ...
    if ( srcStreamType == eSTREAMTYPE_IMAGE_IN ){
        ...
        switch( srcStream.format.format ){
            case PixelFormat::YCBCR_420_888: //default set as NV21 if without HW usage
                streamId = eSTREAMID_IMAGE_ISP_DEVICE_INPUT_YUV;
                if (srcStream.usage & (GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_HW_COMPOSER))
                {
                    streamName += String8("NV12_input");
                    imgFormat = HAL_PIXEL_FORMAT_NV12;
                    MY_LOGD("set input yuv888 -> NV12");
                }
                else
                {
            ...
            case PixelFormat::YCRCB_420_SP:
                streamName += String8("YCRCB_420_SP_input");
                if (srcStream.width <= THUMB_NAIL_WIDTH)
                {
                    streamId = eSTREAMID_IMAGE_ISP_DEVICE_INPUT_THUMBNAIL;
                    mHasThumbnail = true;
                    MY_LOGI("use thumbnail flow");
                }
                else
                {
                    streamId = eSTREAMID_IMAGE_ISP_DEVICE_INPUT_YUV;
            ...
            case PixelFormat::RAW16:
                ...
                imgFormat = (rawbpp == 14) ? eImgFmt_BAYER14_UNPAK : eImgFmt_BAYER12_UNPAK;
                break;
            case PixelFormat::IMPLEMENTATION_DEFINED:
                streamName += String8("RAW_input");
                streamId = eSTREAMID_IMAGE_ISP_DEVICE_INPUT_RAW;
                imgFormat = eImgFmt_BAYER10;
                break;
            case PixelFormat::RAW10:
                streamName += String8("MIPIRAW10_input");
                streamId = eSTREAMID_IMAGE_ISP_DEVICE_INPUT_RAW;
                imgFormat = eImgFmt_BAYER10_MIPI;
                break;
            case PixelFormat::BLOB:
                streamName += String8("BLOB_input");
        ...
        switch( srcStream.format.format ){
            case PixelFormat::YCBCR_420_888:
                ...
                if (srcStream.usage & (GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_HW_COMPOSER))
                {
                    streamName += String8("NV12_output");
                    imgFormat = HAL_PIXEL_FORMAT_NV12;
                    MY_LOGD("set output yuv888 -> NV12");
                }
                else
                {
            ...
            case PixelFormat::BLOB:
                ...
                imgFormat = (srcStream.format.dataspace == static_cast<int32_t>(Dataspace::V0_JFIF)) ?
                            eImgFmt_JPEG : srcStream.format.dataspace == static_cast<int32_t>(Dataspace::HEIF) ?
                            eImgFmt_HEIF : static_cast<MINT>(srcStream.format.format);
                break;
    ...
    if (srcStream.format.format == PixelFormat::RAW16 || srcStream.format.format == PixelFormat::RAW10) {
        ...
        MY_LOGD("Raw16: (sizeInBytes, rowStrideInBytes)=(%zu,%zu)", bufPlanes.planes[0].sizeInBytes, allocBufPlanes.planes[0].rowStrideInBytes);
    }
...
) -> android::sp<IImageStreamInfo>
{
    ...
    switch ( srcStreamId ){
        ...
        case eSTREAMID_IMAGE_ISP_DEVICE_INPUT_RESIZED:
        {
            ...
            allocImgFormat = imgFormat = mUseYUVO ? eImgFmt_MTK_YUV_P010 : eImgFmt_FG_BAYER10;
...
) -> int
{
    ...
    switch ( stream.format.format ){
        case PixelFormat::BLOB:
        {
            ...
            {
                planes.count = 1;
                planes.planes[0].sizeInBytes      = stream.stride[0];
                planes.planes[0].rowStrideInBytes = stream.stride[0];
                //checkPlaneSize(info, 0, (extra.rowNum));
            }
            planes.planes[1].rowStrideInBytes = 0;
            planes.planes[2].rowStrideInBytes = 0;
        }break;
...
)   const -> ISPImageStreamBuffer*
{
    ...
    if (heapFormat == static_cast<int>(eImgFmt_BLOB) && (rawInfo.stride != 0)) {
        if(pStreamInfo->getImgFormat() == eImgFmt_BAYER14_UNPAK) {
            // It is for customer.
            // Left path output raw format maybe not raw14,
            // so use config stream to determine it.
            reqFormat = eImgFmt_BAYER14_UNPAK;
        } else {
    ...
    if (heapFormat == static_cast<int>(eImgFmt_BLOB)
            && (pStreamInfo->getImgFormat() == static_cast<int>(eImgFmt_JPEG)
                || pStreamInfo->getImgFormat() == static_cast<int>(eImgFmt_HEIF)
                || pStreamInfo->getImgFormat() == static_cast<int>(eImgFmt_ISP_TUING))) {
    ...
    android::sp<IGraphicImageBufferHeap> pImageBufferHeap =
        IGraphicImageBufferHeap::create(
            ...
            pStreamInfo->getAllocImgFormat() == eImgFmt_BLOB ? MSize(pStreamInfo->getAllocBufPlanes().planes[0].sizeInBytes, 1): pStreamInfo->getImgSize(),
            heapFormat,
            &bufferHandle,
            acquire_fence,
            release_fence,
    ...
    else if (mDump > 0 && streamInfo->getStreamType() == eSTREAMTYPE_IMAGE_IN) {
            pImageBufferHeap->lockBuf(LOG_TAG, (GRALLOC_USAGE_SW_READ_OFTEN|GRALLOC_USAGE_SW_WRITE_OFTEN
                              |GRALLOC_USAGE_HW_CAMERA_WRITE));
            sp<IImageBuffer> Buffer = pImageBufferHeap->createImageBuffer();
            if (Buffer != nullptr) {
                android::String8 FileName;
                FileName += ISPHIDL_DUMP_PATH;
                FileName += android::String8::format("%#" PRIxPTR "_", reinterpret_cast<intptr_t>(this));
                FileName += android::String8::format("%s_", bufferName.c_str());
                FileName += "image.data";
                Buffer->lockBuf("ispstreamcontrol");
                Buffer->saveToFile(FileName.c_str());
                Buffer->unlockBuf("ispstreamcontrol");
                Buffer = nullptr;
            }
            pImageBufferHeap->unlockBuf(LOG_TAG);
    }
...
)   const -> int
{
    ...
    {
        auto helper = IGrallocHelper::singleton();
        if  ( ! helper || importedTuningBufferHandle == nullptr || ! helper->importBuffer(importedTuningBufferHandle) )
        {
    ...
    pImgBufHeap = IGraphicImageBufferHeap::create(
            ...
            static_cast<int>(eImgFmt_BLOB) /*format*/,
    ...
    if ( pImgBufHeap == nullptr ){
        ...
        MY_LOGF("IGraphicImageBufferHeap::create \"%s\", handle: %p, fd: %d",
            ...
            pImgBufHeap->lockBuf(LOG_TAG, (GRALLOC_USAGE_SW_READ_OFTEN|GRALLOC_USAGE_SW_WRITE_OFTEN
                           |GRALLOC_USAGE_HW_CAMERA_WRITE));
            sp<IImageBuffer> Buffer = pImgBufHeap->createImageBuffer();
            if (Buffer != nullptr) {
                android::String8 FileName;
                FileName += ISPHIDL_DUMP_PATH;
                FileName += android::String8::format("%#" PRIxPTR "_", reinterpret_cast<intptr_t>(this));
                FileName += android::String8::format("%s_", bufferName.c_str());
                FileName += "tuning.data";
                Buffer->lockBuf("ispstreamcontrol");
                Buffer->saveToFile(FileName.c_str());
                Buffer->unlockBuf("ispstreamcontrol");
                Buffer = nullptr;
            }
            pImgBufHeap->unlockBuf(LOG_TAG);
    }
