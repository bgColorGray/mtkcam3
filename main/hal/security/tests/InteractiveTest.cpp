...
int InteractiveTest::run(int* argc, char* argv[]) {
    int waitSecs = 0;
    if (*argc >= 3) {
        waitSecs = atoi(argv[2]);
    }
    std::chrono::seconds durationInSecond(waitSecs);
    ...
    const std::chrono::seconds durationInSecond)
{
    auto navigator = [](std::string message,
        const std::chrono::seconds durationInSecond) {
        if (durationInSecond == 0s) {
        std::cout << "Press ENTER to " << message << "..." << std::endl;
        (void)getchar();
        } else {
        std::cout << "Wait (" << durationInSecond.count() <<
            ") seconds to " << message << "..." << std::endl;
        std::this_thread::sleep_for(durationInSecond);
        }
    };

    if (durationInSecond == 0s) {
        navigator("start capture",0s);
    } else {
        navigator("start capture",2s);
    }
    ...
    if (durationInSecond == 0s) {
        navigator("stop capture",0s);
    } else {
        navigator("stop capture",durationInSecond);
    }
...
{
    if (TestDefs::kEnableSecureImageDataTest) {
        if (flags != AHARDWAREBUFFER_USAGE_PROTECTED_CONTENT) {
