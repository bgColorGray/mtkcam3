...
static bool setFaceDetectionEnable(ACameraManager* CameraManager, const char* CamId, ACaptureRequest* CaptureRequest)
{
    ...
    ACameraMetadata_getConstEntry(staticMetadata, ACAMERA_STATISTICS_INFO_MAX_FACE_COUNT, &entry);
    if (entry.count != 0)
    {
        face_count = entry.data.i32[0];
    }
    if (face_count != 0)
    {
        // set FD enable
        uint8_t enable = ACAMERA_STATISTICS_FACE_DETECT_MODE_SIMPLE;
        ACaptureRequest_setEntry_u8(CaptureRequest, ACAMERA_STATISTICS_FACE_DETECT_MODE, 1, &enable);
    }
    return true;
}
        ...
        bool usePhysicalSettings, ACameraDevice_request_template requestTemplate)
{
    ...
    setFaceDetectionEnable(mCameraManager, mCameraId, mCaptureRequest);
