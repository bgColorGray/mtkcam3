        ...
        size_t size)
{
    ...
    if (!BufferAllocator::CheckIonSupport()) {
        // get secure handle
        CAM_ULOGMD("GRALLOC_EXTRA_GET_ION_FD");
        int32_t heapFD = -1;
        auto err = ::gralloc_extra_query(bufferHandle, GRALLOC_EXTRA_GET_ION_FD,
                                        &heapFD);
        if (android::NO_ERROR != err) {
        CAM_ULOGME("getIonFd(): status[%d]", -err);
        return nullptr;
        }
        CAM_ULOGMD("heapFD:%d",heapFD);
        //
        UREE_SECUREMEM_HANDLE secHandle = -1;
        typedef TZ_RESULT GetChmHandle_t(int, UREE_SECUREMEM_HANDLE*);
        auto getChmHandle = reinterpret_cast<GetChmHandle_t*>
                            (loadSymbol("UREE_GetChmHandle"));

        auto res = (*getChmHandle)(heapFD, &secHandle);
        secureHandle = reinterpret_cast<uint32_t>(secHandle);
        CAM_ULOGMD("UREE_GetChmHandle res(%d)",res);
    } else {
        SECHAND secHandle;
        if (!AImageReaderHelper::getSecureHandle(bufferHandle, secHandle)) {
    ...
    if (createSession == nullptr || registerSharedMem == nullptr || secureHandleToWSM == nullptr || unregisterSharedMem == nullptr || closeSession == nullptr)
    {
        CAM_ULOGME("%s", dlerror());
    ...
    if (__builtin_expect(pageSize < 1, false))
    {
        handle = closeHandle(handle);
        std::unique_ptr<char[], std::function<void(char[])>> wsm;
        return wsm;
    }
    auto memptr = (char *)memalign(pageSize, size);
    ...
    mAcquiredImageCount(0)
{
    ...
    {
        mbDumpBuffer = NSCam::Utils::Properties::property_get_bool(
            "vendor.debug.securecamera_test.enableDump", false);
        //
        std::lock_guard<std::mutex> _l(sAImageReaderHelperObserverLock);
...
bool AImageReaderHelper::getSecureHandle(
        const native_handle_t* bufferHandle, SECHAND& secureHandle)
{
    // Reference:
    // vendor/mediatek/proprietary/hardware/gralloc_extra/include/ui/gralloc_extra.h
    GRALLOC_EXTRA_RESULT ret = gralloc_extra_query(
            bufferHandle, GRALLOC_EXTRA_GET_SECURE_HANDLE, &secureHandle);

    if (ret != GRALLOC_EXTRA_OK)
    {
        CAM_ULOGME("gralloc_extra_query failed: %d", ret);
        return false;
    }
...
bool AImageReaderHelper::getPixelFormat(
        const native_handle_t* bufferHandle, PixelFormat& pixelFormat)
{
    // Reference:
    // vendor/mediatek/proprietary/hardware/gralloc_extra/include/ui/gralloc_extra.h
    GRALLOC_EXTRA_RESULT ret = gralloc_extra_query(
            bufferHandle, GRALLOC_EXTRA_GET_FORMAT, &pixelFormat);
        ...
        const native_handle_t* bufferHandle, size_t imageCount)
{
    ...
    auto memPtr = toWorldSharedMemory(bufferHandle, bufferSize);
