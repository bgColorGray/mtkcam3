...
#ifndef VENDOR_MEDIATEK_HARDWARE_CAMERA3_MAIN_MTKHAL_SECURITY_GRAPHICS_TYPES_H
#define VENDOR_MEDIATEK_HARDWARE_CAMERA3_MAIN_MTKHAL_SECURITY_GRAPHICS_TYPES_H
...
enum class MTKCam_BufferUsage : uint64_t {
   ...
   *       PROT_CAMERA_LEGACY is used for trusted drivers' working buffer in
   * TrustZone.
   *
...
static_assert(
    std::is_same<std::underlying_type_t<BufferUsage>, std::uint64_t>::value,
    "wrong value type");
// make sure that the value type of MTK camera buffer usage is the same as that
// of AOSP buffer usage.
static_assert(std::is_same<std::underlying_type_t<MTKCam_BufferUsage>,
                           std::underlying_type_t<BufferUsage>>::value,
              "mispatched value type");

}  // namespace NSCam

#endif  // VENDOR_MEDIATEK_HARDWARE_CAMERA3_MAIN_MTKHAL_SECURITY_GRAPHICS_TYPES_H
