  ...
  auto ret = mMapper->createDescriptor(
      descriptorInfo, [&descriptor](const auto& err, const auto& desc) {
        if (err != ::android::hardware::graphics::mapper::V4_0::Error::NONE)
          CAM_ULOGME("IMapper::createDescriptor() failed(%s)",
                     toString(err).c_str());
        descriptor = desc;
      });
...
void MapperHelper::importBuffer(buffer_handle_t& outBufferHandle) {
  ...
  auto ret = mMapper->importBuffer(
      hidl_handle(outBufferHandle),
      [&importedHandle](const auto& err, const auto& buffer) {
        if (err != ::android::hardware::graphics::mapper::V4_0::Error::NONE)
          CAM_ULOGME("IMapper::importBuffer() failed(%s)",
                     toString(err).c_str());
...
void MapperHelper::lock(buffer_handle_t bufferHandle,
                        uint64_t cpuUsage,
  ...
  auto ret = mMapper->lock(
      _bufferHandle, cpuUsage, accessRegion, acquireFenceHandle,
...
void MapperHelper::unlock(buffer_handle_t bufferHandle,
                          int& releaseFence) const {
  auto _bufferHandle = const_cast<native_handle_t*>(bufferHandle);
  releaseFence = kNoFenceFd;
  auto ret = mMapper->unlock(
      _bufferHandle, [&](auto err, const auto& tmpReleaseFence) {
        if (err != ::android::hardware::graphics::mapper::V4_0::Error::NONE)
