...
#include <main/mtkhal/custom/hidl/device/3.x/graphics/AllocatorHelper.h>
...
void AllocatorHelper::allocateGraphicBuffer(uint32_t width,
                                            uint32_t height,
                                            uint64_t usage,
                                            PixelFormat format,
                                            hidl_handle* handle,
  ...
  auto ret = mAllocator->allocate(
      descriptor, 1u, [&](auto err, uint32_t stride, const auto& buffers) {
        CAM_ULOGMD("stride(%u)", stride);
        if (rowStride)
          *rowStride = stride;
        if (err != ::android::hardware::graphics::mapper::V4_0::Error::NONE)
          CAM_ULOGME("IAllocator::allocate() failed(%s)",
                     toString(err).c_str());
