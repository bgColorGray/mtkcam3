...
#ifndef MAIN_MTKHAL_CUSTOM_DEVICE_3_X_FEATURE_IFEATURESESSION_H_
#define MAIN_MTKHAL_CUSTOM_DEVICE_3_X_FEATURE_IFEATURESESSION_H_
...
class IFeatureSession {
 ...
 public:  //// Operations.
  ...
  virtual int configureStreams(
      const mcam::StreamConfiguration& requestedConfiguration,
      mcam::HalStreamConfiguration& halConfiguration) = 0;
 ...
 public:  //// Callbacks.
  virtual void notify(const std::vector<mcam::NotifyMsg>& msgs) = 0;

  virtual void processCaptureResult(
      const std::vector<mcam::CaptureResult>& results) = 0;
...
struct CreationParams {
   ...
   */
  const NSCam::IMetadata cameraCharateristics;
   ...
   */
  const std::shared_ptr<mcam::custom::CusCameraDeviceCallback>& callback;
};
...
#endif  // MAIN_MTKHAL_CUSTOM_DEVICE_3_X_FEATURE_IFEATURESESSION_H_
