 ...
 *
 ******************************************************************************/
ACameraDeviceCallback::~ACameraDeviceCallback() {
  MY_LOGI("dtor");
}
 ...
 *
 ******************************************************************************/
ACameraDeviceCallback::ACameraDeviceCallback(const CreationInfo& info)
    : mInstanceId(info.instanceId),
      mLogPrefix(std::to_string(mInstanceId) + "-a-callback"),
      mCallback(info.pCallback) {
  MY_LOGI("ctor");

  if (CC_UNLIKELY(mCallback.lock() == nullptr)) {
 ...
 *
 ******************************************************************************/
auto ACameraDeviceCallback::create(const CreationInfo& info)
    -> ACameraDeviceCallback* {
  auto pInstance = new ACameraDeviceCallback(info);
 ...
 *
 ******************************************************************************/
auto ACameraDeviceCallback::processCaptureResult(
    const std::vector<CaptureResult>& results) -> void {
  CAM_TRACE_CALL();
  if (auto cb = mCallback.lock()) {
    cb->processCaptureResult(results);
  } else {
    MY_LOGW("mCallback does not exist, use_count=%ld",
            mCallback.use_count());
  }
  return;
}
 ...
 *
 ******************************************************************************/
auto ACameraDeviceCallback::notify(const std::vector<NotifyMsg>& msgs)
    -> void {
  CAM_TRACE_CALL();
  if (auto cb = mCallback.lock()) {
    cb->notify(msgs);
...
auto ACameraDeviceCallback::requestStreamBuffers(
 ...
 *
 ******************************************************************************/
auto ACameraDeviceCallback::returnStreamBuffers(
    const std::vector<StreamBuffer>& buffers) -> void {
  CAM_TRACE_CALL();
  if (auto cb = mCallback.lock()) {
