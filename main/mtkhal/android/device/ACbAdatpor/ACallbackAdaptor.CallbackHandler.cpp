...
#define ThisNamespace ACallbackAdaptor::CallbackHandler
...
static void convertToDebugString(
    ACallbackAdaptor::ACallbackParcel const& cbParcel,
    Vector<String8>& out) {
  /*
  ...
  for (auto it = cbParcel.vFrameNumber.begin();
    ...
    if (!CaptureResultIdMap[frameNumber].empty()) {
      ...
      for (auto const& id : CaptureResultIdMap[frameNumber]) {
        ...
        metaSize += get_camera_metadata_entry_count(
                        cbParcel.vCaptureResult[id].result) > 0
                        ? 1
                        : 0;
      }
      if (metaSize)
    ...
    if (!BufferResultIdMap[frameNumber].empty()) {
      ...
      for (auto const& id : BufferResultIdMap[frameNumber]) {
        inImgSize +=
            cbParcel.vBufferResult[id].inputBuffer.streamId == -1 ? 0 : 1;
 ...
 *
 ******************************************************************************/
auto ThisNamespace::waitUntilDrained(int64_t const timeout) -> int {
 ...
 *
 ******************************************************************************/
auto ThisNamespace::push(const ACallbackParcel& item) -> void {
...
auto ThisNamespace::performCallback() -> void {
...

  {
    ...
    freeCameraMetadata(hidlCbParcel.vCaptureResult);
