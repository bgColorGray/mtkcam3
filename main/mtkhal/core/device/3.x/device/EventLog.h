...
#ifndef MAIN_MTKHAL_CORE_DEVICE_3_X_DEVICE_EVENTLOG_H_
#define MAIN_MTKHAL_CORE_DEVICE_3_X_DEVICE_EVENTLOG_H_
//
...
class EventLog {
 ...
 public:  ////    Operations.
  ...
  auto print(IPrinter& printer) const {
    std::lock_guard<std::mutex> _l(mLogLock);
    ...
    for (const auto& item : mLogBuffer1) {
      ...
      if (timeStr.length() > 0 && eventStr.length() > 0) {
        printer.printFormatLine("  %s : %s", timeStr.c_str(), eventStr.c_str());
      }
    }
 ...
 *
 ******************************************************************************/
class EventLogPrinter : public IPrinter {
 protected:  ////    Data Members.
  static const size_t kDefaultOldestBufferCapacity = 0;
  static const size_t kDefaultLatestBufferCapacity = 25;
  EventLog mEventLog;
 ...
 public:  ////    Operations.
  ...
  virtual ~EventLogPrinter() {}

  auto print(IPrinter& printer) const { mEventLog.print(printer); }

 public:  ////    Printer Interface.
  virtual void printLine(const char* string) { mEventLog.add(string); }
...
#endif  // MAIN_MTKHAL_CORE_DEVICE_3_X_DEVICE_EVENTLOG_H_
