...
#ifndef MAIN_MTKHAL_CORE_DEVICE_3_X_DEVICE_DISPLAYIDLEDELAYUTIL_H_
#define MAIN_MTKHAL_CORE_DEVICE_3_X_DEVICE_DISPLAYIDLEDELAYUTIL_H_
...
static void loadPerfAPI(perfLockAcqFunc& lockAcq, perfLockRelFunc& lockRel) {
  ...
  if (libHandle == NULL) {
    ...
    CAM_ULOGD(NSCam::Utils::ULog::MOD_CAMERA_DEVICE,
              "[DisplayIdleDelayUtil] load lib");
  }
  ...
  if (funcA == NULL) {
    ...
    if (funcA == NULL) {
      CAM_ULOGE(NSCam::Utils::ULog::MOD_CAMERA_DEVICE,
                "perfLockAcq error: %s\n", dlerror());
      dlclose(libHandle);
      libHandle = NULL;
      return;
    ...
    CAM_ULOGD(NSCam::Utils::ULog::MOD_CAMERA_DEVICE,
              "[DisplayIdleDelayUtil] load funcA");
  }
  ...
  if (funcR == NULL) {
    ...
    CAM_ULOGD(NSCam::Utils::ULog::MOD_CAMERA_DEVICE,
              "[DisplayIdleDelayUtil] load funcR");
  }
...
class DisplayIdleDelayUtil {
 public:
  DisplayIdleDelayUtil() { loadPerfAPI(_perfLockAcq, _perfLockRel); }

  virtual ~DisplayIdleDelayUtil() { disable(); }
  ...
  virtual bool enable() {
    ...
    if (_perfLockAcq) {
      ...
      int displayIdleDelay = NSCam::Utils::Properties::property_get_int32(
          "vendor.cam3dev.displayidledelay", DISPLAY_IDLE_DELAY_DEFAULT);
      ...
      if (vcmd.size() > 0) {
        _powerHalHandle =
            _perfLockAcq(_powerHalHandle, 0 /*timout, unit is ms*/, vcmd.data(),
                         vcmd.size());
      }

      if (_powerHalHandle != 0) {
        CAM_ULOGD(
            NSCam::Utils::ULog::MOD_CAMERA_DEVICE,
            "[DisplayIdleDelayUtil] Set display idle delay to %d, handle %d",
            displayIdleDelay, _powerHalHandle);
        _isDisplayIdleEnabled = true;
      } else {
        CAM_ULOGE(
            NSCam::Utils::ULog::MOD_CAMERA_DEVICE,
            "[DisplayIdleDelayUtil] Failed to lock perf, invalid handle: %d",
            _powerHalHandle);
      }
    }
  ...
  virtual void disable() {
    ...
    if (_isDisplayIdleEnabled && _perfLockRel && _powerHalHandle != 0) {
      _perfLockRel(_powerHalHandle);
      _isDisplayIdleEnabled = false;
      _powerHalHandle = 0;
      CAM_ULOGD(NSCam::Utils::ULog::MOD_CAMERA_DEVICE,
                "[DisplayIdleDelayUtil] Display idle disabled");
    }
  }
...
#endif  // MAIN_MTKHAL_CORE_DEVICE_3_X_DEVICE_DISPLAYIDLEDELAYUTIL_H_
