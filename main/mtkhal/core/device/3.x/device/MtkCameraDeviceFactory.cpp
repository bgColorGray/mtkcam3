...
createVirtualCameraDevice(CreateVirtualCameraDeviceParams* params) {
  ...
  auto pDevice = new mcam::core::MtkCameraDevice(
      params->pDeviceManager, params->pMetadataProvider,
      params->physicalMetadataProviders, params->instanceId,
      params->virtualInstanceId);

  if (!pDevice) {
    MY_LOGE("Fail to new MtkCameraDevice");
    return nullptr;
  }

  mcam::core::IMtkcamVirtualDeviceSession::CreationInfo const info = {
  ...
  bool bInitSuccess = pDevice->initialize(createMtkCameraDeviceSession(info));
  if (!bInitSuccess) {
    pDevice = nullptr;
  }
