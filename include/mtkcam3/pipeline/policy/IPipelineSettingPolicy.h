...
struct ConfigurationOutputParams
{
    ...
    /**
     * @param pSeamlessSwitchFeatureSetting: The seamless switch feature settings.
     *
     * Callers must ensure it's not a nullptr before this call.
     * Callee will fill its content on this call.
     */
    SeamlessSwitchFeatureSetting*               pSeamlessSwitchFeatureSetting = nullptr;
     ...
     */
    SensorMap<SensorSetting>*                   pSensorSetting = nullptr;
     ...
     * Callee will fill its content on this call.
     */
    SensorMap<P1HwSetting>*                     pP1HwSetting = nullptr;
     ...
     * For example,
     *      (pP1DmaNeed->at(1) & P1_IMGO) != 0 indicates that IMGO is needed for sensorId 1.
     *      (pP1DmaNeed->at(3) & P1_RRZO) != 0 indicates that RRZO is needed for sensorId 3.
     */
    SensorMap<uint32_t>*                        pP1DmaNeed = nullptr;
     ...
     * Callee will fill its content on this call.
     */
    SensorMap<ParsedStreamInfo_P1>*             pParsedStreamInfo_P1 = nullptr;
    ...
    /**
     * A list of stream sets; all the streams within a set try to share the same
     * image buffer pool.
     *
     * Callers must ensure it's not a nullptr before this call.
     * Callee will fill its content on this call.
     */
    std::vector<std::set<StreamId_T>>*          pPoolSharedStreams = nullptr;

    bool*                                       pIsZSLMode = nullptr;

    MINT32*                                     pUniKey = nullptr;
...
struct RequestResultParams
{
     ...
     *  0/0, 1/0: single frame in the request
     *  M/(N-1): frame (N) of M frames in the request
     */
    size_t                                      batchSize       = 0;
    size_t                                      batchIndex      = 0;
     ...
     * The value shows which dma are needed.
     * For example,
     *      (needP1Dma.at(1) & P1_IMGO) != 0 indicates that IMGO is needed for sensorId 1.
     *      (needP1Dma.at(3) & P1_RRZO) != 0 indicates that RRZO is needed for sensorId 3.
     */
    ...
    /**
     * Streams to kept or use for Zsl.
     *
     * For Streaming (keepZslBuffer),
     *  these are the streams that need to be kept track of.
     *
     * For Zsl capture, these are the required streams.
     *  ref. NSCam::ZslPolicy::RequiredStreams
     */
    std::shared_ptr<NSPipelineContext::TrackFrameResultParams>
...
struct RequestOutputParams
{
    ...
    /**
     * true indicates P1 can only output imgo, disable rrzo/lcso/rsso.
     */
    bool                                        isP1OnlyOutImgo = false;

    /**
     *  The stream list that to mark error
     */
    std::vector<StreamId_T>                     markErrorStreamList;

    /**
     *  kick preview request for speed up capture
     */
    bool                                        bkickRequest  = false;
    ...
    std::shared_ptr<TrackFrameResultParams>     zslTrackParam;

    /**
     * mark depth stream as error (or not), set by P2Node_Decision
     */
    bool                                        needDepthStreamMarkError = false;
...
struct RequestInputParams
{
    ...
    /**
     * Configured pipeline topology, built up at the configuration stage.
     */
    PipelineTopology const*                     pConfiguration_PipelineTopology = nullptr;
    ...
    /**
     * Configuration seamless switch info.
     * This info may not be available in every physical featureSetting,
     * so need to be pass as input.
     */
     ...
     *
     * pSensorMode and pSensorSize are the pointers to two maps of sensor modes
     * and sensor sizes, respectively.
     * The maps size is the same to the size of sensor id (i.e. PipelineStaticInfo::sensorId).
     */
    SensorMap<uint32_t> const*                  pSensorMode = nullptr;
    SensorMap<MSize> const*                     pSensorSize = nullptr;
    // for 60 fps +
    SensorMap<uint32_t> const*                  pSensorFps  = nullptr;
    // for 60 fps -
    //
    bool                                        isZSLMode   = false;
    bool                                        isSW4Cell   = false;
    bool                                        isP1HwReady = false;
    //
    SensorMap<P1HwSetting> const*               pP1HwSetting = nullptr;
