...
struct EvaluateRequestParams
{
     ...
     *
     * pSensorSize is the pointer to an unordered_map of sensor sizes.
     * The unordered_map size is the same to the size of sensor id
     * (i.e. PipelineStaticInfo::sensorId).
     */
     ...
     * pAdditionalHal: hal control metadata; the unordered_map size is the same to the
    ...
    SensorMap<std::shared_ptr<IMetadata>>*          pAdditionalHal = nullptr;
    ...
    SensorMap<AdditionalRawInfo> const*             additionalRawInfo = nullptr;

    // Decide which fd node shoud do face detection
    MINT32                                          targetFd = 0;

    bool                                            isFdEnabled = false;
    // multicam metadata
    SensorMap<bool> const*                          pvNeedP1Node = nullptr;
