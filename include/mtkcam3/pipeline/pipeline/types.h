...
struct TrackFrameResultParams
{
     ...
     */
    std::set<StreamId_T>            halImageStreams;

    /**
     * The meta stream Ids which are needed to be kept track of.
     */
    std::set<StreamId_T>            halMetaStreams;
    std::set<StreamId_T>            appMetaStreams;
...
static inline std::string toString(TrackFrameResultParams const& o)
{
    ...
    oss << std::hex;
    oss << " .halImage=[";
    for (auto const& s : o.halImageStreams) { oss << " " << s; }
    oss << " ]";
    oss << " .halMeta=[";
    for (auto const& s : o.halMetaStreams) { oss << " " << s; }
    oss << " ]";
    oss << " .appMeta=[";
    for (auto const& s : o.appMetaStreams) { oss << " " << s; }
    oss << " ]";
    oss << std::dec;
