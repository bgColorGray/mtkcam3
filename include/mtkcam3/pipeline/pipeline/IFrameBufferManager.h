...
public:  ////       Select
    ...
    virtual auto    getInFlightInfos() const -> std::vector<NSCam::ZslPolicy::InFlightInfo>       = 0;

    virtual auto    cloneHBS(HistoryBufferSet const& src) -> HistoryBufferSet = 0;
};

__unused static std::string toString(IHistoryBufferProvider::HistoryBufferSet const& o)
{
    auto toString_vSb = [](auto const& vSb)
    {
        std::ostringstream oss;
        oss << std::showbase << std::hex << "{ ";
        for (auto const& sb : vSb) {
            if (sb) {
                oss << sb->getStreamInfo()->getStreamId() << " ";
            }
        }
        oss << "}";
        return oss.str();
    ...
    oss << "{"
        << " F" << o.frameNo
        << " R" << o.requestNo
        << " Ts" << o.sensorTimestamp
        << " halImage" << toString_vSb(o.vpHalImageStreamBuffers)
        << " halMeta" << toString_vSb(o.vpHalMetaStreamBuffers)
        << " appMeta" << toString_vSb(o.vpAppMetaStreamBuffers)
        << " }";
    return oss.str();
}
...
public:     ////    Configuration
    ...
    virtual auto    addMetaStream(AddMetaStream const& arg) -> int              = 0;
                    ...
                    struct ConfigureStreams
                    {
                        /**
                         * A list of stream sets; the streams in a set share the same buffer pool.
                         */
                        std::shared_ptr<std::vector<std::set<StreamId_T>>>
                                                            configPoolSharedStreams;

                        /**
                         * A list of image streams.
                         */
                        std::vector<AddImageStream>         configImageStreams;

                        /**
                         * A list of meta streams.
                         */
                        std::vector<AddMetaStream>          configMetaStreams;
                    };
    virtual auto    configureStreams(ConfigureStreams const& arg) -> int        = 0;
