...
#define P2_CAM_TRACE_NAME_LENGTH 80
...
#define P2_CAM_TRACE_NAME_INFO(lv,name,info) NSCam::CamSysTrace PASTE(___camtracer, __LINE__) (P2_NEED_TRACE(lv), name, info)
#define P2_CAM_TRACE_INFO(lv,info) P2_CAM_TRACE_NAME_INFO(lv, __FUNCTION__, info)
...
do{                                                         \
    ...
    {                                                       \
        char buf[64];                                       \
        int res = snprintf(buf, sizeof(buf), fmt, ##arg);   \
        if( res < 0 )  buf[0] = '\0';                       \
        CAM_TRACE_BEGIN(buf);                               \
    }                                                       \
}while(0)
...
public:
    ...
    inline CamSysTrace(bool need, const char* name, const char* info) : mNeed(need){
        if(CC_LIKELY(mNeed))
        {
            char str[P2_CAM_TRACE_NAME_LENGTH];
            int res = snprintf(str, P2_CAM_TRACE_NAME_LENGTH, "%s|%s", name, info);
            if( res < 0 )  str[0] = '\0';
            CAM_TRACE_BEGIN(str);
        }
    }
