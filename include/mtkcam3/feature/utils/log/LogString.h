...
private:
    ...
    PRINTF_ARGS_FN TO_PRINTF_ARGS_FN(const char *fmt)
    {
...
public:
  ...
  template <typename... Args>
  int append(const char *fmt, Args... args)
  {
    return _append(TO_PRINTF_ARGS_FN(fmt, args...));
  }
  void clear();
...
private:
  ...
  int _append(const PRINTF_ARGS_FN &argsFn);
...
template <unsigned N>
int FixLenLogString<N>::_append(const PRINTF_ARGS_FN &argsFn)
{
  int len = 0;
  if( mPtr && !mFull )
  {
...
{
  allocate(src.mSize);
  memcpy(mPtr, src.mPtr, std::max<size_t>(src.mLen, 1));
  mLen = src.mLen;
  mFull = src.mFull;
}
