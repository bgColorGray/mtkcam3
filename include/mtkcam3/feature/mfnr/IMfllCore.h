...
public:
    ...
    /**
     *  To set the ISP tuning hint info to MFNR Core Lib
     *  @param info         - int ISP tuning hint
     *  @return             - If OK returns MfllErr_Ok.
     */
    ...
    /**
     *  Retrieve isp stage to devide eFDContainer_Opt.
     *  @return             - ISP stage
     */
    virtual int  getIspTuningHint(void)
    {
        /*default not supported*/
        return 0;
    }
     ...
     *  @param sourceImg    - Full YUV image of queue frame.
     *  @param quarterImg   - Quarter YUV image of queue frame.
     *  @param mssImg       - Mss YUV image of queue frame.
     *  @param dceso        - dceso buffer of queue frame.
     *  @param mv           - Motion vector of queue frame.
     *
     *  @return             - MfllErr_Ok or not.
     */
    virtual enum MfllErr queueYuvCapture(
        ...
        MfllMotionVector        mv)
    {
        /* default not support mss input*/
        return queueYuvCapture(requestNo, sourceImg, quarterImg, mv);
    };
     ...
     *  @param index        - If the selected buffer is not only just a buffer,
     *                        index will be the index of the serial buffers.
     *  @stage              - If the selected buffer is a pyramid,
     *                        stage will be the index of the serial buffers.
     *  @return             - If this function is fail returns sp<T> which contains NULL pointer.
     *  @sa                 - enum MfllBuffer
     */
