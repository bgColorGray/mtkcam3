...
public:
    ...
    /**
     *  Encode YUV image to Pylamid YUVs buffer.
     *
     *  @param pylamid      Inplace buffer, must be following pylamid size rules
     *  @param omc          Input buffer, full size for omc usage
     *  @param mcmv         motion compensation motion vector as input image.
     *  @param dceso        DCESO statistics from the bfbld stage -base frame.
     *  @param index        After BSS which frame number is this stage using
     *  @param s            YuvStage should be YuvStage_Mss
     */
    ...
    /**
     *  Do image blending with pyramid yuvs (since MFNR v4.0)
     *  Before invokes this method, you could prepare weighting memory for IMfllMfb, or
     *  IMfllMfb will invoke IMfllImageBuffer::createInstance to create. And IMfllImageBuffer::initBuffer
     *  is also invoked before use it.
     *
     *  @param base         Base frames as input image.
     *  @param ref          Reference frames as input image.
     *  @param out          Blended images.
     *  @param wt_in        Weighting table input, the first time to blend, set this argument as NULL.
     *  @param wt_out       Weighting table output.
     *  @param wt_ds        Weighting table of dsFrame.
     *  @param difference   differece image as input image.
     *  @param conf         confidence map as input image.
     *  @param dsFrame      dsFrame as input image.
     *  @param dceso        DCESO statistics from the bfbld stage -base frame.
     *  @param index        index-times MSF.
     *  @param s            YuvStage should be YuvStage_Msf
     *  @note               This function must be thread-safe.
     */
