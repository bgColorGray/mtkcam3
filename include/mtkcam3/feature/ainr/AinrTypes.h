...
namespace ainr {
    ...
    enum AinrOnlineDumpInPos
    {
        ONLINE_DUMPIN_BEFORE_UNPACK = 0,
        ONLINE_DUMPIN_BEFORE_BPC = 1,
    };
...
//
    ...
    public:
        ...
        inline void wait(std::unique_lock<std::mutex>&& l) {
            if (!mb_eventdone.load(std::memory_order_relaxed)) {
                cv.wait(l);
            }
        }
    ...
    private:
        ...
        std::mutex              mx;
        std::condition_variable cv;
        std::atomic<bool> mb_eventdone;
    };

    class Semaphore final {
    public:
        explicit Semaphore (int __count = 0)
        : m_count(__count)
        {
        }
