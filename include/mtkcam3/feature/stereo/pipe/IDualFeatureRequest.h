...
enum ResultState
{
    ...
    eRESULT_ERROR,
    eRESULT_YUV_DONE,
    eRESULT_BUF_RELEASED, // node has ability to return buffer by itself
    eRESULT_SKIP_DEPTH,
    eRESULT_FLUSH
};
 ...
 */
class IDualFeatureRequest : public RefBase
{
public:
...
public:
     ...
     *  @return         Frame number
     */
    MUINT64 getFrameNo() const { return mFrameNo; }
    /**
...
public:
    ...
    // ExtraData for tuning or something else
    struct ExtraData {
...
#define MAKE_NAME_CASE(name) \
  ...
  switch(state)
  {
    ...
    MAKE_NAME_CASE(eRESULT_BUF_RELEASED);
    MAKE_NAME_CASE(eRESULT_SKIP_DEPTH);
