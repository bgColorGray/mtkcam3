...
public:
    ...
    FastLogger(const char *TAG, const char *LOG_PROPERTY, const int LOG_VALUE=1)
        : _LOG_PROPERTY((LOG_PROPERTY)?LOG_PROPERTY:"")
    ...
    FastLogger &updateLogStatus()
    {
        __isLogEnabled = ( _TAG.length() > 0 &&
                           _LOG_PROPERTY.length() > 0 &&
     ...
     */
    FastLogger &setSingleLineMode(bool isSingleLine=true)
    {
     ...
     */
    FastLogger &setTrimWhitespace(bool trimWhiteSpace=true)
    {
        android::Mutex::Autolock lock(__logLock);
    ...
    FastLogger& FastLogD(const char *fmt, _T ... args)
    {
        _log(ANDROID_LOG_DEBUG, fmt, args...);
        return *this;
    }
     ...
     */
    template<typename ... _T>
    FastLogger& FastLogE(const char *fmt, _T ... args)
    {
        _log(ANDROID_LOG_ERROR, fmt, args...);
        return *this;
    }
     ...
     */
    template<typename ... _T>
    FastLogger& FastLogW(const char *fmt, _T ... args)
    {
        _log(ANDROID_LOG_WARN, fmt, args...);
        return *this;
    }
     ...
     */
    template<typename ... _T>
    FastLogger& FastLogI(const char *fmt, _T ... args)
    {
        _log(ANDROID_LOG_INFO, fmt, args...);
        return *this;
    }
     ...
     */
    template<typename ... _T>
    FastLogger& FastLogV(const char *fmt, _T ... args)
    {
        _log(ANDROID_LOG_VERBOSE, fmt, args...);
        return *this;
    }
...
protected:
    bool _validateTAG(const char *TAG)
    {
        if(NULL == TAG ||
           0 == strlen(TAG))
        {
    ...
    bool _checkLogProperty(const char* LOG_PROPERTY, const int LOG_VALUE)
    {
        android::Mutex::Autolock lock(__logLock);
    ...
    template<typename ... _T>
    void _log(const int LOG_LEVEL, const char *fmt, _T ... args)
    {
        android::Mutex::Autolock lock(__logLock);
        ...
        size_t lineSize = snprintf(__lineBuffer, MAX_LOG_SIZE, fmt, args...);
        if(0 == lineSize) {
            #ifdef FAST_LOGGER_DEBUG
