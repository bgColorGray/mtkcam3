...
struct StereoArea {
     ...
     */
    inline StereoArea &rotate(int rotation, const bool CENTRALIZE_CONTENT=true) {
        if(CENTRALIZE_CONTENT) {
            NSCam::MSize szContent = contentSize();
            //Only content rotates, padding does not change
            if(StereoHAL::isVertical(rotation)) {
            ...
            else if(!StereoHAL::isHorizontal(rotation))
            {
                if(rotation < 0) {
                    rotation += 360;
                }
                ...
                float a = szContent.w * std::cos(StereoHAL::degreeToRadians(rotation));
                float b = szContent.h * std::sin(StereoHAL::degreeToRadians(90.0f-rotation));
                int finalSize = static_cast<int>(std::round(a+b));
                size.w = finalSize + padding.w;
                size.h = finalSize + padding.h;
            }
        } else {
            switch(rotation)
            {
     ...
     * \details As default we will centralize the content
     *
     * \para ROTATE_WITH_SLANT Rotate according to slant module angle.
     *                  If set to false, will rotate by module type (0, 90, 270)
     * \param CENTRALIZE_CONTENT (default)true if you want to centralize content, padding will not change
     *                           false if you really want to rotate size, padding and start point
     * \return Rotated area
     * \see StereoSettingProvider::getModuleRotation()
     */
    inline StereoArea &rotatedByModule(const bool ROTATE_WITH_SLANT=true,
                                       const bool CENTRALIZE_CONTENT=true) {
        auto angle = StereoSettingProvider::getModuleRotation();
        if (!ROTATE_WITH_SLANT &&
            StereoHAL::isSlant(angle)) {
            angle = 0;
        }
        return rotate(angle, CENTRALIZE_CONTENT);
    }
     ...
     */
    inline StereoArea &applyRatio(STEREO_IMAGE_RATIO_T ratio, ENUM_IMAGE_RATIO_ALIGNMENT alignment=E_KEEP_AREA_SIZE) {
        ...
        const STEREO_IMAGE_RATIO_T KNOWN_RATIOS[] =
        {
            eStereoRatio_16_9,
            eStereoRatio_4_3,
        };
     ...
     */
    inline StereoArea &removePadding(bool keepSize=!IS_KEEP_SIZE)
    {
        if (!keepSize) {
            size = contentSize();
        }
     ...
     */
    inline StereoArea &applyRatioToContent(STEREO_IMAGE_RATIO_T ratio, bool isVertical)
    {
        NSCam::MSize szContent = contentSize();
        int m, n;
        ratio.MToN(m, n);
     ...
     * \return Area with content size change by ratio
     */
    inline StereoArea &applyRatioToSize(STEREO_IMAGE_RATIO_T ratio, bool isVertical)
    {
        int m, n;
        ratio.MToN(m, n);
