...
/* StereoCaptureInfo.h */
class StereoCaptureInfo : public IStereoInfo {
public:
...
/* GoogleStereoInfo.h */
class GoogleStereoInfo : public IStereoInfo {
public:
    ...
    GoogleStereoInfo() :
        ...
        depthFar(255.0),
        depthMime("image/png"),
        hasExtendedXMP("2CFC4BF2EC59CAA9E6B6BF5A22D8C18C"),
        depth_width(-1),
        depth_height(-1) {
    }
...
public:
    ...
    PackInfo() :
        ...
        md5("2CFC4BF2EC59CAA9E6B6BF5A22D8C18C"),
    ...
    StereoString toString() {
        std::stringstream ss;
        if (unpackedCustomizedBufMap != nullptr) {
            ss << "<dump> unpackedCustomizedBufMap";
            for (auto iter = unpackedCustomizedBufMap->begin();
                ...
                ss << "type = " << type.c_str() << ", buffer size = " << buffer.size;
            }
        }
        return ss.str();
    }
...
public:
    ...
    static void encodeDepthToPng(const StereoBuffer_t &inputBuffer,
        int width, int height, StereoBuffer_t &outBuffer);
