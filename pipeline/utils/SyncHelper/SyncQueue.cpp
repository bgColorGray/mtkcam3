...
)
{
    ...
    auto qitem = mvQueue.find(frameno);
    if (qitem != mvQueue.end() ){
        if(qitem->first == frameno&&hasFrametoDrop) {
...
)
{
    ...
    {
        MY_LOGE("Invalid frame (%d) or CamId[%d]", frameno, camId);
        return false;
    }
    // =====================================================================
    mLock.lock();
...
)
{
    ...
    if(pSyncElement != nullptr) {
        ...
        {
            mLock.unlock();
            MY_LOGA("camid is NOT exist in sync target");
        }
    }
    else {
        // =====================================================================
...
)
{
    ...
    if (!checkCandidate(input.frameNum))
    {
        dropFrameNoList.push_back(input.frameNum);
        ret = false;
        MY_LOGD("Drop income Frame %d",input.frameNum);
    }
    return ret;
}
/******************************************************************************
...
{
    bool ret = true;
    for(int i= 0; i <mCandidateQueue.size(); i++) {
        if (mCandidateQueue.at(i) < frameno){
            MY_LOGD("Drop old candidate %d", mCandidateQueue.at(i));
            dropItem(mCandidateQueue.at(i));
            removeCandidate(mCandidateQueue.at(i));
        }
        else if (mCandidateQueue.at(i) > frameno){
            ret = false;
        }
    }
    // Only income frame >= or unexist in candidateQueue can be add in
    if(ret)
        mCandidateQueue.push_back(frameno);
    return ret;
}
/******************************************************************************
 *
...
{
    mCandidateQueue.erase(std::remove(mCandidateQueue.begin(),mCandidateQueue.end(),frameno),mCandidateQueue.end());
}
