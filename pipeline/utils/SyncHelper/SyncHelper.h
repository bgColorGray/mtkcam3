...
{
public:
    SyncHelper(int32_t openId, uint64_t timestamp, int32_t initRequest, bool enableCropOverride);
    virtual ~SyncHelper();
public:
    ...
    bool setSensorCropRegion(int requestNum, std::unordered_map<int32_t, MRect>& cropList) override;
public:
    bool updateData(
                        SyncDataUpdaterType const& updater_type,
                        SyncData &data);
    bool getPreQueue(std::vector<int>& frameNoList);
    int setPreQueue(int frameNo);
private:
    bool isFlush();
    bool getSensorCropRegion(SyncResultInputParams const& input);
public:
...
private:
    ...
    bool mEnableCropOverride = false;
    // crop
    // <sensorId, crop>
    int mCropListRequestNumber = -1;
    std::unordered_map<int32_t, MRect> mvCropList;
    // <framenumber, crop>
    std::unordered_map<int32_t, std::unordered_map<int32_t, MRect>> mvFrameNoCropList;
    std::unordered_map<int32_t, int32_t> mvCropListCount;
    bool needCropReplace = false;
    int32_t mInitReqCnt = 0;
    std::vector<int> mvFrameNoList;
};
/******************************************************************************
 *
 ******************************************************************************/
