...
PhysicalMetaSyncDataUpdater(int32_t openId, uint64_t timestamp)
    ...
    for(auto &sensorId : sensorIDs)
    {
        ...
        if(pMetadataProvider)
        {
            ...
            if(IMetadata::getEntry<MRect>(&staticMeta, MTK_SENSOR_INFO_ACTIVE_ARRAY_REGION, activeArrayRegion))
            {
                ...
                MRect fullCropRegion;
                fullCropRegion.p.x = 0;
                fullCropRegion.p.y = 0;
                fullCropRegion.s.w = activeArrayRegion.s.w;
                fullCropRegion.s.h = activeArrayRegion.s.h;
                MY_LOGD("Sensor %d Full MTK_SCALER_CROP_REGION: (%d, %d) %dx%d", sensorId,
                        fullCropRegion.p.x, fullCropRegion.p.y,
                        fullCropRegion.s.w, fullCropRegion.s.h);
                __fullCropRegion[sensorId] = std::move(fullCropRegion);
            }
            else
            {
...
)
{
    for(auto&& item : data.mvOutputParams) {
        //Update SCALER_CROP_REGION
        {
            ...
            auto iter = __fullCropRegion.find(item.first);
            if(iter != __fullCropRegion.end())
            {
              IMetadata::IEntry scalerCropR(MTK_SCALER_CROP_REGION);
              scalerCropR.push_back(iter->second, Type2Type< MRect >());
              item.second->AppDynamic->update(MTK_SCALER_CROP_REGION, scalerCropR);
