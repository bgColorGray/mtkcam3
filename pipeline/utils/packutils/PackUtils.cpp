...
MERROR
IspTuningDataPackUtil::
packFeoDataToHalMeta_Begin([[maybe_unused]] android::sp<IImageBufferHeap> pInputBufferHeap,
                             ...
                             [[maybe_unused]] IMetadata &halResultMeta)
{
    ...
    CAM_TRACE_CALL();
    MERROR err = OK;
    CAM_LOGD("[%s] packFeoDataToHalMeta_Begin +",__FUNCTION__);
    mFefm = ainr::IAinrFeFm::createInstance();
    if (CC_UNLIKELY(mFefm.get() == nullptr))
    {
    ...
    mFefm->getFeFmSize(feoSize, fmoSize);
    ...
    //
    if(mFeoBuffer == nullptr ||
       (mFeoBuffer != nullptr &&  mFeoBuffer.get() != nullptr &&
       (mFeoBuffer->getImgFormat()!=format || mFeoBuffer->getImgSize()!=feoSize))
       )
    {
        if( mFeoBuffer && mFeoBuffer.get())
        {
            CAM_LOGD("[%s] FEO buffer changed: old format(%d) size(%d,%d) new format(%d) size(%d,%d)",__FUNCTION__,
            mFeoBuffer->getImgFormat(),mFeoBuffer->getImgSize().w,mFeoBuffer->getImgSize().h,
            format,feoSize.w,feoSize.h);
        }
        else
        {
            CAM_LOGD("[%s] FEO buffer is empty, need to create FEO buffer",__FUNCTION__);
        }
        //
        ...
        mFeoBuffer = heap->createImageBuffer();
    }
    else
    {
        CAM_LOGD("[%s] FEO buffer not changed: format(%d) size(%d,%d)",__FUNCTION__,
            format,feoSize.w,feoSize.h);
    }

    //////////////////////////////////////////////////
    // do FE
    mFeoBuffer->lockBuf("feoBuffer",eBUFFER_USAGE_HW_CAMERA_WRITE|eBUFFER_USAGE_SW_READ_OFTEN  );
    mRrzoBuffer = pInputBufferHeap->createImageBuffer();
    mRrzoBuffer->lockBuf("rrzoBuffer",eBUFFER_USAGE_HW_CAMERA_READ|eBUFFER_USAGE_SW_READ_OFTEN  );
    ainr::IAinrFeFm::DataPackage pack;
    pack.appMeta = &appControlMeta;
    pack.halMeta = &halResultMeta;
    pack.inBuf   = mRrzoBuffer.get(); // RRZO/YUVO buffers
    pack.outBuf  = mFeoBuffer.get(); // FEO buffer
    CAM_LOGD("[%s] doFe() +",__FUNCTION__);
    if(0 != mFefm->doFe(&pack))
    {
        err = UNKNOWN_ERROR;
    }
    CAM_LOGD("[%s] doFe() -",__FUNCTION__);
...
MERROR
IspTuningDataPackUtil::
packFeoDataToHalMeta_End([[maybe_unused]] IMetadata &halResultMeta)
{
    MERROR err = OK;
#if MTKCAM_HAVE_AINR_SUPPORT
    //////////////////////////////////////////////////
    // pack feo
    CAM_LOGD("[%s] packFeoDataToHalMeta_End +",__FUNCTION__);
    CAM_LOGD("[%s] waitFeDone +",__FUNCTION__);
    if(0 != mFefm->waitFeDone())
    {
        err = UNKNOWN_ERROR;
    }
    CAM_LOGD("[%s] waitFeDone -",__FUNCTION__);
    ...
    infoPtr->rrzoSize = mRrzoBuffer->getImgSize();
    infoPtr->feoTotalSize = mFeoBuffer->getBufSizeInBytes(0);
    ...
    CAM_LOGD("[%s] memcpy src(%" PRIxPTR ") dst(%p) size(%d)",__FUNCTION__,mFeoBuffer->getBufVA(0),feoData.editArray(),infoPtr->feoTotalSize);
    memcpy(reinterpret_cast<void*>(feoData.editArray()),
                        reinterpret_cast<void*>(mFeoBuffer->getBufVA(0)),
                        infoPtr->feoTotalSize);
    IMetadata::setEntry<IMetadata::Memory>(&halResultMeta, MTK_ISP_FEO_DATA, feoData);
    mFeoBuffer->unlockBuf("PackfeoBufferFeo");
    //mFeoBuffer = nullptr; // need be kept
    mRrzoBuffer->unlockBuf("rrzoBuffer");
    mRrzoBuffer = nullptr;
    mFefm = nullptr;
    CAM_LOGD("[%s] packFeoDataToHalMeta_End -",__FUNCTION__);
#endif //end of #if MTKCAM_HAVE_AINR_SUPPORT
...
downSampleImageBufferHeap(android::sp<IImageBufferHeap> pInputRrzoBufferHeap,
                                    ...
                                    [[maybe_unused]] IMetadata &appControlMeta,
                                    [[maybe_unused]] IMetadata &appResultMeta,
                                    [[maybe_unused]] IMetadata &halResultMeta)
{
    CAM_TRACE_CALL();
    ...
    else
    {
        ...
        //
        if(mBssRrzoBuffer == nullptr ||
           (mBssRrzoBuffer != nullptr &&  mBssRrzoBuffer.get() != nullptr &&
            mBssRrzoBuffer->getBufSizeInBytes(0)!=BssSize)
           )
        {
            if(mBssRrzoBuffer && mBssRrzoBuffer.get())
            {
                CAM_LOGD("[%s] BSS buffer changed: old size(%zu) new size(%d)",__FUNCTION__,
                mBssRrzoBuffer->getBufSizeInBytes(0),
                BssSize);
            }
            else
            {
                CAM_LOGD("[%s] BSS buffer is empty, need to create BSS buffer",__FUNCTION__);
            }
            //
            IImageBufferAllocator::ImgParam imgParam(BssSize,0);
            android::sp<IImageBufferHeap> pHeap = IIonImageBufferHeap::create("BssRrzoBuffer",imgParam,IIonImageBufferHeap::AllocExtraParam(),MTRUE);
            mBssRrzoBuffer = pHeap->createImageBuffer();
            if (pHeap == NULL)
            {
                CAM_LOGE("[%s] heap is NULL",__FUNCTION__);
                return UNKNOWN_ERROR;
        ...
        {
            CAM_LOGD("[%s] BSS buffer not changed: size(%d)",__FUNCTION__,
                BssSize);
        }
        //
        if ( mBssRrzoBuffer.get() == 0 )
        {
        ...
        if(IMetadata::getEntry<MINT32>(&appControlMeta, MTK_CONTROL_CAPTURE_HINT_FOR_ISP_FRAME_INDEX, ispFrameIndex))
        {
            ...
            if ( !mBssRrzoBuffer->lockBuf( "BssRrzoBuffer", (eBUFFER_USAGE_HW_CAMERA_READ|eBUFFER_USAGE_SW_MASK)) )
            {
            ...
            //  Do Zip
            if (spBssCore->doZipWithBss(pRrzoImgBuf.get(),mBssRrzoBuffer.get(),BssSize,&appControlMeta,&appResultMeta, &halResultMeta) == false)
            {
                if(ispFrameIndex == 0)
                {
                    spBssCore->doZip(pRrzoImgBuf.get(),mBssRrzoBuffer.get(),BssSize,&halResultMeta,MTRUE);
                }
                else
                {
                    spBssCore->doZip(pRrzoImgBuf.get(),mBssRrzoBuffer.get(),BssSize,&halResultMeta,MFALSE);
                }
            }
            CAM_LOGD("[%s] doZip -",__FUNCTION__);
            //
            // after do zip, need to unlock image buffer
            if ( !mBssRrzoBuffer->unlockBuf( "BssRrzoBuffer") )
            {
            ...
            if(mBssRrzoBuffer->getImageBufferHeap() == nullptr)
            {
                CAM_LOGE("[%s] pDownSampleImg->getImageBufferHeap() is NULL",__FUNCTION__);
                ret = UNKNOWN_ERROR;
                return ret;
            }
            pOutputBufferHeap = mBssRrzoBuffer->getImageBufferHeap();
        }
        else
        {
    ...
    pOutputBufferHeap = pInputRrzoBufferHeap;
...
packIspTuningDataToHeap(
        ...
        sp<IImageBuffer>& externalBssBuffer,
        sp<IImageBuffer>& externalFeoBuffer
        )
{
    CAM_TRACE_CALL();
    ...
    //
    mBssRrzoBuffer = externalBssBuffer;
    mFeoBuffer = externalFeoBuffer;
    //
    ...
    //
    MINT32 sensorId = -1;
    IMetadata::IEntry sensorIdEntry = inputParam.halResultMeta.entryFor(MTK_HAL_REQUEST_SENSOR_ID);
    if(!sensorIdEntry.isEmpty())
    {
        sensorId = sensorIdEntry.itemAt(0, Type2Type<MINT32>());
    }
    ...
    pInfo->sensorId = sensorId;
    //
    MINT32 deviceId = -1;
    IMetadata::IEntry deviceIdEntry = inputParam.halResultMeta.entryFor(MTK_HAL_REQUEST_DEVICE_ID);
    if(!deviceIdEntry.isEmpty())
    {
        deviceId = deviceIdEntry.itemAt(0, Type2Type<MINT32>());
    }
    else
    {
        CAM_LOGW("[%s] MTK_HAL_REQUEST_DEVICE_ID is empty!",__FUNCTION__);
    }
...
#define COPY_PRIVATE(_buf_,_size_,_msg_) \
    ...
    ///////////////////////////////////////////////////////
    //
    CAM_LOGD("[%s] sensorId(%d) deviceId(%d) requestNo(%d) frameNo(%d) timestamp(%" PRId64 ") pOutBufAddr(%" PRIxPTR ") outImgSizeInBytes(%zu) nowAvailBufSize(%zu) headerSize(%zu)",
        __FUNCTION__, pInfo->sensorId, pInfo->deviceId, inputParam.requestNo, inputParam.frameNo, inputParam.timestamp,
    ...
    if(needPackRrzo > 0)
    {
        ...
        if(needPackFeo > 0)
        {
            err = packFeoDataToHalMeta_Begin(inputParam.pRrzoImageBufferHeap,inputParam.appControlMeta,inputParam.halResultMeta);
            if(err != OK)
            {
                CAM_LOGE("[%s] packFeoDataToHalMeta_Begin() failed!",__FUNCTION__);
            }
        }
        //
        if(needBssDownSample > 0)
        {
            err = downSampleImageBufferHeap(inputParam.pRrzoImageBufferHeap,inputParam.pRrzoStreamInfo,pFinalRrzoImageBufferHeap,inputParam.appControlMeta,inputParam.appResultMeta,inputParam.halResultMeta);
            if(err != OK)
            {
        ...
        if(needPackFeo > 0)
        {
            err = packFeoDataToHalMeta_End(inputParam.halResultMeta);
            if(err != OK)
            {
                CAM_LOGE("[%s] packFeoDataToHalMeta_End() failed!",__FUNCTION__);
            }
        }
        //
...
#undef HAL_FD_DATA
    ...
    /////////////////////////////////////////
    // STT data
#define HAL_STT_DATA "HAL_STT_DATA"
    if (inputParam.pSTTData != nullptr && inputParam.STTDataSize != 0)
    {
        MOVE_OUT_BUF_PTR(flattenRet,HAL_STT_DATA);
        CALCU_AVAIL_SIZE(flattenRet,HAL_STT_DATA);
        COPY_PRIVATE(inputParam.pSTTData, inputParam.STTDataSize,HAL_STT_DATA);
        CALCU_CHECK_SUM(pNowOutBufPtr,flattenRet,HAL_STT_DATA);
        pInfo->privateInfo[STT_DATA].length = flattenRet;
        pInfo->privateInfo[STT_DATA].offset = (pNowOutBufPtr-pOutBufAddr);
        pInfo->privateInfo[STT_DATA].checkSum = checkSumRet;
        //
        CAM_LOGD("[%s] pNowOutBufPtr(%" PRIxPTR ") privateInfo[STT_DATA] offset(%zu) length(%zu) checksum(%u)",
            __FUNCTION__, pNowOutBufPtr, pInfo->privateInfo[STT_DATA].offset, pInfo->privateInfo[STT_DATA].length, pInfo->privateInfo[STT_DATA].checkSum);
    }
#undef HAL_STT_DATA
    //
    /////////////////////////////////////////
...
#undef RRZO_BUF
    ...
    pInfo->totalUsedBytes = headerSize +
        ...
        pInfo->privateInfo[FD_DATA].length + pInfo->rawInfo[STTO_RAW].length + pInfo->rawInfo[RRZO_RAW].length + pInfo->privateInfo[STT_DATA].length;
    //dump total buffer
    if (iDebugLevel >= 2)
    {
        ...
        if(myFptr != NULL)
        {
            ...
            int ret = fwrite(chunk.get(),sizeof(char),total,myFptr);
            MY_LOGE_IF(ret==0,"fwrite() return 0!");
            ret = fclose(myFptr);
            MY_LOGE_IF(ret!=0,"fclose() fail!");
        }
        else
        {
    ...
    //
    externalBssBuffer = mBssRrzoBuffer;
    externalFeoBuffer = mFeoBuffer;
    //
...
#define UNPACK_RAW(_buffer_,_raw_type,_checkSum_,_msg_)    \
    {   \
        ...
        size_t offset = pInfo->rawInfo[_raw_type].offset; \
        ...
        {   \
            ...
            CAM_LOGD("[%s] UNPACK_RAW(%s): memcpy(dst:%" PRIxPTR ", src:(%" PRIxPTR "+%zu)=(%" PRIxPTR "), len:%zu)", \
                __FUNCTION__, _msg_,outPtr,pInBufAddr,offset,(MINTPTR)(pInBufAddr+offset),pInfo->rawInfo[_raw_type].plane_size[plane]);   \
            memcpy((void*)outPtr,(void*)(pInBufAddr+offset),pInfo->rawInfo[_raw_type].plane_size[plane]);  \
            offset += pInfo->rawInfo[_raw_type].plane_size[plane]; \
        }   \
...
#define UNPACK_PRIVATE(_pvecout_,_type_,_checkSum_,_msg_)    \
    ...
    //
    CAM_LOGD("[%s] sensorId(%d) deviceId(%d) requestNo(%d) frameNo(%d) timestamp(%" PRId64 ") pInBufAddr(%" PRIxPTR ") totalUsedBytes(%d) headerSize(%zu)",
        __FUNCTION__, pInfo->sensorId, pInfo->deviceId, pInfo->requestNo, pInfo->frameNo, pInfo->timestamp,
    ...
    //
    if (pInfo->privateInfo[STT_DATA].offset != 0)
    {
        UNPACK_PRIVATE(outputParam.pSTTData, STT_DATA, pInfo->privateInfo[STT_DATA].checkSum, "STT_Data");
    }
    lbExit:
    CAM_LOGD("[%s] -", __FUNCTION__);
#undef UNFLATTEN_META
