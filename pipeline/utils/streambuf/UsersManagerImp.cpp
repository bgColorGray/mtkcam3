...
auto
UsersManager::
addUser(User const& user) -> int
{
    RWLock::AutoWLock _l(mRWLock);

    auto pUserGraph = mUserGroupGraph.empty() ? nullptr : mUserGroupGraph[0];
    if (pUserGraph == nullptr) {
        MY_LOGW("%#" PRIxPTR " %s: Bad pUserGraph", mSubject, getSubjectName());
        return NO_INIT;
    }

    pUserGraph->addUser(user);
    enqueUserGraph_(0/*group 0*/);  // update mProducerMap & mConsumerMap
    //android::LogPrinter printer(LOG_TAG, ANDROID_LOG_DEBUG, "addUser");
