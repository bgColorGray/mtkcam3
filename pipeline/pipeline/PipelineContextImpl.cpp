...
#include "PipelineContextBuilderImpl.h"
...
PipelineContextImpl(char const* name)
    ...
    //
{
    createFrameBufferManager(&mFrameBufferManager, VALIDATE_STRING(name));
...
auto
PipelineContext::PipelineContextImpl::
updateConfig(PipelineContextBuilderImpl const* pBuilder) -> int
{
    if ( pBuilder == nullptr ) {
        return BAD_VALUE;
    }
    ...
    if (auto pFrameBufferManager = getFrameBufferManager();
             pFrameBufferManager != nullptr)
    {
        IFrameBufferManager::ConfigureStreams temp;
        temp.configPoolSharedStreams = pBuilder->mPoolSharedStreams;
        for (auto const& item : pBuilder->mImageStreams) {
                temp.configImageStreams.push_back(
                    IFrameBufferManager::AddImageStream{
                        .pStreamInfo = item.pStreamInfo,
                        .attribute = static_cast<StreamAttribute>(item.attribute),
                        .pProvider = item.pProvider,
                    });
        }
        for (auto const& item : pBuilder->mMetaStreams) {
                temp.configMetaStreams.push_back(
                    IFrameBufferManager::AddMetaStream{
                        .pStreamInfo = item.pStreamInfo,
                        .attribute = static_cast<StreamAttribute>(item.attribute),
                    });
        ...
        int err = pFrameBufferManager->configureStreams(temp);
        if (err != 0) {
            MY_LOGE("Failed on IFrameBufferManager::configureStreams()");
            return err;
        }
    }
...

    {
        ...
        for (auto const& item : pBuilder->mImageStreams) {
                using T = StreamConfig::ItemImageStream;
                android::sp<T> pItem = new T(item.pStreamInfo, item.attribute);
                mpStreamConfig->add(pItem);
        }

        for (auto const& item : pBuilder->mMetaStreams) {
                using T = StreamConfig::ItemMetaStream;
                android::sp<T> pItem = new T(item.pStreamInfo, item.attribute);
                mpStreamConfig->add(pItem);
        }
    }
...
constructRequest(FrameBuilderImpl* pBuilder)
{
    ...
    for (auto const& v : pBuilder->mStreamBuffers_AppImage)
    {
        ...
        MY_LOGD_IF(0, "replace APP stream %#" PRIx64, pInfo->getStreamId() );
    ...
    pFrame->setBatchInfo(pBuilder->getBatchInfo());
    //
...
{
    std::string nodeName = VALIDATE_STRING(pNodeActor->getNodeName());
...
{
    if (auto pNodeName = pIActor->getNodeName()) {
        if (pNodeName != nullptr) {
            CAM_TRACE_NAME((std::string("launchOneNode: ") +
                VALIDATE_STRING(pNodeName)).c_str());
        }
    }
    auto startTime = ::systemTime();

    auto triggerAEE = [=](char const* opsName, MERROR err) {
        std::string msg = std::string{"::"} + VALIDATE_STRING(opsName) + "() failed (called by PipelineContext)";

        if ( pIActor != nullptr && pIActor->getNode() != nullptr ) {
            auto pNode = pIActor->getNode();
            auto pNodeName = pIActor->getNodeName();
            if(pNodeName != nullptr){
                std::string nodeName = VALIDATE_STRING(pNodeName);
            // For example:
...
) -> int
{
    ...
    auto toString = [](const LaunchOneNodeSetT& o){
        ...
        os += "{ ";
        for (auto const& v : o) {
            if (v.pNodeActor->getNodeName() != nullptr) {
                os += VALIDATE_STRING(v.pNodeActor->getNodeName());
                os += " ";
            }
        }
        os += "}";
...
)
{
    ...
    {
        {
            ...
            mAsyncConfigFutures.push_back(
                std::async(isMultiThread ? std::launch::async : std::launch::deferred,
                    ...
                    this,
                    isAsync,
                    [pInitFrameHandler, pDispatcher = mpDispatcher.promote()](int err){
                        if ( pInitFrameHandler != nullptr ) {
                            pInitFrameHandler->setConfigDone(err);
                        }
                        if ( pDispatcher != nullptr ) {
                            pDispatcher->notifyConfigureDone();
                        }
                    }
                )
            );
    ...
    mForcePrintLog = ::property_get_bool("vendor.debug.camera3.pplcontext.log",
                                        false);
    //
                     ...
                     Vector<sp<IPipelineNode>> &vspPipelineNode) -> int
{
    ...
    MY_LOGF_IF(pPipelineBufferSetFrameControl==nullptr, "pPipelineBufferSetFrameControl is nullptr!");
    ...
    do
    {
        ...
        while(it != vspPipelineNode.end() && it != nullptr)
        {
...
sendFrame(android::sp<IPipelineFrame>const& pFrame) -> int
{
    ...
    for(size_t i=0; i<RootNodeObjSet.size();i++)
    {
        ...
        CAM_ULOG_ENTER(pNode, REQ_PIPELINE_FRAME, pFrame->getFrameNo());
...
MERROR
PipelineContext::PipelineContextImpl::
flushNodeByNodeId(NodeId_T const nodeId)
{
    int errConfig = waitUntilConfigDone(__FUNCTION__);
    MY_LOGE_IF(OK!=errConfig, "fail to configure - err:%d", errConfig);

    auto pNodeActor = queryNode(nodeId);
    if(!pNodeActor.get())
    {
        MY_LOGW("get nodeActor of (%" PRIdPTR ") fail", nodeId);
        return UNKNOWN_ERROR;
    }
...
beginFlush()
{
    ...
    {
        MY_LOGD("set flushing +");
        {
            RWLock::AutoWLock _l(mFlushLock);
            mIsFlushing = true;
        }
        MY_LOGD("set flushing -");
        MY_LOGD("pDispatcher->beginFlush +");
        {
        ...
        {
            RWLock::AutoRLock _l(mRWLock);
            MY_LOGD("mpInFlightRequest->abort");
...
onCallback(CallBackParams param)
{
    ...
    if(mForcePrintLog)
    {
        MY_LOGD("param: (id:%" PRIxPTR "), (lastFrameNum = %d), (eNoticeType"
        "= %d)", param.nodeId, param.lastFrameNum,param.noticeType);
    }
...
onHWReady()
{
    MY_LOGD("p1 hw is ready");
    if (auto spCb = mpDataCallback.promote()) {
      spCb->onHWReady();
    }
    return;
}
...
#undef  _IMP_EVALUATE_
    ...
    )
    {
        ...
        for (size_t i = 0; i < pBufMap->size(); i++)
        {
            ...
            MY_LOGF_IF(userGraph==nullptr, "userGraph is nullptr!");
            //Add users
