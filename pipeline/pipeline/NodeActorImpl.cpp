...
protected:
    ...
    virtual auto    onInit() -> int     { return (mpNodeOps->getNode()!=nullptr) ? mpNodeOps->getNode()->init(mInitParam) : -1; }
    virtual auto    onConfig() -> int   { return mpNodeOps->config(mpNodeOps->getConfigParam()); }
    virtual auto    onUninit() -> int   { return (mpNodeOps->getNode()!=nullptr) ? mpNodeOps->getNode()->uninit() : -1; }
...
toString() const -> std::string
{
    ...
    oss << "nodeId=0x" << std::hex << getNodeId() << std::dec
        << " (" << (getNodeName()!=nullptr ? getNodeName() : "") << ")"
