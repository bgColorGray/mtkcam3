...
) -> status_t
{
    ...
    if (!mbInited) {
        ...
        updateBufferCount(maxNumberOfBuffers);
        mbInited = true;
    }
...
auto
BufferHeapPool::
isPoolEmpty() const -> bool
{
    Mutex::Autolock _l(mAvailLock);
    MY_LOGD2("Pool(%s): available(%zu) - inUse(%zu)", poolName(), mAvailableBuf.size(),
            [this](){ Mutex::Autolock _ll(mInUseLock); return mInUseBuf.size(); }());
