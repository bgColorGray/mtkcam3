...
using NSCam::TuningUtils::scenariorecorder::ScenarioLogPrinter;
                    ...
                    {
                        {
                            ...
                            for (size_t i = 0; i < rItemMap.size(); i++) {
                                auto pUsersManager = rItemMap.usersManagerAt(i);
                                if  (pUsersManager == nullptr) {
                                    continue;
                                }
                                if  ( 0 < pUsersManager->getNumberOfProducers() ) {
...
public:     ////    Operations.
    ...
    MVOID           onProducersReleased(ItemMap_HalMetaT::Item& rItem)
                    {
                        android::Mutex::Autolock _l(mLock);
                        auto usersManager = rItem.getUsersManager();
                        if  ( usersManager != nullptr ) {
                            if  ( 0 < usersManager->
                                getNumberOfProducers() ) {
                                mHalMetaNumO_ProducersInFlight--;
                                if  ( rItem.mBuffer != 0 ) {
                                    mHalMetaSetO_ProducersReleased.push_back(
                                        rItem.mBuffer);
                                }
                            }
                        }
    ...
    MVOID           onProducersReleased(ItemMap_AppMetaT::Item& rItem)
                    {
                        ...
                        auto usersManager = rItem.getUsersManager();
                        if  ( usersManager != nullptr ) {
                            if  ( 0 < usersManager->getNumberOfProducers() ) {
                                mAppMetaNumO_ProducersInFlight--;
                                if  ( rItem.mBuffer != 0 ) {
                                    mAppMetaSetO_ProducersReleased.push_back(
                                        rItem.mBuffer);
                                }
                            }
                        }
...
protected:  ////                    Common.
    ...
    // Scenario recorder printer
    std::unique_ptr<ScenarioLogPrinter>
                                    mScenarioLogPrinter;
...
protected:  ////                    Information Keeper
    ...
    bool                            mAlreadyApplyPreRelease = false;
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//  Implementations
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
public:     ////                    Operations.
    ...
    virtual MVOID                   applyRelease(UserId_T userId, bool isRealTime);
...
public:     ////                    Attributes.
    ...
    virtual void                    getBatchInfo(BatchInfo &out) const { out = mBatchInfo; };
    virtual void                    setBatchInfo(BatchInfo const& arg) { mBatchInfo = arg; };
