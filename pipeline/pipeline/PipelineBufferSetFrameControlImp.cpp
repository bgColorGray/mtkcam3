...
castFrom(IPipelineFrame* pPipelineFrame) -> IPipelineBufferSetFrameControl*
{
    ...
    auto name = pPipelineFrame->getMagicName();
    if (CC_UNLIKELY( name == nullptr )) {
        return nullptr;
    }

    if (CC_UNLIKELY( 0 != ::strcmp(name, IPipelineBufferSetFrameControl::magicName()) )) {
        return nullptr;
    }
    ...
    //
{
    ...
    mScenarioLogPrinter = std::make_unique<ScenarioLogPrinter>(arg.requestNo);
}
...
onLastStrongRef(const void* /*id*/)
{
    ...
    auto forceReleaseHalImage = [this](auto const& pItemMap) {
            auto const& map = pItemMap->mMap;

            HalImageSetT aHalImageSet;
            for (size_t i = 0; i < map.size(); i++) {
                auto pItem = map.valueAt(i);
                if (pItem == nullptr || pItem->mBuffer == nullptr)
                    continue;
                aHalImageSet.push_back(pItem->mBuffer);
            }

            /**
             * This is expected in the aiawb case. For example:
             *   0x100000011(Hal:Image:P1FDYuv_main1)
             *       Producer #1 Used:0x80000000 Released:0x80000000
             *         { userId:0x1 ACQUIRE|USED|RELEASE usage:0x20003 }
             *       Consumer #2 Used:0xc0000000 Released:0xc0000000
             *         { userId:0x16 ACQUIRE|USED|RELEASE usage:0x40003 }
             *         { userId:0x100|USED|RELEASE } ==> aiawb
             */
            CAM_ULOGMD("[onLastStrongRef] Force release HalImage: #%zu", aHalImageSet.size());
            handleReturnResult(ReturnResult{
                .pHalResultProcessor = mHalResultProcessor,
                .requestNo = getRequestNo(),
                .frameNo = getFrameNo(),
    ...
    {
        MY_LOGW(
            "%s buffers are not fully released: #(AppImage, AppMeta, HalImage, HalMeta)=(%zd %zd %zd %zd)",
        ...
        if (mpItemMap_HalImage->mNonReleasedNum > 0) {
            forceReleaseHalImage(mpItemMap_HalImage);
            mpItemMap_HalImage->mNonReleasedNum = 0;
        }
    }

    {
        ...
        if  (CC_UNLIKELY( pAppCallback == 0 )) {
            ...
            IAppCallback::Result result =
            {
                ...
                .bIsReprocessFrame = mbReprocessFrame,
            };
...
public:
    ...
    )
    {
        ...
        if  (CC_UNLIKELY( index < 0 )) {
            MY_LOG2("%s not found", toTitleMsg().c_str());
            return MFALSE;
        }
        //
...
protected:
    ...
    updateNodeStatus(NodeId_T const nodeId, IOSet& rIOSet, MBOOL bufStatusCheck)
    {
        ...
        for (; it != rIOSet.end();) {
            ...
            //
            MY_LOGF_IF(pMapItem->getUsersManager() == nullptr, "pMapItem->getUsersManager() is null!");
            ...
            if  ( (status & IUsersManager::UserStatus::RELEASE) ||
                ...
                //
                if  ( pMapItem->getUsersManager()!=nullptr && (
                      (OK == pMapItem->getUsersManager()->haveAllUsersReleased()) ||
                      (OK == pMapItem->getUsersManager()->haveAllUsersReleasedOrPreReleased()))) {
                    if ( bufStatusCheck ){
                        auto pStreamInfo = pMapItem->getStreamInfo();
                        if(pStreamInfo==nullptr)
                        {
                            MY_LOGE("Bad pStreamInfo");
                        }
                        else
                        {
                            if (pMapItem->hasBufferStatus(STREAM_BUFFER_STATUS::ERROR)){
                                ...
                                toHexString(pStreamInfo->getStreamId()).c_str(), bufStatusCheck, mBufferStatus );
...
)
{
    ...
    if ( rPendingReleaseSet.targetStreamId.empty() ) {
        ...
        for (; it != rBufSetToTest.end(); it++) {
            if  (CC_LIKELY( (*it) != 0 )) {
                auto pStreamInfo = (*it)->getStreamInfo();
                MY_LOGF_IF(pStreamInfo==nullptr, "Bad pStreamInfo");
                auto found = rPendingReleaseSet.targetStreamId.find(pStreamInfo->getStreamId());
...
MVOID
MAIN_CLASS_NAME::
handleReleasedBuffers(UserId_T userId, sp<IAppCallback> pAppCallback, bool isRealTime)
{
...
MVOID
MAIN_CLASS_NAME::
applyRelease(UserId_T userId, bool isRealTime)
{
    auto toTitleMsg = [this, nodeId=userId]() { return ::toTitleMsg(ToTitleMsg{.pFrame = this, .nodeId = nodeId,}); };
...
//              mpPipelineDAG = 0;
        ...
        //
        //
        if (userId >= eNODEID_AIAWB_BEGIN && userId < eNODEID_AIAWB_END) {
            auto const& map = *mpItemMap_HalImage;
            MY_LOGD_IF(gLogLevel>=1, "+ R:%d F:%d HalImage:mNonReleasedNum:%zu",
                getRequestNo(), getFrameNo(), map.mNonReleasedNum);
            if (map.mNonReleasedNum > 0) {
                for (size_t i = 0; i < map.size(); i++) {
                  auto pItem = map.itemAt(i);
                  if (!pItem.get())
                    continue;
                  auto pStreamInfo = pItem->getStreamInfo();
                  auto pUsersManager = pItem->getUsersManager();
                  if (!pUsersManager || !pStreamInfo)
                    continue;
                  if (OK == pUsersManager->haveAllUsersReleased()) {
                    auto const& set = mpReleasedCollector->mHalImageSet_AllUsersReleased;
                    auto numBefore = set.size();
                    pItem->handleAllUsersReleased();
                    auto numAfter = set.size();
                    MY_LOGD_IF(gLogLevel>=1, "- R:%d F:%d streamId:%s "
                        "num:%zu->%zu mNonReleasedNum:%zu",
                        getRequestNo(), getFrameNo(),
                        toHexString(pStreamInfo->getStreamId()).c_str(),
                        numBefore, numAfter, map.mNonReleasedNum);
                  }
                }
            }
        }
        //
        //
        handleReleasedBuffers(userId, pAppCallback, isRealTime);
    }
    //
