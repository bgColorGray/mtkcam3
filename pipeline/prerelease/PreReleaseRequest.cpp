...
public:
  ...
  virtual void  lock() {mPreReleaseLock.lock();}
  virtual void  unlock() {mPreReleaseLock.unlock();};
...
private:
    ...
    using RequestsT  = std::vector<RequestItem>;
    mutable android::Mutex                         mReqItemLock;
    RequestsT                                      mReqItems;           /* the items will callback to AP in-order */

    using InFlightRequestsT  = std::vector<InFlightRequestItem>;
    ...
    if (count > 0) {
        ...
        for ( auto i = 0 ; i < count ; i++) {
            ...
            if (mReqItems.size() > 0){
                ...
                auto pEventCallback = EventCallbackMgr::getInstance();
                if (pEventCallback) {
                  pEventCallback->onRequestFinishedCB(readerID, request.frameNo, request.timestamp, request.status);
                } else {
                  MY_LOGW("EventCallbackMgr is nullptr");
                }
            }
        }

        for ( auto i = 0 ; i < count ; i++) {
            mvInFlightRequests.erase(mvInFlightRequests.begin());
        }
    } else {
...
setRequestFinished(uint32_t frameNo, uint32_t preReleaseUid, uint32_t requestStatus)
{
    ...
    while(needCB == true && mReqItems.size() >0) {
        ...
        if(item.status == Status::RELEASE){
            ...
            auto pEventCallback = EventCallbackMgr::getInstance();
            if (pEventCallback) {
              pEventCallback->onRequestFinishedCB(item.imgReaderID, item.frameNo, item.timestamp, item.requestStatus);
            } else {
              MY_LOGW("EventCallbackMgr is nullptr");
            }
            mReqItems.erase(mReqItems.begin());
            MY_LOGD("mReqItems size(%zu)", mReqItems.size());
        }
        else{
            needCB = false;
...
start()
{
    ...
    if (mPreReleaseRequests.isEmpty()) {
      mpContext = nullptr;
      MY_LOGD("set pipeline context to null");
    }
...
//    Mutex::Autolock _l(mLock);
    ...
    NodeId_T FDNodeId = eNODEID_UNKNOWN;
    i = 0;
    do {
        FDNodeId = NodeIdUtils::getFDNodeId(i);
        auto pNode = mpContext->queryINodeActor(FDNodeId);
        if (pNode != NULL) {
          pNode->uninit();
          MY_LOGD("Node(%s) done", toHexString(FDNodeId).c_str());
        }
        i++;
    } while (FDNodeId != eNODEID_UNKNOWN);
