...
#define DEF_IMG_INFO(index) \
    ...
    SUB_IMG_INFO(index, IN_SEAMLESS_FULL,    IO_DIR_IN,  "inSeamlessFull",   IO_FLAG_DEFAULT, IN_S_SEAMLESS_FULL),  \
    ...
    SUB_IMG_INFO(index, IN_RSSO_R2,          IO_DIR_IN,  "inRSSOR2",         IO_FLAG_DEFAULT, IN_S_RSSO_R2),  \
    ...
    SUB_IMG_INFO(index, IN_Tracking_YUV,     IO_DIR_IN,  "inTrackingyuv",    IO_FLAG_DEFAULT, IN_S_Tracking_YUV)
...
const std::unordered_map<ID_IMG, IMG_INFO> P2Img::InfoMap =
{
    ...
    MAIN_IMG_INFO(OUT_APP_DEPTH,       IO_DIR_OUT, "outAppDepth",      IO_FLAG_DEFAULT,     OUT_S_APP_DEPTH),
...
{                                                               \
    ...
    {                                                           \
        ...
        case IN_SEAMLESS_FULL:  return IN_SEAMLESS_FULL_##index;\
        ...
        case IN_RSSO_R2:        return IN_RSSO_R2_##index;         \
        ...
        case IN_Tracking_YUV:   return IN_Tracking_YUV_##index; \
...
{                                                                   \
    ...
    {                                                               \
        ...
        case IN_S_SEAMLESS_FULL:  return IN_S_SEAMLESS_FULL_##index;\
        ...
        case IN_S_RSSO_R2:        return IN_S_RSSO_R2_##index;      \
        ...
        case IN_S_Tracking_YUV:   return IN_S_Tracking_YUV_##index; \
...
#define DEF_STREAM_IMG_MAP(index) \
        ...
        case IN_S_SEAMLESS_FULL##_##index:    return {IN_SEAMLESS_FULL##_##index};              \
        ...
        case IN_S_RSSO_R2##_##index:          return {IN_RSSO_R2##_##index};                    \
        ...
        case IN_S_Tracking_YUV##_##index:     return {IN_Tracking_YUV##_##index};
...
std::list<ID_IMG> getImgsFromStream(ID_STREAM id)
{
    switch(id)
    {
        ...
        case OUT_S_APP_DEPTH:       return {OUT_APP_DEPTH};
...
NSCam::EImageFormat P2Img::getImgFormat() const
{
    NSCam::EImageFormat fmt = NSCam::eImgFmt_UNKNOWN;
    if( isValid() )
    {
        IImageBuffer *img = getIImageBufferPtr();
        if( img )
        {
            fmt = (NSCam::EImageFormat)img->getImgFormat();
        }
    }
    return fmt;
}
...
MVOID P2Img::dumpBuffer(IImageBuffer *buffer, unsigned index) const
{
    if( buffer )
    {
        ...
        char path[256];
        int res = snprintf(path, sizeof(path), DUMP_PATH "/%04d_%04dm%02d_%02d_%s_%dx%d-PW%d-PH%d_%d_%d.%s",
        ...
        MY_LOGE_IF((res < 0), "file dump snprintf FAIL!");
...
MVOID P2Img::dumpNddBuffer() const
{
    ...
    for( IImageBuffer *buffer : this->getIImageBufferPtrs() )
    {
        dumpNddBuffer(buffer, index++);
    }
}

MVOID P2Img::dumpNddBuffer(IImageBuffer *buffer, unsigned index) const
{
    if( buffer )
    {
        char filename[256] = {0};
        ...
        if( index )
        {
            int res = snprintf(indexStr, sizeof(indexStr), "%u", index);
            MY_LOGE_IF((res < 0), "file NDD dump snprintf FAIL!");
        }
        ...
        {
            case IN_FULL:
                genFileName_RAW(filename, sizeof(filename), &hint, RAW_PORT_IMGO, indexStr);
                break;

            case IN_RESIZED:
                {
                    if( NSCam::isHalRawFormat((NSCam::EImageFormat)buffer->getImgFormat()) )
                    {
                        genFileName_RAW(filename, sizeof(filename), &hint, RAW_PORT_RRZO, indexStr);
                    }
                    else
                    {
                        char yuvoStr[16] = "yuvo";
                        if( index )
                        {
                            int res = snprintf(yuvoStr, sizeof(yuvoStr), "yuvo-%s", indexStr);
                            MY_LOGE_IF((res < 0), "file NDD dump snprintf FAIL!");
                        }
                        genFileName_YUV(filename, sizeof(filename), &hint, YUV_PORT_NULL, yuvoStr);
                    }
                }
                break;
           case IN_LCSO:
               genFileName_LCSO(filename, sizeof(filename), &hint, indexStr);
               break;
           ...
           case OUT_YUV:
               {
                   if( this->isDisplay())
                   {
                       genFileName_YUV(filename, sizeof(filename), &hint, YUV_PORT_WDMAO, indexStr);
                   }
                   else if( this->isRecord())
                   {
                       genFileName_YUV(filename, sizeof(filename), &hint, YUV_PORT_WROTO, indexStr);
                   }
                   else
                   {
                       genFileName_YUV(filename, sizeof(filename), &hint,YUV_PORT_UNDEFINED, indexStr);
                   }
               }
               break;
           default:
