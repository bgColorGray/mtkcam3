    ...
    , mEISFactor(EISCustom::getEIS12Factor())
{
    ...
    if( sensorInfo && sensorData )
    {
        ...
        mIsValid = initAppInfo(sensorData) && initHalInfo(sensorData);
        mIsValid = initTransform() && mIsValid;
        initLMV();
        if( needCropperDump() )
        {
            this->dump(mLog);
...
{
    MRectF retRectF = calcViewAngleF(log, size, cropFlag, cropRatio, cropRatio, (DMAConstrain::NONE));
    ...
    const float widthCropRatio, const float heightCropRatio, const MUINT32 dmaConstrain) const
{
    ...
    MBOOL usePhysical = !!(cropFlag & Cropper::USE_PHYSICAL) && isValidCrop(mPhysicalAppCrop);
    MBOOL useRrzoDomainView = useResize && !USE_SENSOR_DOMAIN_VIEW_ANGLE;
    MBOOL needSensorToRrzo = useResize && !useRrzoDomainView;
    TRACE_S_FUNC(log, "size=%dx%d flag=0x%x useResize=%d useEIS12=%d useCropRatio=%d(%f,%f), usePhysical=%d, dmaConstrain(%d), validCropper=%d, useRrzoDomainView=%d",
        size.w, size.h, cropFlag, useResize, useEIS12, useCropRatio, widthCropRatio, heightCropRatio, usePhysical, dmaConstrain, mIsValid, useRrzoDomainView);

    MRectF view(size.w, size.h);
    if( mIsValid )
    {
        const MRect &activeCrop = usePhysical ? mPhysicalAppCrop : mActiveCrop;
        const MRect &sensorCrop = usePhysical ? mPhySensorCrop : mSensorCrop;
        const MRect &resizedCrop = usePhysical ? mPhyResizedCrop : mResizedCrop;

        TRACE_S_CROP(log, "activeCrop", activeCrop.p, activeCrop.s);
        TRACE_S_CROP(log, "sensorCrop", sensorCrop.p, sensorCrop.s);
        TRACE_S_CROP(log, "resizedCrop", resizedCrop.p, resizedCrop.s);

        view = useRrzoDomainView ? resizedCrop : sensorCrop;
...
MBOOL P2Cropper::getMatrix(const MPoint& offset, const MSize& from, const MSize& to, HwMatrix* mat) const
{
    if( mat != nullptr &&
        from.w > 0 && from.h > 0 && to.w > 0 && to.h > 0 )
    {
        *mat = NSCamHW::simplifiedMatrix(
            static_cast<float>(to.w) / static_cast<float>(from.w), offset.x,
            static_cast<float>(to.h) / static_cast<float>(from.h), offset.y);
        return MTRUE;
    }
    return MFALSE;
}
...
{
    MRectF s_crop, a_crop;
    if( resize )
    {
        s_crop.p = inv_transform(mSensor2Resized, cropF.p);
        s_crop.s = inv_transform(mSensor2Resized, cropF.s);
    }
    else
    {
        s_crop.p = cropF.p;
    ...
    TRACE_S_CROPF(mLog, "ViewCrop", cropF.p, cropF.s);
    TRACE_S_CROPF(mLog, "sensorViewCrop", s_crop.p, s_crop.s);
    TRACE_S_CROPF(mLog, "activeViewCrop", a_crop.p, a_crop.s);
    return a_crop.toMRect();
}
...
MVOID P2Cropper::dump(const ILog &log) const
{
    ...
    MY_S_LOGI(log, "activearraySize(%dx%d)", mActiveSize.w, mActiveSize.h);
    ...
    MY_S_LOGI(log, "Active crop (%dx%d)@(%d,%d), AppCrop(%dx%d)@(%d,%d), SimulateAppCrop(%dx%d)@(%d,%d), PhysicalAppCrop(%dx%d)@(%d,%d)",
        ...
        mPhysicalAppCrop.s.w, mPhysicalAppCrop.s.h,
        mPhysicalAppCrop.p.x, mPhysicalAppCrop.p.y);
    MY_S_LOGI(log, "SensorTarget crop (%dx%d)@(%d,%d)",
        mSensorTargetCrop.s.w, mSensorTargetCrop.s.h,
        mSensorTargetCrop.p.x, mSensorTargetCrop.p.y);
    ...
    mSensor2Active.dump("Sensor2Active");
    mActive2Sensor.dump("Active2Sensor");
    TRACE_S_FUNC_EXIT_2(log);
}

MBOOL P2Cropper::needCropperDump() const
{
    static MBOOL sEnableDump = (property_get_int32(KEY_CROPPER_DUMP,0) > 0);
    return sEnableDump || (TRACE_CROPPER > 0) || (mLog.getLogLevel() >= 1 );
}
...
MBOOL P2Cropper::initAppInfo(const P2SensorData *data)
{
    ...
    if( data )
    {
        ...
        mPhysicalAppCrop = data->mPhysicalAppCrop;
...
MBOOL P2Cropper::initHalInfo(const P2SensorData *data)
{
    ...
    if( data)
    {
        ...
        mSensorTargetCrop = data->mSensorTargetCrop;
        ret = MTRUE;
    }
    TRACE_S_FUNC_EXIT_2(mLog);
    return ret;
...
MBOOL P2Cropper::initTransform()
{
    ...
    else
    {
        ...
        if( isValidCrop(mSensorTargetCrop) )
        {
            mSensorCrop = mSensorTargetCrop;
        }
        else
        {
        ...
        if( isValidCrop(mPhysicalAppCrop) )
        {
            mActive2Sensor.transform(mPhysicalAppCrop, mPhySensorCrop);
            mPhySensorCrop = clip(mPhySensorCrop, mP1Crop);
            mPhyResizedCrop = transform(mSensor2Resized, mPhySensorCrop);
        }
    }
    TRACE_S_FUNC_EXIT_2(mLog);
    return ret;
