...
//    MY_LOG_FUNC_ENTER("StreamingNode");
    ...
    mP2Info->mConfigInfo.mAutoTestLog = property_get_bool("vendor.autotesttool.enable", false);
    MY_LOG_FUNC_EXIT();
}
...
MUINT32 P2StreamingNodeImp::getP2PQIndex(const P2UsageHint &usage, P2Type p2Type, MUINT32 presetPQIdx) const
{
    MUINT32 pqIndex = 0;
    // handle EIS flow
    ...
    else if (EIS_MODE_IS_EIS_30_ENABLED(eisInfo.mode)) // Adv EIS mode
    {
    ...
    MY_LOGD("MDPPQ: PQIdx(preset(%d)(%s), final(%d)(%s)), hasVideo=%d, eisInfo(previewEIS=%d, mode=0x%x, videoConfig=%d)",
        presetPQIdx, P2Util::p2PQIdx2String(presetPQIdx),
...
P2UsageHint P2StreamingNodeImp::toP2UsageHint(const P2Common::UsageHint &hint) const
{
    ...
    if( mP2Info->mConfigInfo.mP2Type != P2_BATCH_SMVR )
    {
        ...
        usage.mAISSMode = property_get_int32("vendor.debug.p2.aissMode", hint.mAISSMode);
        ...
        usage.mTotalMarginRatio = hint.mTotalMarginRatio;
        usage.mResizedRawMap = hint.mResizedRawMap;
        usage.mSensorModule = hint.mSensorModule;
        usage.mIsHdr10 = hint.mIsHdr10;
        usage.mHdr10Spec = hint.mHdr10Spec;
        ...
        usage.mInCfg.mReqFps = hint.mInCfg.mReqFps;
        usage.mInCfg.mP1Batch = hint.mInCfg.mP1Batch;
    }
    ...
    usage.mP2PQIndex = getP2PQIndex(usage, mP2Info->mConfigInfo.mP2Type, hint.mPresetPQIdx);
    usage.mTrackingFocusOn = hint.mTrackingFocusOn;
...
MBOOL P2StreamingNodeImp::parseInitParam(const ILog &log, const IPipelineNode::InitParams &initParam)
{
    ...
    mP2Info->mLog = log;
    mP2Info->mConfigInfo.mUseScenarioRecorder = RecorderWrapper::getUseScenarioRecorder();
