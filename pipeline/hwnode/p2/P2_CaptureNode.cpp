        ...
        , mCurFrameIndex(-1)
{
    ...
    mP2Info->mConfigInfo.mUsageHint =
    {
        ...
        .mAppSessionMeta  = usageHint.mAppSessionMeta
    };
    mP2Info->mConfigInfo.mUsageHint.mDualMode = property_get_int32("vendor.debug.p2.dualMode", usageHint.mDualFeatureMode);
    MY_S_LOGI(mLog, "(%p) ctor, uniqueKey:%d, supportedScenarioFeatures:%#" PRIx64 ", bgPreRelease:%d, hidlIsp:%d sessionparam:%p",
        this,
        ...
        &mP2Info->mConfigInfo.mUsageHint.mAppSessionMeta);
...
MERROR P2CaptureNodeImp::queue(sp<IPipelineFrame> frame)
{
    ...
    if (mStatus != STATUS_READY) {
        ...
        MINT32 count = getReadyFrameCountOrPushToList(frame);
        if (count > 0) {
            MBOOL bFirst = MTRUE;
            do {
                sp<IPipelineFrame> curFrame = bFirst ? frame : getFrameFromList();
                mRequestNo = curFrame->getRequestNo();
                MUINT32 uFrameNo = curFrame->getFrameNo();
                ILog frameLog = NSCam::Feature::makeFrameLogger(mLog, uFrameNo, mRequestNo, uFrameNo);
                sp<MWFrame> frameHolder = new MWFrame(frameLog, mNodeId, mNodeName, curFrame);
                ...
                if (pFrameRequest != NULL) {
                    CAM_ULOG_SUBREQS(MOD_P2_CAP_NODE, REQ_PIPELINE_FRAME, curFrame->getFrameNo(), REQ_P2_CAP_REQUEST, curFrame->getFrameNo());
                    CAM_ULOG_ENTER(MOD_P2_CAP_PROC, REQ_P2_CAP_REQUEST, curFrame->getFrameNo());
...
MINT32 P2CaptureNodeImp::getReadyFrameCountOrPushToList(sp<IPipelineFrame> pFrame)
{
    MINT32 count = 1;
    sp<IMetaStreamBuffer> pStreamBuffer;
    auto err = ensureMetaBufferAvailable_(
              pFrame->getFrameNo(),
              mMWInfo->findMetaInfo(IN_P1_HAL)->getStreamId(),
              pFrame->getStreamBufferSet(),
              pStreamBuffer
              );
    IMetadata *pMetadata;
    if ( err == OK && ((pMetadata = pStreamBuffer->tryReadLock(mNodeName, true)) != NULL)) {
        MUINT8 indexOrderUnsure = 0;
        MINT32 frameCount = 0;
        tryGet<MUINT8>(pMetadata, MTK_MULTIFRAME_INDEX_ORDER_UNSURE, indexOrderUnsure);
        tryGet<MINT32>(pMetadata, MTK_HAL_REQUEST_COUNT, frameCount);
        // reorder multiple frame by index
        if (indexOrderUnsure > 0 && frameCount > 1) {

            MINT32 frameIndex = -1;
            tryGet<MINT32>(pMetadata, MTK_HAL_REQUEST_INDEX, frameIndex);

            if (mCurFrameIndex == -1) {  // first incoming frame
                mFrameCount = frameCount;
                if (frameIndex != 0) { // push to list
                  mlPipelineFrame.emplace_back(frameIndex, pFrame);
                  count = 0;
                } else {
                  mCurFrameIndex = 0;
                }
            } else if (mFrameCount == frameCount) { // check if push the frame to list
                if (frameIndex == (mCurFrameIndex + 1)) {  // hit
                    if (!mlPipelineFrame.empty()) {
                        auto iterFrameList = mlPipelineFrame.begin();
                        auto nextIndex = (*iterFrameList).first;
                        if ((frameIndex + 1) == nextIndex) {
                            count = (MINT32)mlPipelineFrame.size() + 1;
                        }
                    }
                    mCurFrameIndex += count;
                } else { // push to list
                    mlPipelineFrame.emplace_back(frameIndex, pFrame);
                    count = 0;
                }
            } else {  // something wrong
                MY_S_LOGE(mLog, "mFrameCount(%d) != frameCount(%d)", mFrameCount, frameCount);
                count = (MINT32)mlPipelineFrame.size() + 1;
                mCurFrameIndex = -1;
            }

            if ((frameIndex + count) == frameCount) { // reorder done
                mCurFrameIndex = -1;
            }
            if (count >= 1) {
                MY_S_LOGD(mLog, "enque %d request(s) to P2C, R/F:%d/%d Index/Count:%d/%d, CurIndex(%d), size(%d)", count, pFrame->getRequestNo(), pFrame->getFrameNo(), frameIndex, frameCount, mCurFrameIndex, (MINT32)mlPipelineFrame.size());
            } else {
                MY_S_LOGD(mLog, "push to temp queue, R/F:%d/%d Index/Count:%d/%d, CurIndex(%d), size(%d)", pFrame->getRequestNo(), pFrame->getFrameNo(), frameIndex, frameCount, mCurFrameIndex, (MINT32)mlPipelineFrame.size());
            }
        }

        pStreamBuffer->unlock(mNodeName, pMetadata);
    }

    return count;
}

sp<IPipelineFrame> P2CaptureNodeImp::getFrameFromList()
{
    if (!mlPipelineFrame.empty()) {
        auto iterFrameList = mlPipelineFrame.begin();
        auto pFrame = (*iterFrameList).second;
        mlPipelineFrame.pop_front();
        return pFrame;
    } else {
        return nullptr;
    }
