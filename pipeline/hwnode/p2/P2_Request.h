...
public:
    P2Request(const ILog &log, const sp<IP2Frame> &frameHolder, const P2Pack &p2Pack, const sp<P2InIDMap> &p2IdMap, const std::set<ID_IMG> &dropImgs);
    virtual ~P2Request();
...
public:
    ...
    MBOOL hasDropped(ID_IMG id) const;
    MBOOL hasDroppedAny() const;
    MBOOL isResized() const;
    MBOOL isReprocess() const;
    MBOOL isPhysic() const;
    MBOOL isLarge() const;
    MVOID setIsPhysic(MBOOL isPhysic);
