    ...
    , mFPS(fps)
{
    ...
    if( mMWFrame != NULL)
    {
        ...
        if( mMWFrame->getInfoIOMapSet(ioMap) )
        {
            ...
            MUINT32 masterId = decideMasterID();
            if(masterId == INVALID_SENSOR_ID)
            {
                masterId = mP2Pack.getConfigInfo().mMainSensorID;
            }
            updateP2FrameData(masterId);
            updateP2SensorData();
            updateP2PackMaster(masterId);
            P2_CAM_TRACE_END(TRACE_ADVANCED);
        }
    }
...
MVOID MWFrameRequest::earlyDispatch()
{
    TRACE_S_FUNC_ENTER(mLog);
    if( mMWFrame != NULL )
    {
        mMWFrame->earlyDispatch();
    }
    TRACE_S_FUNC_EXIT(mLog);
}
...
MBOOL MWFrameRequest::addP2Img(P2ImgMap &imgMap, const StreamId_T &streamID, IO_DIR dir)
{
    ...
    if( mStreamSet.count(streamID) == 0 )
    {
        ...
        MBOOL imgCreate = MFALSE;
        P2_CAM_TRACE_FMT_BEGIN(TRACE_ADVANCED, "MWImg %s_0x%" PRIx64 " ", ImgType2Name(mMWInfo->getImgType(streamID)), streamID);
        ...
        if( !imgCreate )
        {
            std::list<ID_IMG> imgs = mMWInfo->toImgIDs(streamID, dir);
            mDropImgSet.insert(imgs.begin(), imgs.end());
        }
    }
    TRACE_S_FUNC_EXIT(mLog);
    return ret;
...
MVOID MWFrameRequest::updateP2PackMaster(MUINT32 masterId)
{
    TRACE_S_FUNC_ENTER(mLog);
    mP2Pack = P2Pack(mP2Pack, mLog, masterId);
    TRACE_S_FUNC_EXIT(mLog);
}

MVOID MWFrameRequest::updateP2FrameData(MUINT32 masterId)
{
    TRACE_S_FUNC_ENTER(mLog);
    P2FrameData &data = mP2Data->mFrameData;
    // Need decide master first
    data.mMasterSensorID = masterId;
    ...
    if(inApp == NULL)
    {
        inApp = findP2Meta(mMetaMap, mapID(masterId, IN_APP_PHY));
    }
...
MVOID MWFrameRequest::updateP2SensorData()
{
    ...
    for( MUINT32 sensorID : mP2Pack.getConfigInfo().mAllSensorID )
    {
        ...
        sp<P2Meta> inPhyApp = findP2Meta(mMetaMap, mapID(sensorID, IN_APP_PHY));
        if(inApp == NULL)
        {
            inApp = inPhyApp;
        }
       ...
       if( MTK_CONTROL_VIDEO_STABILIZATION_MODE_ON ==
        ...
        data.mSensorTargetCrop = getMeta<MRect>(p1Hal, MTK_P2NODE_SENSOR_CROP_REGION,
                                 MRect(0,0));
	...
	data.mPhysicalAppCrop = getMeta<MRect>(inPhyApp, MTK_SCALER_CROP_REGION,
                        MRect(0,0));
        LMVInfo lmvInfo;
        if( isValid(p1Hal) )
        {
        ...
        const P2PlatInfo *platInfo = P2PlatInfo::getInstance(sensorID);
        if( platInfo && isValid(p1Hal) )
        {
            data.mNvramDsdn = platInfo->queryDSDN(data.mMagic3A, data.mIspProfile);
            data.mNvramMsyuv_3DNR = platInfo->queryMSYUV_RSV(data.mMagic3A, data.mIspProfile);
        }

        MUINT32 aeAvailability = 0;
        aeAvailability |= (tryGet<MINT32>(p1Hal, MTK_3A_AE_BV, data.mAeBv) ? 0x1 : 0);
        aeAvailability |= (tryGet<MINT32>(p1Hal, MTK_3A_AE_CWV, data.mAeCwv) ? 0x2 : 0);
        if( aeAvailability != 0x3 && mP2Pack.getConfigInfo().mUsageHint.mIsHdr10 )
        {
            MY_LOGW("[HDR10] AE info not available: 0x%x; sensor:%d  BV:%d, CWV:%d",
                aeAvailability, sensorID, data.mAeBv, data.mAeCwv);
        }

        // NR
        data.mStreamingNR = getMeta<MINT32>(p1Hal, MTK_DUALZOOM_STREAMING_NR, MTK_DUALZOOM_STREAMING_NR_AUTO);

        // HDR
        data.mHDRHalMode = getMeta<MINT32>(p1Hal, MTK_HDR_FEATURE_HDR_HAL_MODE, MTK_HDR_FEATURE_HDR_HAL_MODE_OFF);
...
MVOID MWFrameRequest::updateP1TS(const sp<P2Meta> &p1App, const sp<P2Meta> &p1Hal, P2SensorData &data)
{
    ...
    data.FrameStartTime = frameTS.size() ? frameTS[0] : 0;
    TRACE_S_FUNC_EXIT(mLog);
}

MVOID MWFrameRequest::updateISO(const sp<P2Meta> &inApp, const sp<P2Meta> &p1App, const sp<P2Meta> &p1Hal, P2SensorData &data)
{
    MINT32 hdrISO = getMeta<MINT32>(p1Hal, MTK_3A_AE_HDR_MIXED_ISO, 0);
    MINT32 realISO = 0;
    if( !tryGet<MINT32>(p1App, MTK_SENSOR_SENSITIVITY, realISO) )
    {
        tryGet<MINT32>(inApp, MTK_SENSOR_SENSITIVITY, realISO);
    }
    data.mISO = (hdrISO != 0) ? hdrISO : realISO;
    TRACE_S_FUNC(mLog, "hdrISO(%d), realISO(%d), finalISO(%d)", hdrISO, realISO, data.mISO);
}
...
MVOID MWFrameRequest::updateP2Metadata(MUINT32 sensorID, const sp<Cropper> &cropper)
{
    ...
    if( !mMWInfo->isCapture() && !inHalMeta->tryGet<MRect>(MTK_3A_PRV_CROP_REGION, rect))
    {
        ...
        if( isValid(target) )
        {
            ...
            if( mP2Pack.getConfigInfo().mUsageHint.mTotalMarginRatio > 1.0f )
            {
                 totalRatio = 1.0f / mP2Pack.getConfigInfo().mUsageHint.mTotalMarginRatio;
                 cropFlag |= Cropper::USE_CROP_RATIO;
            }
            cropFlag |= resized ? Cropper::USE_RESIZED : 0;

            MRectF crop = cropper->calcViewAngleF(mLog, target->getTransformSize(), cropFlag, totalRatio, totalRatio, (DMAConstrain::NONE));
            rect = cropper->toActive(crop, resized);
            TRACE_S_FUNC(mLog, "sID(%d) PreviewCrop: totalRatio(%f), rrzCrop(%f,%f,%fx%f), activeCrop(%d,%d,%dx%d)",
                            sensorID, totalRatio, crop.p.x, crop.p.y, crop.s.w, crop.s.h, rect.p.x, rect.p.y, rect.s.w, rect.s.h);
...
MVOID MWFrameRequest::printIOMap(const IPipelineFrame::InfoIOMapSet &ioMap)
{
    ...
    MY_S_LOGE_IF( (used < 0), mLog, "printIomap snprintf failed!");
...
std::vector<sp<P2Request>> MWFrameRequest::createRequests(IPipelineFrame::InfoIOMapSet &ioMap)
{
    ...
    for(unsigned i = 0, n = imgSet.size(); i < n; ++i )
    {
        ...
        sp<P2Request> request = new P2Request(reqLog, mMWFrame, mP2Pack, mInIDMap, mDropImgSet);
        if( request == NULL )
        {
        ...
        request->initIOInfo();
        if (imgSet[i].isPhysical) {
            MY_S_LOGD(reqLog, "IOMap set is for physical");
            request->setIsPhysic(imgSet[i].isPhysical);
        }
