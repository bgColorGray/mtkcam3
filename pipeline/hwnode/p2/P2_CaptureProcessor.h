...
public:
    virtual MVOID notifyRelease(MINT32 requestNo, MINT32 frameNo, MBOOL isOutput = MFALSE);
...
private:
    ...
    MVOID                   reorderRequest(const vector<sp<P2Request>> &requests, vector<sp<P2Request>>& reorderRequests, bool& hasLogical);
    MVOID                   dumpP1NDD(sp<ICaptureFeatureRequest> pCapRequest, MINT32 sensorId, MINT32 subSensorId);
...
private:
    ...
    std::set<int>                               mRequestRestored;
    std::mutex                                  mNextCaptureLock;
    std::pair<int, bool>                        mNextCaptureCB;
