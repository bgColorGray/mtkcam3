...
P2DrawIDPlugin::P2DrawIDPlugin()
{
    ...
    if(property_str.find("TYPE") !=string::npos) mDrawCtrl.en_bit.img_type = 1;
    if(property_str.find("FLK")  !=string::npos) mDrawCtrl.en_bit.flk = 1;
...
MBOOL P2DrawIDPlugin::onPlugin(const P2Img *img)
{
        ...
        ( img->getID() == OUT_YUV || img->getID() == OUT_JPEG_YUV))
    {
        ...
        for( IImageBuffer *ptr : buffers )
        {
            if( ptr )
            {
                const char* humanName = img->getHumanName();
                std::string imgType = humanName ? humanName : "unknown";
                TRACE_FUNC("draw + img(%s)", imgType.c_str());
                if(mDrawCtrl.en)
                {
                    ...
                    if(mDrawCtrl.en_bit.img_type)
                    {

                        mDrawIDUtil->draw("TYPE "+imgType,
                                          offset_x,
                                          offset_y += linePixel*maskH + 10,
                                          (char*)ptr->getBufVA(0),
                                          ptr->getImgSize().w,
                    ...
                    if(mDrawCtrl.en_bit.flk)
                    {

                        mDrawIDUtil->draw("FLK"+to_string(sensor_data.mFlickerResult),
                                          offset_x,
                                          offset_y += linePixel*maskH + 10,
                                          (char*)ptr->getBufVA(0),
                                          ptr->getImgSize().w,
