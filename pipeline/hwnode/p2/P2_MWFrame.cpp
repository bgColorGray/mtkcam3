...
#ifdef SUPPORT_OPT_NONEED_GRALLOC_VA
    static MBOOL enableVA = ::property_get_bool("vendor.camera.debug.enableApBufVa", 0) ||
                            ::property_get_bool("vendor.camera.dipGetVa.enable", 0);
    MBOOL isStreaming = mNodeID == NSCam::v3::eNODEID_P2StreamNode;
    mNeedRefineBufferUsage = isStreaming && (!enableVA);
#endif
...
MVOID MWFrame::releaseMetaStream(const sp<ReleaseToken> &releaseToken, const sp<IMetaStreamBuffer> &stream, IO_DIR dir, IO_STATUS status)
{
    ...
    if( releaseToken != NULL && stream != NULL && stream->getStreamInfo() != NULL )
    {
        if( dir & IO_DIR_OUT )
        {
...
MVOID MWFrame::metaResultAvailable(IMetadata* partialMeta)
{
    ...
    sp<IPipelineNodeCallback> cb = mFrame->getPipelineNodeCallback();
    if( cb != NULL ) {
        const char *name = mNodeName.string();
        cb->onMetaResultAvailable({partialMeta, mFrame, mNodeID, (name != NULL) ? name : ""});
    }
    TRACE_S_FUNC_EXIT(mLog);
}
...
sp<IMetaStreamBuffer> MWFrame::acquireMetaStream(const StreamId_T &sID)
{
    ...
    else
    {
        ...
        if(sInfo != NULL)
        {
            const char *name = sInfo->getStreamName();
            mReleaseToken->registerStream(sID, (name != NULL) ? name : "");
        }
        else
        {
                                                    ...
                                                    const NSCam::ImageBufferInfo &blobInfo, IO_DIR dir, MBOOL needSWRW, MBOOL isOpaque) const
{
    ...
    if( stream != NULL && heap != NULL)
    {
        ...
        for( sp<IImageBuffer> &buf : images )
        {
            if( buf != NULL )
            {
                if (mNeedRefineBufferUsage)
                {
                    usage = refineBufferUsage(buf, usage, needSWRW);
                }
...
MUINT32 MWFrame::refineBufferUsage(const sp<IImageBuffer>& buf, MUINT32 usage, MBOOL needSWRW)
{
    MBOOL isGraphic = buf->getBufferHeapType() == NSCam::eBufferHeapType::eGraphicBufferHeap;
    MBOOL isUFO = (buf->getImgFormat() >= NSCam::eImgFmt_UFO_START && buf->getImgFormat() <= NSCam::eImgFmt_UFO_END);
    // MBOOL isCustomerNeed = customer && buf->getImgFormat() == eImgFmt_Y8;

    MBOOL needVA = isUFO || needSWRW || (!isGraphic) /* || isCustomerNeed */;
    return needVA ? usage : usage &= (~NSCam::eBUFFER_USAGE_SW_MASK);
}
