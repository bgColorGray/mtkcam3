...
MVOID P2FrameHolder::earlyDispatch()
{
    if( mFrame != NULL )
    {
        mFrame->earlyDispatch();
    }
}
...
P2Request::P2Request(const ILog &log, const sp<IP2Frame> &frameHolder, const P2Pack &p2Pack, const sp<P2InIDMap> &p2IdMap, const std::set<ID_IMG> &dropImgs)
    : P2FrameHolder(frameHolder)
    , mSensorID(p2Pack.getFrameData().mMasterSensorID)
    , mLog(log)
    , mP2Pack(p2Pack.getP2Pack(mLog, mSensorID))
    , mInIDMap(p2IdMap)
    , mDroppedImgSet(dropImgs)
{
}
...
MVOID P2Request::initIOInfo()
{
    ...
    if(mImg[OUT_RAW] && mImg[OUT_RAW]->isPhysicalStream()) {
        mIsPhysic = MTRUE;
    }
}
...
MBOOL P2Request::hasSensorIn(MUINT32 sensorID) const
{
    ...
    return isValidImg(IN_RESIZED, sensorID) || isValidImg(IN_FULL, sensorID)|| isValidImg(IN_SEAMLESS_FULL, sensorID);
    #else
...
MBOOL P2Request::hasInput() const
{
    ...
    {
        #if MTKCAM_SEAMLESS_SWITCH_SUPPORT
        if(isValidImg(IN_RESIZED, sID) || isValidImg(IN_FULL, sID) || isValidImg(IN_SEAMLESS_FULL, sID))
        {
            ret = MTRUE;
            break;
        }
...
MBOOL P2Request::hasOutput() const
{
    ...
    ret = ret || isValidImg(OUT_FD) || isValidImg(OUT_JPEG_YUV) || isValidImg(OUT_THN_YUV) || isValidImg(OUT_POSTVIEW) || isValidImg(OUT_RAW) || isValidImg(OUT_APP_DEPTH);
    return ret;
}

MBOOL P2Request::hasDropped(ID_IMG id) const
{
    return mDroppedImgSet.count(id);
}

MBOOL P2Request::hasDroppedAny() const
{
    return mDroppedImgSet.size();
}
...
MVOID P2Request::setIsPhysic(MBOOL isPhysic)
{
    mIsPhysic = isPhysic;
}
