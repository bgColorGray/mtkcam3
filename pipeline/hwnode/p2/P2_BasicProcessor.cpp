...
MBOOL BasicProcessor::onInit(const P2InitParam &param)
{
    ...
    mLog = mP2Info.mLog;
    mStreamingSize = mP2Info.getConfigInfo().mUsageHint.mStreamingSize;
...
{
    mTimer.startTotal();
    mP2Obj.mRunIndex = NSCam::NSIoPipe::P2_RUN_S_P2A;
}
...
{
    TRACE_S_FUNC_ENTER(payload->mRequest->mLog);
    MUINT32 fps = std::max<MUINT32>(payload->mRequest->mP2Pack.getFrameData().mMinFps , 30);
    DIPParams dipParams = payload->mDIPParams;
    NSCam::Feature::P2Util::updateExpectEndTime(dipParams, mExpectMS);
    NSCam::Feature::P2Util::updateScenario(dipParams, fps, mStreamingSize.w);
...
DIPParams BasicProcessor::prepareEnqueDIPParams(std::vector<sp<P2Payload>> payloads)
{
    ...
    DIPParams dipParams;
    MUINT32 fps = std::max<MUINT32>(payloads[0]->mRequest->mP2Pack.getFrameData().mMinFps , 30);
