    ...
    , public IspTuningDataPackNode
{
    ...
    private:
        ...
        sp<IImageBuffer> mpBssBuffer = nullptr;
        sp<IImageBuffer> mpFeoBuffer = nullptr;
};
...
config(ConfigParams const& rParams)
{
    ...
    // create internal buffer anyway
    {
        ...
        if (mpInternalPackBuffer == NULL)
        {
            ...
            //
            MY_LOGI("Allocate internal pack buffer, mInternalPackBufferSize = %zu (mSourceType:%d)", mInternalPackBufferSize, mSourceType);
            MERROR ret = OK;
        ...
        )
{
    ...
    //
    {
        ...
        if (pInMeta_HalResult == NULL)
        {
            MY_LOGE("No Hal Result Metadata!");
            errorHandle(pFrame);
            err = UNKNOWN_ERROR;
            return err;
        }
        if (pInMeta_P1AppResult == NULL)
        {
            MY_LOGE("No P1 APP Result Metadata!");
            errorHandle(pFrame);
            err = UNKNOWN_ERROR;
            return err;
        }
        if (pInMeta_AppControl == NULL)
        {
            MY_LOGE("No P1 APP Control Metadata!");
            errorHandle(pFrame);
            err = UNKNOWN_ERROR;
            return err;
        }
        ...
        else
        {
            ...
            MY_LOGW("cannot get MTK_P1NODE_FRAME_START_TIMESTAMP, use pFrame->getSensorTimestamp() : %" PRId64 "", HalTimeStamp);
        }
        ...
        MERROR pack_ret = OK;
        MY_LOGD("get FD data : %zu", fdData.size());
        ...
        // STT private info
        auto privData = streamInSttInfo->queryPrivateData();
        packParam.pSTTData = const_cast<uint8_t*>(NSCam::plugin::streaminfo::get_data_if<uint8_t>(privData.privData));
        packParam.STTDataSize = (NSCam::plugin::streaminfo::get_datasize(privData.privData));
        ...
        {
            case YuvFromP2:
                if (pInMeta_P2AppResult)
                    packParam.appResultMeta = (*pInMeta_P1AppResult) + (*pInMeta_P2AppResult);
                else
                {
                    MY_LOGW("YuvFromP2 but pInMeta_P2AppResult is null");
            ...
            case RawFromP2:
                ...
                {
                    MY_LOGW("RawFromP2 but pInMeta_P2AppResult is null");
                    packParam.appResultMeta = *pInMeta_P1AppResult;
                }
                updateTag = MTK_CONTROL_CAPTURE_ISP_TUNING_DATA_RAW;
                break;
        ...
        pack_ret = PackUtils.packIspTuningDataToHeap(pOutImgBufHeap, totalPackSize, packParam, mpBssBuffer, mpFeoBuffer);
        ...
        if(pack_ret == OK)
        {
            ...
            {
                MINT32 bssGoldenIdx = -1;
                if (IMetadata::getEntry<MINT32>(&packParam.appResultMeta, MTK_MFNR_FEATURE_BSS_GOLDEN_INDEX, bssGoldenIdx)) {
                    MY_LOGD("get MTK_MFNR_FEATURE_BSS_GOLDEN_INDEX: %d", bssGoldenIdx);
                    IMetadata::setEntry<MINT32>(pOutMeta_AppResult, MTK_MFNR_FEATURE_BSS_GOLDEN_INDEX, bssGoldenIdx);
                }
            }
        }
        else
...
)
{
    ...
    rStreamBufferSet.applyRelease(getNodeId(), true);
    return OK;
}
...
MERROR
IspTuningDataPackNodeImp::
unlockImgStream(android::sp<IPipelineFrame> const& pFrame __attribute__((unused)),
    sp<IImageStreamBuffer>& pStreamBuffer,
    sp<IImageBuffer>& pImageBuffer,
    MBOOL const isOut,
...
{
    IStreamBufferSet& rStreamBufferSet = pFrame->getStreamBufferSet();
    MY_LOGD_IF(1, "nodeID %#" PRIxPTR " streamID %#" PRIx64 " ",getNodeId(), streamId);
    ...
    if( err != OK )
    {
        MY_LOGD_IF( rpStreamBuffer == NULL, "streamId(%#" PRIx64 ") meta streamBuf not exit", streamId);
        return err;
    }
...
IspTuningDataPackNodeImp::
returnMetadataAndUnlock(
        android::sp<IPipelineFrame> const& pFrame __attribute__((unused)),
        StreamId_T const streamId,
        sp<IMetaStreamBuffer>  rpStreamBuffer,
        IMetadata* rpMetadata,
        MBOOL isOut,
