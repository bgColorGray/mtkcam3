...
{
    //for InImageBuffer is unpack format
    if ( isHalUnpackRawFormat((EImageFormat)(rpInImageBuffer->getImgFormat())) ) {
        if (rpInImageBuffer->getBufSizeInBytes(0) > rpOutImageBuffer->getBufSizeInBytes(0)){
            MY_LOGE("in-imgbuf is oversize for out-imgbuf");
            return MFALSE;
        } else {
            memcpy(reinterpret_cast<void*>(rpOutImageBuffer->getBufVA(0)),
                reinterpret_cast<void*>(rpInImageBuffer->getBufVA(0)),
                rpOutImageBuffer->getBufSizeInBytes(0));
            return MTRUE;
        }
    }
    ...
    switch (rpOutImageBuffer->getImgFormat()) {
        case eImgFmt_BAYER8_UNPAK:
            MyDngopImgInfo.BIT_NUM_DST = 8;
            MyDngopImgInfo.Bit_Depth = 8;
            break;
        ...
        case eImgFmt_BAYER12_UNPAK:
            MyDngopImgInfo.BIT_NUM_DST = 12;
            MyDngopImgInfo.Bit_Depth = 12;
            break;
        case eImgFmt_BAYER14_UNPAK:
            MyDngopImgInfo.BIT_NUM_DST = 14;
            MyDngopImgInfo.Bit_Depth = 14;
            break;
        case eImgFmt_BAYER15_UNPAK:
            MyDngopImgInfo.BIT_NUM_DST = 15;
            MyDngopImgInfo.Bit_Depth = 15;
            break;
        case eImgFmt_BAYER16_UNPAK:
            MyDngopImgInfo.BIT_NUM_DST = 16;
            MyDngopImgInfo.Bit_Depth = 16;
            break;
        default:
            MY_LOGE("no matched unpack stream format");
            break;
    }
...

    {
        if (mLogLevel) {
            char filename[256];
            if (0 > snprintf(filename, sizeof(filename), "/sdcard/DCIM/Camera/RAW10_%d_%d_%d.raw",
...

    {
        if (mLogLevel) {
            char filename[256];
            if (0 > snprintf(filename, sizeof(filename), "/sdcard/DCIM/Camera/RAW16_%d_%d_%d.raw",
