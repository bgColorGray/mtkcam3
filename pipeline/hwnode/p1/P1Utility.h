...
#ifdef P1_STUFF_DEPTH
#undef P1_STUFF_DEPTH
#endif
#define P1_STUFF_DEPTH 2

#ifdef P1_PREALLOC_THRESHOLD
#undef P1_PREALLOC_THRESHOLD
#endif
#define P1_PREALLOC_THRESHOLD (640*480*2)
    ...
    : public virtual android::RefBase
{
    ...
    public:
        enum BUF_STATUS
        {
            BUF_STATUS_AVAILABLE  = 0,
            BUF_STATUS_REFERRED
        };

        struct Info {
        public:
                                Info() : ptr(nullptr), idx(-1) {};
            virtual             ~Info() = default;
            BufCluster*         ptr = nullptr;
            MINT32              idx = (-1);
        };
        ...
        public:
            ...
            virtual             ~Slot() {
                                    if (spImgBuf != nullptr) {
                                        MY_LOGW_IF(
                                            status != BUF_STATUS_AVAILABLE,
                                            "ImgBuf(%p)-NotFree-(%d@%p)",
                                            (void*)spImgBuf.get(), info.idx,
                                            (void*)info.ptr);
                                        spImgBuf = nullptr;
                                    };
                                    //MY_LOGI("BufCluster-Slot END-OK [%s]-(%d)", (info.ptr) ? info.ptr->msName : "NULL-Cluster", info.idx);
                                };
            Info                info;
            BUF_STATUS          status = BUF_STATUS_AVAILABLE;
            sp<IImageBuffer>    spImgBuf;
        };

    public:
                                BufCluster(char const * szPoolName,
                                    MUINT32 uSerialNum,
                                    ImageBufferInfo const& rImgBufInfo)
                                    : mSerialNum(uSerialNum)
                                    , mImgBufInfo(rImgBufInfo)
                                    , spImgBufHeap(nullptr)
                                    , mUsedNum(0)
                                    , mSlotNum(rImgBufInfo.count) {
                                    ::memset(msName, 0, sizeof(msName));
                                    ::memset(msPool, 0, sizeof(msPool));
                                    if (::snprintf(msPool, sizeof(msPool), "%s", szPoolName) < 0) {
                                        MY_LOGW("snprintf msPool error");
                                    }
                                    mvSlotSet.clear();
                                    mvSlotSet.reserve(mSlotNum);
                                    //MY_LOGI("CTR [%s:%d]", msPool, uSerialNum);
                                };
            virtual             ~BufCluster() {
                                    U_if (spImgBufHeap != nullptr ||
                                        mUsedNum != 0) {
                                        MY_LOGW("NOT uninited [%s] used (%d)",
                                            getName(), mUsedNum);
                                    };
                                    mvSlotSet.clear();
                                    spImgBufHeap = nullptr;
                                    //MY_LOGI("DTR [%s]", msName);
                                };
    public:
            MVOID               init(void);
            MVOID               uninit(void);
            MBOOL               acquire(
                                    ...
                                    BufCluster::Info& rInfo);
            MBOOL               release(
                                    sp<IImageBuffer> const& rImgBuf,
                                    BufCluster::Info const& rInfo);
            inline MBOOL        isFull() { return (mUsedNum >= mSlotNum); };
            inline MBOOL        isFree() { return (mUsedNum == 0); };
            inline char const*  getName() { return (char const*)(msName); };
    protected:
            char                msName[512] = {0};
            char                msPool[256] = {0};
            MUINT32             mSerialNum = 0;
            ImageBufferInfo     mImgBufInfo;
            sp<IImageBufferHeap> // might be IIonImageBufferHeap or ...
                                spImgBufHeap;
            MINT32              mUsedNum = 0;
            MUINT32             mSlotNum = 0;
            std::vector<Slot>   mvSlotSet;
...
public:
                            ...
                            , mSecType(secType)
                        {
                            ...
                            mvInfoMap.clear();
                            ::memset(&mClusterImgBufInfo, 0,
                                sizeof(mClusterImgBufInfo));
                            MY_LOGD2("---");
                        };
                        ...
                        StuffBufferPool(
                            ...
                            MUINT32 clusterConfig, // 0:auto, 1:no-need, otherwise:the buf-num of one cluster
                            MINT32  id,
                            MINT32  log,
                            MINT32  logi,
                            ...
                            , mSecType(secType)
                        {
                            ...
                            ::memset(&mClusterImgBufInfo, 0,
                                sizeof(mClusterImgBufInfo));
                            clusterCfgDecision(multiple);
                            MY_LOGI1("[%s] "
                                "[0x%x-%dx%d-%d.%d.%d (%d:%d) *%d +%d] (%d:%d) "
                                "{C:%d-%d-%d} (Ux%x) <S:%d-%d>", szName, format,
                                size.w, size.h, stride0, stride1, stride2,
                                amountBasis, amountMax, multiple, writable,
                                mWaterMark, mMaxAmount,
                                mClusterConfig, mClusterBufNum, mClusterMinCnt,
                                mUsage, mbSecure, mSecType);
                            mvInfoMap.clear();
                            #if (P1_BUF_PRE_ALLOCATION_ENABLE)
                            if (mSize.h * mStride0 <= P1_PREALLOC_THRESHOLD) {
                                std::vector< sp<IImageBuffer> > v;
                                v.reserve(P1_STUFF_DEPTH);
                                for(size_t i = 0; i < P1_STUFF_DEPTH; i++) {
                                    sp<IImageBuffer> buf;
                                    acquireBuffer(buf);
                                    v.push_back(buf);
                                }
    ...
    virtual             ~StuffBufferPool()
                        {
                            ...
                            mvInfoMap.clear();
                            // release cluster after all ImgBuf of mvInfoMap was released
                            size_t cluster_size = mvpCluster.size();
                            for (size_t c = 0; c < cluster_size; c++) {
                                BufCluster* pCluster = mvpCluster[c];
                                if (pCluster != nullptr) {
                                    MY_LOGI2("ClusterCnt(%zu/%d) - uninit[%s]",
                                        mvpCluster.size(), mClusterMinCnt,
                                        pCluster->getName());
                                    pCluster->uninit();
                                    delete pCluster;
                                };
    ...
    MVOID               clusterCreateBuffer(
                            sp<IImageBuffer>& rImgBuf,
                            BufCluster::Info& rInfo);

    MVOID               clusterDestroyBuffer(
                            sp<IImageBuffer> const& rImgBuf,
                            BufCluster::Info const& rInfo);

    MVOID               clusterCfgDecision(MUINT8 multiple = 0);

    inline MBOOL        isUsingCluster() { return (mClusterBufNum > 1); };

private:
    MINT32              mOpenId;
    MINT32              mLogLevel;
    MINT32              mLogLevelI;
    mutable Mutex       mLock; // only use in DTR, acquireBuffer and releaseBuffer
    android::String8    msName;
    MINT32              mFormat;
    MSize               mSize;
    MUINT32             mStride0;
    ...
    DefaultKeyedVector< sp<IImageBuffer>, BufNote >
                        mvInfoMap;
    MUINT32             mClusterConfig; // 0:auto, 1:no-need, otherwise:the buf-num of one cluster // by user configure
    MUINT32             mClusterBufNum; // 0:unknown, 1:disable, otherwise:the buf-num of one cluster
    MUINT32             mClusterMinCnt; // the kept-min-count of cluster in this pool, if cluster-amount > min-count, it start to release free-cluster
    MUINT32             mClusterSN; // cluster serial number
    ImageBufferInfo     mClusterImgBufInfo;
    std::vector< BufCluster* >
                        mvpCluster;
    MBOOL               mbSecure;
    SecType             mSecType;
};
 ...
 *
 ******************************************************************************/
class P1RawInMgr : public virtual android::RefBase
{
public:
    P1RawInMgr(MINT32 openId, MINT32 logLevel, MINT32 logLevelI,
        StuffBufferManager &mgr, MUINT32 pixelMode, MSize size, MINT32 format,
        std::vector<MUINT32> stride, MUINT8 amountBasis, MUINT8 amountMax)
    : mOpenId(openId),
    mLogLevel(logLevel),
    ...
    mFormat(format),
    mvStride(stride),
    mAmountBasis(amountBasis),
    mAmountMax(amountMax)
    {
        if (mpStuffBufMgr)
            mpStuffBufMgr->collectBufferInfo(
                pixelMode,
                MTRUE,/*isFull*/
                format,
                size,
    ...
    MERROR getImageBuffer(sp<IImageBuffer> & imageBuffer);

    MERROR returnImageBuffer(sp<IImageBuffer> & imageBuffer);

private:
    MINT32          mOpenId;
    MINT32          mLogLevel;
    MINT32          mLogLevelI;
    StuffBufferManager* mpStuffBufMgr;
    MSize mSensorSize;
    MINT32 mFormat;
...
public:
    ...
    {   // for new item, please replace one of reservation
        ...
        CcOpTimeout_3AGet,
        CcOpTimeout_ISPGet,
        CcOpTimeout_3AGetCur,
        CcOpTimeout_ISPGetCur,
        CcOpTimeout_Sync,
    ...
    */
    #define ClueCodeListTable \
        ...
        \
        CCLT(CcOpTimeout_3AGet,                               (CC_OP_TIMEOUT),\
            "CcOpTimeout_3AGet",\
            "",\
            CP_3A_GET_BGN, CP_3A_GET_END)\
        \
        CCLT(CcOpTimeout_ISPGet,                               (CC_OP_TIMEOUT),\
            "CcOpTimeout_ISPGet",\
            "",\
            CP_ISP_GET_BGN, CP_ISP_GET_END)\
        \
        CCLT(CcOpTimeout_3AGetCur,                               (CC_OP_TIMEOUT),\
            "CcOpTimeout_3AGetCur",\
            "",\
            CP_3A_GET_CUR_BGN, CP_3A_GET_CUR_END)\
        \
        CCLT(CcOpTimeout_ISPGetCur,                               (CC_OP_TIMEOUT),\
            "CcOpTimeout_ISPGetCur",\
            "",\
            CP_ISP_GET_CUR_BGN, CP_ISP_GET_CUR_END)\
        \
        CCLT(CcOpTimeout_Sync,                               (CC_OP_TIMEOUT),\
            "CcOpTimeout_Sync",\
            "",\
            CP_SYNC_BGN, CP_SYNC_END)\
        \
...
private:
    ...
    class Data {
    public:
        struct timeval  mTv[CP_MAX + 1];
        MINT64          mTime[CP_MAX + 1];
        MUINT32         mTid[CP_MAX + 1];
        //
        MBOOL           mReady;
    ...
    void            bitStr(MUINT64 bitClueCode, android::String8 * str) {
                        ...
                        if (vTag.size() > 0) {
                            ...
                            for (size_t i = 0; i < vTag.size(); i++) {
                                if (vTag[i] >= LogInfo::CP_FIRST && vTag[i] < LogInfo::CP_MAX) {
...
private:
    ...
    Note mNotes[CP_MAX + 1] = {
        ...
        { CP_3A_GET_BGN, "3A.Get+++",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_3A_GET_END, "3A.Get---",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_3A_GET_CUR_BGN, "3A.GetCur+++",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_3A_GET_CUR_END, "3A.GetCur---",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_ISP_GET_BGN, "ISP.Get+++",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_ISP_GET_END, "ISP.Get---",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_ISP_GET_CUR_BGN, "ISP.GetCur+++",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_ISP_GET_CUR_END, "ISP.GetCur---",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_SYNC_BGN, "SYNC.syncResultCheck+++",
            {"MagicNum", "FrameNum", "RequestNum", ""}},

        { CP_SYNC_END, "SYNC.syncResultCheck---",
            {"MagicNum", "FrameNum", "RequestNum", ""}},
