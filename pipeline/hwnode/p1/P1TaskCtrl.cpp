...
P1NodeAct(P1NodeImp * pP1NodeImp, MINT32 id)
    ...
    , dstSize_mono()
    , dstSize_yuv_full()
    , dstSize_yuv_resizer1()
    , dstSize_yuv_resizer2()
    , cropRect_full()
    , cropRect_resizer()
    , cropRect_resizer_orig()
    , cropRect_mono()
    , cropRect_yuv_full()
    , cropRect_yuv_prev()
    ...
    , mspGrouping(NULL)
    , mGroupingNum(0)
    , mGroupingIdx(P1GP_IDX_NULL)
    , mMaxStaggerNum(0)
    , byDeliverReleasing(MFALSE)
    , mbSwtichWithoutWB(MFALSE)
    , mNodeActLock()
    , mbIsReturned(MFALSE)
    , mRawInBufferByStuff(NULL)
    , mRawInBuffer(NULL)
    , mbInSensorZoom(MFALSE)
    , mCap_intent(MTK_CONTROL_CAPTURE_INTENT_CUSTOM)
    , mbFastSwitch()
    , mSensorPattern()
    , mActiveSensorSize()
    , mTargetSensorSize()
    ...
    , mAeTargetMode(MTK_3A_FEATURE_AE_TARGET_MODE_NORMAL)
    , mStaggerType(HDR_NONE)
    , mbStaggerModeSwitched(MFALSE)
    , mbVhdrNeedDrop(MFALSE)
{
    ::memset(portBufIndex, P1_FILL_BYTE, sizeof(portBufIndex));
    if (mpP1NodeImp != NULL) {
        char buffer[16] = {0};
        if (snprintf(buffer, sizeof(buffer), "%s", mpP1NodeImp->getNodeName()) >= 0) {
            mNodeName = buffer;
        } else {
            MY_LOGW("mNodeName snprintf error");
        }
        ...
        mSensorMode = mpP1NodeImp->mSensorParams.mode;
        mTargetSensorSize = mpP1NodeImp->mTargetSensorSize;
        mOffBin = mpP1NodeImp->mOffBin;
    };
...
mapFrameStream()
{
    ...
    mMaxStaggerNum = mpP1NodeImp->mMaxStaggerNum;
    //
    ...
    for (size_t i = 0; i < imageIOMapSet.size(); i++) {
        ...
        if (imageIOMap.vOut.size() > 0) {
            for (size_t j = 0; j < imageIOMap.vOut.size(); j++) {
                ...
                if (mpP1NodeImp->mvStreamImg[STREAM_IMG_OUT_LCS] != NULL &&
                    mpP1NodeImp->mvStreamImg[STREAM_IMG_OUT_LCS]->getStreamId() == streamId &&
                    streamInfo != NULL) {
                    P1NodeImageStreamInfo p1streaminfo;
                    p1streaminfo.info = streamInfo;
                    p1streaminfo.streamImg = STREAM_IMG_OUT_LCS;
                    if(p1streaminfo.isSttExist() == MTRUE) {
                        /* stt exist */
#define REG_STREAM_STT(item, info, p1info)                              \
                        p1info.streamImg = item;                        \
                        if (!streamBufImg[item].bExist &&               \
                            p1info.isSttExist() == MTRUE) {             \
                            streamBufImg[item].bExist |= MTRUE;         \
                            streamBufImg[item].spStreamInfo = info;     \
                            streamBufImg[item].pImgHeapCtrl = &imgHeapCtrlStt;\
                        };
                        REG_STREAM_STT(STREAM_IMG_OUT_LCS, streamInfo, p1streaminfo);
                        REG_STREAM_STT(STREAM_IMG_OUT_LCESHO, streamInfo, p1streaminfo);
#undef REG_STREAM_STT

                    } else {
                        // stt inexist, assign directly
                ...
                if (!streamBufImg[STREAM_IMG_OUT_SEAMLESS_FULL].bExist
                        && mpP1NodeImp->mvStreamImg[STREAM_IMG_OUT_SEAMLESS_FULL] != nullptr
                        && mpP1NodeImp->mvStreamImg[STREAM_IMG_OUT_SEAMLESS_FULL]->getStreamId() == streamId) {
                    streamBufImg[STREAM_IMG_OUT_SEAMLESS_FULL].bExist = MTRUE;
                    streamBufImg[STREAM_IMG_OUT_SEAMLESS_FULL].spStreamInfo = streamInfo;
                    mTargetSensorSize = streamInfo->getImgSize();
                    continue;
                }
                MBOOL found = MFALSE;
                if (EN_STAGGER_MODE) { // (mMaxStaggerNum > 1)
                    #define REG_STREAM_STAGGER(id, item, info)                  \
                    if ((!streamBufImg[item].bExist) &&                         \
                        (mpP1NodeImp->mvStreamImg[item] != NULL) &&             \
                        (mpP1NodeImp->mvStreamImg[item]->getStreamId() == id)) {\
                        streamBufImg[item].bExist |= MTRUE;                     \
                        streamBufImg[item].spStreamInfo = info;                 \
                        streamBufImg[item].pImgHeapCtrl = &imgHeapCtrlStagger;  \
                        found = MTRUE;                                          \
                    };
                    for (auto it : mpP1NodeImp->mvStaggerMap) {
                        if (it.mPortID == NSImageio::NSIspio::EPortIndex_IMGO) {
                            REG_STREAM_STAGGER(streamId, STREAM_IMG_OUT_FULL, streamInfo);
                        }
                        else if (it.mPortID == NSImageio::NSIspio::EPortIndex_CAMSV_IMGO) {
                            REG_STREAM_STAGGER(streamId, STREAM_IMG_OUT_SVIMGO1, streamInfo);
                        }
                        else if (it.mPortID == NSImageio::NSIspio::EPortIndex_CAMSV2_IMGO) {
                            REG_STREAM_STAGGER(streamId, STREAM_IMG_OUT_SVIMGO2, streamInfo);
                        }
                    }
                    if (found) continue;
                    #undef REG_STREAM_STAGGER
                }
                else
                ...
                REG_STREAM_IMG(streamId, STREAM_IMG_OUT_RSS_R2, streamInfo);
                REG_STREAM_IMG(streamId, STREAM_IMG_OUT_YUV_FULL, streamInfo);
                REG_STREAM_IMG(streamId, STREAM_IMG_OUT_YUV_PREV, streamInfo);
    ...
    if (LOGI_LV1) {
        ...
        */
        ...
        for (size_t i = 0; i < imageIOMapSet.size(); i++) {
            ...
            IPipelineFrame::ImageInfoIOMap const& imageIOMap = imageIOMapSet[i];
            U_if (0 > snprintf(strInfo, sizeof(strInfo), "<%zu_I[%zu]{", i,
                imageIOMap.vIn.size())) {
                MY_LOGW("snprintf failed");
            }
            str.append(strInfo);
            ...
            if (imageIOMap.vOut.size() > 0) {
                for (size_t j = 0; j < imageIOMap.vOut.size(); j++) {
                    ...
                    StreamId_T const streamId = imageIOMap.vOut.keyAt(j);
                    U_if (0 > snprintf(strInfo, sizeof(strInfo), "%#" PRIx64 ,
                        streamId)) {
                        MY_LOGW("snprintf failed");
                    }
                    str.append(strInfo);
        ...
        //
        U_if (0 > snprintf(strInfo, sizeof(strInfo), "-Meta[%zu]=",
            metaIOMapSet.size())) {
            MY_LOGW("snprintf failed");
        }
        str.append(strInfo);
        for (size_t i = 0; i < metaIOMapSet.size(); i++) {
            ...
            U_if (0 > snprintf(strInfo, sizeof(strInfo), "}_O[%zu]{",
                metaIOMap.vOut.size())) {
                MY_LOGW("snprintf failed");
            }
            str.append(strInfo);
        ...
        */
        ...
        for (MUINT32 stream = STREAM_ITEM_START; stream < STREAM_IMG_NUM;
            ...
            if (mpP1NodeImp->mvStreamImg[stream] != NULL) {
                ...
                U_if (0 > snprintf(strInfo, sizeof(strInfo), "%s:%#" PRIx64 "(%d)",
        ...
        //
        if (!str.empty())
            msg += String8::format("%s", str.c_str());
    }
    #endif
    //
    return OK;
...
frameImageInit(STREAM_IMG const streamImg)
{
    ...
    SharedImgHeapCtrl* pImgHeapCtrl = streamBufImg[streamImg].pImgHeapCtrl;
    if (pImgHeapCtrl != nullptr) { // this StreamImg use SharedImgHeapCtrl for Heap control
        pImageStreamBuffer = pImgHeapCtrl->getStreamBuf(); // got ImageStreamBuffer while existing
    } // if get null, it means the ImageStreamBuffer is not ready in this SharedImgHeapCtrl, then go-on
    if (pImageStreamBuffer == NULL) { // to get the ImageStreamBuffer
    ...
    L_if (pImageStreamBuffer != NULL) {
        ...
        if (pImgHeapCtrl != nullptr) // must set ImageStreamBuffer for the following heap lock/unlock
            pImgHeapCtrl->setStreamBuf(pImageStreamBuffer);
    ...
    if (pImgHeapCtrl != nullptr) // if SharedImgHeapCtrl is existing, use it to lock heap
        pImageBufferHeap = pImgHeapCtrl->tryLockHeap(getNodeName());
    else // for general case, lock heap by ImageStreamBuffer
        pImageBufferHeap = pImageStreamBuffer->tryWriteLock(getNodeName());
    ...
    MY_LOGD3("ImgInit-frame, " P1INFO_STREAM_STR "BatchEn(%d) BufCnt(%zu) "
        "SImgSBuf(%p_%p) Heap(%p) " P1INFO_ACT_STR, P1INFO_STREAM_VAR(Img),
        streamBufImg[streamImg].bBatchEn,
        streamBufImg[streamImg].sBatchBuf.lockSizeQuery(), // must not be under sBatchBuf.mLock locking
        (void*)(pImgHeapCtrl), ((pImgHeapCtrl != nullptr) ?
        (void*)(pImgHeapCtrl->getStreamBuf().get()) : nullptr),
        (void*)(pImageBufferHeap.get()), P1INFO_ACT_VAR(*this));
    //
    return res;
};
...
frameImageUninit(STREAM_IMG const streamImg)
{
    ...
    */
    ...
    L_if (streamBufImg[streamImg].spStreamHeap != NULL) {
        if (LOGD_LV3)
            logHeap = (MINTPTR)(streamBufImg[streamImg].spStreamHeap.get()); // for log only
        if (pImgHeapCtrl != nullptr) { // if SharedImgHeapCtrl is existing, use it to unlock heap
            pImgHeapCtrl->tryUnLockHeap(getNodeName());
        } else { // for general case, unlock heap by ImageStreamBuffer
    ...
    MY_LOGD3("ImgUninit-frame, " P1INFO_STREAM_STR "BatchEn(%d) BufCnt(%zu) "
        ...
        (void*)(pImgHeapCtrl), ((pImgHeapCtrl != nullptr) ?
        (void*)(pImgHeapCtrl->getStreamBuf().get()) : nullptr),
        (void*)(logHeap), P1INFO_ACT_VAR(*this));
    //
    return res;
};
...
frameImagePut(STREAM_IMG const streamImg, MINT8 const idx)
{
            ...
            if (pImageStreamBuffer == NULL) {
                ...
                if (pImageStreamBuffer == NULL) {
                    MY_LOGI0("null - Check Image " P1INFO_STREAM_STR
                        " in Frame " P1INFO_ACT_STR,
                        P1INFO_STREAM_VAR(Img), P1INFO_ACT_VAR(*this));
                    // acquire StreamBuffer failed, no need to markStatus, just return BAD_VALUE
                    return BAD_VALUE;
                }
            }
            pImageStreamBuffer->markStatus((getFlush() || isErr) ?
    ...
    //
    if (LOGI_LV1 && idx == 0 && !isErr) { // add note in result-string as the first sub-setting
        sp<IImageStreamBuffer> pImageStreamBuffer = NULL;
        pImageStreamBuffer = streamBufImg[streamImg].spStreamBuf;
        if (pImageStreamBuffer != NULL) {
            res += String8::format("<%s:x%x>",
    ...
    //
    if (curLock != STREAM_BUF_LOCK_NONE && !isErr) {
...
poolImageInit(STREAM_IMG const streamImg)
{
    ...
    switch (streamImg) {
        ...
        case STREAM_IMG_OUT_SVIMGO1:
        case STREAM_IMG_OUT_SVIMGO2:
    ...
    MSize const dstSize, sp<IImageBuffer> &rImgBuf)
{
    ...
    if (streamImg == STREAM_IMG_OUT_OPAQUE) {
        ...
        P1NodeImageStreamInfo *streamInfo = NULL;
        P1NodeImageStreamInfo p1streaminfo;
        if (streamBufImg[streamImg].spStreamInfo != NULL) {
            streamInfo = &p1streaminfo;
            streamInfo->info = streamBufImg[streamImg].spStreamInfo;
            streamInfo->streamImg = streamImg;
        } else {
        ...
        MSize bufSize;
        bool isSeamlessStuff =
            mpP1NodeImp->mvStreamImg[STREAM_IMG_OUT_SEAMLESS_FULL] != nullptr
            && streamImg == STREAM_IMG_OUT_FULL
            && dstSize.w != streamInfo->getImgSize().w;
        if (isSeamlessStuff) {
            bufSize = MSize(dstSize_full.w,
                    ((fully) ? dstSize_full.h : dstSize.h));
        } else {
            bufSize = MSize(streamInfo->getImgSize().w,
        ...
        if (reqImgFormat != streamImgFormat || isSeamlessStuff) {
        ...
        } else {  // get stride from stream info.
    ...
    //
    if (rImgBuf != NULL) {
        mpP1NodeImp->mpStuffBufferUserMgr->addBuffer(P1NodeImp::StuffBufferUserMgr::StuffBufferItem{
                                                            .mMagicNum = static_cast<MUINT32>((*this).magicNum),
                                                            .mStreamImg = streamImg,
                                                            .spImgBuf = rImgBuf,
                                                            .mUserCount = 1});
...
stuffImagePut(STREAM_IMG const streamImg, MINT8 const idx)
{
    ...
    //
    if (mpP1NodeImp->mpStuffBufferUserMgr->decUserCount((*this).magicNum, streamImg, pSubImg->spImgBuf->getBufVA(0)) == 0) {
        pSubImg->spImgBuf = NULL;
    }
    //
    pSubImg->eLockState = STREAM_BUF_LOCK_NONE;
...
streamImageInit(STREAM_IMG const streamImg, IMG_BUF_SRC srcType)
{
    ...
    P1NodeImageStreamInfo p1streaminfo;
    P1NodeImageStreamInfo *spStreamInfo;
    if (mpP1NodeImp == nullptr) {
        MY_LOGE("mpP1NodeImp is nullptr");
        return INVALID_OPERATION;
    }
    if(pBufImg->spStreamInfo != NULL) {
        spStreamInfo = & p1streaminfo;
        spStreamInfo->info = pBufImg->spStreamInfo;
        spStreamInfo->streamImg = streamImg;
    } else {
        spStreamInfo = mpP1NodeImp->mvStreamImg[streamImg];
    }
    if (spStreamInfo->info == NULL) {
    ...
    if (EN_STAGGER_MODE && ((streamImg == STREAM_IMG_OUT_FULL) || (streamImg == STREAM_IMG_OUT_SVIMGO1) || (streamImg == STREAM_IMG_OUT_SVIMGO2))) { // (mMaxStaggerNum > 1)
        if (!pBufImg->sBatchBuf.mvMap.empty())
            return OK; // it was assigned by other STREAM_IMG
        U_if (pBufImg->spStreamHeap->getImgFormat() != eImgFmt_BLOB) {
            MY_LOGE("Stagger(%d) ImgFmt(x%x)-ERR", mMaxStaggerNum,
                pBufImg->spStreamHeap->getImgFormat());
            return BAD_VALUE;
        }
        const ImageBufferInfo& info = spStreamInfo->getImageBufferInfo();
        std::vector< IImageBuffer* > vpImageBuffer;
        vpImageBuffer = pBufImg->spStreamHeap->createImageBuffers_FromBlobHeap(
            info, LOG_TAG, MTRUE);
        MUINT8 size = vpImageBuffer.size();
        U_if (mMaxStaggerNum > size) {
            MY_LOGE("Stagger(%d) size(%d)-ERR", mMaxStaggerNum,
                size);
            return BAD_VALUE;
        }

        for (MUINT8 idx = 0; idx < mMaxStaggerNum; idx++) {
            STREAM_IMG staggerImg = STREAM_UNKNOWN;
            // [FIXME]  assume blob order will always as NE/ME/SE
            //          get buffer by IMG_STREAM order so that IMGO will always use NE
            if (mValidExpNum == 1) {
                switch(idx) {
                    case 0:
                        staggerImg = STREAM_IMG_OUT_FULL;
                        break;
                    case 1:
                        staggerImg = STREAM_IMG_OUT_SVIMGO1;
                        break;
                    case 2:
                        staggerImg = STREAM_IMG_OUT_SVIMGO2;
                        break;
                    default:
                        break;
                }
            } else {  // get buffer by port map
                switch(mpP1NodeImp->mvStaggerMap.at(idx).mPortID) {
            ...
            L_if (staggerImg != STREAM_UNKNOWN) {
                ActStreamImg * pStaggerBufImg = (srcType == IMG_BUF_SRC_FRAME) ?
                    (&(streamBufImg[staggerImg])) : (&(poolBufImg[staggerImg]));
                auto bufIdx = mpP1NodeImp->mvStaggerMap.at(idx).mBufIdx;
                sp<IImageBuffer> spImgBuf = vpImageBuffer[bufIdx];
                MY_LOGI3(P1INFO_STREAM_STR " " P1INFO_ACT_STR "S:%d Idx[%d/%d] "
                    "ImgBuf(%p)", P1INFO_STREAM_VAR(Img), P1INFO_ACT_VAR(*this),
                    mMaxStaggerNum, bufIdx, mMaxStaggerNum, spImgBuf.get());
                note.spImgBuf = spImgBuf;
                pStaggerBufImg->bBatchEn = MFALSE;
                pStaggerBufImg->sBatchBuf.mvMap.push_back(note);
            }
        }
    } else if (pBufImg->spStreamHeap->getImgFormat() == eImgFmt_BLOB) {
...
MINT
P1TaskCollector::
requireAct(P1QueAct & rGetAct, MBOOL fromLast)
{
    ...
    Mutex::Autolock _l(mCollectorLock);
    if (!mvqActRoll.empty()) {
        auto it = fromLast ? std::prev(mvqActRoll.end()) : mvqActRoll.begin();
        //P1QueAct qAct = (*it);
