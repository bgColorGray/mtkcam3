...
)
{
    ...
    if (querySrcRect.size().w < bufferSize.w) {
        ...
        if  ( auto pModule = getNormalPipeModule() )
        {
            NSCam::NSIoPipe::NSCamIOPipe::NormalPipe_QueryInfo info;
            pModule->query(
                isHalRawFormat((EImageFormat)imageFormat) ? NSCam::NSIoPipe::PORT_RRZO.index : NSCam::NSIoPipe::PORT_YUVO.index,
...
)
{
    ...
    if (pMeta->count() == 0) {
        // str.clear();
        U_if (0 > snprintf(strInfo, sizeof(strInfo), "%s metadata.count(0)", pInfo)) {
            MY_LOGW("snprintf failed");
        }
        str.append(strInfo);
        if (tag > 0) {
            U_if (0 > snprintf(strInfo, sizeof(strInfo),
                "- while find MetaTag[0x%" PRIX64 "=%" PRId64 "]", tag, tag)) {
                MY_LOGW("snprintf failed");
            }
            str.append(strInfo);
        }
    ...
    for (MUINT32 i = 0; i < pMeta->count(); i++) {
        if (tag != 0) {
            if (tag == pMeta->entryAt(i).tag()) {
                ...
                generateMetaInfoStr(pMeta->entryAt(i), str8);
                MY_LOGI("%s", str.c_str());
                break;
            }
            continue;
        ...
        if (cnt == 0) {
            ...
            // str.clear();
            str = "";
            U_if (0 > snprintf(strInfo, sizeof(strInfo), "%s [%03d~%03d/%03d] ",
    ...
    if ((tag != 0) && (!found)) {
        MY_LOGI("%s NotFound-MetaTag[0x%" PRIX64 "=%" PRId64 "]",
                pInfo, tag, tag);
    }
    // str.clear();
    str = "";
...
createBuffer(sp<IImageBuffer> & imageBuffer)
{
    ...
    sp<IImageBuffer> pImgBuf = NULL;
    BufCluster::Info clusterInfo;

    // add information to buffer name
    // android::String8 imgBufName = android::String8(msName);
    std::string imgBufName = msName.string();
    char str[256] = {0};
    U_if (0 > snprintf(str, sizeof(str), ":Size%dx%d:Stride%d.%d.%d:Sn%d",
        mSize.w, mSize.h, mStride0, mStride1, mStride2, ++mSerialNum)
    ) {
        MY_LOGW("snprintf failed");
    }
    imgBufName.append(str);
    //
    ...
    if (isUsingCluster()) {
        clusterCreateBuffer(pImgBuf, clusterInfo);
    } else { // without cluster
    // create buffer
    ...
    IImageBufferAllocator::ImgParam imgParam =
        IImageBufferAllocator::ImgParam((EImageFormat)mFormat,
                mSize, bufStridesInBytes, bufBoundaryInBytes,
                    (size_t)mPlaneCnt);
    if (!mbSecure) {
        pHeap =
            IIonImageBufferHeap::create(imgBufName.c_str(), imgParam);
        if (pHeap == NULL) {
            MY_LOGE("[%s] Stuff ImageBufferHeap create fail",
                imgBufName.c_str());
            return BAD_VALUE;
        }
        ...
        if (!imgBufName.empty()) {
            pSecHeap =
            ISecureImageBufferHeap::create(imgBufName.c_str(), imgParam,
    ...
    android::String8 str8(imgBufName.c_str());
    BufNote bufNote(str8, BUF_STATE_ACQUIRED, clusterInfo);
    mvInfoMap.add(pImgBuf, bufNote);
    imageBuffer = pImgBuf;
    //
    MY_LOGD2("StuffBuffer-Create (%s) (%zu/%d:%d) "
        "ImgBuf(%p)(0x%X)(%dx%d,%zu,%zu)(P:0x%zx)(V:0x%zx) (isSecure:%d)",
        imgBufName.c_str(), mvInfoMap.size(), mWaterMark, mMaxAmount,
...
destroyBuffer(size_t index)
{
    ...
    if (isUsingCluster()) {
        clusterDestroyBuffer(pImageBuffer, bufNote.mInfo);
    }
    // destroy buffer
...
StuffBufferPool::
clusterCreateBuffer(
    sp<IImageBuffer>& rImgBuf, BufCluster::Info& rInfo)
{
    FUNCTION_IN;
    //
    rImgBuf = nullptr;
    //
    size_t cluster_size = mvpCluster.size();
    for (size_t c = 0; c < cluster_size; c++) {
        BufCluster* pCluster = mvpCluster[c];
        if (pCluster && (!pCluster->isFull())) {
            if (pCluster->acquire(rImgBuf, rInfo)) {
                MY_LOGI3("acquire ImgBuf(%p) - Cluster[%s]",
                    (void*)(rImgBuf.get()), pCluster->getName());
                break;
            }
        }
    }
    if (rImgBuf == nullptr) { // need to create a Cluster
        mClusterSN ++;
        BufCluster* pCluster = new BufCluster(msName.string(), mClusterSN,
            mClusterImgBufInfo);
        if (pCluster == nullptr) {
            MY_LOGE("CANNOT new Cluster");
            return;
        };
        mvpCluster.push_back(pCluster);
        pCluster->init();
        MY_LOGI2("ClusterCount(%zu/%d) after init[%s]",
            mvpCluster.size(), mClusterMinCnt, pCluster->getName());
        if (!pCluster->acquire(rImgBuf, rInfo)) {
            MY_LOGE("CANNOT acquire ImgBuf - Cluster[%s]", pCluster->getName());
            return;
        }
        MY_LOGI3("acquire ImgBuf(%p) - NewCluster[%s]",
                    (void*)(rImgBuf.get()), pCluster->getName());
    }
    //
    FUNCTION_OUT;
...
MVOID
StuffBufferPool::
clusterDestroyBuffer(
    sp<IImageBuffer> const& rImgBuf, BufCluster::Info const& rInfo)
{
    FUNCTION_IN;
    //
    BufCluster* pCluster = rInfo.ptr;
    U_if (pCluster == nullptr) {
        MY_LOGE("CANNOT get Cluster from BufCluster::Info");
        return;
    }
    MY_LOGI3("release ImgBuf(%p) - Cluster[%s]",
        (void*)(rImgBuf.get()), pCluster->getName());
    if (!pCluster->release(rImgBuf, rInfo))
        MY_LOGW("CANNOT release ImgBuf(%p) - Cluster[%s]",
            (void*)(rImgBuf.get()), pCluster->getName());
    if (mvpCluster.size() > mClusterMinCnt) { // tyr to remove cluster while cluster amount > mClusterMinCnt
        std::vector< BufCluster* >::iterator it = mvpCluster.end();
        std::vector< BufCluster* >::iterator it_del = it; // assign the cluster which should be deleted
        it --; // seek from END to BEGIN
        MBOOL check_more = MTRUE;
        while (mvpCluster.size() > mClusterMinCnt && check_more) {
            it_del = mvpCluster.end(); // reset/clear it_del
            BufCluster* pCheckCluster = (*it);
            if (pCheckCluster && pCheckCluster->isFree()) { // delete cluster as no-slot-is-referred
                MY_LOGI2("ClusterCount(%zu/%d) before uninit[%s]",
                    mvpCluster.size(), mClusterMinCnt, pCluster->getName());
                pCheckCluster->uninit();
                it_del = it; // assign it_del for later deleting
            }
            if (it == mvpCluster.begin())
                check_more = MFALSE;
            else
                it --;
            if (it_del != mvpCluster.end()) { // it_del was assigned
                BufCluster* pDelCluster = (*it_del);
                if (pDelCluster != nullptr)
                    delete pDelCluster;
                mvpCluster.erase(it_del);
            }
        }
    }
    //
...
MVOID
StuffBufferPool::
clusterCfgDecision(MUINT8 multiple)
{
    FUNCTION_IN;
    //
    if (mbSecure) { // currently, not support cluster buffer while Secure
        mClusterBufNum = 0;
        return;
    };
    if (mClusterConfig > 1) {
        mClusterBufNum = mClusterConfig;
    } else if (mClusterConfig == 0) { // auto
        MUINT32 const pixNum = (mSize.w * mSize.h);
        MUINT32 const pix_L = (409600); // 2560*160=640*640=409600
        MUINT32 const pix_S = (pix_L >> 1);
        if ((multiple > 1) || (pixNum <= pix_L)) { // 2560*160=640*640=409600
            if (multiple > 1)
                mClusterBufNum = multiple;
            else {
                if (pixNum <= pix_S)
                    mClusterBufNum = mWaterMark;
                else
                    mClusterBufNum = (mWaterMark >> 1);
            };
        }
    }
    if (mClusterBufNum > 1) { // cluster enabled
        // decide min-count
        if (multiple >= mClusterBufNum) // imply (multiple > 1)
            mClusterMinCnt = std::max(mClusterMinCnt,
                (MUINT32)(P1NODE_DEF_PROCESS_DEPTH + 1));
        else
            mClusterMinCnt = (mWaterMark / mClusterBufNum) +
                ((mWaterMark % mClusterBufNum) > 0 ? 1 : 0);
        // calculate Cluster Heap size/length
        NSCam::BufPlanes bufPlanes;
        ::memset(&bufPlanes, 0, sizeof(bufPlanes));
        bufPlanes.count = mPlaneCnt;
        size_t stride[3] = {mStride0, mStride1, mStride2};
        size_t planeCnt = std::min(bufPlanes.count, (size_t)(3));
        mClusterImgBufInfo.bufStep = 0;
        for (size_t p = 0; p < planeCnt; p++) {
            bufPlanes.planes[p].rowStrideInBytes = stride[p];
            bufPlanes.planes[p].sizeInBytes = stride[p] * (mSize.h);
            mClusterImgBufInfo.bufStep += bufPlanes.planes[p].sizeInBytes;
        };
        #if 1 // step align 16
        mClusterImgBufInfo.bufStep =
            ALIGN_UPPER(mClusterImgBufInfo.bufStep, 16);
        #endif
        mClusterImgBufInfo.bufPlanes = bufPlanes;
        mClusterImgBufInfo.count = mClusterBufNum;
        mClusterImgBufInfo.startOffset = 0;
        mClusterImgBufInfo.imgFormat = mFormat;
        mClusterImgBufInfo.imgWidth = mSize.w;
        mClusterImgBufInfo.imgHeight = mSize.h;
        MY_LOGI1("[%s] UseCluster Pool[Multiple:%d-WM(%d:%d)] "
            "Cluster[BufNum:%d-MinCnt:%d-Cfg:%d] "
            "ImgBufInfo[Cnt:%zu-Step:%zu-Size:%dx%d-Fmt:x%x-"
            "Plane(%zu)(%zu.%zu.%zu)(%zu+%zu+%zu)]",
            msName.string(), multiple, mWaterMark, mMaxAmount,
            mClusterBufNum, mClusterMinCnt, mClusterConfig,
            mClusterImgBufInfo.count, mClusterImgBufInfo.bufStep,
            mClusterImgBufInfo.imgWidth, mClusterImgBufInfo.imgHeight,
            mClusterImgBufInfo.imgFormat, mClusterImgBufInfo.bufPlanes.count,
            mClusterImgBufInfo.bufPlanes.planes[0].rowStrideInBytes,
            mClusterImgBufInfo.bufPlanes.planes[1].rowStrideInBytes,
            mClusterImgBufInfo.bufPlanes.planes[2].rowStrideInBytes,
            mClusterImgBufInfo.bufPlanes.planes[0].sizeInBytes,
            mClusterImgBufInfo.bufPlanes.planes[1].sizeInBytes,
            mClusterImgBufInfo.bufPlanes.planes[2].sizeInBytes);
    };
    //
    FUNCTION_OUT;
    return;
 ...
 ******************************************************************************/
MVOID
StuffBufferPool::BufCluster::
init()
{
    // create buffer
    MUINT32 mem_len = mImgBufInfo.bufStep * mImgBufInfo.count;
    MINT32 bufBoundaryInBytes[3] = {0, 0, 0};
    MUINT32 bufStridesInBytes[3] = {mem_len, 0, 0};
    MSize heapSize = MSize(mem_len, 1);
    U_if (0 > snprintf(msName, sizeof(msName),
        "Stuff:%s:%d_Blob:%d_Slot:%zu*%zu_"
        "Fmt:x%x_Img:%dx%d_S:%zu.%zu.%zu", msPool, mSerialNum, mem_len,
        mImgBufInfo.bufStep, mImgBufInfo.count,
        mImgBufInfo.imgFormat,mImgBufInfo.imgWidth, mImgBufInfo.imgHeight,
        mImgBufInfo.bufPlanes.planes[0].rowStrideInBytes,
        mImgBufInfo.bufPlanes.planes[1].rowStrideInBytes,
        mImgBufInfo.bufPlanes.planes[2].rowStrideInBytes)) {
        MY_LOGW("snprintf failed");
    }
    IImageBufferAllocator::ImgParam imgParam =
        IImageBufferAllocator::ImgParam((EImageFormat)eImgFmt_BLOB,
        heapSize, bufStridesInBytes, bufBoundaryInBytes, 1);
    spImgBufHeap = nullptr;
    spImgBufHeap = IIonImageBufferHeap::create(getName(), imgParam);
    U_if (spImgBufHeap == nullptr) {
        MY_LOGE("[%s] Stuff ImageBufferHeap cluster fail", getName());
        return;
    }
    std::vector<IImageBuffer*> vImgBuf = spImgBufHeap->
        createImageBuffers_FromBlobHeap(mImgBufInfo, LOG_TAG);
    size_t got_size = vImgBuf.size();
    U_if (got_size == 0 || got_size != mSlotNum) {
        MY_LOGE("[%s] Stuff cluster got_size(%zu) != SlotNum(%d)",
            getName(), got_size, mSlotNum);
        return;
    }
    for (size_t s = 0; s < got_size; s++) {
        BufCluster::Slot slot;
        slot.spImgBuf = vImgBuf[s];
        slot.status = BufCluster::BUF_STATUS_AVAILABLE;
        slot.info.ptr = this;
        slot.info.idx = s;
        mvSlotSet.push_back(slot);
    }
    return;
};
...
uninit()
{
    size_t slot_size = mvSlotSet.size();
    for (size_t s = 0; s < slot_size; s++) {
        BufCluster::Slot &slot = mvSlotSet[s];
        if (slot.status == BufCluster::BUF_STATUS_REFERRED) {
            MY_LOGW("[%s] slot[%zu] REFERRED:%d", getName(), s, mUsedNum);
            continue;
        }
        slot.spImgBuf = nullptr;
    }
    spImgBufHeap = nullptr;
    return;
};
    ...
    sp<IImageBuffer>& rImgBuf, BufCluster::Info& rInfo)
{
    ...
    for (size_t s = 0; s < slot_size; s++) {
        BufCluster::Slot &slot = mvSlotSet[s];
        if (slot.status == BufCluster::BUF_STATUS_AVAILABLE) {
            mUsedNum ++;
            slot.status = BufCluster::BUF_STATUS_REFERRED;
            rImgBuf = slot.spImgBuf;
            rInfo = slot.info;
            break;
        }
    }
...
release(
    sp<IImageBuffer> const& rImgBuf, BufCluster::Info const& rInfo)
{
    if (rInfo.ptr != this || rInfo.idx >= mSlotNum) {
        MY_LOGW("ClusterSlotInfo-Incorrect Info(%p,%d) Cluster(%p,%d)",
            (void*)(rInfo.ptr), rInfo.idx, (void*)(this), mSlotNum);
        return MFALSE;
    }
    BufCluster::Slot& slot = mvSlotSet[rInfo.idx];
    if (rImgBuf.get() != slot.spImgBuf.get())
        return MFALSE;
    mUsedNum --;
    slot.status = BufCluster::BUF_STATUS_AVAILABLE;
    //
    ...
    MUINT8 multiple, MBOOL writable, MBOOL isSecure, SecType secType)
{
    ...
    if (bufPool == NULL) {
        sp<StuffBufferPool> newPool = new StuffBufferPool(szName, format, size,
            ...
            multiple, writable, 0,/*0:auto*/
            mOpenId, mLogLevel, mLogLevelI, isSecure, secType);
        U_if (newPool == NULL) {
...
MERROR
P1RawInMgr::
getImageBuffer(sp<IImageBuffer> & imageBuffer)
{
    FUNCTION_IN;
    if (!mpStuffBufMgr) {
        MY_LOGW("mpStuffBufMgr not exist");
        return BAD_VALUE;
    }
    char const *szName = "Hal:Image:P1:RAWIN";
    mpStuffBufMgr->acquireStoreBuffer(imageBuffer, szName, mFormat, mSensorSize,
        mvStride, mAmountBasis, mAmountMax, 1/*multiple*/,
        MFALSE/*writable*/, MFALSE/*isSecure*/, SecType::mem_normal);
    if (imageBuffer == NULL) {
        MY_LOGW("RAWI Stuff ImageBuffer not exist");
        return BAD_VALUE;
    }
    MY_LOGI3("acquire ImgBuf:%p", (void*)imageBuffer.get());
    FUNCTION_OUT;
    return OK;
}
...
MERROR
P1RawInMgr::
returnImageBuffer(sp<IImageBuffer> & imageBuffer)
{
    FUNCTION_IN;
    if (!mpStuffBufMgr) {
        MY_LOGW("mpStuffBufMgr not exist");
    ...
    MY_LOGI3("relsase ImgBuf:%p", (void*)imageBuffer.get());
    mpStuffBufMgr->releaseStoreBuffer(imageBuffer);
    FUNCTION_OUT;
    return OK;
}
        ...
        if (ctrl == MTK_P1_SENSOR_STATUS_SW_STANDBY ||
            ...
            switch (mState) {
                case STATE_SUS_READY:
                    while (mStateCond.waitRelative(mLock, 1000000000) != NO_ERROR) {
                        MY_LOGW("mStateCond waiting");
                    }
                    [[fallthrough]];
                case STATE_SUS_DONE:
 ...
 ******************************************************************************/
MBOOL
HardwareStateControl::checkSusReady(void)
{
    if (!isActive()) {
        return MFALSE;
    }
...
get(std::string& rStr) {
    ...
    if ((tm = localtime(&(mLastTv.tv_sec))) == NULL) {
        U_if(0 > snprintf(date_time, sizeof(date_time),
            "%s", "NO_LOCAL_TIME")) {
            MY_LOGW("snprintf failed");
        }
    } else {
        if (strftime(date_time, sizeof(date_time), "%H:%M:%S", tm) == 0) {
            MY_LOGW("strftime failed");
        }
    }
    U_if(0 > snprintf(strInfo, sizeof(strInfo),
        " [Last-Frame-Num(%d_%s.%06ld@%05d) ",
    ...
    MINT64 param0, MINT64 param1, MINT64 param2, MINT64 param3)
{
    ...
    if (cp >= LogInfo::CP_FIRST && cp < LogInfo::CP_MAX) {
...
read(LogInfo::CheckPoint cp, android::String8 * str)
{
    ...
    if (cp < LogInfo::CP_MAX && str != NULL) {
        ...
        if((tm = localtime(&(mSlots[cp].mTv.tv_sec))) == NULL) {
            U_if (0 > snprintf(date_time, sizeof(date_time), "%s", "NO_LOCAL_TIME")) {
                MY_LOGW("snprintf failed");
            }
        } else {
            if (strftime(date_time, sizeof(date_time), "%H:%M:%S", tm) == 0) {
...
analyze(MBOOL bForceToPrint)
{
    ...
    CHECK_OP(CP_SYNC_BGN, CP_SYNC_END, CcOpTimeout_Sync);
    //
