...
#define SUPPORT_CONFIRM_BUF_VA (P1NODE_SUPPORT_CONFIRM_BUF_VA)
...
#define USING_DRV_SET_YUV_CBFP_EXP_SKIP (P1NODE_USING_DRV_SET_YUV_CBFP_EXP_SKIP)
...
#define USING_DRV_SET_RSS_R2_CBFP_EXP_SKIP (P1NODE_USING_DRV_SET_RSS_R2_CBFP_EXP_SKIP)
...
#define P1_LOG                                                      \
    char strLog[P1_LOG_LEN] = {0};                                  \
    U_if (0 > snprintf(strLog, P1_LOG_LEN,                          \
...
#define P1INFO_STREAM_VAR(T) \
    (((mpP1NodeImp == NULL) || ((stream##T) < 0 || (stream##T) >= \
...
#undef  P1TASKGROUPING_STR
#undef  P1TASKGROUPING_VAR
#undef  P1TASKGROUPING_LOG
#define P1TASKGROUPING_STR      "Grouping(%d)(Cnt%d.Done%d.Flush%d.Close%d.NoteX%X)-[%d/%d]={%d:%d}"
#define P1TASKGROUPING_VAR(idx) mKeyId, mCnt, mDone, mIsGroupingFlush, mIsGroupingClose, mNote, idx, mSize, mSet[idx].qAct.id(), mSet[idx].state
#define P1TASKGROUPING_LOG(idx) MY_LOGD3(P1TASKGROUPING_STR, P1TASKGROUPING_VAR(idx));
...
#define P1_PORT_TO_STR(PortID)\
    ...
    ((PortID.index == PORT_RSSO_R2.index) ? "RSS_R2" :\
    ...
    ((PortID.index == PORT_LCESHO.index) ? "LCESHO" :\
    ((PortID.index == PORT_CAMSV_IMGO.index) ? "SVIMGO1" :\
    ((PortID.index == PORT_CAMSV2_IMGO.index) ? "SVIMGO2" :\
     "UNKNOWN"))))))))))))
//
#ifdef P1_RECT_STR
#undef P1_RECT_STR
...
//
#ifdef EN_GROUPING_MODE
#undef EN_GROUPING_MODE
#endif
#define EN_GROUPING_MODE (mMaxGroupingNum > 1)
//
//
#ifdef EN_STAGGER_MODE
#undef EN_STAGGER_MODE
#endif
#define EN_STAGGER_MODE (mMaxStaggerNum > 1)
//
#ifdef EN_RAWIN_FUS
#undef EN_RAWIN_FUS
#endif
#define EN_RAWIN_FUS (mP1RawInFus)
//
...
#undef CROP_RESIZE
#endif
#define CROP_RESIZE(origCrop, afterSize, beforeSize) \
    { \
        origCrop.p.x = ALIGN_UPPER((origCrop.p.x * afterSize.w / beforeSize.w), 4); \
        origCrop.p.y = ALIGN_UPPER((origCrop.p.y * afterSize.h / beforeSize.h), 4); \
        origCrop.s.w = ALIGN_UPPER((origCrop.s.w * afterSize.w / beforeSize.w), 4); \
        origCrop.s.h = ALIGN_UPPER((origCrop.s.h * afterSize.h / beforeSize.h), 4); \
    }
...
//
#ifdef P1_IMGO_TYPE_NOT_SET
#undef P1_IMGO_TYPE_NOT_SET
#endif
#define P1_IMGO_TYPE_NOT_SET ((MINT32)(-1)) // reference: NSCam::NSIoPipe::NSCamIOPipe::EPipeRawFmt
//
...
#define P1_LOG_META(act, meta, info)\
    if (mMetaLogOp > 0) {\
        char strLog[P1_LOG_LEN] = {0};\
        U_if (0 > snprintf(strLog, P1_LOG_LEN,\
            "[P1Meta][%s][Cam::%d]" P1NUM_ACT_STR "",\
            info, getOpenId(), P1NUM_ACT_VAR(act))) {\
            MY_LOGW("snprintf failed");\
        }\
        logMeta(mMetaLogOp, meta, strLog, mMetaLogTag);\
    };

#ifdef P1_BUF_PRE_ALLOCATION_ENABLE
#undef P1_BUF_PRE_ALLOCATION_ENABLE
#endif
#define P1_BUF_PRE_ALLOCATION_ENABLE (1)
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//
enum GROUPING_MSTREAM2PHASE_IDX
{
    GROUPING_MSTREAM2PHASE_IDX_1ST  = 0,
    //
    GROUPING_MSTREAM2PHASE_IDX_SE   = GROUPING_MSTREAM2PHASE_IDX_1ST,
    GROUPING_MSTREAM2PHASE_IDX_NE,
    GROUPING_MSTREAM2PHASE_IDX_LE   = GROUPING_MSTREAM2PHASE_IDX_NE,
    //
    GROUPING_MSTREAM2PHASE_IDX_MAX,
    GROUPING_MSTREAM2PHASE_IDX_NULL = ((MUINT8)(0xFF))
};
static_assert((GROUPING_MSTREAM2PHASE_IDX_MAX < GROUPING_MSTREAM2PHASE_IDX_NULL), "GROUPING_MSTREAM2PHASE_IDX_MAX >= GROUPING_MSTREAM2PHASE_IDX_NULL");
#define GROUPING_MSTREAM2PHASE_NUM_MAX (GROUPING_MSTREAM2PHASE_IDX_MAX)
#define GROUPING_MSTREAM1PHASE_NUM_MAX ((MUINT8)(1))
#define GROUPING_MSTREAM1PHASE_IDX_DEF ((MUINT8)(0))
//
#ifdef GROUPING_NOTE_NONE
#undef GROUPING_NOTE_NONE
#endif
#define GROUPING_NOTE_NONE          ((MUINT32)(0x0))
enum GROUPING_NOTE
{
    GROUPING_NOTE_UNKNOWN           = 0,
    GROUPING_NOTE_CHECK_ON_HANDLE_FLUSHING,
    GROUPING_NOTE_MISSING_BY_FIRST_RECEIVING,
    GROUPING_NOTE_REMAINING_AS_SIZE_CHANGING,
    GROUPING_NOTE_REMAINING_AS_REPROCESS_YUV_PADDING,
    GROUPING_NOTE_RELEASING_DIRECTLY,
    //
    GROUPING_NOTE_TOTAL,
};
static_assert((GROUPING_NOTE_TOTAL <= 32), "GROUPING_NOTE_TOTAL > 32");
#ifdef G_NOTE
#undef G_NOTE
#endif
#define G_NOTE(note)                ((MUINT32)(0x1 << note))
//
enum ACT_TYPE
{
    ACT_TYPE_NULL                   = 0,
...
{                                   // @ bit
    ...
    REQ_OUT_RSSO_R2,        //10    // 0x 0400
    REQ_OUT_RSSO_R2_STUFF,  //11    // 0x 0800
    REQ_OUT_YUV_FULL,       //12    // 0x 1000
    REQ_OUT_YUV_FULL_STUFF, //13    // 0x 2000
    REQ_OUT_YUV_PREV,       //14    // 0x 4000
    REQ_OUT_YUV_PREV_STUFF, //15    // 0x 8000
    REQ_OUT_YUV_RESIZER1,           //16     // 0x 10000
    REQ_OUT_YUV_RESIZER1_STUFF,     //17     // 0x 20000
    REQ_OUT_YUV_RESIZER2,           //18     // 0x 40000
    REQ_OUT_YUV_RESIZER2_STUFF,     //19     // 0x 80000
    REQ_OUT_LCESHO,                 //20     // 0x 100000
    REQ_OUT_LCESHO_STUFF,           //21     // 0x 200000
    REQ_OUT_SVIMGO1,                //22     // 0x 400000
    REQ_OUT_SVIMGO1_STUFF,          //23     // 0x 800000
    REQ_OUT_SVIMGO2,                //24     // 0x 1000000
    REQ_OUT_SVIMGO2_STUFF,          //25     // 0x 2000000
    REQ_OUT_MAX
};
static_assert((REQ_OUT_MAX <= 32), "REQ_OUT_MAX > 32");
//
#ifdef EXP_REC
#undef EXP_REC
#endif
...
enum EXP_EVT
{
    ...
    EXP_EVT_NOBUF_LCSO,
    EXP_EVT_NOBUF_RSSO,
    EXP_EVT_NOBUF_RSSO_R2,
    EXP_EVT_NOBUF_YUVO,
    EXP_EVT_NOBUF_YUVO_PREV,
    EXP_EVT_NOBUF_CRZ1O,
    EXP_EVT_NOBUF_CRZ2O,
    EXP_EVT_NOBUF_LCESHO,
    EXP_EVT_NOBUF_SVIMGO1,
    EXP_EVT_NOBUF_SVIMGO2,
    EXP_EVT_MAX
};
static_assert((EXP_EVT_MAX <= 32), "EXP_EVT_MAX > 32");
//
...
enum P1_OUTPUT_PORT
{
    ...
    P1_OUTPUT_PORT_RSSO_R2, //5
    P1_OUTPUT_PORT_YUVO,    //6
    P1_OUTPUT_PORT_CRZ1O,   //7
    P1_OUTPUT_PORT_CRZ2O,   //8
    P1_OUTPUT_PORT_LCESHO,  //9
    P1_OUTPUT_PORT_RAWI,    //10
    P1_OUTPUT_PORT_SVIMGO1, //11
    P1_OUTPUT_PORT_SVIMGO2, //12
    P1_OUTPUT_PORT_TOTAL    // (max:32) for CONFIG_PORT (MUINT32)
};
static_assert((P1_OUTPUT_PORT_TOTAL <= 32), "P1_OUTPUT_PORT_TOTAL > 32");
//
#ifdef IS_PORT
#undef IS_PORT
#endif
...
enum CONFIG_PORT
{
    ...
    CONFIG_PORT_RSSO_R2     = (0x1 << P1_OUTPUT_PORT_RSSO_R2), // 0x20
    CONFIG_PORT_YUVO        = (0x1 << P1_OUTPUT_PORT_YUVO), // 0x40
    CONFIG_PORT_CRZ1O       = (0x1 << P1_OUTPUT_PORT_CRZ1O), // 0x80
    CONFIG_PORT_CRZ2O       = (0x1 << P1_OUTPUT_PORT_CRZ2O), // 0x100
    CONFIG_PORT_LCESHO      = (0x1 << P1_OUTPUT_PORT_LCESHO), // 0x200
    CONFIG_PORT_RAWI        = (0x1 << P1_OUTPUT_PORT_RAWI), // 0x400
    CONFIG_PORT_SVIMGO1     = (0x1 << P1_OUTPUT_PORT_SVIMGO1), // 0x800
    CONFIG_PORT_SVIMGO2     = (0x1 << P1_OUTPUT_PORT_SVIMGO2), // 0x1000
...
enum P1_FLUSH_REASON
{
    ...
    P1_FLUSH_REASON_MISMATCH_SYNC,      //19
    P1_FLUSH_REASON_MISMATCH_GROUPING,  //20
    P1_FLUSH_REASON_MISMATCH_UNRELIABLE,//21
    P1_FLUSH_REASON_TOTAL // (max:32) for FLUSH_TYPE (MUINT32)
};
static_assert((P1_FLUSH_REASON_TOTAL <= 32), "P1_FLUSH_REASON_TOTAL > 32");
#ifdef IS_FLUSH
#undef IS_FLUSH
#endif
...
enum FLUSH_TYPE
{
    ...
    FLUSH_MIS_GROUPING  = (0x1 << P1_FLUSH_REASON_MISMATCH_GROUPING),           // 0x100000
    FLUSH_MIS_UNRELIABLE= (0x1 << P1_FLUSH_REASON_MISMATCH_UNRELIABLE),         // 0x200000
...
//
enum STREAM_IMG: MUINT32
{
    STREAM_IMG_IN_RAW           = STREAM_ITEM_START,
    STREAM_IMG_IN_YUV,
    STREAM_IMG_IN_OPAQUE,
    STREAM_IMG_OUT_START,
    STREAM_IMG_OUT_OPAQUE       = STREAM_IMG_OUT_START,
    STREAM_IMG_OUT_SEAMLESS_FULL,
    STREAM_IMG_OUT_FULL,
    STREAM_IMG_OUT_FULL_APP,
    STREAM_IMG_OUT_RESIZE,
    STREAM_IMG_OUT_LCS,
    STREAM_IMG_OUT_LCESHO,
    STREAM_IMG_OUT_RSS,
    STREAM_IMG_OUT_RSS_R2,
    STREAM_IMG_OUT_YUV_FULL,
    STREAM_IMG_OUT_YUV_PREV,
    STREAM_IMG_OUT_YUV_RESIZER1,
    STREAM_IMG_OUT_YUV_RESIZER2,
    STREAM_IMG_OUT_SVIMGO1,
    STREAM_IMG_OUT_SVIMGO2,
    STREAM_IMG_NUM,
    STREAM_UNKNOWN              = STREAM_IMG_NUM
