...
queue(android::sp<IPipelineFrame> pFrame)
{
    ...
    //
    MY_LOGD("- Request queue size(%d)", (int)size);
    return OK;
}
...
)
{
    ...
    if(pInImageStreamBufferRaw)
    {
        ...
        //  Input Image Stream: YUV
        {
            ...
            MUINT const groupUsage = pInImageStreamBufferRaw->queryGroupUsage(getNodeId());

            //
            pInImageBufferHeapRaw = pInImageStreamBufferRaw->tryReadLock(getNodeName());
            if( !pInImageBufferHeapRaw)
            {
                MY_LOGE("tryReadLock pInImageStreamBufferRaw fail, groupUsage(%d)", (int)groupUsage);
                goto lbExit;
            }
