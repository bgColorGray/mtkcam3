...
static constexpr size_t JPEG_SOI_MARKER_SIZE = 2;
...
{
    char filename[256] = {0};
    if (0 > ::snprintf(filename, sizeof(filename), "/data/vendor/JpegYuv/Buffer_frame%d_%d_%dx%d_%d_%d.yuv", reqNo, frameNo,
    ...
    MY_LOGI("[DUMP_YUV] SaveFile[%s]:(%d)", filename, rets);
...
JpegNodeImp()
    ...
    , mpInYuv_main2(NULL)
    , mpInYuv_Y16(NULL)
    , mpInAppYuv_main(NULL)
    , mpOutJpeg(NULL)
    , mpOutJpegAppSegments(NULL)
    , mpOutInternalJpeg(NULL)
    , mBufPool(NULL)
    , mpInYuv_Y16_r(NULL)
    , mpOutJpeg_r(NULL)
...
)
{
    ...
    if (jpg.mIsIspHidlHeif) {
        jpg.processHeif(enc_frame);
        {
            std::lock_guard<std::mutex> lock(jpg.mWorkingBufferLock);
            jpg.mvWorkingBuffer.push_back(JpegNodeImp::WorkingBuffer{
                std::move(enc_frame->mpHeifWorkingBuffer),
                std::move(enc_frame->mpAppSegmentWorkingBuffer)
            });
        }
        jpg.mWorkingBufferCV.notify_one();
    }
...
static void extractRequestLoop(JpegNodeImp& jpg)
{
    ...
    while(!stop) {
        ...
        pFrame = nullptr;
        if (jpg.mIsPreReleaseRequest && jpg.mOutputJpegStreamBuf) {
        MY_LOGD("mark Jpeg buffer release");
        jpg.mOutputJpegStreamBuf->markUserStatus(jpg.getNodeId(),
    ...
    while(1)
    {
        ...
        if ((bIsEncodeError == MFALSE) && jpg.info.mbPackBitStream)
        {
            ...
            if (buf1 != NULL && buf2 != NULL &&
                ...
                //
                JpgPack::StereoInfo info(
                    ATTRIBUTES::MAIN_IMAGE_MIME_JEPG,
                    ATTRIBUTES::DEPTH_MIME_PNG,
                    ATTRIBUTES::DEPTH_FORMAT_RANGE_LINEAR,
                    0.0, 255.0);

                JPG_PACK_FLOW packFlow;
                if (reqList[0].getEncodeFrame()->mParams.pakType == 0)
                    packFlow = JPG_PACK_FLOW::MTK_STYLE_PACK;
                else
                ...
                if (jpg.mIsPreReleaseRequest == MTRUE) {
                    ...
                    if (pGraphicImageBufferHeap != NULL) {
                        if (auto bufferHandlePtr = pGraphicImageBufferHeap->getBufferHandlePtr()) {
                            int rc = IGrallocHelper::singleton()->query(*bufferHandlePtr, &staticInfo, NULL);
                            if (rc != OK) {
                            ...
                            MY_LOGE("pGraphicImageBufferHeap->getBufferHandlePtr() is nullptr");
                        }
                    } else {
            ...
            if (jpg.info.muDumpBuffer) {
                sp<encode_frame> pEncodeFrame = (reqList.size() > 1) ? reqList[0].getEncodeFrame() : NULL;
                const char* filename = (pEncodeFrame != NULL) ? pEncodeFrame->filename_pack : "pack";
                ...
                MY_LOGI("[DUMP_JPG_PACKED] SaveFile[%s]!", filename);
            }
...
init(InitParams const& rParams)
{
...

    {
        ...
        {
            MUINT8 maxAppsSegment;
            if(tryGetMetadata<MUINT8>(
                        &static_meta, MTK_HEIC_INFO_MAX_JPEG_APP_SEGMENTS_COUNT,
                        maxAppsSegment)
              ) {
                info.mAppSegmentMaxSize = (maxAppsSegment < 1 ? 1 : maxAppsSegment > 16 ? 16 : maxAppsSegment) * (2 + 0xFFFF) + sizeof(CameraBlob);
                MY_LOGD("AppsSegment:%zu AppSegmentMaxSize:%zu", static_cast<size_t>(maxAppsSegment), info.mAppSegmentMaxSize);
            }
        }
    }
...
config(ConfigParams const& rParams)
{
    ...
    flush();
    {
        ...
        mpInAppYuv_main = rParams.pInAppYuv_Main;
        mpOutJpeg = rParams.pOutJpeg;
        mpOutJpegAppSegments = rParams.pOutJpegAppSegment;
        mpOutInternalJpeg = rParams.pOutInternalJpeg;
        ...
        // ISP HIDL HEIF
        if (mpOutJpeg != nullptr
                && mpOutJpeg->getImgFormat() == eImgFmt_HEIF) {
            if (mpInYuv_main != nullptr){
                mHeicYuvSize = MSize(
                        mpInYuv_main->getImgSize().w,
                        mpInYuv_main->getImgSize().h);
            } else {
                MY_LOGE("no mpInYuv_main");
            }
            mbAppnOnly = MTRUE;
            mIsIspHidlHeif = MTRUE;
            initIspHidlHeif();
        }
        ...
        } else if (mbAppnOnly && !mIsIspHidlHeif){
        ...
        if (mpInAppYuv_main != NULL) {
            MY_LOGD("mpInAppYuv_main:%dx%d",
                mpInAppYuv_main->getImgSize().w,
                mpInAppYuv_main->getImgSize().h);
        }
        ...
        if (mpOutInternalJpeg != NULL) {
            MY_LOGD("mpOutInternalJpeg:%dx%d",
                mpOutInternalJpeg->getImgSize().w, mpOutInternalJpeg->getImgSize().h);
        }
...
#define MARK_USER_STATUS(sBufSet, streamBuffer, nId, status)   \
    ...
    if (!mIsPreReleaseEnable) {
        ...
        {
            // always wait until wait done queue idle cond,
            // after bgs flush frame, there supose no multi-thread encode
            ...
            for (; it != mRequestQueue.end(); ) {
                ...
                // dst bistream buffer
                {
                    ...
                    if (pGraphicImageBufferHeap != NULL) {
                        ...
                        if (auto bufferHandlePtr = pGraphicImageBufferHeap->getBufferHandlePtr()) {
                            graphicBufferHandle = *bufferHandlePtr;
                        } else {
                            MY_LOGE("pGraphicImageBufferHeap->getBufferHandlePtr() is nullptr");
                        }
...
queue(android::sp<IPipelineFrame> pFrame)
{
    ...
    if (mIsIspHidlHeif) {
        std::call_once(mWorkingBufferThreadFlag, [this] {
            {
                std::lock_guard<std::mutex> lock(mWorkingBufferLock);
                mvWorkingBuffer.reserve(mWorkingBufferNum);
            }
            mWorkingBufferThread = std::thread([this]() {
                IImageBufferAllocator::ImgParam appSegmentImgParam(
                        info.mAppSegmentMaxSize, 0);
                IImageBufferAllocator::ImgParam heifImgParam(
                        NSSImager::ISImager::getHeifDstBufferSize(
                            mpInYuv_main->getImgSize(),
                            static_cast<EImageFormat>(
                                mpInYuv_main->getImgFormat())),
                        0);
                heifImgParam.imgFormat = eImgFmt_HEIF;
                WorkingBuffer workingBuffer;
                {
                    for (int i = 0; i < mWorkingBufferNum; ++i) {
                        initWorkingBuffer(
                                heifImgParam,
                                "HeifWorkingBuffer",
                                workingBuffer.mpHeifWorkingBuffer
                                );
                        initWorkingBuffer(
                                appSegmentImgParam,
                                "AppSegmentWorkingBuffer",
                                workingBuffer.mpAppSegmentWorkingBuffer
                                );
                        {
                            std::lock_guard<std::mutex> lock(
                                    mWorkingBufferLock);
                            mvWorkingBuffer.push_back(std::move(workingBuffer));
                        }
                        mWorkingBufferCV.notify_one();
                    }
                }
            });
        });
...
) const
{
    ...
    if  ( ! (rParams.pOutJpeg.get() || rParams.pOutInternalJpeg.get()) && ! rParams.pOutJpegAppSegment.get()) {
    ...
    */
    ...
    MY_LOGD_IF(rParams.pInAppYuv_Main.get(),
            "stream: [img] in App Yuv %#" PRIx64,
            rParams.pInAppYuv_Main->getStreamId()
            );
    ...
    MY_LOGD_IF(rParams.pOutInternalJpeg.get(),
            "stream: [img] out InternalJpeg %#" PRIx64,
            rParams.pOutInternalJpeg->getStreamId()
            );
    //
    return OK;
}
...
processJpeg(android::sp<IPipelineFrame>& pFrame, JpegNodeImp& jpg) {
    ...
    if (jpg.mIsAsyncWaitEncodeReqEnable == MTRUE && jpg.mIsPreReleaseRequest == MFALSE)
    {
    ...
    static_info& info)
{
    ...
    // image
    {
        ...
        for( size_t i = 0; i < imageIOMap.vIn.size(); i++ )
        {
            ...
            if ((isStream(mpInYuv_main, streamId) || isStream(mpInAppYuv_main, streamId))) {
        ...
        for( size_t i = 0; i < imageIOMap.vOut.size(); i++ )
        {
            ...
            if(isStream(mpOutJpeg, streamId) || isStream(mpOutInternalJpeg, streamId)) {
                MY_LOGD_IF(info.mLogLevel, "accept Jpeg(%#" PRIx64 ")!", streamId);
                sp<IImageStreamInfo> jpeg = imageIOMap.vOut.valueAt(i);
                mpOutJpeg_r = jpeg;
            }
        }
        // pre-release flow
    ...
    if (!mbAppnOnly) {
        for (size_t i = 0; i < mvInYuv_main.size(); i++) {
            ...
            Request r(mvInYuv_main[i], mvInYuv_thumbnail[i], mpOutJpeg_r,
    ...
    } else if (mIsIspHidlHeif) {
        if (mvInYuv_main.size() > 0 && mvInYuv_thumbnail.size() > 0) {
            sp<IImageStreamInfo> mainInfo = NULL;
            if (mvInYuv_thumbnail[0] == NULL) {
                MY_LOGD("app not set thumbnail size, so streamInfo is NULL");
            }
            std::unique_lock<std::mutex> lock(mWorkingBufferLock);
            mWorkingBufferCV.wait(lock, [this]() { return !mvWorkingBuffer.empty(); });
            auto& workingBuffer = mvWorkingBuffer.back();
            Request r(mvInYuv_main[0], mvInYuv_thumbnail[0], nullptr,
                mvInAppMeta[0], mvInHalMeta[0], mpOutMetaStreamInfo_Result,
                rpFrame, getNodeId(), getNodeName(), info, mBufPool, MFALSE, mbAppnOnly,
                mHeicYuvSize,
                workingBuffer.mpHeifWorkingBuffer,
                workingBuffer.mpAppSegmentWorkingBuffer);
            mvWorkingBuffer.pop_back();
            reqList.push_back(r);
        } else {
            MY_LOGE("Error iomap setting no yuv or no thumbnail input");
            return BAD_VALUE;
        }
    } else {
    ...
    sp<encode_frame>& pEncodeFrame)
{
    ...
    params.pDst = pEncodeFrame->mpHeifWorkingBuffer ? pEncodeFrame->mpHeifWorkingBuffer.get() : pEncodeFrame->mpJpeg_Main.get();
    params.transform = 0;
    ...
    if (pEncodeFrame->mpHeifWorkingBuffer == nullptr) {
    ...
    sp<encode_frame>& pEncodeFrame)
{
    ...
    if (mbAppnOnly) {
        if (!mIsIspHidlHeif) {
            auto buffer = reinterpret_cast<unsigned char*>(pOutImgBufferHeap->getBufVA(0));
            // shift two byte
            std::copy(buffer + JPEG_SOI_MARKER_SIZE, buffer + JpegBitStreamSize, buffer);
        }
        JpegBitStreamSize -= JPEG_SOI_MARKER_SIZE;
        pEncodeFrame->appSegmentBlobSize = JpegBitStreamSize;
    }

    if (!mIsIspHidlHeif) {
        pOutImgBufferHeap->setBitstreamSize(
                JpegBitStreamSize);
        sp<IGraphicImageBufferHeap> pGraphicImageBufferHeap = NULL;
        mcam::GrallocStaticInfo staticInfo = {};
        pGraphicImageBufferHeap = IGraphicImageBufferHeap::castFrom(pOutImgBufferHeap.get());
        if (pGraphicImageBufferHeap != NULL) {
            if (auto bufferHandlePtr = pGraphicImageBufferHeap->getBufferHandlePtr()) {
                ...
                if (rc != OK) {
                    ...
                    size_t jpegBufSize = staticInfo.widthInPixels;
                    CameraBlob* pTransport = reinterpret_cast<CameraBlob*>(jpegBuf + jpegBufSize - sizeof(CameraBlob));
                    pTransport->blobId = mbAppnOnly ?
                        static_cast<CameraBlobId>(0x0100) : CameraBlobId::JPEG;
                    pTransport->blobSize = jpegDataSize;
    ...
    if (info.muDumpBuffer && mbAppnOnly) {
        MBOOL rets = pOutImageBuffer->saveToFile(pEncodeFrame->filename);
        MY_LOGI("[DUMP_EXIF] SaveFile[%s]:(%d)", pEncodeFrame->filename, rets);
    }
    pOutImageBuffer->unlockBuf(getNodeName());

    if (info.muDumpBuffer && pEncodeFrame->mpJpeg_Main != NULL && !info.mbPackBitStream) {
        ...
        MBOOL rets = dumpImgBuffer->saveToFile(pEncodeFrame->filename);
        MY_LOGI("[DUMP_JPG] SaveFile[%s]:(%d)", pEncodeFrame->filename, rets);
    ...
    if (pOutImgStreamBuffer != NULL) {
        ...
        if (mIsPreReleaseRequest) {
            MY_LOGD("mark Jpeg buffer released after");
        } else {
...
{
    CAM_TRACE_NAME(__FUNCTION__);
    MY_LOGD("+ frameNo(%d), requestNo(%d), src: %dx%d, dst: %dx%d, format: 0x%x, crop(%d,%d;%d,%d), bsOffset(%zu)",
            rParams.frameNo, rParams.requestNo,
...
*/
        ...
        {
            // OutJpeg buffer clean/flush cache first
            MUINT const usage = rpStreamBuffer->queryGroupUsage(mNodeId);
            auto ret = rpImageBufferHeap->lockBuf(mNodeName, usage);
            MY_LOGA_IF(!ret, "%s", "lockBuffer failed, please check VA usage!");
            rpImageBufferHeap->syncCache(eCACHECTRL_FLUSH);
            rpImageBufferHeap->unlockBuf(mNodeName);
        }
    }
    if (rpEncodeFrame->mpYUV_MainStreamInfo.get()) {
        ...
        MBOOL ret = rpImageBuffer->lockBuf(mNodeName, groupUsage);
        MY_LOGA_IF(!ret, "%s", "lockBuffer failed, please check VA usage!");
        rpEncodeFrame->mbBufferLocked = MTRUE;
    ...
    )
{
    ...
    if (rpEncodeFrame->mbBufferLocked) {
        ion_fd = rpImageBufferHeap->getHeapID();
        buf_va = rpImageBufferHeap->getBufVA(0);
    } else {
        MBOOL ret = rpImageBufferHeap->lockBuf(mNodeName, groupUsage);
        MY_LOGA_IF(!ret, "%s", "lockBuffer failed, please check VA usage!");
        ion_fd = rpImageBufferHeap->getHeapID();
        buf_va = rpImageBufferHeap->getBufVA(0);
        rpImageBufferHeap->unlockBuf(mNodeName);
    }
    if (ion_fd == -1 || buf_va == 0) {
        MY_LOGE("qurey info for heap is wrong. fd : %d, va : %#" PRIxPTR, ion_fd, buf_va);
        return BAD_VALUE;
    }

    PortBufInfo_v1 portBufInfo = PortBufInfo_v1(
            ion_fd, (MUINTPTR)(buf_va + thumbnailOffset)
            );
...
prepareMeta()
{
    ...
    if (!tryGetMetadata<MINT64>(mInHalMeta, MTK_P1NODE_FRAME_EXP_TIMESTAMP,
                mpEncodeFrame->timestamp)) {
        MY_LOGE("no tag: MTK_P1NODE_FRAME_EXP_TIMESTAMP");
    }
...
prepareExif()
{
    ...
    if (!mbAppnOnly || mpHeifWorkingBuffer != nullptr) {
        ...
        if((mpEncodeFrame->mParams.orientation == 90  &&  transform & eTransform_ROT_90)  ||
            ...
            mpEncodeFrame->needExifRotate = false;
        }
    } else {
        imageSize = mHeicYuvSize;
    }
    //
    ...
    std::map<MUINT32, MUINT32> debugInfoList;
    {
        ...
        debugInfoList[CMN_TAG_PRI_UNIQUE_KEY] = CUSTOM_UNIQUE_KEY;
    }
    ...
    if (mInfo.muDumpBuffer) {
        ...
        auto tmp = localtime(&stdParams.seconds);
        uint32_t timestamp = 0;
        if (tmp != nullptr) {
            timestamp = tmp->tm_hour*10000000 + tmp->tm_min*100000 + tmp->tm_sec*1000 + stdParams.microSeconds/1000;
        }
        if (!mInfo.mbPackBitStream) {
            genFileName_JPG(mpEncodeFrame->filename, sizeof(mpEncodeFrame->filename), &hint, nullptr, timestamp);
            MY_LOGD("enable muDumpBuffer FileName[%s]", mpEncodeFrame->filename);
        } else {
            // pack jpeg file dump
            genFileName_JPG(mpEncodeFrame->filename_pack, sizeof(mpEncodeFrame->filename_pack), &hint, "packed", timestamp);
            MY_LOGD("enable muDumpBuffer FileName_pack[%s]", mpEncodeFrame->filename_pack);
        }
    }
...
)
{
    ...
    if( rpHalMeta && rpAppMeta ) {
        ExifUtils::updateStdExifParam_3A(*rpHalMeta, *rpAppMeta, rStdParams, enableLog);
    }
    else {
    ...
    // zoom ratio
    MFLOAT zoomRatio = 100.0f;
    if( rpAppMeta && tryGetMetadata<MFLOAT>(rpAppMeta, MTK_CONTROL_ZOOM_RATIO, zoomRatio)) {
        rStdParams.u4ZoomRatio = (MUINT32) (zoomRatio * 100);
    } else {
        MY_LOGW("no tag: MTK_CONTROL_ZOOM_RATIO, calculate via jpeg params");
        rStdParams.u4ZoomRatio = calcZoomRatio(rParams.configured, rParams.cropRegion, rParams.activeArray, rSize, enableLog);
    }
    rStdParams.u4Facing = (rParams.facing == MTK_LENS_FACING_BACK) ? 0 : 1;

    struct timeval tv;
    gettimeofday(&tv, NULL);
    rStdParams.seconds = tv.tv_sec;
    rStdParams.microSeconds = tv.tv_usec;
...
#define getParam(meta, tag, type, param, needWarn)    \
    ...
    IMetadata exifMeta;
    if ( tryGetMetadata<IMetadata>(&rHalMeta, MTK_3A_EXIF_METADATA, exifMeta) ) {
        getParam(&exifMeta, MTK_3A_EXIF_FNUMBER,             MINT32, rStdParams.u4FNumber, MTRUE); /**/
        getParam(&exifMeta, MTK_3A_EXIF_FOCAL_LENGTH,        MINT32, rStdParams.u4FocalLength, MTRUE); /**/
        getParam(&exifMeta, MTK_3A_EXIF_CAP_EXPOSURE_TIME,   MINT32, rStdParams.u4CapExposureTime, MTRUE); /**/
        getParam(&exifMeta, MTK_3A_EXIF_AE_ISO_SPEED,        MINT32, rStdParams.u4AEISOSpeed, MTRUE); /**/
        getParam(&exifMeta, MTK_3A_EXIF_FOCAL_LENGTH_35MM,   MINT32, rStdParams.u4FocalLength35mm, MFALSE);
        getParam(&exifMeta, MTK_3A_EXIF_AWB_MODE,            MINT32, rStdParams.u4AWBMode, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_LIGHT_SOURCE,        MINT32, rStdParams.u4LightSource, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_EXP_PROGRAM,         MINT32, rStdParams.u4ExpProgram, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_SCENE_CAP_TYPE,      MINT32, rStdParams.u4SceneCapType, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_FLASH_LIGHT_TIME_US, MINT32, rStdParams.u4FlashLightTimeus, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_AE_METER_MODE,       MINT32, rStdParams.u4AEMeterMode, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_AE_EXP_BIAS,         MINT32, rStdParams.i4AEExpBias, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_AE_BRIGHTNESS_VALUE, MINT32, rStdParams.i4Brightness, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_FLASH_FIRING_STATUS, MINT32, rStdParams.u4FlashFiringStatus, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_FLASH_RETURN_DETECTION, MINT32, rStdParams.u4FlashReturnDetect, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_FLASH_MODE,          MINT32, rStdParams.u4FlashMode, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_FLASH_FUNCTION,      MINT32, rStdParams.u4FlashFunction, MTRUE);
        getParam(&exifMeta, MTK_3A_EXIF_FLASH_REDEYE,        MINT32, rStdParams.u4FlashRedEye, MTRUE);
    }
    else {
        // from result meta of 3A
        // for Hal3 yuv reprocessing [must need!]
        MY_LOGW("no tag: MTK_3A_EXIF_METADATA, miss in Hal meta find APP meta");
        MFLOAT fNumber = 0.0f;
    ...
    if (!mbAppnOnly || mpHeifWorkingBuffer != nullptr) {
        ...
        if (getInImageBuffer(mpFrame, mpMainStreamInfo->getStreamId(),
            ...
            if (mInfo.mLogLevel >= 2) {
                dumpYUVBuffer(mpFrame->getRequestNo(), mpFrame->getFrameNo(), mpInMainImageBuffer, 0);
            }
        }
    }
    if (mpEncodeFrame->mbHasThumbnail) {
        ...
        if (getInImageBuffer(mpFrame, mpThumbStreamInfo->getStreamId(),
            ...
            if (mInfo.mLogLevel >= 2) {
                dumpYUVBuffer(mpFrame->getRequestNo(), mpFrame->getFrameNo(), mpInThumbImageBuffer, 1);
            }
        }
    }
...
lockOutImg()
{
    if (mInfo.mbPackBitStream) {
        ...
        if (mpAppSegmentWorkingBuffer != nullptr) {
            mpEncodeFrame->mpOutImgBufferHeap = mpAppSegmentWorkingBuffer->getImageBufferHeap();
        } else if (getOutJpegImageBuffer(mpFrame, mpJpegStreamInfo->getStreamId(),
                    mpOutJpegImageStreamBuffer, mpOutJpegImageBuffer,
        ...
        if (mpAppSegmentWorkingBuffer != nullptr) {
            mpEncodeFrame->mbBufferLocked = MTRUE;
            if (mpEncodeFrame->mbHasThumbnail &&
                    getOutThumbImageBuffer(-1,
                        mpEncodeFrame, mpAppSegmentWorkingBuffer->getImageBufferHeap(),
                        mpOutThumbImageBuffer) != OK) {
        ...
        } else if (mpEncodeFrame->mbHasThumbnail &&
                getOutThumbImageBuffer(mpJpegStreamInfo->getStreamId(),
...
unlock()
{
    ...
    if (mpAppSegmentWorkingBuffer != nullptr) {
        if (returnImageBuffer(mpFrame, -1,
                    mpOutJpegImageStreamBuffer, mpOutJpegImageBuffer,
                    mpOutThumbImageBuffer) != OK) {
            MY_LOGE("return Thumbnail image buffer failed!");
            return BAD_VALUE;
        }
    } else if (mpJpegStreamInfo && returnImageBuffer(mpFrame, mpJpegStreamInfo->getStreamId(),
        mpOutJpegImageStreamBuffer, mpOutJpegImageBuffer,
        mpOutThumbImageBuffer) != OK) {
        MY_LOGE("return Thumbnail image buffer failed!");
        return BAD_VALUE;
        ...
        )
{
    sp<IImageBufferHeap> pHeap =
        IIonImageBufferHeap::create(bufferName,
                imgParam);
    if (pHeap == nullptr) {
        MY_LOGE("Create image buffer heap fail");
        return;
    }
    buffer = pHeap->createImageBuffer();
    if (buffer == nullptr) {
        MY_LOGE("Create image buffer fail");
        return;
    }
    auto ret = buffer->lockBuf(LOG_TAG,
...
void
JpegNodeImp::
deinitWorkingBuffer(const sp<IImageBuffer>& buffer)
{
    if (buffer != nullptr) {
        buffer->unlockBuf(mNodeName);
    }
}
...
#if !MTKCAM_ISP_HIDL_HEIF
    MY_LOGE("Config isp hidl heif but no lib");
#endif
}
...
deinitIspHidlHeif()
{
    if (mWorkingBufferThread.joinable()) {
        mWorkingBufferThread.join();
        {
            std::lock_guard<std::mutex> lock(mWorkingBufferLock);
            if (mvWorkingBuffer.size() != mWorkingBufferNum) {
                MY_LOGE("mvWorkingBuffer size incorrect, "
                        "expected:%zu actual:%zu",
                        mWorkingBufferNum, mvWorkingBuffer.size());
            }
            for (auto& workingBuffer : mvWorkingBuffer) {
                deinitWorkingBuffer(workingBuffer.mpHeifWorkingBuffer);
                deinitWorkingBuffer(workingBuffer.mpAppSegmentWorkingBuffer);
            }
        }
    } else {
        MY_LOGW("mWorkingBufferThread is not joinable");
    }
}

void
JpegNodeImp::
setupMediaFormat(encode_frame& frame, AMediaFormat& mediaFormat) {
#if MTKCAM_ISP_HIDL_HEIF
    auto& pInYuv = frame.mpYUV_Main;
    if (pInYuv == nullptr) {
        MY_LOGE("No in yuv");
        return;
    }
    auto width = static_cast<int32_t>(pInYuv->getImgSize().w);
    auto height = static_cast<int32_t>(pInYuv->getImgSize().h);

    // FIXME: frame rate
    AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_FRAME_RATE, 30);
    AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_TILE_WIDTH, GRID_WIDTH);
    AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_TILE_HEIGHT,
            GRID_HEIGHT);
    auto gridRows = static_cast<int32_t>(
            std::ceil(static_cast<float>(height) / GRID_HEIGHT));
    AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_GRID_ROWS, gridRows);
    auto gridCols = static_cast<int32_t>(
            std::ceil(static_cast<float>(width) / GRID_WIDTH));
    AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_GRID_COLUMNS, gridCols);
    frame.mGridNum = gridRows * gridCols;
    MY_LOGD("gridRows:%" PRId32 " gridCols:%" PRId32 " gridNum:%u",
            gridRows, gridCols, frame.mGridNum);
    ...
    AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_HEIGHT, height);
    {
        ...
        int32_t colorStandard = 0;
        int32_t colorTransfer = 0;
        // TODO(MTK): get colorRange, colorStandard, colorTransfer.
        AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_COLOR_RANGE,
                colorRange);
        AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_COLOR_STANDARD,
                colorStandard);
        AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_COLOR_TRANSFER,
                colorTransfer);
    }

    AMediaFormat_setInt32(&mediaFormat, AMEDIAFORMAT_KEY_IS_DEFAULT, 1);
    AMediaFormat_setString(&mediaFormat, AMEDIAFORMAT_KEY_MIME,
...
void
JpegNodeImp::
processHeif(const sp<encode_frame>& pEncodeFrame)
{
#if MTKCAM_ISP_HIDL_HEIF
    CAM_TRACE_CALL();
    MY_LOGD("start");
    std::ostringstream tempFileName;
    tempFileName << "HEIF-" << pthread_self() << "-F" << pEncodeFrame->mpFrame->getFrameNo()
        << "-R" << pEncodeFrame->mpFrame->getRequestNo();
    auto fd = syscall(__NR_memfd_create, tempFileName.str().c_str(), MFD_CLOEXEC);
    if (fd < 0) {
        MY_LOGE("Failed to create file: %s", std::strerror(errno));
        return;
    }
    class FdHandle {
    public:
    ...
    pEncodeFrame->mpMediaFormat = AMediaFormat_new();
    auto pMediaFormat = pEncodeFrame->mpMediaFormat;
    if (pMediaFormat == nullptr) {
        MY_LOGE("Create AMediaFormat failed");
        return;
    }
    class MediaFormatHandle {
      public:
        explicit MediaFormatHandle(AMediaFormat* pFormat) : pFormat(pFormat) {
        }
        MediaFormatHandle(const MediaFormatHandle&) = delete;
        MediaFormatHandle& operator=(const MediaFormatHandle&) = delete;
        ~MediaFormatHandle() {
            AMediaFormat_delete(pFormat);
        }
      private:
        AMediaFormat* const pFormat;
    } formatHandle(pMediaFormat);

    setupMediaFormat(*pEncodeFrame, *pMediaFormat);

    // MediaMuxer::OUTPUT_FORMAT_HEIF == 3
    pEncodeFrame->mpMediaMuxer = AMediaMuxer_new(fd, static_cast<OutputFormat>(3));
    auto pMediaMuxer = pEncodeFrame->mpMediaMuxer;
    if (pMediaMuxer == nullptr) {
        MY_LOGE("Failed to create AMediaMuxer");
        return;
    }
    class MediaMuxerHandle {
    public:
        explicit MediaMuxerHandle(AMediaMuxer* pMuxer)
            : pMuxer(pMuxer)
        {}
        MediaMuxerHandle(const MediaMuxerHandle&) = delete;
        MediaMuxerHandle& operator=(const MediaMuxerHandle&) = delete;
        bool start()
        {
            return (isStarted = AMediaMuxer_start(pMuxer) == AMEDIA_OK);
        }
        bool stop()
        {
            isStarted = false;
            return AMediaMuxer_stop(pMuxer) == AMEDIA_OK;
        }
        ~MediaMuxerHandle()
        {
            if (isStarted) {
            ...
            AMediaMuxer_delete(pMuxer);
        }
    private:
        AMediaMuxer* const pMuxer;
        bool isStarted = false;
    } muxerHandle(pMediaMuxer);

    if (AMediaMuxer_setOrientationHint(pMediaMuxer,
                pEncodeFrame->needExifRotate ?
                pEncodeFrame->mParams.orientation : 0)
            != AMEDIA_OK) {
        MY_LOGE("Failed to setOrientationHint");
        return;
    }

    const auto heifWorkingBuffer = reinterpret_cast<const unsigned char*>(pEncodeFrame->mpHeifWorkingBuffer->getBufVA(0));
    if (heifWorkingBuffer == nullptr) {
        MY_LOGE("heifWorkingBuffer == nullptr");
        return;
    }

    auto csd0Buffer = heifWorkingBuffer;
    size_t csd0Size = 79;
    AMediaFormat_setBuffer(pMediaFormat, AMEDIAFORMAT_KEY_CSD_0, csd0Buffer,
            csd0Size);

    auto trackId = AMediaMuxer_addTrack(pMediaMuxer, pMediaFormat);
    if (trackId < 0) {
        MY_LOGE("Failed to addTrack");
        return;
    }
    pEncodeFrame->trackId = trackId;

    if (!muxerHandle.start()) {
        MY_LOGE("Failed to start muxer");
        return;
    }

    // write appSegment
    processAppSegment(pEncodeFrame);

    // write bitstream
    auto bitstreamBuffer = heifWorkingBuffer + csd0Size;
    constexpr size_t gridInfoMaxSize = 4096;
    auto heifWorkingBufferSize = pEncodeFrame->mpHeifWorkingBuffer->getBitstreamSize();
    MY_LOGD("heifWorkingBuffer:%p bitstreamSize:%zu",
            const_cast<void*>(static_cast<const void*>(heifWorkingBuffer)),
            static_cast<size_t>(heifWorkingBufferSize));
    if (heifWorkingBufferSize <= gridInfoMaxSize) {
        MY_LOGE("HeifWorkingBufferSize:%zu <= gridInfoMaxSize:%zu",
                static_cast<size_t>(heifWorkingBufferSize),
                gridInfoMaxSize);
        return;
    }
    auto gridInfo = heifWorkingBuffer + heifWorkingBufferSize - gridInfoMaxSize;
    processBitstream(pEncodeFrame, bitstreamBuffer, gridInfo);

    if (!muxerHandle.stop()) {
        MY_LOGE("Failed to stop muxer");
        return;
    }

    processCompleteFrame(pEncodeFrame, fd);
...
void
JpegNodeImp::
processAppSegment(const sp<encode_frame>& pEncodeFrame) {
#if MTKCAM_ISP_HIDL_HEIF
    auto pMediaMuxer = pEncodeFrame->mpMediaMuxer;
    const auto appSegmentWorkingBuffer = reinterpret_cast<const unsigned char*>(pEncodeFrame->mpAppSegmentWorkingBuffer->getBufVA(0));
    if (appSegmentWorkingBuffer == nullptr) {
        MY_LOGE("appSegmentWorkingBuffer == nullptr");
        return;
    }

    // Assemble the APP1 marker buffer required by MediaCodec
    const unsigned char exifMarker[] = {'E', 'x', 'i', 'f'};
    auto blobSize = pEncodeFrame->appSegmentBlobSize;
    MY_LOGD("blobSize:%zu", blobSize);
    size_t appSegmentBufferSize = sizeof(exifMarker) + blobSize;
    std::unique_ptr<unsigned char[]> appSegmentBuffer(new unsigned char[appSegmentBufferSize]);
    memcpy(appSegmentBuffer.get(), exifMarker, sizeof(exifMarker));
    memcpy(appSegmentBuffer.get() + sizeof(exifMarker), appSegmentWorkingBuffer + JPEG_SOI_MARKER_SIZE,
            blobSize);

    // MediaCodec::BUFFER_FLAG_MUXER_DATA == 16
    AMediaCodecBufferInfo info = {0, static_cast<int32_t>(appSegmentBufferSize),
        pEncodeFrame->timestamp, 16};
    if (AMediaMuxer_writeSampleData(pMediaMuxer, pEncodeFrame->trackId,
                appSegmentBuffer.get(), &info) != AMEDIA_OK) {
        MY_LOGE("Failed to writeSampleData");
        return;
    }
    MY_LOGD("success");
#endif
...
JpegNodeImp::
processBitstream(
        const sp<encode_frame>& pEncodeFrame,
        const unsigned char* bitstreamBuffer,
        const unsigned char* gridInfo
        )
{
#if MTKCAM_ISP_HIDL_HEIF
    auto pMediaMuxer = pEncodeFrame->mpMediaMuxer;
    const auto gridInfoBegin = gridInfo;
    uint32_t gridIndex;
    uint32_t gridSize;
    constexpr auto elementSize = sizeof(gridIndex) + sizeof(gridSize);
    auto timestamp = pEncodeFrame->timestamp;
    auto trackId = pEncodeFrame->trackId;
    MY_LOGD("timestamp:%" PRId64, timestamp);
    auto gridNum = pEncodeFrame->mGridNum;
    for (decltype(gridNum) i = 0; i < gridNum;
            ++i, gridInfo += elementSize, bitstreamBuffer += gridSize) {
        memcpy(&gridIndex, gridInfo, sizeof(gridIndex));
        memcpy(&gridSize, gridInfo + sizeof(gridIndex), sizeof(gridSize));
        if (i != gridIndex) {
            MY_LOGW("Grid index incorrect, expected:%u actual:%" PRIu32 " size:%" PRIu32,
                    i, gridIndex, gridSize);
        }
        if (gridSize == 0 || (bitstreamBuffer + gridSize) > gridInfoBegin) {
            MY_LOGE("GridInfo address:%p, Grid index:%u, gridSize:%" PRIu32 " <= 0 "
                    "or bitstreamBuffer and gridInfo overlap",
                    const_cast<void*>(static_cast<const void*>(gridInfo)),
                    i, gridSize);
            return;
        }
        {
            AMediaCodecBufferInfo info = {0, static_cast<int32_t>(gridSize), timestamp, 0};
            if (AMediaMuxer_writeSampleData(pMediaMuxer, trackId,
                        bitstreamBuffer, &info) != AMEDIA_OK) {
                MY_LOGE("Failed to writeSampleData");
                return;
            }
        }
...
void
JpegNodeImp::
processCompleteFrame(const sp<encode_frame>& pEncodeFrame, int fd)
{
#if MTKCAM_ISP_HIDL_HEIF
    auto fSize = lseek(fd, 0, SEEK_END);
    lseek(fd, 0, SEEK_SET);
    sp<IImageStreamBuffer> outStreamBuffer;
    sp<IImageBufferHeap> outImageBufferHeap;
    lockOutJpeg(pEncodeFrame->mpFrame->getStreamBufferSet(),
            outStreamBuffer, outImageBufferHeap);
    if (outImageBufferHeap == nullptr) {
        MY_LOGE("outImageBufferHeap == nullptr");
        return;
    }
    class Handle {
    public:
        Handle(JpegNodeImp* pNode,
                const sp<IImageStreamBuffer>& outStreamBuffer,
                const sp<IImageBufferHeap>& outImageBufferHeap)
            : pNode(pNode),
            outStreamBuffer(outStreamBuffer),
            outImageBufferHeap(outImageBufferHeap)
        {}
        Handle(const Handle&) = delete;
        Handle& operator=(const Handle&) = delete;
        ~Handle()
        {
            pNode->unlockOutJpeg(outStreamBuffer, outImageBufferHeap);
        }
    private:
        JpegNodeImp* const pNode;
        const sp<IImageStreamBuffer>& outStreamBuffer;
        const sp<IImageBufferHeap>& outImageBufferHeap;
    } handle(this, outStreamBuffer, outImageBufferHeap);
    auto dstBuffer = reinterpret_cast<unsigned char*>(outImageBufferHeap->getBufVA(0));
    std::size_t dstBufferSize = getOutputBufferSize(outImageBufferHeap);
    MY_LOGD("dstBufferSize:%zu", dstBufferSize);
    if (dstBuffer == nullptr) {
        MY_LOGE("dstBuffer == nullptr");
        return;
    }
    if (dstBufferSize < static_cast<size_t>(fSize)) {
        MY_LOGE("dstBufferSize:%zu < fSize:%zu",
                dstBufferSize,
                static_cast<size_t>(fSize));
        return;
    }
    auto bytesRead = read(fd, dstBuffer, fSize);
    if (bytesRead < fSize) {
        MY_LOGE("Only %zd of %ld bytes read", bytesRead, fSize);
        return;
    }
    auto width = static_cast<int32_t>(mpInYuv_main->getImgSize().w);
    auto height = static_cast<int32_t>(mpInYuv_main->getImgSize().h);
    auto maxHeicBufferSize = width * height * 3 / 2 + info.mAppSegmentMaxSize;
    if (dstBufferSize < maxHeicBufferSize) {
        MY_LOGW("dstBufferSize:%zu < maxHeicBufferSize:%zu",
                dstBufferSize,
                static_cast<size_t>(maxHeicBufferSize));
    }
    if (dstBufferSize < fSize + sizeof(CameraBlob)) {
        MY_LOGE("dstBufferSize:%zu < fSize + sizeof(CameraBlob):%zu",
                dstBufferSize,
                (fSize + sizeof(CameraBlob)));
        return;
    }
    CameraBlob cameraBlob = {
        .blobId = static_cast<CameraBlobId>(0x00FE),
        .blobSize = static_cast<uint32_t>(fSize)
    };
    static_assert(std::is_trivially_copyable<decltype(cameraBlob)>::value,
            "CameraBlob is not trivially copyable!");
    memcpy(dstBuffer + dstBufferSize - sizeof(cameraBlob), &cameraBlob,
            sizeof(cameraBlob));
    MY_LOGD("blobSize:%zu",
            static_cast<size_t>(fSize));
    MY_LOGD("success");
...
{
    auto streamId = mpOutJpeg->getStreamId();
        outStreamBuffer = streamBufferSet.getImageBuffer(streamId, mNodeId);
    if (outStreamBuffer == nullptr) {

        MY_LOGD("[node:%ld], streamID(%#" PRIx64 ")",
                mNodeId, streamId);
        return;
    }
    //  Mark this buffer as ACQUIRED by this user.
    streamBufferSet.markUserStatus(
            streamId, mNodeId, IUsersManager::UserStatus::ACQUIRE);
    //  Check buffer status.
    if (outStreamBuffer->hasStatus(STREAM_BUFFER_STATUS::ERROR)) {
        //  The producer ahead of this user may fail to render this buffer's content.
        MY_LOGE("[node:%ld][stream buffer:%s] bad status:%d",
                mNodeId, outStreamBuffer->getName(), outStreamBuffer->getStatus());
        return;
    }
    outImageBufferHeap = outStreamBuffer->tryWriteLock(mNodeName);

    if (outImageBufferHeap == nullptr) {
        MY_LOGE("outImageBufferHeap is nullptr");
        return;
    }

    MBOOL ret = outImageBufferHeap->lockBuf(mNodeName, outStreamBuffer->queryGroupUsage(mNodeId));
    MY_LOGA_IF(!ret, "%s", "lockBuffer failed, please check VA usage!");
}
...
{
    MBOOL ret = outImageBufferHeap->unlockBuf(mNodeName);
    MY_LOGA_IF(!ret, "%s", "unlockBuffer failed");

    outStreamBuffer->unlock(mNodeName, outImageBufferHeap.get());
    outStreamBuffer->markUserStatus(
            mNodeId,
            IUsersManager::UserStatus::USED |
            IUsersManager::UserStatus::RELEASE
            );
...
getOutputBufferSize(
        const sp<IImageBufferHeap>& outImageBufferHeap)
{
    if (auto pGraphicImageBufferHeap = IGraphicImageBufferHeap::castFrom(
                outImageBufferHeap.get())) {
        if (auto bufferHandlePtr =
                pGraphicImageBufferHeap->getBufferHandlePtr()) {
            mcam::GrallocStaticInfo staticInfo{};
            if (auto rc = IGrallocHelper::singleton()->query(
                    *bufferHandlePtr, &staticInfo, NULL); rc == OK) {
                return staticInfo.widthInPixels;
            } else {
                MY_LOGE("Fail to query blob buffer - rc:%d", rc);
            }
        } else {
    ...
    return outImageBufferHeap->getBufSizeInBytes(0);
}
...
putBufHeap(StreamId_T const streamId, MINT32 ion_fd)
{
    ...
    if (search != mPools.end()) {
        ...
        if (found) {
            MY_LOGE("cannot find the BufferLot by id(%" PRId64 ")", streamId);
            return BAD_VALUE;
        }
    }
