...
public:
        ...
        // ISP HIDL HEIF
        sp<IImageBuffer> mpHeifWorkingBuffer;
        sp<IImageBuffer> mpAppSegmentWorkingBuffer;
        size_t appSegmentBlobSize;
        unsigned int mGridNum = 0;
        ...
        char filename[256] = {0};
        char filename_pack[256] = {0};
        //
        AMediaMuxer* mpMediaMuxer;
        AMediaFormat* mpMediaFormat = nullptr;
        size_t trackId;
        MINT64 timestamp;
        bool needExifRotate = true;
        encode_frame(
                sp<IPipelineFrame> const pFrame,
                MBOOL const hasThumbnail,
                MBOOL const skipParse,
                const sp<IImageBuffer>& heifWorkingBuffer = nullptr,
                const sp<IImageBuffer>& appSegmentWorkingBuffer = nullptr
                )
            : mpFrame(pFrame)
              ...
              , mpHeifWorkingBuffer(heifWorkingBuffer)
              , mpAppSegmentWorkingBuffer(appSegmentWorkingBuffer)
              , appSegmentBlobSize(0)
              , mpOutImgStreamBuffer(NULL)
              , mpOutImgBufferHeap(NULL)
              , thumbnailMaxSize(0)
              , muJpegSwMode(0)
              , mpMediaMuxer(nullptr)
              , trackId(0)
              , timestamp(0)
    {}
};
...
public:
    Request(sp<IImageStreamInfo>& main,
        ...
        const sp<IImageStreamInfo>& Jpeg,
    ...
    {
        mpEncodeFrame =
            new encode_frame(mpFrame, (mpThumbStreamInfo != NULL)/*useThumbnail*/, mbSkipParseMeta, heifWorkingBuffer, appSegmentWorkingBuffer);
    }
...
private:    ////to sync main yuv & thumbnail yuv
    ...
    // ISP HIDL HEIF
    void initIspHidlHeif();
    void deinitIspHidlHeif();
    void initWorkingBuffer(
            const IImageBufferAllocator::ImgParam& imgParam,
            const char* bufferName,
            sp<IImageBuffer>& buffer
            );
    void deinitWorkingBuffer(const sp<IImageBuffer>& buffer);
    void processHeif(const sp<encode_frame>& pEncodeFrame);
    void processAppSegment(const sp<encode_frame>& pEncodeFrame);
    void processBitstream(
            const sp<encode_frame>& pEncodeFrame,
            const unsigned char* bitstreamBuffer,
            const unsigned char* gridInfo
            );
    void processCompleteFrame(const sp<encode_frame>& pEncodeFrame, int fd);
    void lockOutJpeg(
    ...
    void setupMediaFormat(encode_frame& frame, AMediaFormat& mediaFormat);
...
protected:  ////Data Members. (Config)
    ...
    sp<IImageStreamInfo> mpInAppYuv_main;
    sp<IImageStreamInfo> mpOutJpeg;
    sp<IImageStreamInfo> mpOutJpegAppSegments;
    sp<IImageStreamInfo> mpOutInternalJpeg;
    ...
    std::vector<WorkingBuffer> mvWorkingBuffer;
    std::thread mWorkingBufferThread;
    mutable std::once_flag mWorkingBufferThreadFlag;
    static constexpr size_t mWorkingBufferNum = 1;
    mutable std::mutex mWorkingBufferLock;
    std::condition_variable mWorkingBufferCV;
...
protected:   //Data Members (Request)
    ...
    sp<IImageStreamInfo> mpOutJpeg_r;
