...
FunctionType_Configuration_StreamInfo_P1 makePolicy_Configuration_StreamInfo_P1_SMVR()
{
    return [](Configuration_StreamInfo_P1_Params const& params) -> int {
        ...
        MINT32 iBurstNum=1;
        {
            ...
            {
                MY_LOGE("smvrC: Can not get metadata provider for search burst number!! set burst number to 1");
            }
            else
            {
                ...
                if (!entry.isEmpty())
                {
                    ...
                    for ( size_t i=0; i<entry.count(); i+=5 )
                    {
                        ...
                        if ( pParsedAppImageStreamInfo->maxYuvSize.w == entry.itemAt(i,   Type2Type<int32_t>()) &&
                              pParsedAppImageStreamInfo->maxYuvSize.h == entry.itemAt(i+1, Type2Type<int32_t>()) )
                        {
                            iHitMaxYuv = i;
                        }
                    }
                    if ( !hit )
                    {
                        MY_LOGW("smvrC: no matching high speed profile by videoImg(%dx%d)",
                            pParsedAppImageStreamInfo->videoImageSize.w, pParsedAppImageStreamInfo->videoImageSize.h);

                        // check maxYuv
                        if ( iHitMaxYuv != -1 )
                        {
                            iBurstNum = entry.itemAt(iHitMaxYuv+4, Type2Type<MINT32>());
                            MY_LOGD("smvrC: matching high speed profile by maxYuv(%dx%d), "
                                "iHitMaxYuv(%d), burstNum(%d)",
                                pParsedAppImageStreamInfo->maxYuvSize.w, pParsedAppImageStreamInfo->maxYuvSize.h,
                                iHitMaxYuv, iBurstNum);
                        }
                        else
                        {
                            MY_LOGF("smvrC: no matching high speed profile by maxYuv(%dx%d), "
                                "check MTK_CONTROL_AVAILABLE_HIGH_SPEED_VIDEO_CONFIGURATIONS",
                                pParsedAppImageStreamInfo->maxYuvSize.w,
                                    pParsedAppImageStreamInfo->maxYuvSize.h);
                        }
                    }
                }
                else
                {
                    MY_LOGW("smvrC: no static MTK_CONTROL_AVAILABLE_HIGH_SPEED_VIDEO_CONFIGURATIONS");
                }
            }
        }
        ...
        for (const auto& [_sensorId, _needP1] : pPipelineNodesNeed->needP1Node)
        {
            ...
            if (_needP1)
            {
                if (nSensorIdx < 0)
                {
                    MY_LOGE("smvrC: no matched sensorIdx from sensorId:%d", _sensorId);
                    continue;
                }
                ...
                //IMGO
                if (pvP1DmaNeed->at(_sensorId) & P1_IMGO)
                {
                    P1ImageStreamInfoBuilder builder(builderCtorParams);
                    builder.setP1Imgo_Default(P1_streamBufCnt*iBurstNum, pvP1HwSetting->at(_sensorId));
                ...
                {
                    P1ImageStreamInfoBuilder builder(builderCtorParams);
                    builder.setP1Rrzo_Default(P1_streamBufCnt*iBurstNum, pvP1HwSetting->at(_sensorId));
                ...
                {
                    P1ImageStreamInfoBuilder builder(builderCtorParams);
                    builder.setP1Rsso_Default(P1_streamBufCnt * iBurstNum, pvP1HwSetting->at(_sensorId));
