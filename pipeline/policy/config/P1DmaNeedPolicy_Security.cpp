...
FunctionType_Configuration_P1DmaNeedPolicy makePolicy_Configuration_P1DmaNeed_Security()
{
    ...
    ) -> int
    {
        ...
        bool needFDYUV = (pStreamingFeatureSetting->bNeedP1FDYUV && ((*pP1HwSetting).at(main1SensorId).fdyuvSize.size() > 0));
        MY_LOGD("pStreamingFeatureSetting->bNeedP1FDYUV(%d),(*pP1HwSetting)[main1].fdyuvSize.size(%d),needFDYUV(%d)",
            pStreamingFeatureSetting->bNeedP1FDYUV, (*pP1HwSetting).at(main1SensorId).fdyuvSize.size(),needFDYUV);
        //
        ...
        {
            uint32_t setting = 0;
            auto p1HwSetting = (*pP1HwSetting).at(sensorId);
            ...
            if (needFDYUV && sensorId == main1SensorId) {
                setting |= P1_FDYUV; // for isp6.0
            }
#endif
            MY_LOGD("setting(0x%x)", setting);
            (*pvOut).emplace(main1SensorId, setting);
        }
        return OK;
