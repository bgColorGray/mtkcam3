...
#include "custom/FeatureSettingPolicy_Streaming_Custom.h"
...
private:
    ...
    bool mScEnable = false;  // IScenarioRecorder enable option

    // Hal3a
    std::mutex mHal3aLocker;
    SensorMap<std::shared_ptr<NS3Av3::IHal3A>> mHal3a;
    ...
    uint32_t                                     mReqMasterId = 0;
    std::vector<uint32_t>                        mReqSubSensorIdList;
    MUINT32                                      mPeriodForStreamingProcessDepth = 1;

    // for HDR
    SensorMap<std::shared_ptr<HDRPolicyHelper>>  mHDRHelper;
    SensorMap<std::shared_ptr<CHDRPolicyHelper>> mCHDRHelper;
    MINT32 mForcedVHDRMode = 0;

    // for Seamless Switch
    bool                                         mIsReturnFromSeamless = false;
    uint32_t                                     mCurrentSensorMode = 0;
    int32_t                                      mRcfFrameCnt = 0;
...
private:
    ...
    virtual auto    evaluateSeamlessSwitchConfiguration(
                        ConfigurationOutputParams* out,
                        ConfigurationInputParams const* in
    ) -> bool;
    ...
    virtual auto    updateStreamConfiguration(
                        ConfigurationOutputParams* out,
                        ConfigurationInputParams const* in
    ) -> bool;

    virtual auto    updateStreamConfiguration_Sensor(
                        ConfigurationOutputParams* out,
                        ConfigurationInputParams const* in,
                        uint32_t sensorId
    ) -> bool;

    virtual auto    extractStreamConfiguration(
                        ConfigurationOutputParams* out,
                        ConfigurationInputParams const* in
    ) -> bool;

    virtual auto    extractStreamConfiguration_Default(
                        ConfigurationOutputParams* out,
                        ConfigurationInputParams const* in
    ) -> bool;

    virtual auto    extractStreamConfiguration_Custom(
                        ConfigurationInputParams const* in
    ) -> CustomStreamConfiguration;

    virtual auto    negotiateStreamConfiguration(
    ...
    virtual auto    evaluateSeamlessSwitchSetting(
                        RequestOutputParams* out,
                        RequestInputParams const* in
                    ) -> bool;
    ...
    virtual auto updateDualCamRequestOutputParams(
                        ...
                        std::unordered_map<uint32_t, NSPipelinePlugin::MetadataPtr>& halMetaPtrList,
                        RequestInputParams const* in
                    ) -> bool;

    virtual auto updateStreamMultiFrame(
                        RequestOutputParams* out,
                        RequestInputParams const* in,
                        NSPipelinePlugin::MetadataPtr pOutMetaApp_Additional,
                        NSPipelinePlugin::MetadataPtr pOutMetaHal_Additional,
    ...
    virtual auto updateMtkCaptureFeatureHint(
                        MINT64 combinedKeyFeature, /*eFeatureIndexMtk and eFeatureIndexCustomer*/
                        RequestOutputParams* out,
                        RequestInputParams const* in
                    ) -> bool;

    virtual auto updateCaptureVHDRInfo(
                        RequestOutputParams* out
                    ) -> void;

    // ScenarioRecorder for debug
    virtual auto writeScenarioRecorderResult(
                        const int32_t requestNo,
                        const uint32_t sensorId,
                        const char* log,
                        const bool toHealline = false
                    ) -> void;
    ...
    virtual auto updateRequestResultParams_Streaming(
                        RequestInputParams const* in,
                        std::shared_ptr<RequestResultParams> &requestParams,
                        NSPipelinePlugin::MetadataPtr pOutMetaApp_Additional,
                        NSPipelinePlugin::MetadataPtr pOutMetaHal_Additional,
    ...
    virtual auto updateStreamData_Sensor(
                        RequestOutputParams* out,
                        RequestInputParams const* in,
                        uint32_t sensorId
                    ) -> bool;

    virtual auto emplaceStreamData(
                        RequestOutputParams* out,
                        RequestInputParams const* in
                    ) -> bool;

    virtual auto extractStreamData_Custom(
                        RequestInputParams const* in
                    ) -> CustomStreamData;

    virtual auto decideReqMasterAndSubSensorlist(
                        RequestInputParams const* in,
    ...
    virtual auto collectHDRCaptureInfo() -> void;
    ...
    virtual auto getStreamP1DmaConfig(
                        uint32_t &needP1Dma,
                        RequestInputParams const* in,
                        uint32_t sensorId = 0
                    ) -> bool;
    ...
    virtual bool checkNeedUpdateSetting(
                        std::vector<uint32_t> const& ids
                    );

private:
    void getPIPInfo(ParsedPIPInfo &outPIPInfo);
    ...
    void appendPrivateDataUpdateFunc_Streaming(
                    RequestOutputParams* out,
                    RequestInputParams const* in,
                    uint32_t sensorId);
