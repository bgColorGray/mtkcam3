...
) -> int
{
    ...
    auto main1SensorId = mPipelineStaticInfo->sensorId[0];
    ...
    if (!mInit)
    {
        ...
        if( ! infohelper->updateInfos() ) {
            ...
            mbNeedPDE = infohelper->isType3PDSensorWithoutPDE((*in.pSensorMode).at(main1SensorId));
            MY_LOGD("SensorMode %d, isType3PDSensorWithoutPDE %d", (*in.pSensorMode).at(main1SensorId), mbNeedPDE);
        }
        mInit = true;
    }
        ...
        && (!noP2Node))
    {
        ...
        //bNeedPDE
        out.zslPolicyParams.needPDProcessedRaw       = true;
    }
    ...
    auto buildOutParam = [&] (std::shared_ptr<featuresetting::RequestResultParams> const setting, int dummy, bool isMain) -> int
    {
          ...
          && CC_LIKELY(mPolicyTable->fIOMap_NonP2Node != nullptr) )
        {
            ...
            if (setting != nullptr)
            {
                ...
                if (setting->needP1Dma.size() > 0 && topologyOut.pNodesNeed!=nullptr && topologyOut.pNodesNeed->needRaw16Node)
                {
                    setting->needP1Dma.at(main1SensorId) |= P1_IMGO;
                }
        ...
        if (bNeedPDE && misPureRaw)
        {
            ...
            result->additionalHal.at(main1SensorId)->update(entry.tag(), entry);
