...
std::shared_ptr<IRequestSettingPolicyMediator> makeRequestSettingPolicyMediator_AOV(MediatorCreationParams const& params);
...
) -> std::shared_ptr<IPipelineSettingPolicy>
{
    ...
    //  Always-On Vision
    ////////////////////////////////////////////////////////////////////////////
    if (pParsedAppConfiguration->aovMode > 0) {
        MY_LOGD("Always-On Vision (AOV) mode:%d", pParsedAppConfiguration->aovMode);
        // AOV mode: step1
        if (pParsedAppConfiguration->aovMode == 1) {
            _MEDIATOR_(pRequestSettingPolicyMediator, makeRequestSettingPolicyMediator_AOV);
        }
        // AOV mode: step1/step2
        _POLICY_(fConfigPipelineNodesNeed, makePolicy_Configuration_PipelineNodesNeed_AOV());
        _POLICY_(fConfigSensorSetting, makePolicy_Configuration_SensorSetting_AOV());
        _POLICY_(fConfigP1DmaNeed, makePolicy_Configuration_P1DmaNeed_AOV());
        goto lbDefault;
    }
    ...
    if(pPipelineStaticInfo->isDualDevice)
    {
        if(pParsedMultiCamInfo->mDualDevicePath == DualDevicePath::MultiCamControl)
        {
            ...
            _POLICY_( fConfigBufferPoolSharedStream,makePolicy_Configuration_BufferPoolSharedStream_Multicam() );
            ...
            for(size_t i = 0;i<params.pPipelineStaticInfo->sensorId.size();i++)
            {
                ...
                if(physicalPipelineStaticInfo != nullptr)
                {
                    ...
                    physicalPipelineStaticInfo->numsP1YUV = params.pPipelineStaticInfo->numsP1YUV;
...
lbDefault:
    {
        ...
        _POLICY_IF_EMPTY_( fNDDMetadata,                     makePolicy_NDDMeta_Default() );
