...
auto
isCustomPrvStream(
    IMetadata const* pRequest_AppControl __attribute__((unused)),
    android::sp<IImageStreamInfo> pImgInfo __unused
) -> bool
{
    return false;
...
) -> bool
{
    ...
    // add app name check +
    if (pRequest_AppImageStreamInfo->previewStreamId != -1)
    {
        return false;
    }
    // add app name check -
    if (!(pRequest_AppImageStreamInfo->hasVideoConsumer) || !checkIsVSSControl(in->pRequest_AppControl, in->pRequest_ParsedAppMetaControl))
    {
        ...
        if ( ! ret )
        {
            //[ITS test] captureIntent==manual => P2CaptureNode
...
) -> void
{
    ...
    if (pRequest_AppImageStreamInfo->pAppImage_Jpeg != nullptr) {
        // Heic : no need Hal jpeg yuv
        if (pConfiguration_StreamInfo_NonP1->pHalImage_Jpeg_YUV != nullptr) {
...
) -> void
{
    ...
    for (auto const& it : imageIds_Capture)
    {
        ...
        {
            MY_LOGD("find output YUV stream (%#" PRIx64 ") from P2C in imageIds_Capture", it);
    ...
    {
        for (auto const& streamid : it.second)
        {
            if(checkIsYuvStream(streamid, pConfiguration_StreamInfo_NonP1, pRequest_AppImageStreamInfo))
            {
                MY_LOGD("find output YUV stream (%#" PRIx64 ") from P2C in imageIds_Capture_Physical (sensor id:%u)", streamid, it.first);
                out->needYuvOutputFromP2C = true;
                return;
            }
        }
...
) -> void
{
    ...
    needP2StreamNode = false;
    {
        ...
        for (auto const& it : pRequest_AppImageStreamInfo->vAppImage_Output_Proc)
        {
            if (isCustomCapStream(pRequest_AppControl, it.second))
            {
                ...
                MAX_IMAGE_SIZE(out->maxP2CaptureSize, it.second->getImgSize());
                  ...
                  || (pRequest_AppImageStreamInfo->previewStreamId == it.first) )
            {
            ...
            for(auto const& info:it.second)
            {
                if (isCustomCapStream(pRequest_AppControl, info))
                {
                    ...
                    MAX_IMAGE_SIZE(out->maxP2CaptureSize, info->getImgSize());
                        ...
                        || (pRequest_AppImageStreamInfo->previewStreamId == info->getStreamId()) )
                {
                    needP2StreamNode = true;
                    addStreamToPhysicalList(
        ...
        // currenly, depth stream just available on P2Streaming
        if (pRequest_AppImageStreamInfo->pAppImage_Depth != nullptr) {
            MY_LOGD("P2S+P2C: Add depth to StreamNode");
            needP2StreamNode = true;
            imageIds_Stream.push_back(pRequest_AppImageStreamInfo->pAppImage_Depth->getStreamId());
            MAX_IMAGE_SIZE(out->maxP2StreamSize, pRequest_AppImageStreamInfo->pAppImage_Depth->getImgSize());
        }

        if (pRequest_AppImageStreamInfo->pAppImage_Jpeg != nullptr)
        {
            StreamId_T streamId_Thumbnail_YUV = (in->needThumbnail) ? pConfiguration_StreamInfo_NonP1->pHalImage_Thumbnail_YUV->getStreamId() : (-1);
            // Heif capture use p2c
            auto imgFormat = pRequest_AppImageStreamInfo->pAppImage_Jpeg->getImgFormat();
    ...
    // metadata
    {
        ...
        if (needP2StreamNode)
        {
            ...
            if (pRequest_AppImageStreamInfo->hasVideoConsumer || !needP2CaptureNode) {
...
) -> int
{
    ...
    if (!(in.hasP2CaptureNode))
    {
        ...
        // currenly, depth stream just available on P2Streaming
        if (pRequest_AppImageStreamInfo->pAppImage_Depth != nullptr) {
            MY_LOGD("P2S-only: add depth to StreamNode");
            out.vImageStreamId_from_StreamNode.push_back(pRequest_AppImageStreamInfo->pAppImage_Depth->getStreamId());
        }
        out.needP2CaptureNode = false;
