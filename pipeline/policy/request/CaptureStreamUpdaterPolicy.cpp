...
) -> int
{
    ...
    if  ( entryThumbnailSize.isEmpty() ) {
        if (!pCfgJpegYUV.get()) {
            MY_LOGW("AP doesn't set thumbnail size setting, "
                "and hal Jpeg Yuv is null, so skip thumbnail stream setting");
            return OK;
        }
    ...
    //
    MSize const yuvthumbnailsize = (pCfgJpegYUV.get()) ?
        calcThumbnailYuvSize(pCfgJpegYUV->getImgSize(),thumbnailSize) : thumbnailSize;
    //
