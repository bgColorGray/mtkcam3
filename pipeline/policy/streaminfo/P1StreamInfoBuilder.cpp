...
)
{
    ...
    if (CC_UNLIKELY( ! plugin.has_value() || plugin->pluginId == PluginId::BAD )) {
        MY_LOGD("plugin may not exist - has_value:%d", plugin.has_value());
        return ret;
    }
...
GETP1STREAMID(P1SeamlessImgo,
    eSTREAMID_IMAGE_PIPE_SEAMLESS_RAW_OPAQUE_00,
    eSTREAMID_IMAGE_PIPE_SEAMLESS_RAW_OPAQUE_01,
    eSTREAMID_IMAGE_PIPE_SEAMLESS_RAW_OPAQUE_02
)
...
auto P1ImageStreamInfoBuilder::queryMinBufNum(char const* name) -> int32_t
{
    ...
    MY_LOGF_IF(name==nullptr,"name is nullptr!");
...
) -> P1ImageStreamInfoBuilder&
{
    ...
    return setP1Imgo_Default(maxBufNum, rP1HwSetting,false);
}

auto P1ImageStreamInfoBuilder::setP1SeamlessImgo_Default(
    P1HwSetting const& rP1HwSetting
) -> P1ImageStreamInfoBuilder&
{
    size_t maxBufNum = 1;
    return setP1Imgo_Default(maxBufNum, rP1HwSetting,true);
}

auto P1ImageStreamInfoBuilder::setP1Imgo_Default(
    size_t maxBufNum,
    P1HwSetting const& rP1HwSetting,
    bool isSeamlessSwitchImgo
) -> P1ImageStreamInfoBuilder&
{
    auto const& setting = isSeamlessSwitchImgo? rP1HwSetting.imgoSeamlessRequest:rP1HwSetting.imgoDefaultRequest;
    MINT const imgFormat = setting.format;
    MSize const& imgSize = setting.imageSize;
    IImageStreamInfo::BufPlanes_t defaultBufPlanes;
    bool ret = mHwInfoHelper->getDefaultBufPlanes_Imgo(defaultBufPlanes, imgFormat, imgSize,isSeamlessSwitchImgo);
    ...
    ret = mHwInfoHelper->getDefaultBufPlanes_Imgo(configBufPlanes, imgConfigFormat, imgConfigSize,isSeamlessSwitchImgo);
    ...
    {
        setStreamName((std::string("Hal:Image:P1:SeamlessRaw_")+getP1StreamNamePostfix(mIndex)));
        setStreamId(getStreamId_P1SeamlessImgo(mIndex));
    }
...
auto P1ImageStreamInfoBuilder::setP1Imgo_Stagger(
    P1HwSetting const& rP1HwSetting,
    MINT batchNum
) -> P1ImageStreamInfoBuilder&
{
    ...
    IImageStreamInfo::BufPlanes_t allocBufPlanes;
    allocBufPlanes.count = 1;
    allocBufPlanes.planes[0] = IImageStreamInfo::BufPlane{allocSize*batchNum, allocSize*batchNum};
    setStreamName((std::string("Hal:Image:P1:Fullraw_")+getP1StreamNamePostfix(mIndex)));
    setStreamId(getStreamId_P1Imgo(mIndex));
    setStreamType(eSTREAMTYPE_IMAGE_INOUT);
    setMaxBufNum(maxBufNum);
    ...
    setImgFormat(imgFormat);
    setImgSize(imgSize);
    setBufCount(batchNum);
    setStartOffset(0);
    setBufStep(allocSize);
    setBufPlanes(defaultBufPlanes);
    setSecureInfo(mpPipelineUserConfiguration->pParsedAppImageStreamInfo->secureInfo);
    return *this;
}
...
auto P1STTImageStreamInfoBuilder::setPluginQueryParam(void *param) -> void
{
    mPluginQueryParam = param;
}
...
) -> P1ImageStreamInfoBuilder&
{
    ...
    // Adjust maxBufNum by fps
    maxBufNum = maxBufNum * ((sensorFps + 29) / 30);
    MY_LOGD("Sensor fps: %d, FD maxBufNum: %lu", sensorFps, maxBufNum);

    MINT imgFormat = eImgFmt_YUY2;
    MSize const& imgSize = rP1HwSetting.fdyuvSize.size() != 0 ? rP1HwSetting.fdyuvSize : rP1HwSetting.AIAWBYuvSize;
    ...
    {
        case 0:
            setStreamId(eSTREAMID_IMAGE_PIPE_CRZOR1_main1);
            break;
        case 1:
            setStreamId(eSTREAMID_IMAGE_PIPE_CRZOR1_main2);
            break;
        case 2:
            setStreamId(eSTREAMID_IMAGE_PIPE_CRZOR1_main3);
            break;
        default:
            setStreamId(eSTREAMID_IMAGE_PIPE_CRZOR1_main1);
            break;
