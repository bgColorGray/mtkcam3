...
struct BuildPipelineContextInputParams
{
     ...
     * image buffer pool.
     */
    std::vector<std::set<StreamId_T>> const*    pPoolSharedStreams = nullptr;
    ...
    /**
     *  BuildStream with SW 4Cell settings. (raw from P2C)
     */
    bool                                        bIsSW4Cell = false;

    /**
     * The sensor settings.
     */
    SensorMap<SensorSetting> const*             pSensorSetting = nullptr;
    ...
    /**
     * The seamless switch feature settings.
     */
     ...
     */
    SensorMap<uint32_t> const*                  pBatchSize = nullptr;
     ...
     * isSwitchSensor is used to indicates current config is sensor switch flow or not.
     */
    int32_t                                        initRequest = false;

    MBOOL                                       bEnableCaptureFlow = MFALSE;
    /**
...
/**
 * Unsed on the call preConfigureP1ForPipelineContext().
 */
struct PreConfigInputParams
{
    /**
     * enable preQueue
     */
    bool    bEnablePreQueue = false;
    NodeId_T         nodeId = 0;
};
...
/**
 * preConfigure P1 by original pipeline context.
 *
 * @param[out] pContext: a original pipeline context and conifg P1 for new setting.
 *
 * @param[in] in: input parameters.
