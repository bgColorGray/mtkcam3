...
) -> std::shared_ptr<IFrameBuilder>
{
    ...
    auto setAppImage = [&builder](auto const& bufferMap,
        auto const& pStreamInfo){
            if ( pStreamInfo ) {
                ...
                if( gLogLevel >= 1 ){
                    MY_LOGD("setAppImageStreamBuffer: %#" PRIx64 " buffer:%p",
                    streamId, pStreamBuffer.get());
                }
            }
        };
#define SET_APP_IMAGE(_BufferMap_, _Arg_...) setAppImage(pAppImageStreamBuffers->_BufferMap_, _Arg_)
        ...
        for ( auto&& v : pAppImageStreamInfo->vAppImage_Output_RAW10 ) {
            SET_APP_IMAGE( vOImageBuffers, v.second );
        }

        SET_APP_IMAGE( vOImageBuffers, pAppImageStreamInfo->pAppImage_Depth );
        // physical stream
        ...
        for ( auto&& s : pAppImageStreamInfo->vAppImage_Output_RAW10_Physical ) {
            for ( auto&& v : s.second ) {
                SET_APP_IMAGE( vOImageBuffers, v );
            }
        }
...
#undef  SET_APP_IMAGE
    ...
    if  (auto&& pSBuffers = in.pAppMetaStreamBuffers) {
        for ( auto&& pSBuffer : *pSBuffers ) {
            if (pSBuffer->getStreamInfo() != nullptr) {
