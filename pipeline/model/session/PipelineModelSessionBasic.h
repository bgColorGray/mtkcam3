...
protected:  ////    2nd configuration info (unchangable unless re-configuration)
    struct ConfigInfo2
    {
        ...
        SeamlessSwitchFeatureSetting                mSeamlessSwitchFeatureSetting;
        PipelineNodesNeed                           mPipelineNodesNeed;
        PipelineTopology                            mPipelineTopology;
        SensorMap<SensorSetting>                    mvSensorSetting;
        SensorMap<P1HwSetting>                      mvP1HwSetting;
        SensorMap<uint32_t>                         mvP1DmaNeed;
        SensorMap<ParsedStreamInfo_P1>              mvParsedStreamInfo_P1;
        ParsedStreamInfo_NonP1                      mParsedStreamInfo_NonP1;
        bool                                        mIsZSLMode = false;
        MINT32                                      mUniKey = 0;
        std::vector<std::set<StreamId_T>>           mPoolSharedStreams;
    };
...
protected:  ////    private request info (changable)
     ...
     */
    SensorMap<uint32_t>             mSensorMode;
    SensorMap<MSize>                mSensorSize;
    // for 60 fps +
    SensorMap<uint32_t>             mSensorFps;
    // for 60 fps -

    /**
     * seamless switch enable
     */
    mutable android::RWLock         mRWLock_seamlessEnable;
    bool isHWReadyForSeamlessSwitch = false;
...
protected:  ////    Operations (Request Stage).
    ...
    virtual auto    getSeamlessSwitchAvailable() -> bool;
    /**
     * Process frame(s) which have been evaluated.
     *
...
public:     ////
    ...
    virtual MVOID   onHWReady() override;
};
