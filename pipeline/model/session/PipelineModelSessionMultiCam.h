...
public:
    auto sendNotify(
            ...
            MUINTPTR arg2,
            MUINTPTR arg3) -> bool;
    auto isAlreadyNotify(uint32_t reqNo, TYPE type) -> bool;
    auto requestFinish(uint32_t reqNo) -> bool;
private:
    //
    std::weak_ptr<policy::pipelinesetting::IPipelineSettingPolicy> mpSettingPolicy;
    //
    std::mutex mLock;
    std::unordered_map<TYPE, std::vector<uint32_t>> mvCallback_id;
};
/******************************************************************************
 *
 ******************************************************************************/
...
public:     ////    Instantiation.
    ...
    virtual MVOID   onConfigureDone() override;
    MVOID   onRequestDone(
                    MUINT32   requestNo
                );
...
private:
    ...
    auto    getAfState(MUINT32 const requestNo,
                       MINTPTR const userId,
                       Result const& result,
                       MUINT8 &afstate) -> bool;
    auto    handleIspQualityCheck(
                        MUINT32 const requestNo,
                        Result const& result) -> bool;
    auto    doSwitchSensorFlow(
                policy::pipelinesetting::RequestOutputParams const& reqOutput) -> bool;
    auto    doP1ReConfigFlow(
                policy::pipelinesetting::RequestOutputParams const& reqOutput) -> bool;
...
private:
    ...
    std::shared_ptr<NSCam::JobQueue<void()>> mConfigJobQueue = nullptr;
    //
    std::unordered_map<uint32_t, std::shared_ptr<NS3Av3::IHal3A>>
                                            mvHal3AList;
    //
    bool isIspQualityAccept = false;
    //
