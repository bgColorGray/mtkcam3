...
ThisNamespace(
    ...
    : PipelineModelSessionBase(
        ...
        rCtorParams)
{
    auto PowerHalService = IMtkPower::getService();
    int32_t coreNum = 0;
    int32_t clusterNum = PowerHalService->querySysInfo(MTKPOWER_CMD_GET_CLUSTER_NUM,0);
    for( int32_t i = 0; i < clusterNum; i++)
    {
        coreNum += PowerHalService->querySysInfo(MTKPOWER_CMD_GET_CLUSTER_CPU_NUM, i);
    }
...
static android::String8 toString(const std::unordered_map<T, U>& o)
{
    android::String8 os;
    for (const auto& [sensorId, v] : o) {
        os += android::String8::format(" [%d]: ", sensorId);
        os += toString(v);
    }
    return os;
};
...
{
    auto toStringP1DmaNeed = [](std::unordered_map<SensorId_T, uint32_t>const& o) -> android::String8 {
        android::String8 os;
        for (const auto& [sensorId, v] : o) {
            if  (o.size() > 1) { os += android::String8::format(" [%d] ", sensorId); }
            os += "{ ";
    ...
    printer.printFormatLine(".mUniKey=%d", o.mUniKey);
}
...
) -> int
{
    ...
    HwInfoHelper helper(main1sensorId);
    if (!helper.getSensorOutputSize(pConfigInfo2->mvSensorSetting.at(main1sensorId).sensorMode, param.sensorSize))
    {
        param.sensorSize = pConfigInfo2->mvSensorSetting.at(main1sensorId).sensorSize;
    }
    param.sensorFps  = pConfigInfo2->mvSensorSetting.at(main1sensorId).sensorFps;
    ...
    for (const auto& [_sensorId, _sensorSetting] : pConfigInfo2->mvSensorSetting)
    {
        ...
        {
            auto parsedStreamInfo_P1 = pConfigInfo2->mvParsedStreamInfo_P1.at(_sensorId);
            bool useYUVO = false;

            if (parsedStreamInfo_P1.pHalImage_P1_Rrzo != nullptr)
            {
                useYUVO = isHalYuvb10Format((EImageFormat)(parsedStreamInfo_P1.pHalImage_P1_Rrzo->getImgFormat()));
            }
            Sensor.P1YuvOut =  ( parsedStreamInfo_P1.pHalImage_P1_FDYuv != nullptr
                              || parsedStreamInfo_P1.pHalImage_P1_ScaledYuv != nullptr
                              || parsedStreamInfo_P1.pHalImage_P1_RssoR2 != nullptr
                              || useYUVO
                              );
            if (isCapture && parsedStreamInfo_P1.pHalImage_P1_Imgo != nullptr)
            {
                bbp = (float)(parsedStreamInfo_P1.pHalImage_P1_Imgo->getAllocBufPlanes().planes[0].sizeInBytes) /
                        parsedStreamInfo_P1.pHalImage_P1_Imgo->getImgSize().w /
                        parsedStreamInfo_P1.pHalImage_P1_Imgo->getImgSize().h;
                if (bbp > 2.0)
                {
                    bbp = 2.0;
                }
...
) -> int
{
    ...
    uint32_t lastFrameNo = -1;
    uint32_t lastZslFrameNo = -1;
    RETURN_ERROR_IF_NOT_OK( onProcessEvaluatedFrame(lastFrameNo, lastZslFrameNo, in),
            "[requestNo:%u] onProcessEvaluatedFrame failed", request->requestNo);

    mpScenarioCtrl->setCPUProfile(reqOutput.CPUProfile);
    return OK;
}
...
onHWReady()
{
    android::RWLock::AutoWLock _l(mRWLock_seamlessEnable);
    isHWReadyForSeamlessSwitch = true;
}
...
auto
ThisNamespace::
getSeamlessSwitchAvailable() -> bool
{
    android::RWLock::AutoRLock _l(mRWLock_seamlessEnable);
    return isHWReadyForSeamlessSwitch;
}
