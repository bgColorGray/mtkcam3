...
) -> void
{
    ...
    if (CC_LIKELY( pTargetFrameControl != nullptr )) {
        ...
        if ( forceSelect ) {
            ...
            MY_LOGW("%s=true", key);
            if ( pTargetFrameControl != nullptr ) {
                MY_LOGW("[workaround] "
...
transferBuffersFromFrame(TransferBuffersFromFrameParams const& params) -> void
{
    ...
    )
    {
        ...
        for (size_t i = 0; i < vStreamBuffer.size(); i++) {
            ...
            if (CC_LIKELY( pBuffer != nullptr )) {
                auto streamInfo = pBuffer->getStreamInfo();
                MY_LOGF_IF(streamInfo == nullptr, "Bad pBuffer->getStreamInfo()");
    ...
    transferBuffers(
        ...
        /*ignoreBuffer*/[ ](auto b){ if (b != nullptr) { b->releaseBuffer(); } },
        pHistoryFrameControl,
