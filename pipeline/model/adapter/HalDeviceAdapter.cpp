    ...
    virtual auto    powerOn() -> bool override
                    {
                        ...
                        auto initSensor = [ this ](size_t i) {
                            std::string const name(std::string("powerOnSensor")+std::to_string(i));
                            CAM_TRACE_NAME(name.c_str());
                            ::prctl(PR_SET_NAME, (unsigned long)name.c_str(), 0, 0, 0);
                            CAM_ULOGMD("[initSensor] %s", name.c_str());
                                    //
                                    {
                        ...
                        std::vector<std::future<bool>> futures_initSensor;
                        for (size_t i = 0; i < mvPhySensorId.size(); i++)
                        {
                            futures_initSensor.push_back(
                        ...
                        //(3) Wait for Sensor
                        for (auto& future_initSensor : futures_initSensor)
                        {
    ...
    virtual auto    powerOff() -> void override
                    {
                        ...
                        auto uninit = [ this ](size_t i, bool isAsync) {
                            std::string const name(std::string("powerOff")+std::to_string(i));
                            CAM_TRACE_NAME(name.c_str());
                            if ( isAsync ) {
                                ::prctl(PR_SET_NAME, (unsigned long)name.c_str(), 0, 0, 0);
                            }
                        ...
                        std::vector<std::future<void>> futures_uninit;
                        for (size_t i = 0; i < mvPhySensorId.size(); i++)
                        {
                            bool isAsync = (mvPhySensorId.size() > 1);
                            futures_uninit.push_back(
                        ...
                        for (auto& future_uninit : futures_uninit) {
                            future_uninit.get();
                        }
