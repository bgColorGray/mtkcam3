...
template<typename T>
std::map<MINT32, typename PipelinePlugin<T>::WeakPtr>
PipelinePlugin<T>::mInstances;

template<typename T>
std::mutex PipelinePlugin<T>::mInstancesMutex;
...
PipelinePlugin<T>::getInstance(MINT32 iUniqueKey)
{
    ...
    std::lock_guard<std::mutex> mapLock(mInstancesMutex);
    Ptr sp =  mInstances[hash].lock();
    if (sp == nullptr) {
        sp = std::make_shared<PipelinePlugin<T>>(hash);
        mInstances[hash] = sp;
        MY_LOGI("%s: create plugin(%p), hash:%d, uniqueKey:%d, mInstances size:%zu",
            getName<T>().c_str(), sp.get(), hash, iUniqueKey, mInstances.size());
    }
...
template<typename T>
PipelinePlugin<T>::PipelinePlugin(MINT32 hash)
: mpInterface(nullptr)
, mInstanceHash(hash)
{
    MY_LOGI("%s: ctor(%p), hash:%d", getName<T>().c_str(), this, mInstanceHash);
}
...
PipelinePlugin<T>::~PipelinePlugin()
{
    ...
    std::lock_guard<std::mutex> mapLock(mInstancesMutex);
    auto it = mInstances.find(mInstanceHash);
    if (it != mInstances.end()) {
        Ptr sp =  mInstances[mInstanceHash].lock();
        mInstances.erase(it);
        MY_LOGI("remove plugin(%p), hash:%d, mInstances size:%zu", sp.get(), mInstanceHash, mInstances.size());
    }
}
...
MVOID BufferSelection::getAlignment(MSize& rSize) const
{
    rSize = mImpl->mAlignment;
}
...
MVOID BufferSelection::setCapability(eBufferCapability cap)
{
    mImpl->mCapability |= (MINT32)cap;
}

MBOOL BufferSelection::hasCapability(eBufferCapability cap) const
{
    return (mImpl->mCapability & cap);
}
...
MVOID BufferSelection::dump(std::ostream& os) const
{
    ...
    os << " Capability:(" << mImpl->mCapability << ")";
