...
getTuningFromNvram(MUINT32 openId, MINT32 magicNo, MINT64 featureCombination, const IMetadata* pInHalMeta) const
{
    ...
    {
        pMapInfo->eIspProfile = EIspProfile_MFNR_Single;  // EIspProfile_MFNR_After_Blend;
        MY_LOGD("get threshold from ISP profile(%d)", pMapInfo->eIspProfile);
    }
    else
    {
        pMapInfo->eIspProfile = EIspProfile_Capture;
        MY_LOGD("get threshold from ISP profile(%d)", pMapInfo->eIspProfile);
    }
#else
    ...
    UNREFERENCED_PARAMETER(featureCombination);
        ...
        RequestCallbackPtr pCallback)
{
    ...
    MY_LOGD("debug: new test");
    {
        ...
        if (mpSwnr == NULL && !mbLpcnrEn) {
            ...
            if(sensorId < 0 ) {
                ...
                mSensoorID = sensorId;
                mpSwnr = (ISwNR*)MAKE_SwNR(mSensoorID);
            }
            CAM_TRACE_FMT_END();
        }
        else if (mpSwnr != NULL && mSensoorID != sensorId && sensorId >= 0) {
            MY_LOGD("sensorID change from %d to %d, re-make SwNR", mSensoorID, sensorId);
            delete mpSwnr;
            mpSwnr = nullptr;
            mSensoorID = sensorId;
            mpSwnr = (ISwNR*)MAKE_SwNR(mSensoorID);
    ...
    // 2. enque
    {
        int debug_sync = ::property_get_int32("debug.plugin.swnr.sync", 1);
        ...
        if((!pCallback.get()) || debug_sync) {
            ...
            mvFutures.insert(
                    ...
                    std::async(std::launch::deferred,
                        [=]() -> int32_t {
                            ::prctl(PR_SET_NAME, "Cam@doSWNR", 0, 0, 0);
                            int32_t err = OK;
...
)
{
    ...
    if(mLpcnr.get() && mbLpcnrEn) {
        ...
        if (mpSwnr == nullptr) {
            ...
            mSensoorID = sensorId;
            mpSwnr = MAKE_SwNR(mSensoorID);
            CAM_TRACE_END();
        }
        else if (mSensoorID != sensorId && sensorId >= 0) {
            MY_LOGD("sensorID change from %d to %d, re-make SwNR", mSensoorID, sensorId);
            delete mpSwnr;
            mpSwnr = nullptr;
            mSensoorID = sensorId;
