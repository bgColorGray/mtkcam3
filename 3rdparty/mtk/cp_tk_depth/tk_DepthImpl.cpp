    ...
    mEnable(0)
{
    ...
    //
    mStereoModeType = StereoSettingProvider::getMtkDepthFlow();// 0: tk | 1: pure 3rd | 2: tk depth + 3rd
...
MERROR cp_tk_DepthProvider::process(RequestPtr requestPtr, RequestCallbackPtr callbackPtr)
{
    ...
    if (!setInputData(pDepMapReq, requestPtr) && !setOutputData(pDepMapReq, requestPtr))
    {
        ...
        if(!mpDepthMapPipe->enque(pDepMapReq)) {
            // enque failed, request failed
            processDone(requestPtr, callbackPtr, BAD_VALUE);
            return OK;
        }
    }
    else
...
MERROR cp_tk_DepthProvider::_copy2Target(IImageBuffer* pSrc,IImageBuffer* target)
{
    ...
    size_t outBufSize = 0, strides_diff = 0;
    size_t planeCnt = std::min(pSrc->getPlaneCount(), target->getPlaneCount());
