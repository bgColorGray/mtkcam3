...

{
    ...
    if (mSetImgSize) {
        StereoSizeProvider* pSizeProvider = StereoSizeProvider::getInstance();
        if (!pSizeProvider) {
            MY_LOGI("null StereoSizeProvider!!");
            return BAD_VALUE;
        }
        sel.mCfgInfo.mMaxOutSize = pSizeProvider->getPreviewSize();
        //
...
void S_SEMI_Plugin::init()
{
    ...
    case 90:  mTarRotate = 270; break;
    case 270: mTarRotate = 90;  break;
    case 180: mTarRotate = 180; break;
    case 0:
    default:
        break;
    }
    ...
    if (0 != mTarRotate) {
        ...
        MSize targetSize = pSizeProvider->getBufferSize(E_DEPTH_MAP);
        if (StereoHAL::isVertical(mTarRotate))//Need to Rotate
...
MERROR S_SEMI_Plugin::process(RequestPtr pRequest, RequestCallbackPtr /*pCallback*/)
{
        ...
        pRequest->mIBufferDepth.get() != nullptr)
    {
        ...
        if (mRotatedDepthImgPtr != nullptr && mbDumpBuffer && (0 != mTarRotate))
        {
            IMetadata* pInHalMeta;
            pInHalMeta = pRequest->mIMetadataHal1->acquire();
