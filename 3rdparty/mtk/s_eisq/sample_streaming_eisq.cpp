...
private:
    ...
    int   mMargin = 0;
    int   mQueueCount = 0;
    int   mStage1CountDown = 0; // direct output rec frame with eis algo
    int   mStage2CountDown = 0; // queue rec frame
    int   mOpenID1 = 0;
    int   mOpenID2 = 0;
...
MERROR S_EISQ_Plugin::getConfigSetting(Selection &sel)
{
    ...
    IMetadata::getEntry(meta, MTK_STREAMING_FEATURE_DEMO_EIS, mDemoMode);

    mEISPreview = property_get_bool("vendor.debug.tpi.s.eisq.preview", (mDemoMode == 2) ) ;
    // margin & qcount should sync with customer_scenario_mgr.cpp
    mMargin = property_get_int32("vendor.debug.tpi.s.eisq.margin", 0);
    ...
    sel.mCfgSharedInput = property_get_bool("vendor.debug.tpi.s.eisq.shareinput", MTRUE);
    //sel.mCfgEnableFD = MTRUE;
    sel.mCfgRun = property_get_bool("vendor.debug.tpi.s.eisq", (mDemoMode != 0) );
...
MERROR S_EISQ_Plugin::process(RequestPtr pRequest, RequestCallbackPtr pCallback)
{
    ...
    if( needRun && pRequest->mIBufferMain1 != NULL && cropValid)
    {
        ...
        // mOQueueCmd define how APP get record result when record buffer exists.
        // 1) PUSH: mark this record buffer as ERROR. it cause video codec skip this frame
        // 2) PUSH_POP: use record buffer as result
        // 3) default(error handle): use DISPLAY buffer as record result. it cause MP4 encoding the display content

        // For this queue flow:
        // i.e:
        //   stage1: direct output count(18)
        //   stage2: eis queue count(12)
        //
        //   frame 1~18 will process immediatly (record to video file)
        //   frame 19~30 will be queued ( record buffer mark error)
        //   frame 31 start to process frame 19 from queue ( record to video file with timestamp modification)
        ...
        if( rec == NULL )
        {
            ...
            // reset stage 1 counter
            // direct output frame to avoid < 1 seconds recording fail
            // suggest>13 due to encoder may drop 10 frames to avoid record start sound
            mStage1CountDown = property_get_int32("vendor.debug.tpi.s.eisq.stage1.countdown", 18);

            // reset stage 2 counter
            // eis queue numbers
            mStage2CountDown = mQueueCount;
        }
        else if ( mStage1CountDown > 0 )
        {
            MY_LOGD("Stage 1 countdown %d", mStage1CountDown);
            // Just direct handle recording
            mStage1CountDown--;

            // Start Output record. push current frame and pop immediate
            // ONLY PUSH_POP will send record buffer to encoder
        ...
        else if( mStage2CountDown > 0 )
        {
            MY_LOGD("Stage 2 countdown %d", mStage2CountDown);
            mStage2CountDown--;
            // Just do eis display, start queue
            // ONLY PUSH will mark record buffer as ERROR buffer (skip encoder)
