...
S_FOVA_Plugin::S_FOVA_Plugin()
{
    ...
    mbDebugTest  = ::property_get_int32("vendor.debug.vsdof.fova.debug", 0);
...

{
    ...
    sel.mCfgRun       = property_get_bool("vendor.debug.tpi.s.fova", MTRUE);
    sel.mCfgSharedInput = property_get_bool("vendor.debug.tpi.s.fova.shareinput", MFALSE);
        ...
        switch (state) {
            ...
            case BOTH_VALID:
            {
                ...
                if (mCoordinate == 0) {
                    ...
                    // Now just copy master data to output to simulate fusion
                    ret = (imgCpy2Target(pRequest,
                                         pRequest->mIBufferMain1->acquire(),
                                         pRequest->mOBufferMain1->acquire()))
                          ?  OK : UNKNOWN_ERROR;
                }
                ...
                if (mbDebugTest) {  // Test use Src image
                    int32_t debugSrc = ::property_get_int32("vendor.debug.vsdof.fova.debug.src", 0);
                    int32_t src = (state == BOTH_VALID && debugSrc > 0) ? debugSrc : 1;
                    if (src == 1) {
                        pRequest->mOBufferMain1_Info.mOSrcImageID = eJoinImageID_MAIN1;
                    } else if (src == 2) {
                        pRequest->mOBufferMain1_Info.mOSrcImageID = eJoinImageID_MAIN2;
                    }
                    MY_LOGD("debug src id = %d, final src = %d", debugSrc, src);
                }
#endif
            }
            break;
