...
#if 0
    // This is workaround when we use AINR param as AIHDR param
    if (strategyCfg.vEvTypes.size() != 7) {
        ainrLogE("handle mismatch issue");
        strategyCfg.vEvTypes = {exposureType::SE, exposureType::NE,
                                exposureType::SE, exposureType::NE,
                                exposureType::SE, exposureType::NE,
                                exposureType::LE};
        strategyCfg.bssEvTypes = exposureType::SE;
        strategyCfg.frameCapture = 7;
    }
#endif

    ainrLogCAT("[AIHDR] currentIso:%d lowThreshold:%d highThreshold:%d frameCapture:%d bssEvTypes:%d",
...
bool AiHdrShot::initAinrCore() {
    ...
    cfg.needDRC    = true;
    cfg.mdlaMode    = MTK_FEATURE_AINR_MDLA_MODE_DRCOUT_16BIT;
...
void AiHdrShot::configAinrCore(IMetadata* pHalMeta, const IMetadata* pMetaDynamic) {
    CAM_ULOGM_APILIFE();
