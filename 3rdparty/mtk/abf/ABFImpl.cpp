...
isAbfEnable(IMetadata* pIMetadata_App)
{
  ...
  {
    case 0:
      MY_LOGW("force disable abf");
      result = false;
      break;
    case 1:
      MY_LOGW("force enable abf");
      result = true;
      break;
    default:
      if(CC_LIKELY(pIMetadata_App != NULL)){
        if(tryGetMetadata<MINT32>(pIMetadata_App, MTK_ABF_FEATURE_ABF_MODE, abfmode)){
  ...
  MY_LOGD("App set abfmode as : %d",abfmode);
  return result;
}
 /******************************************************************************
 *
 ...
 ******************************************************************************/
 AbfAdapter*
 AbfPluginProviderImp::
 getAbfAdapter(MINT32 openId)
 {
   if (mpAbf == NULL) {
     MY_LOGD("debug: create instance openId %d", openId);
     if(openId < 0 ) {
       MY_LOGE("need set openId before init() !!");
     }
     else {
       mpAbf = new AbfAdapter(openId);
     }
   }
   return mpAbf;
 }
        ...
        RequestCallbackPtr pCallback)
{
    ...
    if(mpAbf ==NULL)
      mpAbf = getAbfAdapter(sensorId);//get AbfAdapter instance
            ...
            mvFutures.insert(
                    ...
                    std::async(std::launch::deferred,
                        [=]() -> int32_t {
                            ...
                            int32_t err = OK;
                            err = this->doAbf(pRequest,sensorId);
                            if(pCallback.get() != nullptr)
