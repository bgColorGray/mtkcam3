...
public:
    ...
    struct CtrlerNDDParam {
        int32_t uniqueKey;
        int32_t requestNumber;
        explicit CtrlerNDDParam(int32_t uKey,
            int32_t reqNum)
            : uniqueKey(uKey)
...
// Ainr operations
public:
    // Provide NDD information
    void                        setTuningNddParam(const CtrlerNDDParam &param);
    // Update AINR params before take a picture.
    void                        updateAinrStrategy(NSCam::MSize size);

    void                        queryMDLAMode(IMetadata* pHalMeta);
...
public:
    ...
    inline int                  getMdlaMode() const noexcept      { return m_MDLAMode; }
    ...
    inline void                 setMdlaMode(int32_t m) noexcept { m_MDLAMode = m; }
...
protected:
    ...
    volatile bool               m_bFired; // represents if MFNR has been fired or not.
    bool                        m_bInit;
