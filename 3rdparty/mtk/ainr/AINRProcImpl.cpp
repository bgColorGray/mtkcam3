...
template<typename T>
static inline IMetadata *getMetaPointer(T const &metaInfo) {
    auto spMeta = metaInfo.getControl();
    if (spMeta) {
        return spMeta.get();
    } else {
        // We all know that it is not good to return nullptr
        // But it makes sense that we check handle before using it.
        // EX:IMetadata::getEntry & IMetadata::setEntry
        return nullptr;
    }
}
...
public:
    ...
    virtual MERROR queryRecommendedFrameSetting(RecommendedParam &param) {
        MERROR error = android::OK;
        IMetadata const* inAppMeta = param.pAppMetadata;
        if(inAppMeta == nullptr ) {
            ainrLogF("Cannot judge feature hint because of metadata nullptr");
            return android::BAD_VALUE;
        }

        int32_t tuningHint = MTK_CONTROL_CAPTURE_HINT_FOR_ISP_TUNING_DEFAULT_NONE;
        IMetadata::getEntry<MINT32>(inAppMeta, MTK_CONTROL_CAPTURE_HINT_FOR_ISP_TUNING, tuningHint);
        if (tuningHint != MTK_CONTROL_CAPTURE_HINT_FOR_ISP_TUNING_AINR) {
            ainrLogD("No need to do AINR, due to tuninghint(%d)", tuningHint);
            return android::BAD_VALUE;
        }

        int32_t iso = 0;
        MINT64 shutter = 0;
        NSCam::IMetadata::getEntry<MINT32>(inAppMeta, MTK_SENSOR_SENSITIVITY, iso);
        NSCam::IMetadata::getEntry<MINT64>(inAppMeta, MTK_SENSOR_EXPOSURE_TIME, shutter);
        ainrLogD("Current iso(%d) shutter(%" PRId64 " ns)", iso, shutter);

        int openId = 0; // no use
        uint32_t uKey = NSCam::Utils::TimeTool::getReadableTime();
        int32_t reqNum = -1;  // cannot get request number in this stage
        AinrCtrler::CtrlerNDDParam nddParam(uKey, reqNum);
        AinrMode mode = AinrMode_NormalAinr;
        std::shared_ptr<AinrCtrler> ctrler
                = std::make_shared<AinrCtrler>(openId, mode,
                        iso, shutter, false /*flashOnOff*/);
        ctrler->setTuningNddParam(nddParam);
        if(CC_UNLIKELY(ctrler.get() == nullptr)) {
            ainrLogF("Create controller fail");
            return android::BAD_VALUE;
        }

        // TODO(Yuan Jung) Fix query sensor size
        MSize sensorSize {};
        ctrler->updateAinrStrategy(sensorSize);

        // Judge whether to execute ainr or not
        if( (mEnable == 1) || ctrler->getIsEnableAinr()) {
            ainrLogD("Enable ainr mEnable(%d), ctrlerEnable(%d)"
                , mEnable, ctrler->getIsEnableAinr());

            auto frameCount = ctrler->getCaptureNum();
            auto &framesSetting = param.framesSetting;
            for(int i = 0; i < frameCount; i++) {
                std::shared_ptr<IMetadata> appMetaPtr = std::make_shared<IMetadata>();
                IMetadata::setEntry<MINT32>(appMetaPtr.get(), MTK_CONTROL_CAPTURE_HINT_FOR_ISP_TUNING, tuningHint);
                IMetadata::setEntry<MINT32>(appMetaPtr.get(), MTK_CONTROL_CAPTURE_HINT_FOR_ISP_FRAME_INDEX, i);
                IMetadata::setEntry<MINT32>(appMetaPtr.get(), MTK_CONTROL_CAPTURE_HINT_FOR_ISP_FRAME_COUNT, frameCount);
                framesSetting.push_back(appMetaPtr);
            }
        } else {
            ainrLogD("No need to execute ainr");
            return android::BAD_VALUE;
        }
    ...
    virtual MERROR negotiate(Selection& sel)
    {
        ...
        int32_t ainrSupport = ::property_get_int32("vendor.debug.plugin.ainr.support", 1);
        if(CC_UNLIKELY(ainrSupport == 0)) {
            ainrLogD("AINR feature not support");
            return BAD_VALUE;
        }

        mEnable = ::property_get_int32("vendor.debug.camera.ainr.enable", -1);
        mForceMDLAMode = ::property_get_int32("vendor.debug.camera.ainr.mdla.mode", -1);
        if(CC_UNLIKELY(mEnable == 0)) {
            if (mScenarioRecorderEn) {
                auto spHalMeta = sel.mIMetadataHal.getControl();
                IMetadata* pHalMeta = spHalMeta.get();
                if (!IMetadata::getEntry<MINT32>(pHalMeta,
                        MTK_PIPELINE_REQUEST_NUMBER, mRequestNumber)) {
                    ainrLogW("Get MTK_PIPELINE_REQUEST_NUMBER fail");
                }

                DecisionParam decParam;
                auto ukey = NSCam::Utils::TimeTool::getReadableTime();
                submitTriggerFeatureLog(ukey, mRequestNumber,
                        sel.mSensorId, decParam);
                WRITE_DECISION_RECORD_INTO_FILE(decParam, "Force off AINR");
            }
    ...
    /* update output buffer format */
    virtual MERROR negotiateUpdate(Selection& sel) override
    {
        MUINT32 imgFmt = eImgFmt_BAYER12_UNPAK;
        std::shared_ptr<AinrCtrler> ctrler = nullptr;
        bool mainFrame = 0;

        if (sel.mIspHidlStage == ISP_HIDL_STAGE_DISABLED) {
            mainFrame = ((sel.mRequestIndex == 0) ? 1 : 0);
        } else {
            mainFrame = ((sel.mStateIspHidl.frameIndex == 0) ? 1 : 0);
        }

        /* update main frame output fmt for isp6s only */
        if (!mainFrame) {
            return OK;
        }
        ...
        IMetadata* pHalAddMeta = sel.mIMetadataHal.getAddtional().get();
        if (!IMetadata::getEntry<MINT32>(pHalAddMeta,
        ...
        ctrler = getCtrlerInfo(processUniqueKey);
        if (CC_UNLIKELY(ctrler.get() == nullptr)) {
        ...
        ctrler->queryMDLAMode(pHalMeta);

        if (mForceMDLAMode >= 0) {
            ainrLogI("force set MDLAMode:%d", mForceMDLAMode);
            ctrler->setMdlaMode(mForceMDLAMode);
        }

        if (ctrler->getMdlaMode() == MTK_FEATURE_AINR_MDLA_MODE_NNOUT_16BIT) {
            imgFmt = eImgFmt_BAYER16_UNPAK;
        }

out:
        /* If cannot get ctrl set 12bits AINR directly */
        sel.mOBufferFull.addAcceptedFormat(imgFmt);
    ...
    MERROR negotiateHal(Selection& sel)
    {
        ...
        if (sel.mRequestIndex == 0) {
            ...
            if(gShotCount > 1) {
                ...
                if (mScenarioRecorderEn) {
                    DecisionParam decParam;
                    submitTriggerFeatureLog(mUniqueKey, mRequestNumber,
                            sel.mSensorId, decParam);
                    WRITE_DECISION_RECORD_INTO_FILE(decParam,
                            "No need to do AINR. ShotCount(%d)"
                            , gShotCount.load(std::memory_order_relaxed));
                }
                return BAD_VALUE;
            }
            ...
            IMetadata* pHalMeta = getMetaPointer(sel.mIMetadataHal);
            if (!IMetadata::getEntry<MINT32>(pHalMeta,
                    MTK_PIPELINE_REQUEST_NUMBER, mRequestNumber)) {
                ainrLogW("Get MTK_PIPELINE_REQUEST_NUMBER fail");
            }
            ...
            if (CC_UNLIKELY(mMfbMode == AinrMode_Off)) {
                if (mScenarioRecorderEn) {
                    ...
                    submitTriggerFeatureLog(mUniqueKey, mRequestNumber,
                            sel.mSensorId, decParam);
                    WRITE_DECISION_RECORD_INTO_FILE(decParam,
                            "App mode is not support multiframe capture");
                }

                ainrLogD("App mode is not support multiframe capture");
                return BAD_VALUE;
            }
            ...
            if (mEnable == 1) {
                ...
                if (mScenarioRecorderEn) {
                    ...
                    submitTriggerFeatureLog(mUniqueKey, mRequestNumber,
                            sel.mSensorId, decParam);
                    WRITE_DECISION_RECORD_INTO_FILE(decParam,
                            "Decide to trigger AINR because of force on");
                }
            } else {
                if (mScenarioRecorderEn &&
                        sel.mRequestIndex == 0) {
                    ...
                    submitTriggerFeatureLog(mUniqueKey, mRequestNumber,
                            sel.mSensorId, decParam);
                    WRITE_DECISION_RECORD_INTO_FILE(decParam,
                            "Decide to trigger AINR because of scene detect");
                }
            }
        }
        ...
        if (!enableAinr
            ...
            if (mScenarioRecorderEn &&
                ...
                submitTriggerFeatureLog(mUniqueKey, mRequestNumber,
                        sel.mSensorId, decParam);
                WRITE_DECISION_RECORD_INTO_FILE(decParam,
                        "Not to trigger AINR because of scene detect");
            }
            removeCtrlerInfo(getUniqueKey());
        ...
        if (sel.mIMetadataApp.getControl() != nullptr) {
            ...
            bool isNonZsl = !sel.mDecision.mZslEnabled;
            if (isNonZsl) {
    ...
    MERROR negotiateIspHidlCamera(Selection& sel)
    {
        ...
        sel.mDecision.mZslPolicy.mPolicy
                ...
                | ZslPolicy::ZeroShutterDelay;
        sel.mDecision.mZslPolicy.mTimeouts = 1000;
        sel.mDecision.mZslPolicy.burstIndex = frameIndex;
        sel.mDecision.mZslPolicy.totalFrameSize = frameCount;
    ...
    MERROR negotiateIspHidlHal(Selection& sel)
    {
        ...
        if (frameIndex == 0) {
            ainrLogCAT("[AINR] frameCapture:%d", frameCount);
            // Query 3A information from caller
            mZSDMode = sel.mState.mZslRequest && sel.mState.mZslPoolReady;
            mFlashOn = sel.mState.mFlashFired;
            mRealIso = sel.mState.mRealIso;
        ...
        */
        ...
        if (pRequest->mRequestBSSIndex == pRequest->mRequestBSSCount - 1)
        {
            ...
            acquireJobQueue();
            {
                ...
                gShotCount++;
                ainrLogD("Ainr shot count(%d)", gShotCount.load(std::memory_order_relaxed));
            }
        }
        return 0;
    }
        ...
        , mMinorJobQueue(nullptr)
    {
        ...
        mUsingRrz = ::property_get_int32("vendor.debug.camera.ainr.userrz", 0);
        mScenarioRecorderEn
...
#error "CUST_MFLL_AUTO_MODE is no defined in custom/feature/mfnr/camera_custom_mfll.h"
    ...
    void createCtrlerInfo(MINT32 sensorId)
    {
        ...
        AinrCtrler::CtrlerNDDParam param(mUniqueKey, mRequestNumber);
        auto ctrler = std::make_shared<AinrCtrler>(sensorId, mMfbMode, mRealIso, mShutterTime, mFlashOn);
        ctrler->setTuningNddParam(param);

        {
            std::lock_guard<decltype(mCtrlerMx)> lk(mCtrlerMx);
            auto checkPair = mCtrlerContainer.emplace(mUniqueKey, ctrler);
    ...
    void acquireJobQueue()
    {
        ...
        if (mMinorJobQueue.get() == nullptr) {
            mMinorJobQueue
                    = std::make_shared<NSCam::JobQueue<void()>>("AinrCbJob");
        }
        return;
    }
    ...
    void submitTriggerFeatureLog(int32_t ukey,
            int32_t reqNum, int32_t openId, DecisionParam &outDecParam) {
        DebugSerialNumInfo &dbgNumInfo = outDecParam.dbgNumInfo;
        dbgNumInfo.uniquekey = ukey;
        dbgNumInfo.reqNum = reqNum;
...
private:
    ...
    // scenario recorder enable flag
    bool                            mScenarioRecorderEn;
