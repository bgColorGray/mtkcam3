...
public:
    ...
    virtual MERROR negotiate(Selection& sel)
    {
        ...
        if (sel.mIMetadataApp.getControl() != NULL) {
            ...
            const CaptureParam_T& captureParam(m_CurrentCapParams[requestIndex]);

            IMetadata::Memory capParams;
            capParams.resize(sizeof(CaptureParam_T));
            memcpy(capParams.editArray(), &captureParam, sizeof(CaptureParam_T));
...
private:
    ...
    bool getCurrentCaptureParam(bool bZsdFlow)
    {
        ...
        {
            CaptureParam_T& captureParam(m_CurrentCapParams.at(i));

            // copy original capture parameter
            captureParam = tmpCap3AParam;
        #if 0
            // adjust exposure settings
            CaptureParam_T& modifiedCapExpParam(captureParam);
    ...
    void dumpCaptureParam(
        const CaptureParam_T& captureParam, const char* msg)
    {
    ...
    std::vector<CaptureParam_T>  m_CurrentCapParams;
    std::vector<CaptureParam_T>  m_DelayCapParams;
    int                          m_captureNum;
    int                          m_delayFrame;
    int                          m_dump;
    int                          m_enable;
