...
{
    return ImageTransformPtr(IImageTransform::createInstance("TPFusionProvider", -1, IImageTransform::OPMode_DisableWPE), [](IImageTransform *p)
    {
        p->destroyInstance();
    });
}
...
process(RequestPtr requestPtr, RequestCallbackPtr callbackPtr)
{
    ...
    //
    {
        ...
        {
            // face data
            IMetadata::IEntry entryFaceRects = inMainHalMetaPtr->entryFor(MTK_FEATURE_FACE_RECTANGLES);
            IMetadata::IEntry entryPoseOriens = inMainHalMetaPtr->entryFor(MTK_FEATURE_FACE_POSE_ORIENTATIONS);
