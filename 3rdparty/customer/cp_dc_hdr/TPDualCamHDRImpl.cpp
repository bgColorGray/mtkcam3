...
{
    return ImageTransformPtr(IImageTransform::createInstance("TPDualCamHDRPrivider", -1, IImageTransform::OPMode_DisableWPE), [](IImageTransform *p)
    {
        p->destroyInstance();
    });
}
...
getCaptureParams(const IMetadata& matadata, CaptureParam_T& captureParam)
{
    ...
    bool ret = IMetadata::getEntry<IMetadata::Memory>(&matadata, MTK_3A_AE_CAP_PARAM, captureMemory);
    if( !ret ){
        MY_LOGE("cannot get MTK_3A_AE_CAP_PARAM");
        return;
    }
...
processDone(const RequestPtr& requestPtr, const RequestCallbackPtr& callbackPtr, MERROR status)
{
    ...
    if( isLastFrame )
    {
        ...
        {
            const auto it = mCollectedRequests.begin();
            const RequestPtr& tmpReqPtr = it->mRequestPtr;
            const RequestCallbackPtr& cbReqPtr = it->mCallbackPtr;
            const MERROR status = it->mStatus;
