...
) -> bool
{
    ...
    MY_LOGD("scenarioHint(isCShot:%d, isDualCam:%d, dualDevicePath:%d, multiCamFeatureMode:%d, isLogical:%d captureScenarioIndex:%d)",
    ...
    if (scenarioHint.captureScenarioIndex > 0) {  // force by vendor tag (ex:Pro mode))
        ...
        if (scenarioHint.dualDevicePath == static_cast<int32_t>(DualDevicePath::MultiCamControl)) {
            if (scenarioHint.multiCamFeatureMode == MTK_MULTI_CAM_FEATURE_MODE_ZOOM) {
                ...
                MY_LOGD("Default Multicam scenario!");
    ...
    } else if (scenarioHint.isVideoMode) {
        MY_LOGD("VSS scenario!");
        scenario = MTK_CAMERA_SCENARIO_CAPTURE_VSS;
    } else {
    ...
    switch (scenario) {
        ...
        default:
            if (gIsLowMemoryDevice) {
                ...
                MUINT32 jpegNum = static_cast<MUINT32>(::property_get_int32("vendor.debug.default.jpegnum", 5));
...
{
    // Customer should runtime check TP eis enable or not via paramters
    // i.e. pAppMetadata/scenario/scenarioFeatures/scenarioConfig/scenarioHint
    // For demo TP eis run or not, currently use prop control
    int eisDemoMode = 0;
    IMetadata::getEntry(pAppMetadata, MTK_STREAMING_FEATURE_DEMO_EIS, eisDemoMode);

    bool isEIS = property_get_bool("vendor.debug.tpi.s.eisq", (eisDemoMode != 0) );
    if(isEIS)
    {
        // margin & qcount should sync with TP eis plugin
        ...
        MUINT32 eisQCnt = (MUINT32)::property_get_int32("vendor.debug.scenario.s.eisQCount", 12);
        if( scenarioHint.isVideoMode ) // Only video mode need queue count
        {
            scenarioConfig.queueCount = eisQCnt;
        }
    }

    MY_LOGI("streaming scenario config, fixMarginList(%zu), qCount(%d)", scenarioConfig.fixedMargins.size(), scenarioConfig.queueCount);
